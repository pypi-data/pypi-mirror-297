        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {{
                ...
                eventSource.onmessage = function(event) {{
                    ...
                    // Update the element by its ID
                    const element = document.getElementById("{element_id}");
                    if (element) {{
                        console.log("Updating element:", '{element_id}');
                        element.value = newValue;  // Update the DOM element's value

                        // Dispatch an input event to trigger any bound events (this mimics user input)
                        const event = new Event('input', {{ bubbles: true }});
                        element.dispatchEvent(event);  // This tells NiceGUI the value has changed
                    }} else {{
