        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {{
                ...
                eventSource.onmessage = function(event) {{
                    ...
                    // Trigger NiceGUI's set_value method using JavaScript
                    if (window.nicegui) {{
                        const element = window.nicegui.get_element('{element_id}');
                        if (element) {{
                            console.log("Using NiceGUI to update element:", '{element_id}');
                            element.set_value(newValue);  // Use NiceGUI's set_value method
                        }} else {{
                            console.log("Element not found in NiceGUI:", '{element_id}');
                        }}
                    }} else {{
                        console.log("NiceGUI not defined, retrying...");
                    }}
                }};
