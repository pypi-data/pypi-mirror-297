...
class SSEManager:
    ...
    def notify_listeners(cls, model_name, object_id, field_name, new_value):
        ...
        for listener in listeners:
            listener(new_value)  # Directly call each listener

    @classmethod
    def stream_updates(cls, request, app_label, model_name, object_id, field_name):
        # Create an event stream to yield events to the client
        def event_stream():
            ...
            # Listener that sends updates via the stream
            ...
            # Send the initial value if present
            if last_value is not None:
                yield from send_update(last_value)

            # Keep the connection open for further updates
            try:
                while True:
                    # This loop ensures the SSE connection remains open until the client disconnects
