...
class RecyclingPeakEncoder(torch.nn.Module):
    """Encode mass spectrum.
    ...
    def __init__(
        ...
        # TODO: Change this when you use a larger esm model in production
        self.dim_embedding = 1280
        self.embedding_encoder = torch.nn.Linear(
            self.dim_embedding, self.dim_model, bias=False
        )
    ...
    def forward(self, X, Y):
        ...
        X : torch.Tensor of shape (n_spectra, n_peaks, 2)
            ...
            should be zero-padded, such that all of the spectra in the batch
            are the same length.
        Y : torch.Tensor of shape (n_spectra, dim_embedding), optional
            The embeddings of the sequences corresponding to the particular spectra,
            which are generated by the ESM model.
        Returns
        -------
        torch.Tensor of shape (n_spectra, n_peaks, dim_model)
            The encoded features for the mass spectra.
        ...
        if Y is not None:
            embedding = self.embedding_encoder(Y).unsqueeze(1).repeat(1, X.shape[1], 1)
            return encoded + intensity + embedding
