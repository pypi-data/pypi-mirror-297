...
function init_voice_quick_search(quicksearch)
{
    ...
    const columns = Array.from(document.querySelectorAll('#SDA th'));
    const rows = Array.from(document.querySelectorAll('#SDA tbody tr'));
    const headers = Array.from(document.querySelectorAll('#SDA thead th'));
    ...
    columns.forEach(th => {
        if (!start) {
            start = (lower_case(th.textContent.trim()) === "start") ? th : null;
        }
        if (!end) {
            end = (lower_case(th.textContent.trim()) === "end") ? th : null;
        }
    });
