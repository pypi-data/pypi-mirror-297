...
def to_gaussian_number_method(src):
    ...
    '''
    if src is None:
        return INVALID_GAUSSIAN_NUMBER_METHOD
    if src in ['', 'nan']:
        return INVALID_GAUSSIAN_NUMBER_METHOD
    else:
...
def to_wiener_process_build_method(src):
    ...
    '''
    if src is None:
        return INVALID_WIENER_PROCESS_BUILD_METHOD
    if src in ['', 'nan']:
        return INVALID_WIENER_PROCESS_BUILD_METHOD
    else:
...
def create_matrix(data: np.array):
