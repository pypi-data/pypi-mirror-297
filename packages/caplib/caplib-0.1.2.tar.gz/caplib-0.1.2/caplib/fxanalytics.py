...
def fx_european_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    pb_input = dqCreateProtoFxEuropeanOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
...
def fx_american_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxAmericanOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_asian_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxAsianOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_digital_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxDigitalOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_single_barrier_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxSingleBarrierOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_double_barrier_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxDoubleBarrierOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_one_touch_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxOneTouchOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_double_touch_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxDoubleTouchOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_range_accrual_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxRangeAccrualOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_single_shark_fin_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxSingleSharkFinOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_double_shark_fin_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxDoubleSharkFinOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_ping_pong_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxPingPongOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_airbag_option_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxAirbagOptionPricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_snowball_auto_callable_note_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxSnowballAutoCallableNotePricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_phoenix_auto_callable_note_pricer(instrument, pricing_date, mkt_data_set, pricing_settings, risk_settings, scn_settings):
    try:
        pb_input = dqCreateProtoFxPhoenixAutoCallableNotePricingInput(dqCreateProtoFxPricingInput(create_date(pricing_date), instrument.underlying, 
                                                                                     mkt_data_set, pricing_settings, risk_settings, 
                                                                                     False, '', '', '', '', '', 
                                                                                     scn_settings),
                                                         instrument)
...
def fx_delta_to_strike_calculator(delta_type, delta, expiry_date, vol_surf, fx_spot_rate, domestic_discount_curve, foreign_discount_curve):
    try:
        option_type='Call'
        if delta < 0:
            option_type = 'Put'
...
def fx_atm_strike_calculator(atm_type, expiry_date, vol_surface, fx_spot_rate, domestic_discount_curve, foreign_discount_curve):
    try:
