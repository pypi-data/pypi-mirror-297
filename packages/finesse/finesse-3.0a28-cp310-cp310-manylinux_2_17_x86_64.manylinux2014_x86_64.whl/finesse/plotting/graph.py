...
def plot_graph(
    ...
    **kwargs,
    ...
    if graphviz:
        return graphviz_draw(network=network, layout=layout, **kwargs)
    else:
        return plot_nx_graph(network, layout, **kwargs)
...
def graphviz_draw(
    ...
    if _in_ipython():
        ...
        path = tempfile.NamedTemporaryFile(suffix=f".{format}", delete=False)
        byt = A.draw(path=path, format=None, prog=layout)
        if format == "svg":
