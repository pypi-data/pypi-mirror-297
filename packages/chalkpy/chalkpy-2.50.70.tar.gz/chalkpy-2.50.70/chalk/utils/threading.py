...
from chalk.utils.log_with_context import add_logging_context, get_logging_context
    ...
    if __debug__:
        ...
        def submit(  # pyright: ignore[reportRedeclaration]
            ...
            def wrapped_fn(*args: P.args, **kwargs: P.kwargs):
                ...
                # task
                with add_logging_context(_merge=False, **logging_context):
        ...
        def _wrapped_fn_no_debug(
            self, original_fn: Callable[P, T], logging_context: Mapping[str, Any], /, *args: P.args, **kwargs: P.kwargs
        ) -> T:
            # Setting _merge=False to override the existing context,
            # in case if there is context left behind in the current thread from a previous
            # task
        ...
        def submit(self, fn: Callable[P, T], /, *args: P.args, **kwargs: P.kwargs) -> Future[T]:
            """Submit a new task to the thread pool."""
            logging_context = get_logging_context()
            return super().submit(self._wrapped_fn_no_debug, fn, logging_context, *args, **kwargs)
