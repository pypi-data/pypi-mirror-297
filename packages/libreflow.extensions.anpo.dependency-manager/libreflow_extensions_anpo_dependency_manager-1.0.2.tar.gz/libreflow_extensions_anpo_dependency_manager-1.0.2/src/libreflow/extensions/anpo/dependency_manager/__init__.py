...
class CheckDependencies(flow.Action):
    ...
    def __init__(self, parent, name):
        super(CheckDependencies, self).__init__(parent, name)
...
class CheckRevisionDependencies(CheckDependencies):
...
class CheckLayoutDependencies(CheckDependencies):
    """
...
class CheckBlockingDependencies(CheckDependencies):
    """
    Action to check the dependencies of the latest
...
def check_dependencies(parent):
    if isinstance(parent, TrackedFile) and parent.name().endswith('_blend'):
        cd = flow.Child(CheckDependencies)
        cd.name = 'check_dependencies'
        cd.index = None
