...
from typing import Generic, TypeVar, Optional, List, Any, Type, Union

from pydantic import BaseModel
from sqlalchemy import func
from sqlmodel import Session, select, desc, asc

from vovo.persistence.page import Page
...
class GenericRepository(Generic[T], ABC):
    ...
    @abstractmethod
    def get_page_list(self, page_number: int, page_size: int, order_by: Optional[Union[str, List[str]]] = None,
                      **filters: Any) -> Page[T]:
        """Get a paginated list of records."""
        raise NotImplementedError()

    @abstractmethod
    def get_count(self, **filters: Any) -> int:
...
class GenericSqlRepository(GenericRepository[T], ABC):
    ...
    def get_page_list(self, page_number: int = 1, page_size: int = 10, order_by: Optional[Union[str, List[str]]] = None,
                      **filters: Any) -> Page[T]:
        """Get a paginated list of records with optional ordering."""
        # Calculate the total number of records
        total_records = self.get_count(**filters)
        ...
        result = self.session.exec(query).all()
        elements = [self.model(**dict(row)) for row in result]
