...
from . import RestOptions, HttpMethod, RestResponse, RestFul, Hooks, _utils
from ._constants import _OPTIONAL_ARGS_KEYS, _HTTP_RE, _Constant, _BODY_SHOW_MAX_LEN, _HTTP_INFO_TEMPLATE
...
class _RestFast(object):
    ...
    def opts(self, opts: RestOptions) -> '_RestFast':
        """
    ...
    def method(self, method: Union[HttpMethod, str]) -> '_RestFast':
        """
...
class _Rest(_BaseRest):
    ...
    def __request(self, func: callable, kwargs: dict, api_name: str = None, server_name: str = None, host: str = None,
        ...
        _utils.build_log_message(log_builder, f'{func_name} Request Start'.center(41, '*'))
        ...
        _server_name: str = _utils.server_name_handler(self.server_name, server_name, func)
        _api_name: str = _utils.api_name_handler(api_name, func)
        _server_dict: dict = _utils.server_dict_handler(self.server, self.server_list, _server_name)
        server_description = _utils.server_desc_handler(self.description, _server_dict)
        _host: str = _utils.host_handler(self.host, host, _server_dict)
        _api_info: dict = _utils.api_handler(_server_dict, _api_name)
        optional_args: dict = _utils.optional_args_handler(_api_info, kwargs)
        optional_args[_Constant.ALLOW_REDIRECTS] = allow_redirection
        _api: str = ObjectsUtils.none_of_default(_api_info.get(_Constant.API_PATH), api)
        ObjectsUtils.check_non_none(_api)
        api_description = _utils.api_desc_handler(description, _server_dict, _api_name, _Constant.DESC)
        _method: str = _api_info.get(_Constant.HTTP_METHOD, _utils.http_method_handler(method))
        ObjectsUtils.check_non_none(HttpMethod.get_by_value(_method.upper()))
        _headers: dict = _api_info.get(_Constant.HEADERS, {})
        _utils.header_handler(optional_args, self.headers, self.cookies, _method.upper(), _headers, headers,
        ...
        # noinspection PyBroadException
        try:
            req_args = _utils.run_before_hooks(self.__hooks_before, hooks, optional_args.get("hooks"), req_args)
            resp: Response or None = _utils.action(self.__session, _method.lower(), url, **req_args)
            if check_status_:
            ...
            _utils.build_log_message(log_builder, f"{func_name} Request End".center(43, '*'))
            _LOGGER.log(level=RestConfig.http_log_level.value, msg=log_builder, stacklevel=2)
            kwargs[_Constant.RESPONSE] = _utils.run_after_hooks(self.__hooks_after,
                                                                hooks,
...
__all__ = [_Rest, _BaseRest, _RestFast]
