...
class TimeUnit(_UnitEnum):
    ...
    def format(value: [float, int, '_UnitEnum', '_Converter'], precision: int) -> str:
        ...
        if _value >= TimeUnit.DAY.value:
            ...
            return f"{round(_value / TimeUnit.SECOND.value, precision)}Sec(s)"
        elif _value >= TimeUnit.MILLI_SECOND.value:
            return f"{round(_value / TimeUnit.MILLI_SECOND.value, precision)}Mill(s)"
        elif _value >= TimeUnit.MICRO_SECOND.value:
            return f"{round(_value / TimeUnit.MICRO_SECOND.value, precision)}Micro(s)"
        else:
            return f"{round(_value, precision)}Nano(s)"
