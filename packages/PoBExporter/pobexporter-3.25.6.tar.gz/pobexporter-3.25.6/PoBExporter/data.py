...
def get_gem_dict():
    ...
    with open("data/gems.json", "w") as file:
...
def get_skilltree():
    ...
    with open("data/skilltree.json", "w") as file:
...
class Data:
    ...
    def __init__(self) -> None:
        with open("data/gems.json", "r") as file:
            self.gem_data: dict[str, GemData] = json.load(file)

        with open("data/skilltree.json", "r") as file:
            self.skilltree = json.load(file)

        self.coordinate_to_jewel_hash = [
        ...
        for class_id, base_class in enumerate(self.skilltree["classes"]):
            self.base_class_to_class_id[base_class["name"]] = str(class_id)
            for ascendancy_class_id, ascendancy_class in enumerate(base_class["ascendancies"]):
                self.ascendancy_class_to_ascendancy_class_id[ascendancy_class["name"]
                                                             ] = str(ascendancy_class_id + 1)
                self.ascendancy_class_to_base_class[ascendancy_class["name"]
                                                    ] = base_class["name"]
