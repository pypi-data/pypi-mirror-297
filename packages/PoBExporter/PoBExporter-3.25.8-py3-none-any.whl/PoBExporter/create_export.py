...
from PoBExporter.fetch import Data
from PoBExporter.item import get_item_text, get_slot, get_slots, set_gems
from PoBExporter.passive_encode import get_passive_tree_url
from PoBExporter.schema import Character

Data = Data()


def create_pob_string(character: Character) -> str:
    if character["class"] in Data.ascendancy_class_to_base_class:
        base_class = Data.ascendancy_class_to_base_class[character["class"]]
        ascendant_class = character["class"]
        class_id = Data.base_class_to_class_id[base_class]
        ascendancy_class_id = Data.ascendancy_class_to_ascendancy_class_id[ascendant_class]
    else:
    ...
    build = ET.SubElement(
        ...
        targetVersion="3_0",
        pantheonMajorGod=character["passives"].get("pantheon_major", "None"),
        pantheonMinorGod=character["passives"].get("pantheon_minor", "None"),
        bandit=character["passives"].get("bandit_choice", "None"),
    ...
    for jewel in character["jewels"]:
        node_id = Data.coordinate_to_jewel_hash[jewel["x"]]
