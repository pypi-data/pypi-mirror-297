...
class Detector_UnevenMultiAccum(Detector_MultiAccum):
    ...
    def slope_variance(self, rate: np.ndarray, unsat_nframes: np.ndarray, weights: str = "optimal") -> tuple[np.ndarray, np.ndarray]:
        ...
        Last validated against RomanISim v0.5.2 on 2024.0808 (JETC-4466)
        ...
        # Calculate mean times for each resultant (RomanISIM, fit_ramps_casertano_no_dq.read_pattern_to_tbar, 2024.0807)
        tmean = tframe * np.array([np.mean(resultant) for resultant in self.exposure_spec.readout_pattern])
        ...
        slope_var = slope_rn_var + slope_var * np.sum(rate["fp_pix_variance"] * k_i, axis=0)
    ...
    def weightcalc(self, uns: float, w_ij: np.ndarray, tmean: np.ndarray) -> np.ndarray:
        """
    ...
    def _frames_before_sat(self, slope: np.ndarray) -> np.ndarray:
        ...
        # number of frames is total time / frame time. If reference frames are used but
        # not downlinked, all of the frame counts will be lower by one frame (the
        # reference frame). A frame_before_saturation of -1 is possible if the source is
        # bright enough to saturate during the reference frame. 
        # 
        ...
        frames_before_saturation = np.floor(time_to_saturation / self.exposure_spec.tframe)
        if self.exposure_spec.nreference > 0 and not self.exposure_spec.reference_downlink:
            frames_before_saturation -= self.exposure_spec.nreference
        if self.exposure_spec.nreset > 0 and self.exposure_spec.reset_downlink:
            frames_before_saturation += self.exposure_spec.nreset
