...
class GBSDCalculator(Calculator):
    ...
    def calculate(
        ...
        fractional_positions = self._move_to_device_and_time(atoms.get_scaled_positions())
        species = atoms.get_chemical_symbols()
        cell = self._move_to_device_and_time(atoms.get_cell().array)
        ...
        if "forces" in properties:
            ...
            self.results.update(forces=self._move_to_cpu_and_time(forces))

        if "stress" in properties:
            stress = (1 / get_cell_volume(cell)) * torch.autograd.grad(energy, strain)[0]
            self.results.update(stress=self._move_to_cpu_and_time(stress).flat[[0, 4, 8, 5, 2, 1]])  # voigt stress
