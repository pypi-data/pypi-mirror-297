...
try:
    opsgenie = Opsgenie('420fb17d-4542-4344-81a7-b6770d061413')
...
response = opsgenie.get_team_by_name('saas')
print(response)
response = opsgenie.list_schedules()
print(response)
response = opsgenie.get_schedules_by_name('SaaS schedule')
print(response)
response = opsgenie.list_teams()
print(response)
team_name = 'team3'
schedules = opsgenie.list_schedules()
teams_with_ownerteam_key = [x for x in schedules['data'] if 'ownerTeam' in x]
...
if len(filtered_schedules) == 1:
    team_schedule_name = filtered_schedules[0]['name']
result = opsgenie.list_schedule_timeline_by_team_name(team_schedule_name)
...
for rotation in rotations:
    ...
    for period in periods:
        if not history and pendulum.now().in_tz('UTC') > pendulum.parse(period['startDate']):
            continue
        else:
            ...
            format_table.add_row([period['recipient']['name'], f'{start_dt.format("ddd")} {start_dt.to_datetime_string()}', f'{end_dt.format("ddd")} {end_dt.to_datetime_string()}'])
    print(format_table)


team_id = "027af724-a97d-4c1a-989b-f20e52ff2a9e"
team_name = "saas"
response = opsgenie.teams_get(team_id, team_name)
print(response)
