...
hex_chars = string.hexdigits[:16]  # الحصول على أحرف سداسية
...
class Email_services:
    def Hotmail(username):
        guid = uuid.uuid4().hex
        hash_value = uuid.uuid4().hex[:4]
        lv = f"{random.randint(202400, 202499)}"
        v = "4"
        def generate_base64_string(length):
            random_bytes = uuid.uuid4().bytes
            return base64.b64encode(random_bytes).decode('utf-8')
        lu = str(random.randint(1000000000000, 9999999999999))
        msfpc_value = f"GUID={guid}&HASH={hash_value}&LV={lv}&V={v}&LU={lu}"
        random_value = uuid.uuid4().hex
        formatted_value = random_value[:8] + random_value[8:12] + random_value[12:16] + random_value[16:20] + random_value[20:]
        cookies = {
            'MicrosoftApplicationsTelemetryDeviceId': str(uuid.uuid4),
            'MSFPC': msfpc_value,
            'MUID': formatted_value,
            '_pxvid': str(uuid.uuid4),
            'fptctx2': 'taBcrIH61PuCVH7eNCyH0Iitb%252bEMfwlgK%252fM8w%252f28EbfI3jNwNw7TTOLpA6HyicQrxwiMFA3I%252fCurwwJswZYq7Y6PKUxFm1Z%252bxJ0uT1A1kVgBZTm%252bpT9S3HKMRv3hjrc9T%252b%252bEzny7ZDxXx1foDXveMAXGiqNg0iyptv2CddVuPGgP922p9wgZ1uI12xzK97EZlh3VfAblsZBTFjbvK42yJRriXCW3UOzZy4Lb1z2bIbe%252bjnfwIaObfWxD3NTFvS7FwcRVpxyl2jfdJX8ezxiFuzCjjzLloJEEj%252f7HqU0%252bAweRP77VKoLnVb4275JW0ARCR%252fYEHDdAsantibdDnagPQg%253d%253d',
            'PPLState': '1',
            'logonLatency': f"LGN01={random.randint(1000000000000000000, 9999999999999999999)}",
            'MSPAuth': random.choice(['Enabled', 'Disabled']),
            'MSPProf': random.choice(['Enabled', 'Disabled']),
            'ANON': f"A={uuid.uuid4().hex}&E={random.randint(1, 100)}&W={random.randint(1, 10)}",
            'NAP': f"V={random.randint(1, 2)}&E={uuid.uuid4().hex}&C={uuid.uuid4().hex[:8]}",
            'WLSSC': generate_base64_string(200),
            'ANON': '',
            'NAP': '',
            'amsc': ':2:3c',
            'ai_session': f"{uuid.uuid4().hex}|{random.randint(1000000000000, 9999999999999)}|{random.randint(1000000000000, 9999999999999)}",
            '_px3': generate_base64_string(150),
            '_pxde': generate_base64_string(100),
        }

        headers = {
            'accept': 'application/json',
            'accept-language': 'en-US,en;q=0.9',
            'canary': ':2:3c',
            'client-request-id': uuid.uuid4().hex,
            ...
            'priority': 'u=1, i',
            'referer': 'https://signup.live.com/signup?lcid=1033&wa=wsignin1.0&rpsnv=161&ct=1727093233&rver=7.5.2211.0&wp=MBI_SSL&wreply=https%3a%2f%2foutlook.live.com%2fowa%2f%3fnlp%3d1%26signup%3d1%26cobrandid%3dab0455a0-8d03-46b9-b18b-df2f57b9e44c%26RpsCsrfState%3da12f5cec-c7f7-5617-3ad3-ffbe190a719c&id=292841&CBCXT=out&lw=1&fl=dob%2cflname%2cwld&cobrandid=ab0455a0-8d03-46b9-b18b-df2f57b9e44c&lic=1&uaid=e368f946ee29487ca193db242429a22f',
            'sec-ch-ua': '"Google Chrome";v="129", "Not=A?Brand";v="8", "Chromium";v="129"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
        ...
        response = requests.post(
            'https://signup.live.com/API/CheckAvailableSigninNames?lcid=1033&wa=wsignin1.0&rpsnv=161&ct=1727093233&rver=7.5.2211.0&wp=MBI_SSL&wreply=https%3a%2f%2foutlook.live.com%2fowa%2f%3fnlp%3d1%26signup%3d1%26cobrandid%3dab0455a0-8d03-46b9-b18b-df2f57b9e44c%26RpsCsrfState%3da12f5cec-c7f7-5617-3ad3-ffbe190a719c&id=292841&CBCXT=out&lw=1&fl=dob%2cflname%2cwld&cobrandid=ab0455a0-8d03-46b9-b18b-df2f57b9e44c&lic=1&uaid=e368f946ee29487ca193db242429a22f',
            cookies=cookies,
            headers=headers,
            json=json_data,
        )
        try:
            if '"isAvailable":true' in response.text:
                ...
                return 'Error With request Talk to @KKKKKQ9'
        except:
            return 'FIELD EMPTY'
