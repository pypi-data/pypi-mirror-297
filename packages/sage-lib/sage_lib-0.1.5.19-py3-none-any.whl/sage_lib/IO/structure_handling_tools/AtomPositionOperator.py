...
class AtomPositionOperator:
    ...
    def set_latticeVectors(self, new_latticeVectors:np.array, edit_positions:bool=True):
        ...
        self._distance_matrix = None if edit_positions and self._latticeVectors else self._distance_matrix
        self._kdtree = None if edit_positions and self._latticeVectors else self._kdtree
    ...
    def compress(self, compress_factor: list = None, scale_positions: bool = True, verbose: bool = False):
        """
        Compresses the lattice vectors and optionally the atomic positions by specified factors.

        This method scales the lattice vectors stored in the class by the given compress factors. 
        Optionally, it can also scale the atomic positions, depending on the value of `scale_positions`.
        ...
        - scale_positions (bool): If True, both the lattice vectors and atomic positions are scaled. 
          If False, only the lattice vectors are scaled.
        ...
        if not isinstance(compress_factor, np.ndarray) or compress_factor.shape[0] != 3:
            raise ValueError("Compress factors must be a list or numpy array of exactly three elements.")

        if self.latticeVectors is not None:
            # Scale the lattice vectors
            self.set_latticeVectors(self.latticeVectors * compress_factor, edit_positions=scale_positions)
            
        else:
            #raise ValueError("Lattice vectors must be set before compression.")
            new_positions = self.atomPositions * compress_factor
            self.set_atomPositions(new_positions)

        # Optional verbose output
        if verbose:
            print("Lattice vectors compressed successfully.")
            if scale_positions:
