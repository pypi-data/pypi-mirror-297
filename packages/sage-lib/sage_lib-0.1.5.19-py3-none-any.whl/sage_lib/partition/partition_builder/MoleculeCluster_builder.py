...
class MoleculeCluster_builder(PartitionManager):
    ...
    def add_solvent(self, container, 
                        ...
                        molecules:dict={'H2O':1.0}, density:float=1.0, max_iteration:int=6000, verbosity:bool=True, molecules_number:dict=None):
        """
        ...
        Returns:
            None
        """
        max_iteration = max_iteration if isinstance(max_iteration,int) else 3000
        ...
        for container_index, container in enumerate(containers):
            for v_key, v_item in values.items():
                ...
                if v_key.upper() == 'ADD_SOLVENT':
                    ...
                    molecules_number = v_item.get('molecules_number', None)
                    ...
                    if not self.add_solvent(container=container_copy, shape=shape, cluster_lattice_vectors=vacuum_box, 
                                ...
                                molecules_number=molecules_number, max_iteration=max_iteration):
                        return False
                    else:
