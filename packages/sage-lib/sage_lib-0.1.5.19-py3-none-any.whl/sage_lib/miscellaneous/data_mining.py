...
class LinearAnalysis:
    ...
    def analysis(self, methods:list=None, plot:bool=True, output_dir:str='./'):
        ...
        if 'dynamic_permutation' in methods:         
            ...
            self.plot_dynamic_permutation_impact(self.analysis_results['dynamic_permutation']['mean_errors'], self.analysis_results['dynamic_permutation']['n_permutations_list'], output_file=f'{output_dir}/dynamic_permutation_impact.png')

        for m in methods:
            if not m in self.linear_methods:            
                print(f' (!) Method {m} is not recognizeble.')           

    def sensitivity_analysis(self, linear_predict, compositions, energies, noise_levels=np.logspace(-5, -4, num=30), n_iterations=300):
        """
        ...
        Returns:
            dict: Dictionary containing mean coefficients, std of coefficients, and noise levels.
        """
        mean_coeffs = []
        std_coeffs = []
        mean_abs_noise = []
        ...
        for noise in tqdm(noise_levels, desc="Processing noise levels"):
            ...
            for _ in range(n_iterations):
                noise_array = np.random.normal(0, np.abs(np.max(energies)) * noise, size=energies.shape)
                noisy_energies = energies + noise_array

                noisy_energies = energies + np.random.normal(0, np.abs(np.max(energies)) * noise, size=energies.shape)
                coeffs, _, _ = linear_predict(compositions, noisy_energies, regularization=1e-5, verbose=False, zero_intercept=True, force_negative=False)
                coeffs_for_noise.append(coeffs[1:])
                abs_noise_for_level.append( np.std(noise_array))

            mean_coeffs.append(np.mean(coeffs_for_noise, axis=0))
            std_coeffs.append(np.std(coeffs_for_noise, axis=0))
            mean_abs_noise.append(np.mean(abs_noise_for_level))

        return {'mean_coefficients': mean_coeffs, 'std_coefficients': std_coeffs, 'noise_levels': mean_abs_noise}
    ...
    def evaluate_dynamic_permutation_impact(self, X, y, linear_predict, tolerance=10, min_sampling=5):
        """
        Dynamically evaluates the impact of permuting input features on the predictive accuracy of a linear model.
        This function automates the determination of iteration count and permutation numbers to explore how 
        feature permutations affect model error.

        Parameters:
        - linear_predict (callable): Function that performs linear predictions, returning coefficients, predictions, and residuals.
        - X (np.ndarray): Input feature matrix with shape (n_samples, n_features).
        - y (np.ndarray): Target vector of length n_samples.
        - tolerance (float): Convergence criterion for the stopping condition based on the mean error variation.
        - min_sampling (int): Minimum number of samples to consider before assessing error stability.

        Returns:
        - dict: Dictionary containing 'n_permutations_list' and 'mean_errors' as keys with corresponding lists.
        """
        rows, cols = X.shape
        max_permutations = int((rows * cols)/2)
        n_permutations_list = list(range(max_permutations))  # Range from 0 to the total number of elements in X
        mean_errors = []
        previous_error = float('inf')

        # Iterate over different numbers of permutations
        for n_permutations in n_permutations_list:
            errors_per_permutation = []
            change_in_error = float('inf')

            # Continue permuting until the change in error is less than the tolerance or enough samples are gathered
            while change_in_error > tolerance or len(errors_per_permutation) < min_sampling:
                x_permuted = np.copy(X)
                efficient_vectorized_permutations(x_permuted, n_permutations)

                # Predict and calculate error using the linear predictor
                coefficients, predictions, residuals = linear_predict(
                    X=x_permuted, y=y, regularization=1e-7, force_negative=False, zero_intercept=True, verbose=False
                )
                mse = np.mean((predictions - y) ** 2)
                errors_per_permutation.append(mse)

                # Determine the change in error
                current_error = np.mean(errors_per_permutation)
                change_in_error = abs(current_error - previous_error)
                previous_error = current_error

            mean_errors.append(current_error)

        return {'n_permutations_list': n_permutations_list, 'mean_errors': mean_errors}
    ...
    def plot_sensitivity_analysis(self, result, feature_names=None, output_file='coefficients_distribution.png'):
        ...
        mean_coeffs = np.array(result['mean_coefficients'])
        std_coeffs = np.array(result['std_coefficients'])
...
class ClusteringAnalysis:
    ...
    def _create_clustering_methods(self, params: Dict[str, Any]) -> Dict[str, callable]:
        ...
        if 'dbscan' in methods:
            if not 'eps' in params or not 'min_samples' in params:
                eps, min_samples = self.optimize_dbscan_params(self.data_scaled)
            params['eps'] = params.get( 'eps', eps )
            params['min_samples'] = params.get( 'min_samples', min_samples )
...
class Compress:
    ...
    def _factor_analysis_compression(self, data, n_components=2, random_state=42):
        data = np.array(data)
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(data)
    ...
    def verify_and_load_or_compress(self, data, n_components=2, method='umap', load=True, **kwargs):
        """
