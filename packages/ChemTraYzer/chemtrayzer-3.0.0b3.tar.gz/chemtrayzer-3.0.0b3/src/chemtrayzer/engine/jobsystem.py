...
class Memory:
    ...
    def __eq__(self, other) -> bool:
        if type(other) == Memory:
            return self.amount_in_unit(Memory.UNIT_KB) == \
                other.amount_in_unit(Memory.UNIT_KB)
...
class Job(ABC, Submittable):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # use keyword arguments even for required arguments to avoid errors with
        # the order of this huge list of arguments

        # required arguments
        missing = set(
            ['n_tasks', 'n_cpus', 'memory', 'runtime']
        ).difference(set(kwargs.keys()))

        if len(missing) > 0:
            raise TypeError(f'__init__() missing required keyword '
                f'arguments: {missing}')
...
class PythonJob(Job):
    ...
    '''name of the file that contains the path to the file in which the callable
    is defined'''
    ...
    def __init__(self, c: Callable, *args,
                 n_tasks = 1, memory = Memory(1, unit=Memory.UNIT_GB),
                 n_cpus = 1, runtime = timedelta(days=1),
                 **kwargs) -> None:
        super().__init__(**kwargs, n_tasks=n_tasks, memory=memory,
                         n_cpus=n_cpus, runtime=runtime)
        self.args = args
        self.kwargs = kwargs
        self.callable = c
    ...
    def gen_input(self, path):
        ...
        # store the path to that file in the job's folder so that the job script
        # can import it.
        try:
            callable_path = Path(inspect.getsourcefile(self.callable)).resolve()
        except TypeError as err:
            if inspect.isfunction(self.callable):
                raise TypeError('Cannot pickle the callable. It may be a lambda'
                                ' function') from err
            elif hasattr(self.callable, '__class__'):
                # assume that the callable is an instance of a class that
                # defines the __call__ method
                callable_path = Path(inspect.getsourcefile(
                                        self.callable.__class__)).resolve()
            else:
                raise TypeError('Cannot determine the file in which the '
                                'callable is defined. This may lead to problems'
                                ' when unpickling it.') from err


        with open(path/self._CALLABLE_DEF_PATH, 'w', encoding='utf-8') as file:
...
class PythonScriptJob(Job):
    ...
    def __init__(self, *, script: os.PathLike, python = None,
                 arguments = None, working_dir = None,
                 **kwargs) -> None:
        if python is None:
...
class JobTemplate:
    ...
    def __init__(self, job : Job, cmd_tmpl : str, input_tmpls : Mapping[str, str]) -> None:
        self.job = job
    ...
    def gen_input(self, path):
        ...
        for filename in self.input_tmpls:
            with open(os.path.join(path, filename), encoding='utf-8',
                ...
                file.write(
                    Template(
                        self.input_tmpls[filename]
                    ).substitute(self._get_job_properties())
                )
...
class _JobDatabase(Database):
    ...
    def list_jobs(self, name: str = None, state: State = None)  \
            -> Collection[int]:
        '''
        ...
        if name is not None and state is not None:
            cur = self._con.execute('SELECT job_id FROM job_metadata '
                'WHERE name=(?) AND state=(?)', (name, str(state)))
    ...
    def save_job(self, job: Job) -> int:
        ...
        try:
            ...
            cur.execute('INSERT INTO job_metadata VALUES (?,?,?)',
            ...
            try:
                ...
                cur.execute('INSERT INTO job_blobs VALUES (?, ?)',
                    (job_id, job_blob))
            except Exception:
    ...
    def submit(self, job: Job, wait_for: Iterable[int] = None) -> int:
        ...
        try:
            ...
            if slurm_id.isnumeric():
                self._slurm_ids[job.id] = slurm_id
            else:
