...
class Event:
    ...
    def __eq__(self, __o: object) -> bool:
        if isinstance(__o, Event):
            return type(self) == type(__o) and self.spec == __o.spec
        else:
            return False
    ...
    def register_listener(self, listener: Listener, event: Event = None,
        ...
        if event is None and not (event_type is None and spec is None):
            ...
            raise EventError('The event needs to be defined by either suppling '
                             'an event object or by supplying its type and specification.')

        if event_def not in self._listeners:
            self._listeners[event_def] = []

        if listener not in self._rev_listeners:
            self._rev_listeners[listener] = []
