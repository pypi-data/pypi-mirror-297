...
from chemtrayzer.core.coords import ChainOfStates, Geometry, TSGeometry
...
class IntegrityError(DatabaseError):
    """raised when the structural or relational integrity of the database is affected"""
...
GeoId = TypeVar("GeoId", bound=int)


class Database:
    """A class to help create sqlite databases based on Python's sqlite3 module.
    ...
    ids of objects stored in the database which can than be passed to ``load_...
    (id)`` to retrieve a single object at a time.
    ...
    Using context managers ensures that the internal connection to the database is properly opened and closed.

    .. note::
        Database classes should be given a version by extending the ``_VERSION`` dictionary. The keys in this dictionary can be any unique name corresponding to the database class, but it should be the class name. If non-optional columns/tables are added, the main version number should be increased. If optional columns are added, one only needs to increase the minor version.
    ...
    :cvar _VERSION: used to check if the version of this class is compatible with the database file. The version info is written into the sqlite files when a new database is created. This dict may be extended by child classes. That way each class in a hierarchy can have a separate version.
    :cvar _TABLES: defines the tables of the database. The keys of the dictionary are the table names, the values contains a list of tuples, where each tuple corresponds to a column. Its first element contains the column name, the second the sql column type.
    :type _TABKES: dict
    """
    ...
    def __connect(self):
        ...
        if self._mode == DBOpenMode.WRITE:
            ...
            raise NotImplementedError(f"Cannot open database with mode {self._mode}")
    ...
    def __create(self):
        ...
        with con:
            ...
            for cls_name, version in self._VERSION.items():
                con.execute("INSERT INTO db_info VALUES(?,?)", (cls_name, version))
            ...
            try:
                ...
                if file_version[0] != class_version[0]:
                    raise IntegrityError(
                        ...
                        f"{file_version[0]}.{file_version[1]} while the version"
                        f" of class {cls} is {class_version[0]}."
                        f"{class_version[1]}."
                    )
                ...
                raise DatabaseError(
                    "The db_info table of the database may " "be corrupted."
                ) from e
    ...
    def __check_tables(self):
        ...
        """
        expected_sql = self._sql_table_schema()
        actual_sql = {
            name: sql
            for name, sql in self._con.execute("SELECT name, sql " "FROM sqlite_master")
        }
        ...
        for table, row_definitions in self._TABLES.items():
            # check that all tables are there (it is ok, if there are additional
            # tables in the file)
            ...
            for expected_name, expected_type_constr in row_definitions:
                ...
                if expected_name not in cols:
                    raise IntegrityError(
                        f"Table {table} is missing expected " f"column {expected_name}."
                    )
    ...
    def __getattribute__(self, __name: str) -> Any:
        ...
        if __name.startswith("save") and self._mode == DBOpenMode.READ:
            raise DBOpenModeError(
                "Cannot save object, because database was " "opened in read-only mode."
            )
