...
class Failure:
    ...
    def __str__(self) -> str:
        if not hasattr(self, 'msg'):
            raise ProgrammingError('msg attribute for failure not set. This can'
...
class Submittable(HasLogger, HasState, HasId):
    ...
    class Result:
        ...
        reason: Failure = field(init=False, default=None)
        """reason for failure. Usually set by the `fail()` method of the submittable
        """
        submittable_id: int = field(init=False, default=None)
        """id of the submittable. Set automatically when result or id is
        assigned to the submittable
        """
        submittable_type: type = field(init=False, default=None)
        """type of the submittable. Set automatically when result is assigned to
        the submittable
        """
    ...
    def __init__(self):
        super().__init__()
        self.__result: Submittable.Result = None

    @property
    def result(self) -> Submittable.Result:
        return self.__result

    @result.setter
    def result(self, result: Submittable.Result):
    ...
    def __set_failure_reason(self, reason: str|Failure|Exception):
        ...
        if self.result is None:
            # self.Result may be overridden with a class containing non-default fields
            kwargs = {}
            for field in fields(self.Result):
                # check if field has default and is in constructor
