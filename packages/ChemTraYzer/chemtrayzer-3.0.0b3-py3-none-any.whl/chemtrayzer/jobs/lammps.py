...
class Lammps(Program):
    ...
    def geometry2input(self, geometry: Geometry, path: Union[str, PathLike],
        ...
        def gen_atoms_section(id_map: Dict[Element, int],
                              charges:Optional[Iterable[float]]=None) -> str:
            atoms_str = 'Atoms\n\n'
            if charges is None:
                charges = np.zeros(geometry.n_atoms)
            for i, (elem, q, (x, y, z)) in enumerate(
                    zip(geometry.atom_types, charges, geometry.coords),
                    start=1):
                atoms_str += f'{i:3d} {id_map[elem]:2d} {q:.2f} '\
...
class LammpsBondOrderParser:
    ...
    class Result:
        ...
        steps: np.ndarray
        '''n_steps long array containg the timestep numbers for which data is
    ...
    def __init__(self, path: Union[str, PathLike], skip: int = 0) -> None:
        self.path = path
        self.skip = skip
    ...
    def parse(self, n_steps) -> Result:
        ...
        with open(self.path, 'r', encoding='utf-8') as file:
            ...
            # allocate memory
            result = self.Result(n_steps=n_steps,
                steps=np.zeros(n_steps),
                bond_orders=[dict() for i in range(n_steps)],
            ...
            if result.n_steps != n_steps:
                ...
                result.total_bond_orders.resize((n_steps, n_atoms), refcheck=False)
    ...
    def _skip(self, file: TextIOBase):
    ...
    def _read_step(self, file: TextIOBase) \
        ...
        for line in file:
            ...
            if line.startswith('# Timestep'):
                if step is not None:    # sanity check
                    raise LammpsParsingError('unexpected timestep definition '
                        'encountered.')
                ...
                # sanity checks
                if step is None:
                    raise LammpsParsingError('Number of particles encountered '
                        'before timesteps')
            ...
            # data line
                ...
                # sanity checks
                if step is None:
                    raise LammpsParsingError('Non-commented line encountered '
    ...
    def _fill_result_with_table(self, step, n_atoms, table, result: Result):
        for i in range(n_atoms):
            ...
            for j in range(n_BOs):
                # atom ids connected to atom_id start at postion 3 and bond
...
class LammpsDumpCustomParser:
    ...
    .. note::
        ...
        LAMMPS uses one-based indexing for atoms. If your custom styles included
        the atom ids, they will be one-based in the result returned by this
        parser.
        ...
        try:
            with open(self.path, 'r', encoding='utf-8') as file:
                ...
                for step in range(1, n_steps):
                    ...
                    # sanity checks
                    if n_atoms_curr != n_atoms or pbe_curr != pbe or\
                            labels_curr != labels:
                        raise LammpsParsingError('Assumption violated: '
                            f'{self.__class__.__name__} assumes that the number'
                            ' of steps, the periodic boundary conditions and '
                            'the data labels do not change in the trajectory')
            ...
            raise LammpsParsingError(
                f'An error occured while parsing {self.path}') from err
...
class LammpsReaxFFJob(MDJob):
    ...
    .. note::
        ...
        remebers the job's id. When ``parse()`` is called on this parser object
        later, the parser gets the relevant data from the job system using the
        job's id and constructs a trajectory object from the data in the job
        directory.

    :param metadata: details of the job, that should be submitted
    :param intial_geometry: simulation box of the first frame. The coordinates
                            here are absolute, i.e. **not** relative to
                            metadata.box_origin
    :param reaxff_path: path to a ReaxFF parameter file.
    :param lammps: LAMMPS software
    :ivar result: result set by ``parse_result``. If successful, it will contain
                  a dictionary with the key "trajectory_parser" and a
...
pair_coeff      * * ${REAXFF_FILE} ${atoms_in_order}
    # perform charge equilibration (QEq) and read the respective parameters from
    # the reaxff file
...
compute         ape all pe/atom
    # write id, type, coordinates, velocities and energy per atom energy in file
...
'''
    ...
    def __init__(self, *, metadata: MDMetadata, initial_geometry: Geometry,
        ...
        if metadata.box_type != BoxType.ORTHOGONAL:
            raise NotImplementedError('Non-orthogonal boxes not supported')
        if isinstance(metadata.thermostat, NoseHooverTStat)\
                and metadata.integration_method != MDIntegrator.VELOCITY_VERLET:
            raise ValueError('Invalid integration scheme for '
                'Nosé-Hoover thermostat')
        if isinstance(metadata.thermostat, BerendsenTStat)\
                and metadata.integration_method != MDIntegrator.VELOCITY_VERLET:
            raise ValueError('Invalid integration scheme for '
    ...
    def thermostat_str(self) -> str:
        ...
        if self.metadata.seed is not None:
            ...
            thermo_str += 'velocity        all create '\
                        + str(self.metadata.temperature) + ' '\
                        + str(self.metadata.seed) + ' mom yes rot yes\n'

        T = self.metadata.temperature
        tau = self.metadata.thermostat.tau

        if isinstance(self.metadata.thermostat, NoseHooverTStat):
            # check the integration scheme
            if self.metadata.integration_method != MDIntegrator.VELOCITY_VERLET:
                raise ValueError('Invalid integration scheme for '
                    'Nosé-Hoover thermostat')
...
'''
    ...
    def atoms_in_order(self) -> str:
        ...
        sorted_elems = sorted(set(self.initial_geometry.atom_types),
                              key=extract_atomic_nr)
...
class LammpsReaxFFJobFactory(MDJobFactory):
    ...
    def __init__(self, *,
            reaxff_path: Union[str, os.PathLike], lammps: Lammps,
            n_cpus: int, n_tasks: int, memory: Memory,
    ...
    def create(self,
        ...
        """
        super().create(metadata=metadata, initial_geometry=initial_geometry,
                       name=name)

        return LammpsReaxFFJob(metadata=metadata,
                               initial_geometry=initial_geometry,
                               reaxff_path=self.reaxff_path, lammps=self.lammps,
