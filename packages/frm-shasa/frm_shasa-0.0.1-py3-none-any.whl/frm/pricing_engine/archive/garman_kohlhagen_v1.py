...
def garman_kohlhagen(S: float, 
    ...
    :param task: task to perform; 'px' (default), 'strike', 'solve_implied_vol'
    ...
    # For numerical solving, need to allow negative vol
    # assert (σ > 0.0).all(), σ
        ...
        if Δ_convention in {'regular_spot_Δ','regular_forward_Δ'}:
            ...
            if bool_cond.any(): 
                ...
                if solution.converged:
                    ...
                    raise ValueError('the numerical solver for the premium adjusted strike did not converge')
...
def gk_solve_implied_σ(X: float, S: float, K: float, r_d: float, r_f: float, tau: float, cp: int, σ_guess: float) -> float:
    """
