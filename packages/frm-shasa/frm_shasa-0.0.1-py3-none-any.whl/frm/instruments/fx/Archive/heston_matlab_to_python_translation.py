...
@jit(nopython=True)
def HestonVanillaInt(phi, m, s, k, v0, vv, rd, rf, tau, kappa, theta, lambda_, rho):
    a = kappa * theta
    u = [0.5, -0.5]
...
def HestonVanilla(cp, s, k, v0, vv, rd, rf, tau, kappa, theta, lambda_, rho):
    ...
    P1 = 0.5 + 1/np.pi * quad(lambda phi: HestonVanillaInt(phi, 1, s, k, v0, vv, rd, rf, tau, kappa, theta, lambda_, rho), 0, np.inf, epsrel=1e-8)[0]
    P2 = 0.5 + 1/np.pi * quad(lambda phi: HestonVanillaInt(phi, 2, s, k, v0, vv, rd, rf, tau, kappa, theta, lambda_, rho), 0, np.inf, epsrel=1e-8)[0]
...
def HestonVanillaFitSmile(delta, marketvols, spot, rd, rf, tau, cp):
    ...
    def HestonVanillaSSE(param, cp, spot, strikes, v0, marketvols, rd, rf, tau, kappa):        
        ...
        for i in range(nostrikes):
            ...
            IV[i] = ImplVolFX(P[i], spot, strikes[i], rd, rf, tau, cp, marketvols[i])
        return np.sum((marketvols - IV)**2)

    def ImplVolFX(X, S, K, rd, rf, tau, cp, σ_guess):
        try:
            # Try Netwon's method first
            return newton(lambda σ: (garman_kohlhagen(S=spot,σ=σ,r_f=rf,r_d=rd,tau=tau,cp=cp,K=K,Δ=delta,task='px') - X), x0=σ_guess, tol=1e-4, maxiter=50)
        except RuntimeError:
            # Fallback to Brent's method
            return root_scalar(lambda σ: (garman_kohlhagen(S=spot,σ=σ,r_f=rf,r_d=rd,tau=tau,cp=cp,K=K,Δ=delta,task='px') - X), bracket=[0.001, 2], method='brentq').root
