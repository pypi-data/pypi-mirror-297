...
class FXVolatilitySurface:
    ...
    def __post_init__(self, day_count_basis):
        ...
        if self.Δ_σ_pillar is not None:
            ...
            for col in self.Δ_σ_pillar.columns:
                if self.Δ_σ_pillar[col].apply(isinstance, args=(float,)).all():
                    self.Δ_σ_pillar[col] = pd.to_numeric(self.Δ_σ_pillar[col])
            if 'tenor_date' not in self.Δ_σ_pillar.columns:
                result = calc_tenor_date(self.curve_date, self.Δ_σ_pillar['tenor_name'], self.curve_ccy, holiday_calendar=holiday_calendar)
                holiday_rolled_offset_date, cleaned_tenor_name, spot_date = result            
                self.Δ_σ_pillar['tenor_date'] = holiday_rolled_offset_date
                self.Δ_σ_pillar['tenor_name'] = cleaned_tenor_name
            self.Δ_σ_pillar.set_index('tenor_date', inplace=True,drop=True)       
            self.Δ_σ_pillar.insert(loc=0, column='tenor_years', value=self.daycounter.year_fraction(self.curve_date, self.Δ_σ_pillar.index).values)                    

            cols = [v for v in self.Δ_σ_pillar.columns if v not in ['tenor_date','tenor_name','tenor_years','Δ_convention']]
            for i,v in enumerate(cols):
                pattern = r'^σ_(\d{1,2})Δ(call|put)$'  # σ_5Δput, σ_5Δcall, σ_10Δput, σ_10Δcall, ...
                allow_list = ['σ_atmΔneutral']
                if (re.match(pattern, v) and 1 <= int(re.match(pattern, v).group(1)) <= 99) or v in allow_list:   
                    pass
                else:
                    msg = 'User added column' + "'" + v + "'" + ' does not ' \
                        + 'match regex pattern ' + "'" + pattern + "'" \
                        + ' and is not in the allowed list (' + ', '.join(allow_list) + ').'
                    raise ValueError(msg)
            self.Δ_σ_pillar = self.__interp_daily_FXF_and_IR_rates(self.Δ_σ_pillar.copy())
            ...
            if True:                
                ...
                self.Δ_σ_pillar_input_Δ_convention = self.Δ_σ_pillar.copy()
                Δ_K_pillar = self.Δ_σ_pillar.copy()
                Δ_K_pillar.loc[:, Δ_K_pillar.columns.str.contains('σ')] = np.nan
                Δ_K_pillar.rename(columns=lambda x: x.replace('σ', 'k'), inplace=True)
    
                helper_cols = ['tenor_date','tenor_name','tenor_years','fx_forward_rate','foreign_ccy_simple_interest_rate','domestic_ccy_simple_interest_rate','Δ_convention']
                dict_K_σ = {v.replace('σ', 'k'): v for v in self.Δ_σ_pillar.columns if v not in helper_cols}
                for date, row in Δ_K_pillar.iterrows():
                    # Scalars
                    S = self.fx_spot_rate
                    r_f=row['foreign_ccy_simple_interest_rate']
                    r_d=row['domestic_ccy_simple_interest_rate']
                    tau=row['tenor_years']                
                    ...
                    σ = self.Δ_σ_pillar.loc[date, dict_K_σ.values()].values
                                        
                    K = gk_solve_strike(S=S,tau=tau,r_f=r_f,r_d=r_d,σ=σ,Δ=Δ,Δ_convention=Δ_convention,F=F)
                    Δ_K_pillar.loc[date,dict_K_σ.keys()] = K
                    
                # Given the ATM Δ Neutral formula doesn't include a discount factor
                # This conversion, results in a kinked curve. 
                # Hence will ignore for now                     
                #     if Δ_convention in {'regular_spot_Δ','premium_adjusted_spot_Δ'}:
                #         def convert_spot_σ_to_forward_σ(σ, S, r_f, r_d, tau, Δ, Δ_convention, F, K_target):
                #             K = gk_solve_strike(S=S,tau=tau,r_f=r_f,r_d=r_d,σ=σ,Δ=Δ,Δ_convention=Δ_convention,F=F)
                #             return K - K_target
                        
                #         for i,v in enumerate(dict_K_σ.values()):
                #             K_target = K[i]
                #             Δ_target = Δ[i]
                #             σ_init = σ[i]
                #             forward_Δ_convention = Δ_convention.replace('spot','forward')
                #             solution = root_scalar(convert_spot_σ_to_forward_σ, args=(S, r_f, r_d, tau, Δ_target, forward_Δ_convention, F, K_target), x0=σ_init ,bracket=[0.00001, 2.0*σ_init])
                
                #             if solution.converged:
                #                 self.Δ_σ_pillar.loc[date,v] = solution.root
                #                 self.Δ_σ_pillar.loc[date,'Δ_convention'] = forward_Δ_convention
                #             else:
                #                 raise ValueError('the numerical solver for the σ did not converge')
                            
                # Δ_K_pillar['Δ_convention'] == self.Δ_σ_pillar['Δ_convention']
                self.Δ_K_pillar = Δ_K_pillar

            ###################################################################
            # Interpolate the term structure of volatility to get a daily volatility smile            
            dates = pd.date_range(min(self.Δ_σ_pillar.index),max(self.Δ_σ_pillar.index),freq='d')
            ...
            df_pillar = self.Δ_σ_pillar.copy()
    ...
    def __interp_daily_FXF_and_IR_rates(self, df):
        ...
        df.insert(loc=1+offset, column='foreign_ccy_simple_interest_rate', value=np.nan)
        df.insert(loc=2+offset, column='domestic_ccy_simple_interest_rate', value=np.nan)
    ...
    def fit_and_plot_smile(self):
        ...
        i = 0
        for date, row in self.Δ_σ_pillar.iterrows():
            ...
            # Arrays
            Δ = [convert_colunm_name_to_delta(c) for c in self.Δ_σ_pillar.columns if c[0] == 'σ']
            Δ_for_plt = [convert_colunm_name_to_delta_for_plt(c) for c in self.Δ_σ_pillar.columns if c[0] == 'σ']
            cp = np.sign(Δ)
            σ = row[[c for c in self.Δ_σ_pillar.columns if c[0] == 'σ']].values             
