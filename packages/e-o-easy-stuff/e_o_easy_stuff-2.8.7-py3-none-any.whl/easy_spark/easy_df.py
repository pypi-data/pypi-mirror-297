...
class EasyDF:
    ...
    def add_hash_column_from_columns(self, column_names: list[str], hash_column_name: str) -> 'EasyDF':
    ...
    def add_missing_columns_based_on_schema(self, schema: StructType) -> 'EasyDF':
        new_schema = schema if schema is not None else self.current_schema
        for field in new_schema.fields:
            if field.name not in self._df.columns:
                self._df = self._df.withColumn(field.name, lit(None).cast(field.dataType))
        return self

    def remove_columns_based_on_schema(self, schema: StructType) -> 'EasyDF':
        new_schema = schema if schema is not None else self.current_schema
        for coloumn in self._df.columns:
            if coloumn not in new_schema.names:
                self._df = self._df.drop(coloumn)

        return self

    def distinct(self, column_names: list[str] = None) -> 'EasyDF':
        if column_names:
            self._df = self._df.distinct(column_names)
        else:
    ...
    def save_from_table_path(self, path: TablePath, df_format="delta", mode="overwrite",
        ...
        return self.save_from_path(path.path, df_format, mode, merge_option)
    ...
    def save_from_path(self, path: str, df_format="delta", mode="overwrite",
                       merge_option: str = "overwriteSchema", add_audit_fields=False) -> 'EasyDF':
