...
import pytest
from drt_sdk_core import (Address, Message, MessageComputer,
                                 Transaction, TransactionComputer)

from drt_sdk_wallet.crypto.randomness import Randomness
from drt_sdk_wallet.user_keys import UserSecretKey
...
def test_user_signer_from_pem_file():
    pubkey = UserSigner.from_pem_file(Path("./drt_sdk_wallet/testdata/alice.pem"), 0).get_pubkey()
    assert Address(pubkey.buffer, "moa").to_bech32() == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"

    pubkey = UserSigner.from_pem_file(Path("./drt_sdk_wallet/testdata/bob.pem"), 0).get_pubkey()
    assert Address(pubkey.buffer, "moa").to_bech32() == "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk"

    pubkey = UserSigner.from_pem_file(Path("./drt_sdk_wallet/testdata/carol.pem"), 0).get_pubkey()
    assert Address(pubkey.buffer, "moa").to_bech32() == "moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h"
...
def test_user_wallet_to_keyfile_object_using_known_test_wallets_with_their_randomness():
    ...
    alice_saved_path = Path("./drt_sdk_wallet/testdata/alice.saved.json")
    bob_saved_path = Path("./drt_sdk_wallet/testdata/bob.saved.json")
    carol_saved_path = Path("./drt_sdk_wallet/testdata/carol.saved.json")

    alice_wallet.save(alice_saved_path, "moa")
    bob_wallet.save(bob_saved_path, "moa")
    carol_wallet.save(carol_saved_path, "moa")

    assert alice_saved_path.read_text().strip() == Path("./drt_sdk_wallet/testdata/alice.json").read_text().strip()
    assert bob_saved_path.read_text().strip() == Path("./drt_sdk_wallet/testdata/bob.json").read_text().strip()
    assert carol_saved_path.read_text().strip() == Path("./drt_sdk_wallet/testdata/carol.json").read_text().strip()
...
def test_user_wallet_encrypt_then_decrypt():
    ...
    alice_keyfile_object = alice_wallet.to_dict("moa")
    ...
    bob_keyfile_object = bob_wallet.to_dict("moa")
    ...
    carol_keyfile_object = carol_wallet.to_dict("moa")
...
def test_sign_transaction():
    """
    Also see: https://github.com/DharitriOne/drt-chain-go/blob/master/examples/construction_test.go
    """
    ...
    signer = UserSigner.from_pem_file(Path("./drt_sdk_wallet/testdata/alice.pem"))
    verifier = UserVerifier.from_address(Address.new_from_bech32("moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"))
    transaction_computer = TransactionComputer()

    tx.signature = signer.sign(transaction_computer.compute_bytes_for_signing(tx))
    assert tx.signature.hex() == "686bcb4057948d1a6109ea27c350807855b7ef874cd6ca0ab353acaec5bfa2c8b42aa4755cd2316c54cbc82d440ca16c4d87b878f5283371be93a5adf838630b"
...
def test_user_pem_save():
    path = Path("./drt_sdk_wallet/testdata/alice.pem")
...
def test_load_secret_key_but_without_kind_field():
    keystore_path = Path("./drt_sdk_wallet/testdata/withoutKind.json")
    secret_key = UserWallet.load_secret_key(keystore_path, "password")
    actual_address = (secret_key.generate_public_key().to_address("moa")).to_bech32()
    assert actual_address == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"


def test_load_secret_key_with_unecessary_address_index():
    keystore_path = Path("./drt_sdk_wallet/testdata/alice.json")
...
def test_create_keystore_with_mnemonic_with_randomness():
    expected_dummy_wallet_json = Path("./drt_sdk_wallet/testdata/withDummyMnemonic.json").read_text()
...
def test_load_secret_key_with_mnemonic():
    keystore_path = Path("./drt_sdk_wallet/testdata/withDummyMnemonic.json")

    assert UserWallet.load_secret_key(keystore_path, "password", 1).generate_public_key().to_address("moa").to_bech32() == "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk"
    assert UserWallet.load_secret_key(keystore_path, "password", 2).generate_public_key().to_address("moa").to_bech32() == "moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h"
    assert UserWallet.load_secret_key(keystore_path, "password", 0).generate_public_key().to_address("moa").to_bech32() == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"
