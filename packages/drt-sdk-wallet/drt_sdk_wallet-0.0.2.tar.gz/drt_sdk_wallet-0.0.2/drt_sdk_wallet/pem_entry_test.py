...
from drt_sdk_wallet.constants import USER_SEED_LENGTH
from drt_sdk_wallet.pem_entry import PemEntry


def test_from_text_all():
    text = Path("./drt_sdk_wallet/testdata/alice.pem").read_text()
    entries = PemEntry.from_text_all(text)
    entry = entries[0]
    assert len(entries) == 1
    assert entry.label == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"
    assert entry.message[0:USER_SEED_LENGTH].hex() == "413f42575f7f26fad3317a778771212fdb80245850981e48b58a4f25e344e8f9"

    text = Path("./drt_sdk_wallet/testdata/multipleUserKeys.pem").read_text()
    entries = PemEntry.from_text_all(text)
    entry = entries[0]
    ...
    entry = entries[1]
    assert entry.label == "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk"
    assert entry.message[0:USER_SEED_LENGTH].hex() == "b8ca6f8203fb4b545a8e83c5384da033c415db155b53fb5b8eba7ff5a039d639"

    entry = entries[2]
    assert entry.label == "moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h"
...
def test_from_text_all_for_validators():
    text = Path("./drt_sdk_wallet/testdata/validatorKey00.pem").read_text()
    ...
    text = Path("./drt_sdk_wallet/testdata/multipleValidatorKeys.pem").read_text()
    entries = PemEntry.from_text_all(text)
