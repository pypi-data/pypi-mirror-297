...
class CertoraBuildGenerator:
    ...
    def _handle_optimize(self, contract_file_path: str, settings_dict: Dict[str, Any],
        ...
        if self.context.solc_optimize_map:
            ...
            if not self.context.strict_solc_optimizer and (
                ...
                solc0_8_26_to_0_8_27 = "dhfoDgvulfnTUtnIfxa[r]EscLMVcul[j]Trpeulxa[r]cLCTUca[r]LSsTFOtfDnca[r]" + \
                ...
                elif minor == 8 and 26 <= patch <= 27:
                    yul_optimizer_steps = solc0_8_26_to_0_8_27
                assert yul_optimizer_steps is not None, 'Yul Optimizer steps missing for requested Solidity version. Please contact Certora team.'
    ...
    def get_contract_in_sdc(self,
        ...
        storage_layout = self.collect_storage_layout(contract_data)
    ...
    def handle_contract_extensions(self):
        ...
        for extended, extending in self.context.contract_extensions.items():
            ...
            for ext in extending:
                ...
                if extension not in contracts_by_name.keys():
                    raise Util.CertoraUserInputError(f"Can't find extending contract {extension} for contract extension")
                extension_contract = contracts_by_name[extension]
                for f in ext["exclude"]:
                    if extension_contract.find_method(f) is None:
                        raise Util.CertoraUserInputError(f"Can't find a method named {f} in contract {extension_contract.name}")
