...
from fastdev.utils.tensor_utils import to_numpy, to_torch
...
try:
    ...
    from fastdev.robo.curobo_utils import CustomCudaRobotModel, CustomCudaRobotModelConfig
...
class Mesh(Geometry):
    ...
    vertices: Optional[np.ndarray] = None  # un-scaled vertices
...
def _build_joint_transform(joint: Joint, joint_value: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def __post_init__(self):
        ...
        if self.enable_curobo:
            if not CUROBO_AVAILABLE:
                raise ImportError("Curobo is not available.")
            if self.device == "cpu":
                raise ValueError("Curobo model can only be loaded on CUDA device.")
            if self.format != "urdf":
                raise ValueError("Curobo model can only be loaded from URDF file.")

            if len(self.curobo_spheres) != 0:
                if not all(isinstance(k, str) and isinstance(v, list) for k, v in self.curobo_spheres.items()):
                    raise ValueError("Invalid curobo_spheres format.")


class RobotModel:
    """Robot model. Slighly faster than `pytorch_kinematics`.
    ...
    def __init__(self, config: RobotModelConfig):
        ...
        self._has_mimic_joints: bool = any(
        ...
        if self._has_mimic_joints:
            ...
            self._mimic_joint_indices: Optional[torch.Tensor] = torch.tensor(
                mimic_joint_indices, device=self.device, dtype=torch.long
            )
            self._mimic_multipliers: Optional[torch.Tensor] = torch.tensor(
                mimic_multipliers, device=self.device, dtype=torch.float32
            )
            self._mimic_offsets: Optional[torch.Tensor] = torch.tensor(
                mimic_offsets, device=self.device, dtype=torch.float32
            )
        else:
            self.mimic_joint_names = []
            self._mimic_joint_indices = None
            self._mimic_multipliers = None
            self._mimic_offsets = None
        ...
        # lazy initialization for some tensors
        self._joint_limits_pt: Optional[torch.Tensor] = None
        self._joint_axes: Optional[torch.Tensor] = None
        self._joint_origins: Optional[torch.Tensor] = None
        self._joint_types: Optional[torch.Tensor] = None
        self._link_joint_indices: Optional[torch.Tensor] = None
        self._link_joint_types: Optional[torch.Tensor] = None
        self._link_joint_origins: Optional[torch.Tensor] = None
        self._link_indices_topological_order: Optional[torch.Tensor] = None
        self._parent_link_indices: Optional[torch.Tensor] = None

        # load curobo model if enabled
        if self.config.enable_curobo:
            if not CUROBO_AVAILABLE:
                raise ImportError("Curobo is not available.")
            cfg_dict: dict[str, Any] = {
                "urdf_path": os.path.abspath(self.config.urdf_or_mjcf_path),
                "base_link": self.base_link_name,
                "ee_link": self.ee_link_names[0],
                "link_names": self.link_names,
            }
            if len(self.config.curobo_spheres) > 0:
                collision_link_names = list(self.config.curobo_spheres.keys())
                cfg_dict.update(
                    {
                        "collision_spheres": self.config.curobo_spheres,
                        "collision_link_names": collision_link_names,
                        "collision_sphere_buffer": 0.0,
                        "self_collision_buffer": {name: 0.0 for name in collision_link_names},
                        "self_collision_ignore": {
                            name: collision_link_names for name in collision_link_names
                        },  # ignore all
                    }
                )

            tensor_args = TensorDeviceType(device=self.device)  # type: ignore
            cuda_robot_model_cfg = CustomCudaRobotModelConfig.from_config(  # type: ignore
                CudaRobotGeneratorConfig(**cfg_dict, tensor_args=tensor_args)  # type: ignore
            )
            self._curobo_model: Optional[CustomCudaRobotModel] = CustomCudaRobotModel(cuda_robot_model_cfg)  # type: ignore
            if self._curobo_model.all_articulated_joint_names != self.active_joint_names:
                raise ValueError("Active joint names do not match between RobotModel and CudaRobotModel.")
            if self._curobo_model.link_names != self.link_names:
                raise ValueError("Link names do not match between RobotModel and CudaRobotModel.")
        else:
            self._curobo_model = None
    ...
    def link_joint_indices(self) -> torch.Tensor:
        if self._link_joint_indices is None:
            ...
            self._link_joint_indices = torch.tensor(link_joint_indices, device=self.device, dtype=torch.int32)
        return self._link_joint_indices

    @property
    def link_joint_types(self) -> torch.Tensor:
        if self._link_joint_types is None:
            link_joint_types = [
                self.joint_map[self.link_map[link_name].joint_name].type.value for link_name in self.link_names
            ]
            self._link_joint_types = torch.tensor(link_joint_types, device=self.device, dtype=torch.int32)
        return self._link_joint_types

    @property
    def link_joint_origins(self) -> torch.Tensor:
        if self._link_joint_origins is None:
            link_joint_origins = [
                self.joint_map[self.link_map[link_name].joint_name].origin for link_name in self.link_names
            ]
            link_joint_origins = np.stack(link_joint_origins, axis=0)
            self._link_joint_origins = torch.from_numpy(link_joint_origins).to(device=self.device, dtype=torch.float32)
        return self._link_joint_origins

    @property
    def link_indices_topological_order(self) -> torch.Tensor:
        if self._link_indices_topological_order is None:
            link_indices = [self.link_names.index(link_name) for link_name in self._link_names_topological_order]
            self._link_indices_topological_order = torch.tensor(link_indices, device=self.device, dtype=torch.int32)
        return self._link_indices_topological_order

    @property
    def parent_link_indices(self) -> torch.Tensor:
        if self._parent_link_indices is None:
            parent_link_indices = []
            for link_name in self.link_names:
            ...
            self._parent_link_indices = torch.tensor(parent_link_indices, device=self.device, dtype=torch.int32)
        return self._parent_link_indices

    def _apply_mimic_joints(
        self, joint_values: Float[torch.Tensor, "b num_dofs"]
    ) -> Float[torch.Tensor, "b num_joints"]:
        if not self._has_mimic_joints:
            return joint_values
        else:
            mimic_joint_values = torch.index_select(joint_values, dim=-1, index=self._mimic_joint_indices)  # type: ignore
            mimic_joint_values = mimic_joint_values * self._mimic_multipliers + self._mimic_offsets
            joint_values = torch.cat([joint_values, mimic_joint_values], dim=-1)
            return joint_values

    def forward_kinematics_curobo(
        self,
        joint_values: Float[torch.Tensor, "b num_dofs"],
        joint_names: Optional[List[str]] = None,
        clamp_joint_values: bool = True,
    ):
        """Forward kinematics using curobo model. Much faster than `forward_kinematics`.

        Returns:
            tuple[Float[torch.Tensor, "b num_links 3"], Float[torch.Tensor, "b num_links 4"]]: links_position, links_quaternion (wxyz)

        .. note::
            The gradients of `links_quaternion` are different from `forward_kinematics`, do not use it for optimization.
        """
        if self._curobo_model is None:
            raise ValueError("Curobo model is not loaded.")
        ...
        if clamp_joint_values:
            joint_limits = self.get_joint_limits(return_tensors="pt")
            if joint_limits is not None:
                joint_values = torch.clamp(joint_values, joint_limits[:, 0], joint_limits[:, 1])

        state = self._curobo_model.get_state(joint_values)
        return state
        # return state.links_position, state.links_quaternion
    ...
    def forward_kinematics(
        ...
        if self._has_mimic_joints:
            joint_values = self._apply_mimic_joints(joint_values)
        if use_warp:
    ...
    def to(self, device: Device) -> "RobotModel":
        ...
        self._joint_axes = None
        self._joint_origins = None
        self._joint_types = None
        self._link_joint_indices = None
        self._link_joint_types = None
        self._link_joint_origins = None
        self._link_indices_topological_order = None
        self._parent_link_indices = None
        if self._curobo_model is not None:
            raise NotImplementedError("Curobo model does not support device change.")
        return self
