...
class Report:
    ...
    def annual_pf_values_to_csv(self, output_dir="", file_name: str = "annual_pf_values.csv"):
        ...
        annual_pf_values = returns_to_pf_values(self.dt_pf_returns)

        annual_pf_values.sort_index(ascending=False).to_frame().T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="year"
        )
    ...
    def daily_returns_to_csv(self, lookback: int = 30, output_dir="", file_name: str = "daily_returns.csv"):
        ...
        self.daily_returns.iloc[-lookback:].sort_index(ascending=False).to_frame().T.to_csv(
    ...
    def daily_pf_values_to_csv(self, lookback: int = 30, output_dir="", file_name: str = "daily_pf_values.csv"):
        ...
        daily_pf_values.iloc[-lookback:].sort_index(ascending=False).to_frame().T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="date"
        )
    ...
    def monthly_pf_values_to_csv(self, lookback: int = 12, output_dir="", file_name: str = "monthly_pf_values.csv"):
        ...
        monthly_pf_values = returns_to_pf_values(self.dt_pf_returns)

        monthly_pf_values.iloc[-lookback:].sort_index(ascending=False).to_frame().T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="month"
        )
    ...
    def quarterly_pf_values_to_csv(self, lookback: int = 8, output_dir="", file_name: str = "quarterly_pf_values.csv"):
        ...
        quarterly_pf_values = returns_to_pf_values(self.dt_pf_returns)

        quarterly_pf_values.iloc[-lookback:].sort_index(ascending=False).to_frame().T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="quarter"
        )
...
class ReportComparison(object):
    ...
    def get_benchmark_metrics(self):
        ...
        return pd.DataFrame(
            [self.batting_averages, self.pearson_correlations, self.alphas, self.betas],
            index=["batting_average", "pearson_correlation", "alpha", "beta"],
        ).T
    
    def benchmark_daily_returns_to_csv(self, lookback=30, output_dir="", file_name: str = "benchmark_daily_returns.csv"):
        os.makedirs(os.path.join(self.log_dir, output_dir), exist_ok=True)

        pd.DataFrame(self.get_benchmark_daily_returns(lookback=lookback), index=[self.portfolio_name] + [x for x in self.benchmarks.keys()]).T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="date"
        )
    ...
    def benchmark_daily_pf_values_to_csv(self, lookback=30, output_dir="", file_name: str = "benchmark_daily_pf_values.csv"):
        ...
        daily_return = self.get_benchmark_daily_returns(lookback=lookback)
        daily_pf_values = [returns_to_pf_values(pd.Series(x)) for x in daily_return]

        pd.DataFrame(daily_pf_values, index=[self.portfolio_name] + [x for x in self.benchmarks.keys()]).T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="date"
        )

    def get_benchmark_daily_returns(self, lookback=30):
        """Provides daily returns for the portfolio and benchmarks in recent days

        Args:
            lookback (int): number of days to show for daily returns. Default is 30.
        """
        ...
        for report in benchmarks:
            daily_ret = report.daily_returns.iloc[-lookback:].to_dict()
    ...
    def benchmark_monthly_returns_to_csv(self, lookback=5, output_dir="", file_name: str = "benchmark_monthly_returns.csv"):
        ...
        pd.DataFrame(self.get_benchmark_monthly_returns(lookback=lookback), index=["Portfolio"] + [x for x in self.benchmarks.keys()]).T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="month"
        )
    ...
    def benchmark_monthly_pf_values_to_csv(self, lookback=5, output_dir="", file_name: str = "benchmark_monthly_pf_values.csv"):
        ...
        monthly_return = self.get_benchmark_monthly_returns(lookback=lookback)
        monthly_pf_values = [returns_to_pf_values(pd.Series(x)) for x in monthly_return]

        pd.DataFrame(monthly_pf_values, index=["Portfolio"] + [x for x in self.benchmarks.keys()]).T.to_csv(
            os.path.join(self.log_dir, output_dir, file_name), index_label="month")

    def get_benchmark_monthly_returns(self, lookback=5):
        ...
        for report in benchmarks:
            monthly_ret = report.monthly_returns.iloc[-lookback:].to_dict()
    ...
    def get_benchmark_annual_returns(self):
        ...
        Args:
            lookback (int): number of years to show for annual returns. Default is 5.
        """
