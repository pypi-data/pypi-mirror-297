...
def selection_reading(file_select,option):
    ...
    df = pd.read_csv(file_select, usecols=[i])
...
def cif_remove_process(filename,option,select_list):
    ...
    if option == "2":
        filename_new = f'{newname}_NoDiesel.cif'
    elif option == "3":
        filename_new = f'{newname}_TIPLOC.cif'
    elif option == "4":
        filename_new = f'{newname}_TOC.cif'
    elif option == "5":
        filename_new = f'{newname}_Train.cif'
    else:
...
if __name__ == "__main__":
    ...
    text_input = "cifexample.txt"  # Adjust as needed
