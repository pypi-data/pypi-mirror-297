...
NullableDateInterval = list[str | None]


def validate_nullable_number_interval(v: list[int | float | None]) -> None:
...
def validate_nullable_date_interval(v: list[str | None]) -> None:
    """Validate the input value."""
    if not isinstance(v, list):
        raise TypeError("List required")
...
class Model(msgspec.Struct):
    ...
    def validate(self) -> None:
        ...
        msgspec.json.decode(msgspec.json.encode(self), type=SelfModel)
...
class MappingJob(Model, omit_defaults=True, kw_only=True):
    ...
    @classmethod
    def required_for_pool(cls, v: list | None, values: dict) -> None:
...
class Filter(Model, omit_defaults=True, kw_only=True):
    ...
    @classmethod
    def required_for_some_security_type_2(cls, v: list, values: dict) -> None:
        """Validate the expiration field."""
        if (
            "security_type_2" in values
            and values["security_type_2"] in ["Option", "Warrant"]
