...
Options:
  ...
  --timeout=x or -t x: set a timeout for report generation (in seconds, defaults to 60 if not set)
...
def _query_insightcollector(
    workflow_id: str, name_pattern: str, using: str, save_to: str, timeout: str
) -> None:
...
def generate_report(workflow_id: str, using: str):
    ...
    name_pattern, save_to, timeout = _get_report_options()
    if not using:
    ...
    if using and not os.path.exists(using):
        ...
        sys.exit(1)
    try:
        _query_insightcollector(workflow_id, name_pattern, using, save_to, timeout)
    except Exception as err:
        _fatal(
