...
class SendMe:
    ...
    def backfill(self, json_backfill_task_id, json_path=None):
        json_path = json_path if json_path is not None else "./report/remote/"
        ...
        for _ip, res in json_res.items():
            for case_info in res.get("test_cases"):
                case_py_path = case_info.get("case_py")
                module, *_, case_py = case_py_path.split("/")
                tpl["case"] = case_py
                case_id = re.findall(r"test_.*?_(\d+)", case_py)
                if case_id:
                    tpl["case"] = case_id[0]
                tpl["module"] = module
                result = case_info.get("py_outcome")
...
if __name__ == '__main__':
    SendMe(
        ...
        username="",
        password="",
    ).backfill("d3439082af374a94b95e1d3c0613f513")
