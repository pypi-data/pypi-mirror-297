...
from src.EducRating.elo import Elo, EloRating
...
def test_elo() -> None:
    ...
    assert result["user_rating"].value < 0.3
    assert result["user_rating"].value > 0.1
    assert isinstance(result["resource_rating"], EloRating)
    assert isinstance(result["resource_rating"].value, float)
    assert result["resource_rating"].value < -0.1
    assert result["resource_rating"].value > -0.3
