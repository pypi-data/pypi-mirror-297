...
#设置日历控件的最小、最大日期
def f_dtmin(dt, min=365, max=365):
...
def f_connes(name):
    match name:
        #本地
        case 'local':
            es = ES(hosts=[{'host': 'localhost', 'port':9200}])
            return (es)
            pass
...
def f_getes(es,index,body={"query":{"match_all":{}}},flag=0):
    match flag:
        case 0:
...
def f_getesindex(es,flag=0):
    ...
    pass
#获取所有记录
def f_getesall(es,index,flag=0):
    match flag:
        case 0:
