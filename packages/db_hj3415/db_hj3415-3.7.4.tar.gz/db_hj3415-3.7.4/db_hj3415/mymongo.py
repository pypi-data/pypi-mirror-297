...
class Base:
    ...
    setting_manager = db_cli.DbSettingsManager(db_cli.SETTINGS_FILE)
    ...
    def __init__(self, db: str, table: str):
        ...
        if not Base.is_there(db=db, table=table):
            self.mongo_client[db].create_collection(table)
            print(f"{db}데이터베이스에 빈 컬렉션 {table}이 생성되었습니다.")
