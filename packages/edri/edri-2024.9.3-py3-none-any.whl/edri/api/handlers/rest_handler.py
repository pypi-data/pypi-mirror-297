...
class RESTHandler[T: HTTPResponseDirective](HTTPHandler):
    ...
    async def response_error(self, status: HTTPStatus, response: Event | dict | None = None, *args, **kwargs: Unpack[ResponseErrorKW]):
        data = self.response_error_prepare(status, response)
        response = dumps(data, ensure_ascii=False, cls=self.CustomJSONEncoder).encode("utf-8")
        return await super().response_error(status, response, *args, **kwargs)
