...
"""
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QSpinBox, QDoubleSpinBox, QLineEdit, QCheckBox, QComboBox, QPushButton, QApplication, QSlider
        ...
        * 当描述符与Type(str|bool)结合时，会抛出异常
        ...
        if _type == int:
            if not isinstance(start, int):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.start must be int. but got range.start={start}")
            if not isinstance(stop, int):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.stop must be int. but got range.stop={stop}")
            if not isinstance(step, int):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.step must be int. but got range.step={step}")
        elif _type == float:
            if isinstance(start, int):
                start = float(start)
            elif not isinstance(start, float):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.start must be float. but got range.start={start}")
            if isinstance(stop, int):
                stop = float(stop)
            elif not isinstance(stop, float):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.stop must be float. but got range.stop={stop}")
            if isinstance(step, int):
                step = float(step)
            elif not isinstance(step, float):
                raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of range.step must be float. but got range.step={step}")
        else:
            raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type '{_type}' is not supported for Range.")
        return True


    def _check_enum_with_type(self, _enum, _type):
        """
        * 当描述符与Type(int|float|str)结合时，提升QXXX为QComboBox
        * 当描述符与Type(bool)结合时，会抛出异常
        """
        if _enum is None:
            return True
        if _type == bool:
            raise FTMonoaRuntimeEvalError(self.cmonoa, f"Type of Enum can not constrait to type:bool.")
        return True

    def __call__(self, monoa:MonoAttr, monoe:dict) -> dict:
        monoa.iterall(_api_prehandle_single_attr)
        res, self.cmonoa = {}, monoa
        env = monoe.copy()
        ...
        self._check_enum_with_type(res['enum'], res['type'])
...
class QMonoWidget(QWidget):
    def __init__(self, attr_dict:dict, parent=None):
        super().__init__(parent)
        self.ad = attr_dict  # attr_dict
        self.value = self.ad['value']
        self._rootL = QVBoxLayout()
        self._rootL.setContentsMargins(2, 2, 2, 2)
        self._rootL.setSpacing(2)
        self._mainL = QHBoxLayout()
        self._mainL.setContentsMargins(0, 0, 0, 0)
