...
class Mono:
    ...
    def __init__(self, ESO, EEO, VSO, VEO,
                 ...
                 LPSO=None, LPBK=None, LPCT=None, LPEO=None,
        ...
        * env: 用于在构造eval和exec用的虚拟环境时执行的代码。一般可以指定为几条import语句
        ...
        self._binding = {}  # 用于存储attr和exec的绑定关系  # dict{str: list[FTMatched]}
        ...
        * 使用api_parse_python_single_line_vardef获取变量名
        ...
        for i, s in enumerate(_items):
            if s._MONO_TYPE == TYPE_ATTRS:
                ...
                if _acc_attrs:
                    var_name = api_parse_python_single_line_vardef(str(s))  # 获取变量名
                    if var_name:
                        _res[var_name] = _acc_attrs
                        _acc_attrs = []  # 重置属性列表
