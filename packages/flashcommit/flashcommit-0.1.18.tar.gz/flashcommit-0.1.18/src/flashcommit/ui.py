...
class LayoutApp(App):
    ...
    def get_header_text(self):
        return f"'{self.get_comment()}' in [i][b]{self.get_file()}[/b][/i]"
    ...
    def __init__(self, driver_class: Type[Driver] | None = None, css_path: CSSPathType | None = None,
                 ...
                 review_steps: list[Tuple[str, str, str]] = None, comments: str = None) -> None:
    ...
    def on_ready(self) -> None:
        self.push_screen(ApplyScreen(diff=self.get_diff(), header_text=f"{self.comments}: {self.get_header_text()}"))

    def unpack(self) -> tuple[str, str, str]:
        return (self.review_steps[self.step_index][0],
                self.review_steps[self.step_index][2],
                self.review_steps[self.step_index][1])
...
class UI:
    ...
    def __init__(self, fcommit: FlashCommitAdapter, review_steps: list[Tuple[str, str, str]], comments: str = None):
        super().__init__()
        self.app = LayoutApp(fcommit=fcommit, review_steps=review_steps, comments=comments)
