...
class Database(Protocol):
    ...
    def table_to_records(self, table_name:str, columns:str="*", where_clause:str="") -> tuple[list[dict], SQLSchema]:
    ...
    def table_to_csv(self, table_name:str, file_name:str="", save_path:str=".", columns:str="*", where_clause:str="") -> None:
        table_records, schema = self.table_to_records(table_name, columns, where_clause)
    ...
    def print_table(self, table_name:str, col_width:int=12, start_row:int=0, 
        ...
        def get_table_attributes(schema:SQLSchema, max_cols:int) -> tuple[list[dict], int, list[str], int]:
            if max_cols == 1:
                records, schema = self.table_to_records(table_name, schema.col_name_list[0], where_clause)
                ...
                records, schema = self.table_to_records(table_name, col_filter, where_clause)
                col_names = front + ["..."] + back
                col_count = len(col_names)
                concat_col = col_names.index("...")
            else:
                records, schema = self.table_to_records(table_name, columns, where_clause)
                col_count = schema.col_count
                col_names = schema.col_name_list
                concat_col = -1
