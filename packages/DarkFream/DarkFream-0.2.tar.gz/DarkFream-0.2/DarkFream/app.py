...
class MyApp(BaseHTTPRequestHandler):
    ...
    def handle_404(self):
        ...
        self.send_header("Content-type", "text/html")
        self.end_headers()
        self.wfile.write(render(self, '404.html', templates='templates', status=1).encode('utf-8'))
    ...
    def do_GET(self):
        if not urlpatterns:
            ...
            if self.path == '/':
                ...
                self.send_header("Content-type", "text/html")
                self.end_headers()
                self.wfile.write(render(self, 'index.html', templates='templates', status=1).encode('utf-8'))
                return
            else:
                self.send_response(404)
                self.send_header("Content-type", "text/html")
