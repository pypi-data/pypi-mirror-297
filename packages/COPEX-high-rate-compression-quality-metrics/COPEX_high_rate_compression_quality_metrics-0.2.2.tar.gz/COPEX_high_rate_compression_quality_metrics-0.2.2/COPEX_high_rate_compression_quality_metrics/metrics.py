...
def calculate_lrsp(image1_path, image2_path, data_range_mode="data_type", verbose=False, lpips_block_size=5000) -> dict:
    """
    ...
    lpips_values, lpips_value = lpips_func(image1, image2, loss_fn, lpips_block_size)

    mean_ssim_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    mean_ssim = calculate_ssim_multiband(image1, image2, data_range_mode=data_range_mode, verbose=verbose)
