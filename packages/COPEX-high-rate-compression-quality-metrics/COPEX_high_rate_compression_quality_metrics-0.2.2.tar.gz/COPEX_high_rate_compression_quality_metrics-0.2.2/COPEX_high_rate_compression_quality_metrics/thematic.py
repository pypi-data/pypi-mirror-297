...
thematic_usecase_bands = {
    'S1': {  # Type de donnée S
        'Kmeans++': {  # Thématique Kmeans++
            ...
            'bands_needing_resampling': [([], 2)]  # Bandes à rééchantillonner pour S et Kmeans++
        },
...
def load_multispectral_images(directory, valid_bands, resample_bands, resample_factor, resampling_method = Resampling.bilinear):
    """
...
def compute_kmeans_score_for_multiband(original_folder_path, decompressed_folder_path, satellite_type: str,
                                       sample_interval=10, n_clusters=10, random_state=42, resampling_method = Resampling.bilinear):
    """
        Calcule la similarité entre deux images classifiées avec K-means via l'accuracy et le coefficient de kappa.
    ...
    image_original, geo_transform, projection, base_filename = load_multispectral_images(directory=original_folder_path,
                                                                                         ...
                                                                                         resample_factor=resample_factor,resampling_method=resampling_method)
    ...
    # Retourne le nom du modèle K-means et les métriques de similarité.
    return "kmeans++" + satellite_type + "-" + str(sample_interval) + "-" + str(n_clusters) + "-" + str(
        random_state), data
