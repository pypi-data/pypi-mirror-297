...
def process_field(medisoft_field, csv_row, parsed_address_components, reverse_mapping, csv_data, fixed_values):
    ...
    try:
        ...
        if not value and medisoft_field in fixed_values:
            value = fixed_values[medisoft_field][0]  # Use the fixed value
        elif not value and medisoft_field in reverse_mapping:
            if medisoft_field in ["Ins1 Insurance ID", "Primary Insurance Company"]: # TODO add secondary insurance company to this list at some point.
                MediLink_ConfigLoader.log("Detected {}: {}".format(medisoft_field, value))
        ...
        if proceed:
            ...
            if existing_patients:
                ...
                for index, (surgery_date, patient_name, patient_id) in enumerate(patient_info):
                    print("{:03d}: {} (ID: {}) {}".format(index + 1, patient_name, patient_id, surgery_date.strftime('%m-%d')))
