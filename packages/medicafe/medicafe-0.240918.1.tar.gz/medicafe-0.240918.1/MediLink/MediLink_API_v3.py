...
class APIClient(BaseAPIClient):
    ...
    def get_access_token(self, endpoint_name):
        MediLink_ConfigLoader.log("[Get Access Token] Called for {}".format(endpoint_name))
        ...
        if cached_token:
            ...
            MediLink_ConfigLoader.log("Cached token expires at {}".format(expires_at), level="INFO")
            return cached_token
    ...
    def make_api_call(self, endpoint_name, call_type, url_extension="", params=None, data=None, headers=None):
        ...
        if token:
            MediLink_ConfigLoader.log("[Make API Call] Token found for {}".format(endpoint_name), level="INFO")
        else:
...
def fetch_payer_name_from_api(payer_id, config, primary_endpoint='AVAILITY'):
    ...
    # I think this is getting confused because the UHC API doesn't have a payer list yet and 
    # its throwing an error here.
    MediLink_ConfigLoader.log("[Fetch payer name from API] Overriding {} with AVAILITY.".format(primary_endpoint))
