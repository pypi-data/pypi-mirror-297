...
def simulate_operation(operation_type, files, config):
    ...
    if operation_type == 'upload' and files:
        MediLink_ConfigLoader.log("Simulating upload operation for files: {}".format(files))
        return [os.path.normpath(file) for file in files if os.path.exists(file)]
    elif operation_type == 'download':
        MediLink_ConfigLoader.log("Simulating download operation. No files to download in test mode.")
        return []
    else:
