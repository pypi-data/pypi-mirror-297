...
def format_single_claim(patient_data, config, endpoint, transaction_set_control_number, crosswalk):
    """
...
def convert_files_for_submission(detailed_patient_data, config, crosswalk): 
    """
    ...
    for endpoint, patient_data_list in data_by_endpoint.items():
        ...
        if submission_type == 'single':
            ...
            for patient_data in patient_data_list:
                ...
                converted_path = process_claim(config, endpoint, [patient_data], crosswalk, suffix)
                if converted_path:
                    converted_files_paths.append(converted_path)
        else:
            # Process all patient data together for batch submissions
            converted_path = process_claim(config, endpoint, patient_data_list, crosswalk)
            if converted_path:
                converted_files_paths.append(converted_path)

    return converted_files_paths

def process_claim(config, endpoint, patient_data_list, crosswalk, suffix=""): 
    """
    ...
    for patient_data in patient_data_list:
        ...
        if is_valid:
            ...
            formatted_claim = format_single_claim(patient_data, config, endpoint, transaction_set_control_number, crosswalk)
