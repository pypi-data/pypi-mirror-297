...
class Segment:
    ...
    """
    audio_narration_filename: Union[str, None] = None
    """
    The custom audio narration filename provided by the user if a custom
    audio narration is expected to be used. This is a filename that the
    user brings to be narrated as the audio narration, so no artificial
    voice over narration is created (even if 'text' and 'voice' parameters
    are set).
    """
    transcription = None
    """
    ...
    """
    A paramter that is set when the audio part has been created
    ...
    def __init__(self, json, project_id, segment_index):
        ...
        # more dynamic
        self.status = json.get('status', SegmentStatus.TO_START.value)
        ...
        self.audio_filename = json.get('audio_filename') # Filename of the used audio
        self.transcription = json.get('transcription') # Audio transcription
        self.video_clip = None
        self.video_filename = json.get('video_filename') # Filename of the used video
        self.full_clip = None
        self.full_filename = json.get('full_filename')

        self.__load_previously_created_content()

    """
            [  F U N C T I O N A L I T Y  ]
    """
    def __load_previously_created_content(self):
        """
        This method looks for previously created content in the database
        and loads it in memory to avoid generating it again. After this
        method is executed, all available files will be loaded as moviepy
        audio and/or video clips.
        """
        # TODO: I'm not about this if but here it is, I copied it
        # from previous code. I will check later
        if self.type != 'meme': # 'youtube_video' (?)
            if self.audio_filename:
                self.audio_clip = AudioFileClip(self.audio_filename)
                self.calculated_duration = self.audio_clip
    ...
    def __update_audio_filename_in_database(self):
        """
        Stores the current 'self.audio_filename' in the database by using
        the 'self.project_id' and the 'self.segment_index' parameters to
        identify it in the database.
        """
        return self.database_handler.set_project_segment_audio_filename(self.project_id, self.segment_index, self.audio_filename)
    
    def __update_video_filename_in_database(self):
        """
        Stores the current 'self.video_filename' in the database by using
        the 'self.project_id' and the 'self.segment_index' parameters to
        identify it in the database.
        """
        return self.database_handler.set_project_segment_video_filename(self.project_id, self.segment_index, self.video_filename)
    
    def __update_full_filename_in_database(self):
        """
        Stores the current 'self.full_filename' in the database by using
        the 'self.project_id' and the 'self.segment_index' parameters to
        identify it in the database.
        """
        return self.database_handler.set_project_segment_full_filename(self.project_id, self.segment_index, self.full_filename)
    
    def __update_transcription_in_database(self):
        """
        Stores the current 'self.transcription' in the database by using
        the 'self.project_id' and the 'self.segment_index' parameters to
        identify it in the database.
        """
        return self.database_handler.set_project_segment_transcription(self.project_id, self.segment_index, self.transcription)
    ...
    def __handle_narration(self):
        ...
        if not self.audio_clip:
            ...
            # parameter that allows choosing the engine and the voice
                ...
                self.audio_filename = create_ai_narration(self.text, output_filename = segment_part_filename)
                print_completed('Voice narration created successfully')
                
            self.__update_audio_filename_in_database()
    ...
    def __handle_transcription(self):
        ...
        if not self.transcription: 
            ...
            self.__update_transcription_in_database()
    ...
    def __process_video(self):
        ...
        if not self.video_clip:
            ...
            self.__update_video_filename_in_database()
    ...
    def __process_full_part(self):
        ...
        if not self.full_clip:
            ...
            self.__update_full_filename_in_database()
    ...
    def build(self):
        ...
        text_pre_building_without_shortcodes_sanitized = self.shortcode_parser.parse(self.text)
        print(text_pre_building_without_shortcodes_sanitized)

        # We will process the basic audio and video parts to enhance
        # them later. Some segment types doesn't have basic audio part
        # (youtube_video, memes, premades, etc.) because the video part
        # has its own and the only audio (this means 'self.audio_clip'
        # will be None in these cases).

        # We handle BASIC AUDIO part
        self.__process_audio()

        # We handle BASIC VIDEO part
        self.__process_video()

        # We put them together to build the BASIC
