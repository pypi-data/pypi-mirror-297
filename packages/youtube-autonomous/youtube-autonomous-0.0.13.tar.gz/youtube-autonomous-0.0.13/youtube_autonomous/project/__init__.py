...
class Project:
    ...
    def __init__(self, id: Union[str, ObjectId], status: Union[ProjectStatus, str], segments: list[Segment]):
        self.id = id
        ...
        for index, segment in enumerate(segments):
            tmp_segments.append(Segment(segment, self.id, index))
    ...
    def _do_use_setter(self, value: bool = True):
        if not isinstance(value, bool):
            raise Exception('The "value" parameter provided is not a bool.')

        self.__do_use_setter = value
    ...
    def build(self, output_filename: str):
        ...
        final_clip = concatenate_videoclips([segment.full_filename for segment in self.segments])
