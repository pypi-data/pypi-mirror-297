...
class YoutubeAutonomous:
    ...
    def process_unfinished_projects(self):
        ...
        for db_project in db_projects:
            # TODO: This should go to a 'Project' class
            print(db_project)
            self.process_project(db_project)

    # TODO: Move this to a new 'Project' class
    def process_project(self, db_project):
        project = Project(db_project['_id'], db_project['status'], db_project['segments'])
        project.build(f'project_{project.id}.mp4')

        # segments = []
        # for index, db_segment in enumerate(db_project['segments']):
        #     segment = Segment(db_segment, str(db_project['_id']), index)
        #     print(segment.text)
        #     segment.build()
        #     # TODO: When finished, change status in the 'segment.py'
        #     segments.append(segment)

        # # TODO: Concatenate all segments in one file
        # full_clips = [segment.full_clip for segment in segments]
        # project_clip = concatenate_videoclips(full_clips)
        # project_clip.write_videofile(f'project_{str(db_project["_id"])}.mp4')
