...
class Segment:
    ...
    def text(self, text: str):
        if not self.__do_use_setter:
            ...
            self.__shortcode_parser.parse(self.text)
            self._text_sanitized = self.shortcode_parser.text_sanitized
            self._text_sanitized_without_shortcodes = self.shortcode_parser.text_sanitized_without_shortcodes
            self._text_sanitized_with_simplified_shortcodes = self.shortcode_parser.text_sanitized_with_simplified_shortcodes
    ...
    def frame(self, frame: str):
        if not frame and frame != '':
            raise Exception('No "frame" provided.')
    ...
    @shortcodes.setter
    def shortcodes(self, shortcodes):
        # TODO: Apply type to 'shortcodes'
        if not shortcodes:
    ...
    def audio_filename(self, audio_filename: str):
        if not self.__do_use_setter:
            ...
            if not audio_filename and audio_filename != '':
    ...
    def video_filename(self, video_filename: str):
        if not self.__do_use_setter:
            ...
            if not video_filename and video_filename != '':
    ...
    def full_filename(self, full_filename: str):
        if not self.__do_use_setter:
            ...
            if not full_filename and full_filename != '':
