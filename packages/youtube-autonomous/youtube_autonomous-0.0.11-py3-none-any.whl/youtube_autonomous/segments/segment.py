...
class Segment:
    ...
    def status(self, status: Union[SegmentStatus, str] = SegmentStatus.TO_START):
        ...
        if isinstance(status, str) and not SegmentStatus.is_valid(status):
        ...
        if self.__do_use_setter:
            self.database_handler.__update_project_segment_status(self.project_id, self.segment_index, status)

    @audio_filename.setter
    def audio_filename(self, audio_filename: str):
        if not self.__do_use_setter:
            ...
            if audio_filename is not None:
                ...
                if not file_is_audio_file(audio_filename):
                    raise Exception('The "audio_filename" parameter provided is not a valid audio file.')
            
            self.audio_filename = audio_filename
            self.database_handler.set_project_segment_audio_filename(self.project_id, self.segment_index, self.audio_filename)
    ...
    def video_filename(self, video_filename: str):
        if not self.__do_use_setter:
            ...
            self.video_filename = video_filename
            self.database_handler.set_project_segment_video_filename(self.project_id, self.segment_index, self.video_filename)
    ...
    def full_filename(self, full_filename: str):
        if not self.__do_use_setter:
            ...
            self.full_filename = full_filename
            self.database_handler.set_project_segment_full_filename(self.project_id, self.segment_index, self.full_filename)
    ...
    def transcription(self, transcription):
        if not self.__do_use_setter:
            ...
            self.transcription = transcription
            self.database_handler.set_project_segment_transcription(self.project_id, self.segment_index, self.transcription)

    @audio_narration_filename.setter
    def audio_narration_filename(self, audio_narration_filename: str):
        if not self.__do_use_setter:
            self.audio_narration_filename = audio_narration_filename
        else:
    ...
    def text(self, text: str):
        if not self.__do_use_setter:
            ...
            self.text = text
            # TODO: What about this below (?) I keep it by now to check
            self.shortcode_parser.parse(self.text)
            self.text_sanitized = self.shortcode_parser.text_sanitized
            self.text_sanitized_without_shortcodes = self.shortcode_parser.text_sanitized_without_shortcodes
            self.text_sanitized_with_simplified_shortcodes = self.shortcode_parser.text_sanitized_with_simplified_shortcodes

    @text_sanitized.setter
    def text_sanitized(self, text: str):
        if not self.__do_use_setter:
            self.text_sanitized = text
        else:
            if not text and text != '':
                raise Exception('No "text" provided.')
            
            self.text_sanitized = text
            # TODO: Update (insert maybe) this field in database

    @text_sanitized_without_shortcodes.setter
    def text_sanitized_without_shortcodes(self, text: str):
        if not self.__do_use_setter:
            self.text_sanitized_without_shortcodes = text
        else:
            if not text and text != '':
                raise Exception('No "text" provided.')
    ...
    def text_sanitized_with_simplified_shortcodes(self, text: str):
        if not self.__do_use_setter:
            self.text_sanitized_with_simplified_shortcodes = text
        else:
            if not text and text != '':
                raise Exception('No "text" provided.')
