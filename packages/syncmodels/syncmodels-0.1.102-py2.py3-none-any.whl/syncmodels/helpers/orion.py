...
class OrionInjector:
    ...
    def _to_fiware(self, snap: Dict):
        ...
        #     }
        try:
            # TODO: extract the information from GEOJSON:
            # TODO: Point --> direct
            # TODO: Multipolygon: compute X,Y average (gravity center)
            geojson = data.get(GEOJSON_KEY)
            if geojson:
                centroid = GeojsonManager.centroid(geojson)
                # Note reverse order: lng, lat --> lat, lng
                data["location"] = "{1},{0}".format(*centroid["coordinates"])
