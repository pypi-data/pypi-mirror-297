...
class iBot(iAgent):
    ...
    async def old_process_stream(
        self, stream, context, **task
    ):  # TODO: delete
        """TBD"""
        ...
        # transform the stream when is needed before proccesing each single item
        # obtaing mandatory information (i.e: `geospec` etc) from each item
        # and filter before going for `each` plugins
        self.process(iPrePlugin, stream, context)
        stream = stream[: iPlugin.MAX_RECORDS]

        # process each item from stream
        # TODO: use context to store all samples with meta-data
        # TODO: use same instance of context, avoiding creating a new one on each call
        sample = None
        # plugins can modify stream and add more elements
        context[STREAM_KEY] = _stream = list(stream)
        i = 0
        while _stream:
            org = _stream.pop(0)
            data = {**org}
            ...
            i += 1
            if not (i % 100):
                await asyncio.sleep(0)  # to be nice with other fibers
