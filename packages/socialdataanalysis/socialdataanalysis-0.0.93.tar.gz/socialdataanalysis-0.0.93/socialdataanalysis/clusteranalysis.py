...
def hierarchical_clustering_analysis(df, columns, method='average', metric='sqeuclidean'):
    ...
    return tabulate(agglomeration_schedule[['Cluster 1', 'Cluster 2', 'Coefficients', 'Number of Points in Cluster']], 
                    headers=['Cluster 1', 'Cluster 2', 'Distance (Coefficients)', 'Number of Points in Cluster'], 
                    tablefmt='fancy_grid')
...
def cluster_membership_analysis(df, columns, method='average', metric='sqeuclidean', num_clusters=[3, 4]):
    ...
    table = tabulate(cluster_membership, headers=['Case'] + [f'{n_clusters} Clusters' for n_clusters in num_clusters], 
...
def perform_kmeans(df, columns, n_clusters=3, random_state=0):
    """
