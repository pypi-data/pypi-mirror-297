...
def _update_node_outbound_input(model, node_to_remove):
    target_value_info = model.find_value_info_by_name(node_to_remove.output[0])

    # Case where node_to_remove is the last node
    if target_value_info in model.output:
        ...
        model.replace_input_of_all_nodes(node_to_remove.output[0], node_to_remove.input[0])

        # Remove node_to_remove output value info
        model.graph().value_info.remove(target_value_info)
...
def _remove_redudant_flatten_reshape(model):
    ...
    while True:
        ...
        _update_node_outbound_input(model, node_to_remove)
    ...
    for node in model.nodes():
        if node.op_type in ('Reshape', 'Flatten'):
            ...
            if input_shape == output_shape:
                _update_node_outbound_input(model, node)
