...
@pytest.mark.parametrize(
    ...
    [
        (None, None, "Please provide exactly one Kubernetes namespace.", None),
        ([], None, "Please provide exactly one Kubernetes namespace.", None),
        (
            ["namespace1", "namespace2"],
            None,
            "Please provide exactly one Kubernetes namespace.",
            None,
        ),
        (
            ["namespace1"],
            ["us-west-1a,1.2.3.4"],
            None,
            [NFSMountTarget(address="1.2.3.4", zone="us-west-1a")],
        ),
        (["namespace1"], ["1.2.3.5"], None, [NFSMountTarget(address="1.2.3.5")]),
        (
            ["namespace1"],
            ["us-west-1a,1.2.3.4,extra"],
            "Invalid mount target us-west-1a,1.2.3.4,extra; expected (zone,address) tuple or a singular address.",
            None,
        ),
    ],
)
def test_register_generic_cloud(
    kubernetes_namespaces, nfs_mount_targets, expected_exception, expected_mount_targets
):
    # Set up mocks
    mock_cloud = Mock(result=Mock(id="mock_cloud_id"))
    mock_api_client = Mock(
        create_cloud_api_v2_clouds_post=Mock(return_value=mock_cloud),
        update_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_put=Mock(),
        delete_cloud_api_v2_clouds_cloud_id_delete=Mock(),
    )
    mock_log_spinner = MagicMock()
    mock_log_spinner.return_value.__enter__.return_value = Mock()
    mock_cloud_event_producer = Mock()

    # Initialize CloudController with mocks
    cloud_controller = CloudController()
    cloud_controller.api_client = mock_api_client
    cloud_controller.log = Mock(spinner=mock_log_spinner)
    cloud_controller.cloud_event_producer = mock_cloud_event_producer

    name = "test-generic-cloud"

    if expected_exception:
        with pytest.raises(ClickException) as exc_info:
            cloud_controller.register_generic_cloud(
                name=name,
        ...
        # Verify cloud creation API call
        ...
        # Verify cloud resource update API call
        mock_api_client.update_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_put.assert_called_once()
        (
            _,
            kwargs,
        ) = (
            mock_api_client.update_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_put.call_args
        )
        assert kwargs["cloud_id"] == "mock_cloud_id"
        update_cloud_with_cloud_resource = kwargs["update_cloud_with_cloud_resource"]
        assert isinstance(
            update_cloud_with_cloud_resource, UpdateCloudWithCloudResource
        )
        create_cloud_resource = (
            update_cloud_with_cloud_resource.cloud_resource_to_update
        )
        assert create_cloud_resource.compute_stack == ComputeStack.K8S
        assert create_cloud_resource.kubernetes_namespaces == kubernetes_namespaces

        # Compare the expected and actual NFSMountTarget objects directly
        assert create_cloud_resource.nfs_mount_targets == expected_mount_targets

        # Verify no cloud deletion
...
def test_register_cloud_aws(  # noqa: PLR0913
    ...
    with patch(
        ...
        get_organization_id=Mock(return_value="org_"),
    ):
        cloud_controller = CloudController()
        cloud_controller.api_client = mock_api_client
...
def test_delete_cloud(provider: str, no_resource_record: bool):
    ...
    with patch.multiple(
        ...
        cloud_controller.delete_all_aws_resources = Mock()  # type: ignore[assignment]
        cloud_controller.delete_all_gcp_resources = Mock()  # type: ignore[assignment]
        ...
        if provider == "AWS":
            cloud_controller.delete_all_aws_resources.assert_called_with(
                mock_cloud.result
            )
            ...
            cloud_controller.delete_all_gcp_resources.assert_called_with(
                mock_cloud.result
            )
...
def test_delete_aws_managed_cloud(pre_deleted: bool):
    ...
    mock_log_spinner.return_value.__enter__.return_value = Mock()
    cloud_controller.log = Mock(spinner=mock_log_spinner)
    mock_try_delete_customer_drifts_policy = Mock()

    with patch.object(
        cloud_controller,
        "delete_aws_cloudformation_stack",
        return_value=cloud_controller.delete_aws_cloudformation_stack,
    ) as mock_delete_aws_cloudformation_stack, patch.multiple(
        "anyscale.controllers.cloud_controller",
        try_delete_customer_drifts_policy=mock_try_delete_customer_drifts_policy,
    ):
        ...
        mock_try_delete_customer_drifts_policy.assert_called_once()
...
@pytest.mark.parametrize(
    ("is_hosted", "is_k8s"),
    [pytest.param(True, False, id="hosted"), pytest.param(False, True, id="k8s"),],
)
def test_delete_all_aws_resources_skip_hosted_or_k8s_clouds(
    is_hosted: bool, is_k8s: bool
):
    cloud_controller = CloudController()
    cloud_controller.delete_aws_lb_cfn_stack = Mock()  # type: ignore[assignment]
    assert (
        cloud_controller.delete_all_aws_resources(
            cloud=Mock(is_hosted=is_hosted, is_k8s=is_k8s)
        )
        is True
    )
    cloud_controller.delete_aws_lb_cfn_stack.assert_not_called()
...
def test_delete_all_gcp_resources_skip_hosted_or_k8s_clouds(
    is_hosted: bool, is_k8s: bool
):
    cloud_controller = CloudController()
    mock_try_import_gcp_managed_setup_utils = Mock()
    with patch.multiple(
        "anyscale.controllers.cloud_controller",
        try_import_gcp_managed_setup_utils=mock_try_import_gcp_managed_setup_utils,
    ):
        assert (
            cloud_controller.delete_all_gcp_resources(
                cloud=Mock(is_hosted=is_hosted, is_k8s=is_k8s)
            )
            is True
        )
    mock_try_import_gcp_managed_setup_utils.assert_not_called()


@pytest.mark.parametrize("is_bring_your_own_resource", [True, False])
def test_delete_all_gcp_resources(is_bring_your_own_resource: bool):
    mock_gcp_utils = Mock(get_google_cloud_client_factory=Mock(return_value=Mock()))
    mock_setup_utils = Mock(delete_gcp_tls_certificates=Mock())
    cloud_controller = CloudController()
    cloud_controller.delete_gcp_managed_cloud = Mock()  # type: ignore[assignment]
    cloud_controller.log = Mock(warning=Mock())
    ...
    mock_cloud = Mock(
        ...
        credentials=json.dumps(
            {
                "project_id": "mock_project_id",
                "gcp_workload_identity_pool_id": "mock_pool_id",
                "service_account_email": "mock_service_account_email",
            }
        ),
        is_bring_your_own_resource=is_bring_your_own_resource,
    ...
    with patch.multiple(
        ...
        assert cloud_controller.delete_all_gcp_resources(cloud=mock_cloud)

    mock_setup_utils.delete_gcp_tls_certificates.assert_called_once()
    if not is_bring_your_own_resource:
        cloud_controller.delete_gcp_managed_cloud.assert_called_once()
        cloud_controller.log.warning.assert_not_called()
    else:
        cloud_controller.log.warning.assert_called_once()
        cloud_controller.delete_gcp_managed_cloud.assert_not_called()


@pytest.mark.parametrize("is_bring_your_own_resource", [True, False])
def test_delete_all_aws_resources(is_bring_your_own_resource: bool):
    cloud_controller = CloudController()
    cloud_controller.delete_aws_lb_cfn_stack = Mock()  # type: ignore[assignment]
    ...
    mock_cloud = Mock(
        is_aioa=False, is_bring_your_own_resource=is_bring_your_own_resource,
    )

    assert cloud_controller.delete_all_aws_resources(cloud=mock_cloud)

    cloud_controller.delete_aws_lb_cfn_stack.assert_called_once()
    cloud_controller.delete_aws_tls_certificates.assert_called_once()
...
def test_delete_cloud_force_delete(cloud_test_data):
    cloud_controller = CloudController()
    cloud_controller.delete_all_aws_resources = Mock(side_effect=Exception("failed"))  # type: ignore[assignment]
    cloud_controller.delete_all_gcp_resources = Mock(side_effect=Exception("failed"))  # type: ignore[assignment]
    assert cloud_controller.delete_cloud(
        cloud_name=cloud_test_data.name, cloud_id=None, skip_confirmation=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "org_abc_valid",
        "not_valid_id",
        "org_abc_not_contained_in_policy",
        "org_123",
    ],
)
def test_register_aws_cloud_external_id(external_id: Optional[str]):
    mock_cloud = Mock(result=Mock(id="mock_cloud_id"))
    mock_api_client = Mock(
        create_cloud_api_v2_clouds_post=Mock(return_value=mock_cloud)
    )
    ...
    mock_subnet_ids = ["mock_subnet"]
    mock_efs_id = "mock_efs_id"
    mock_anyscale_iam_role = "arn:aws:iam::123:role/mock_anyscale_role"
    mock_anyscale_iam_role_name = mock_anyscale_iam_role.split("/")[-1]
    mock_instance_iam_role = "arn:aws:iam::123:role/instance_role"
    mock_security_group_ids = ["security_group_id"]
    mock_s3_bucket_id = "mock_s3_id"
    mock_role = Mock(
        assume_role_policy_document={
            ...
            "Statement": [
                ...
                },
                {
                    ...
                    "Principal": {"AWS": "arn:aws:iam::123:root"},
                    "Action": "sts:AssumeRole",
                    "Condition": {"StringEquals": {"sts:ExternalId": "org_abc_valid"}},
                },
            ],
        }
    )
    mock_get_role = Mock(return_value=mock_role)
    mock_get_aws_efs_mount_target_ip = Mock(return_value="mock_efs_ip")
    ...
    cluster_management_stack_version = ClusterManagementStackVersions.V2
    mock_verify_aws_cloud_resources = Mock(return_value=True)
    mock_update_external_ids_for_policy = Mock(return_value={"mock_policy": "test"})
    with patch(
        "anyscale.controllers.cloud_controller.CloudController.verify_aws_cloud_resources",
        new=mock_verify_aws_cloud_resources,
    ), patch.multiple(
        "anyscale.controllers.cloud_controller",
        associate_aws_subnets_with_azs=mock_associate_subnet_ids_with_policy,
        _update_external_ids_for_policy=mock_update_external_ids_for_policy,
        _get_role=mock_get_role,
        _get_aws_efs_mount_target_ip=mock_get_aws_efs_mount_target_ip,
        _get_memorydb_cluster_config=mock_get_memorydb_cluster_config,
        validate_aws_credentials=Mock(),
        ...
        if external_id and external_id != "org_abc_valid":
            with pytest.raises(ClickException) as e:
                cloud_controller.register_aws_cloud(
                    region=mock_region,
                    compute_stack=ComputeStack.VM,
                    ...
                    subnet_ids=mock_subnet_ids,
                    efs_id=mock_efs_id,
                    anyscale_iam_role_id=mock_anyscale_iam_role,
                    instance_iam_role_id=mock_instance_iam_role,
                    security_group_ids=mock_security_group_ids,
                    s3_bucket_id=mock_s3_bucket_id,
                    memorydb_cluster_id=None,
                    functional_verify=None,
            ...
            if not external_id.startswith(mock_org_id):
                assert e.match(
                    f"Cloud registration failed! `--external-id` must start with the organization ID: {mock_org_id}"
                )
                mock_api_client.create_cloud_api_v2_clouds_post.assert_not_called()
            else:
                assert e.match(
                    f"Cloud registration failed! External ID {external_id} is not in the assume role policy of {mock_anyscale_iam_role}."
                )
                mock_get_role.assert_called_once_with(
                    mock_anyscale_iam_role_name, mock_region
                )
                mock_api_client.create_cloud_api_v2_clouds_post.assert_called_with(
                    write_cloud=WriteCloud(
                        provider="AWS",
                        region=mock_region,
                        credentials=mock_anyscale_iam_role,
                        name=mock_cloud_name,
                        is_bring_your_own_resource=True,
                        is_private_cloud=False,
                        cluster_management_stack_version=cluster_management_stack_version,
                ...
                mock_api_client.delete_cloud_api_v2_clouds_cloud_id_delete.assert_called_once()
                mock_role.AssumeRolePolicy.assert_not_called()
        else:
            cloud_controller.register_aws_cloud(
                region=mock_region,
                compute_stack=ComputeStack.VM,
            ...
            if external_id is None:
                mock_update_external_ids_for_policy.assert_called_once()
                mock_role.AssumeRolePolicy.assert_called_once()
            else:
                mock_update_external_ids_for_policy.assert_not_called()
                mock_role.AssumeRolePolicy.assert_not_called()
