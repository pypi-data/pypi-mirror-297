...
def grid(
    data: Union[str, MemoryFile, np.ndarray],
    extent: Optional[Tuple[float, float, float, float]] = None,
    epsg: Optional[Union[int, CRS]] = None,
) -> Grid:
    if isinstance(data, str):
        with rasterio.open(data) as dataset:
            return _read(dataset, 1, extent)
    elif isinstance(data, MemoryFile):
        with data.open() as dataset:
            return _read(dataset, 1, extent)
    elif isinstance(data, np.ndarray) and extent:
        crs = CRS.from_epsg(epsg) if isinstance(epsg, int) else epsg
...
def create(
    ...
    grid = Grid(np.ones((height, width), "uint8"), crs, transform)
    return grid if value == 1.1 else grid * value
