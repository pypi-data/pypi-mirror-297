...
class PodEnvironment(OdfEnvironment):
    ...
    def __init__(self, context, inserts, expressionsHolders):
        ...
        # Info about the currently parsed pod graphic
    ...
    def isPodGraphic(self, tag, attrs):
        '''Does this p_tag represent a POD graphic, ie, a graphic whose range
           of cells is dynamic and depends on the result of a pod "for"
           statement ?'''
        if tag != 'draw:frame' or 'draw:name' not in attrs: return
        name = attrs['draw:name']
        # A POD-compliant graphic has a name of the form:
        #
        #                 <rangeStart>:<rangeEnd>:<podIter>
        #
        return name.split(':') if name.count(':') == 2 else None
            ...
            if e.state == e.IGNORING:
                ...
                if tag in e.impactableTags:
                    if e.mode == e.ADD_IN_SUBBUFFER:
                        e.addSubBuffer()
                    e.currentBuffer.addElement(e.currentTag.name)
                    graphic = e.isPodGraphic(tag, attrs)
                else:
                    graphic = None
