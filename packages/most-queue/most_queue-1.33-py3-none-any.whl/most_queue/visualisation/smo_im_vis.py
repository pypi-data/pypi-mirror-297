    ...
    def run(self):
        ...
        if not self.is_smo_created:
            if self.model_type == "FCFS один класс":
                self.smo = QueueingSystemSimulator(self.n, buffer=self.r, calc_next_event_time=True)
                ...
                self.smo = PriorityQueueSimulator(self.n, self.k, self.model_type, buffer=self.r, calc_next_event_time=True)
                sources = []
                servers_params = []
        ...
        if self.k == 1:
            for i in range(self.queue_count + 1):
                ...
                if i >= self.queue_count - self.number_of_jobs and i != self.queue_count:
                    ...
                    #
                    clr_num = np.random.randint(0, len(COLOR_LIST))
                    qp.setPen(QPen(QColor(*COLOR_LIST[clr_num]), 1, Qt.SolidLine))
                    qp.setBrush(QBrush(QColor(*COLOR_LIST[clr_num]), Qt.SolidPattern))
