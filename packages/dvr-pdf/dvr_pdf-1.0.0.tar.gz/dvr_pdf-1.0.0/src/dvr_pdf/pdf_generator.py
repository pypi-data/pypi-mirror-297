...
class NumberedPageCanvas(Canvas):
    ...
    def __init__(self, *args, render_total_pages: bool, font_name: Font, render_text: Callable,
    ...
    def _render_page_elements(self, page_count: int):
        ...
        for element in render_elements:
            if element.element_type == ElementType.TEXT:
                ...
                _render_image_element(element=element, canvas=self, resources=self._resources)

    def _render_background_elements(self, *, render_elements: list[PDFElement]):
        for element in render_elements:
            if element.element_type != ElementType.BACKGROUND:
                continue
            _render_background_element(element=element, canvas=self)
...
class PDFGenerator:
    def __init__(self, *, file_path: PathLike, template: PDFTemplate, merge_tag_map: dict[str, str],
        ...
        self._table_element: Optional[PDFTableElement] = None
    ...
    def _split_elements(self):
        ...
        for element in elements:
            element_type = element.get('element_type', element.get('elementType'))
            if element_type == ElementType.TABLE.value:
                element = PDFTableElement.model_validate(element)
            elif element_type == ElementType.IMAGE.value:
                element = PDFImageElement.model_validate(element)
            elif element_type == ElementType.BACKGROUND.value:
                element = PDFBackgroundElement.model_validate(element)
            elif element_type == ElementType.TEXT.value:
                element = PDFTextElement.model_validate(element)
            if element.element_type == ElementType.TABLE:
                self._table_element = PDFTableElement.model_validate(element)
                continue
            if (page_behavior := element.page_behaviour) == PageBehavior.FIRST_PAGE:
    ...
    def _render_fixed_elements(self, canvas: NumberedPageCanvas, _document: BaseDocTemplate):
        ...
        for element in render_elements:
            ...
            if element_type == ElementType.TEXT:
                self._render_text_element(element=element, canvas=canvas)
            elif element_type == ElementType.IMAGE:
                _render_image_element(element=element, canvas=canvas, resources=self._resources)
    ...
    def _render_text_element(self, *, element: PDFTextElement, canvas: Canvas):
        ...
        width, height = paragraph.wrapOn(self._document, _inner_width, _inner_height)
    ...
    def _render_table_element(self):
        ...
        table = Table(rows, colWidths=column_widths, minRowHeights=row_heights, style=styles, repeatRows=1)
    ...
    def _render_table_rows(self, *, rows: list[PDFRowElement], font_size: int) -> \
        ...
        if 'table_rows' not in self._entities:
            return list(), list()
    ...
    def _render_vat_rows(self, *, row: PDFRowElement, start_y: int, font_size: int) -> \
        ...
        for idx, vat_row in enumerate(vat_rows):
            _row, _styles = self._render_table_row(row_data=vat_row, row=row, y=start_y + idx, ignore_tags=True,
                                                   font_size=font_size)
            rows.append(_row)
            styles.extend(_styles)
        return rows, styles

    def _render_table_row(self, *, row_data: Optional[Union[dict, tuple]], row: PDFRowElement, y: int,
    ...
    def _render_table_cell(self, *, row_data: Optional[Union[dict, tuple]], cell: PDFCellElement, x: int, y: int,
    ...
    def _get_configurable_table_element_styles(self, *, element: ConfigurableTableElement, x_start: int, y_start: int,
                                               x_end: int, y_end: int, bold: Optional[bool] = None,
                                               italic: Optional[bool] = None) -> list[TableStyleProp]:
        if bold is None:
            bold = element.bold
        if italic is None:
        ...
        styles: list[TableStyleProp] = [
            (TableStyleName.FONTNAME.value, *coords, font_name),
            (TableStyleName.FONTSIZE.value, *coords, font_size)
        ]
        if background_color := element.background_color:
            styles.append((TableStyleName.BACKGROUND.value, *coords, background_color))
        if color := element.color:
            styles.append((TableStyleName.TEXTCOLOR.value, *coords, color))
        if border_bottom := element.border_bottom_color:
            styles.extend([
                (TableStyleName.LINEBELOW.value, *coords, 1, border_bottom),
        ...
        if border_top := element.border_top_color:
            styles.append((TableStyleName.LINEABOVE.value, *coords, 1, border_top))
        return styles
    ...
    def _get_table_frame_coordinates(self, *, elements: list[PDFElement], table_element: PDFTableElement) -> \
        ...
        for element in elements:
            if not self._element_has_content(element=element):
                continue
...
def _render_background_elements(*, render_elements: list[PDFElement], canvas: NumberedPageCanvas):
    for element in render_elements:
        if element.element_type != ElementType.BACKGROUND:
            continue
        _render_background_element(element=element, canvas=canvas)
...
def _initialize_styles():
    ...
    styles.add(ParagraphStyle(leading=16, alignment=TA_LEFT, parent=styles['BodyText'], name='TextStyle'))
    styles.add(ParagraphStyle(valign='MIDDLE', parent=styles['BodyText'], name='TableText'))
    styles.add(ParagraphStyle(alignment=TA_RIGHT, parent=styles['TableText'], name='TableTextRight'))
