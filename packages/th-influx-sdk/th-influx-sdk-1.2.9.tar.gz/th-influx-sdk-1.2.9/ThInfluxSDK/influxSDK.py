    ...
    if result is not None:
        ...
        if result.error:
            ...
            for series in result.raw['series']:
                data_map[series['tags']['UUID']] = series['values']
        ...
        while True:
            ...
            if data_map is None:
                ...
                for time_stamp in time_list:
                    ...
                    for point in pointList:
                        values_list = result_value_map.get(point)
                        data_map_value = data_map.get(point)
                        if values_list and values_list[index][1]:
                            # 假设 values_list 是一个包含时间戳和值的元组列表
                            record.append(values_list[index][1])
                        elif data_map_value and data_map_value[index][1]:
                            record.append(data_map_value[index][1])
                        else:
                            record.append('')
