...
def map(self, device, hw_only=False, mode=MapMode.AllNps):
    ...
    """
    # workaround to force model remapping
    self._map(None)

    if mode == MapMode.Minimal or device is None:
        self._map(device, hw_only=hw_only)
    elif mode == MapMode.AllNps:
        _map_search(self, device, hw_only=hw_only, min_pass=True)
    elif mode == MapMode.HwPr:
        _map_search(self, device, hw_only=hw_only, min_pass=False)
    else:
