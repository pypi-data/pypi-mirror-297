...
from petdb.service.server import Server, DEFAULT_STORAGE_PATH
...
def init_service(password: str, port: int = DEFAULT_PORT, storage_path: str = DEFAULT_STORAGE_PATH):
...
def run_service(password_hash: str, port: int):
	Server(password_hash=password_hash, port=port, storage_path=os.getcwd()).run()
