...
class orderclass:
    def __init__(self,api = None , targetposition = None, sendmail = None, sendmailpwd = None, receivemail = None , filename = None, ifbalancehis = False ):
    ...
    def sendorder(self, symbol= 'CFFEX.IF2409', number = 5, offset = 'CLOSE',direction = 'SELL'):
        ...
        while True:
            ...
            if (not order and self.api.is_changing(quote)) or (
                ...
                if direction == 'SELL':
                    order = self.api.insert_order(symbol= symbol , direction=direction, offset='CLOSE',
    ...
    def sendmailto(self, body = None, subject = None ):
        try:
            ...
            message["From"] = self.sendmail
            message["To"] = self.receivemail
            message["Subject"] = subject
            message.attach(MIMEText(body, "plain"))
            server = smtplib.SMTP("outlook.office365.com", 587)
            server.starttls()
            server.login(self.sendmail, self.sendmailpwd)
            text = message.as_string()
            server.sendmail(self.sendmail, self.receivemail, text)
            server.quit()
        except:
            pass
