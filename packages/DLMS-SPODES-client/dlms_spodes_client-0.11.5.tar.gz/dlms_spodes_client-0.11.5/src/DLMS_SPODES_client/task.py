...
class CreateType(ExTask):
    ...
    def __init__(self, col_id: CollectionID):
...
class MapTypeCreator:
    adapter: Adapter
    lock: asyncio.Lock
    con: dict[[CollectionID], CreateType]
    keep: set[CollectionID]
    ...
    async def get_collection(self, client: Client, col_id: CollectionID) -> collection.Collection:
    ...
    async def exchange(self, c: Client) -> bool:
        ...
        match c.objects.firm_ver:
            ...
            case AppVersion(0, 0, 0) | AppVersion(-1):
                ...
                return False
            case AppVersion():
                c.set_error(Application.VERSION_ERROR, F'Version {c.objects.firm_ver} don\'t read update properties')
                return False
            case _:
        ...
        for key in firmwares:
            ...
            if compare_version > suitable_firmware:
                ...
                temp_key = key
        if suitable_firmware != c.objects.firm_ver and temp_key is not None:
            ...
            c.log(logL.INFO, F"{c.objects.firm_ver=} is actual")
        # search boot image
        temp_key = None
        for key in boots:
            match key:
        ...
        match c.objects.firm_ver:
            ...
            case AppVersion(0, 0, 0):
                ...
                return False
            case AppVersion():
                c.set_error(Transmit.EXECUTE_ERROR, F'Невозможно обновить версию {c.objects.firm_ver}')
                return False
            case _:
                return False
