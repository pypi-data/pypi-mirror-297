...
class MasterDetailViewSet(
    ...
    def get_filter_class(self):
        assert self.filter_class is not None, (
            "'%s' should either include a `filter_class` attribute, "
            "or override the `get_filter_class()` method."
            % self.__class__.__name__
        )
    ...
    def filter_options(self, request, *args, **kwargs):
        ...
        options_info = FilterClass.get_options_info(self, serializer_info)
...
class ImageAttachmentViewSet(mixins.DocumentViewSetMixin, MasterDetailViewSet):
    queryset = ImageAttachment.objects.all()
    serializers = {
        'default': serializers.ImageAttachmentSerializer,
        'retrieve': serializers.ImageAttachmentSerializer,
        'update': serializers.ImageAttachmentEditSerializer,
    }
