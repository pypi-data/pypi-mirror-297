...
from texta_parsers.settings import ELASTIC_BULK_BATCH_SIZE, ELASTIC_GLOBAL_TIMEOUT, META_FIELD
from texta_parsers.tools import utils
from texta_parsers.tools.utils import ParserOutputType, slugify
...
class ESImporter:
    ...
    def __init__(self, elastic_url: str, index_prefix: str, index_per_collection: bool = False):
        """
        :param elastic_url: Url to elasticsearch
        :param index_prefix: Prefix used for naming indices in elasticsearch
        """
        self.index_prefix = index_prefix
        self.es = elasticsearch.Elasticsearch(elastic_url, request_timeout=ELASTIC_GLOBAL_TIMEOUT)
        self.index_per_collection = index_per_collection

    @staticmethod
    def build_flavor(elastic_url: str) -> str:
        es = elasticsearch.Elasticsearch(elastic_url, request_timeout=ELASTIC_GLOBAL_TIMEOUT)
    ...
    def _add_texta_facts_mapping(self, index):
        ...
        self.es.indices.put_mapping(body=mapping, index=index)

    def _get_index_name(self, item):
        item_type = utils.get_output_type(item, self.index_per_collection)
        ...
        if (item_type == ParserOutputType.EMAIL):
            ...
            return self.index_prefix + "_collection_" + slugify(filename)
        else:
