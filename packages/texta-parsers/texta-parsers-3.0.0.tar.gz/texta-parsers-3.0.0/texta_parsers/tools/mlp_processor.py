...
from texta_parsers.settings import META_FIELD, CONTENT_FIELD
...
class MLPProcessor:
    ...
    def __init__(self, mlp):
        self.mlp = mlp
        self.IGNORE_FIELDS = ("texta_facts", "properties")
    ...
    def _apply_mlp(self, document: dict, field: str, analyzers: list):
        ...
        if content:
            ...
            mlp_res_path = f"{field}_mlp"
            # Add the MLP output dictionary.
            document[mlp_res_path] = mlp_res[f"text_mlp"]
    ...
    def _apply_mlp_to_mails(self, email: dict, attachments: list, analyzers: list):
        self._apply_mlp(email, CONTENT_FIELD, analyzers)
        for attachment in attachments:
            self._apply_mlp(attachment, CONTENT_FIELD, analyzers)
