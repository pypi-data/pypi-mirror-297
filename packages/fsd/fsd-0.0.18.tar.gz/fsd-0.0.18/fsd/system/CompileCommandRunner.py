...
class CompileCommandRunner:
    ...
    async def execute_steps(self, steps_json, compile_files, code_files):
        ...
        for step in steps:
            logger.info(f"Step: {step['prompt']} - {step['command']}")
            ...
            logger.info(f"Executing step: {step['prompt']}")
            ...
            while retry_count < self.max_retry_attempts:
                if step['method'] == 'bash':
                    ...
                    if return_code != 0:
                        ...
                        error_message = error_check.get('error_message', "")

                        if error_type == 1:
                            await self.print_code_error(error_message, code_files)
                            retry_count += 1
    ...
    async def execute_fixing_steps(self, steps_json, compile_files, code_files):
        ...
        for step in steps:
            logger.info(f"Step: {step['error_resolution']} - {step['command']}")
            user_permission = input(f"Press Tab to execute this step, or Enter to skip: ")
            ...
            logger.info(f"Executing step: {step['error_resolution']}")
