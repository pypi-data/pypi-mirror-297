...
class CompileFileFinderAgent:
    ...
    async def get_compile_file_planning(self, userRequest, tree):
        ...
        prompt = (
            ...
            "{\n"
            "    \"crucial_files\": [\"file1.extension\", \"file2.extension\", ...]\n"
            "}\n\n"
            "CRITICAL: Return ONLY the valid JSON object. Do NOT include any explanatory text, markdown formatting, or extraneous characters. Ensure the JSON is parseable and contains only the file names (not full paths) of main relevant files for compilation, running, and building the project."
        )
