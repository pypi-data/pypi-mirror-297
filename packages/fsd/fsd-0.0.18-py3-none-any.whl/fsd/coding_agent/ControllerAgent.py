...
class ControllerAgent:
    def __init__(self, repo):
        ...
        self.techExplainer = TechnicalExplainerAgent(repo)
    ...
    def scan_for_single_file(self, filename):
        ...
        logger.info(f"Couldn't find {filename} in: {self.directory_path}")
        return None
    ...
    async def get_explainer(self, user_prompt, language, role):
        """Generate idea plans based on user prompt and available files."""
        return await self.techExplainer.get_technical_plans(user_prompt, language, role)
    ...
    async def get_taskPlanner(self, instruction):
        """Generate idea plans based on user prompt and available files."""
        return await self.taskPlanner.get_task_plans(instruction)
    ...
    async def get_coding_requests(self, instructions, final_working_files, context, context_files, role, crawl_logs):
        ...
        plan = await self.get_taskPlanner(instructions)
        logger.info(f"Completed preparing task!")

        is_first = True
        for step in plan.get('steps', []):
            file_name = os.path.basename(step.get('file_name'))
    ...
    async def regular_code_task_pipeline(self, finalPrompt, role):
        ...
        totalfile = await self.get_coding_requests(idea_plan, list(final_working_files), all_context, context_files, role, crawl_logs)
        await self.image.get_started(idea_plan)
        logger.info(f"Next, I will build to check if any compile error was made")
        await self.build_and_fix_compile_error(idea_plan, totalfile, role)
    ...
    async def get_started(self, user_prompt):
        ...
        if pipeline == "0":
            await self.explainer_task_pipeline(finalPrompt, language, role)
        elif pipeline == "1":
