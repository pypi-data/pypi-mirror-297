...
def auto_by_study_id(study_ids: List[int]):
    li = list()
    for study_id in study_ids:
        for app_env in [AppEnvEnum.DEV.description]:
            li.append("{0}-{1}".format(study_id, app_env))
    return li
...
if __name__ == '__main__':
    ...
    # handle_list = auto_by_study_id([221, 220, 219, 218, 217])
    handle_list = auto_by_study_id([1002])
    app_name = AppEnum.DESIGN.code
