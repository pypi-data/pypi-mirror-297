...
from pydenticore.authentication import DefaultAuthenticationDataProtector, AuthenticationResult
from pydenticore.authentication.interfaces import IAuthenticationHandler, IAuthenticationDataProtector
...
class CookieAuthenticationHandler(IAuthenticationHandler):
    ...
    def _encode_authentication_cookie(self, scheme: str, principal: ClaimsPrincipal, **properties) -> dict[str, str]:
        ep = principal_serialize(principal)
        data = {"principal": ep}
    ...
    def _decode_authentication_cookie(self, scheme: str, cookies: dict[str, str]) -> AuthenticationResult:
        ...
        if value := cookies.get(key):
            ...
            principal = principal_deserialize(unprotected_data.pop("principal"))
