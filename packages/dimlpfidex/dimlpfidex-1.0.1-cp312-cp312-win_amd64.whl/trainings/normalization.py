...
def get_and_check_parameters(init_args):
    ...
    parser.add_argument("--output_rule_files", type=lambda x: [sanitizepath(args.root_folder, file, "w") for file in list_type(x, dict(func=str))], help="List of paths where the denormalized rule files will be saved, it is mandatory to specify each of them if one is specified (default: <original_name>_denormalized<original_extension>)", metavar="<list<str>>", action=TaggableAction, tag="Normalization")
