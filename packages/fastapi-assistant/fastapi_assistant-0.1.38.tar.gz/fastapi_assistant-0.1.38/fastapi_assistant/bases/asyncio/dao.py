...
class BasicDao(object):
    ...
    async def get_or_create(self, db: AsyncSession, defaults=None, is_commit: bool = True, **kwargs) -> Tuple[EnhancedModel, bool]:
        """
        获取一个或创建一个 defaults
        :param db:
        :param defaults: 扩充条件
    ...
    async def update_or_create(self, db: AsyncSession, defaults=None, is_commit=True, **kwargs) -> Tuple[EnhancedModel, bool]:
        """
        更新或创建一个
        :param db:
        :param defaults: 更新内容
    ...
    async def update(self, db: AsyncSession, pk: int, update_data: dict, is_commit: bool = True):
        ...
        try:
            await db.execute(
                self.Model.__table__.update().where(*filters).values(update_data)
            )
            await db.flush()
        except Exception as e:
            if is_commit:
    ...
    async def delete(self, db: AsyncSession, pk: int, is_commit: bool = True):
        ...
        try:
            if filters:
                filters.append(self.Model.id == pk)
                await db.execute(
                    self.Model.__table__.update().where(*filters).values(is_deleted=True)
                )
            else:
                await db.execute(
                    self.Model.__table__.delete().where(self.Model.id == pk)
                )
            await db.flush()
        except Exception as e:
            if is_commit:
