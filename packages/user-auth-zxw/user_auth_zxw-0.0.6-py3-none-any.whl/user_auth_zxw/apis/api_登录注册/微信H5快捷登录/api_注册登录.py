...
from user_auth_zxw.apis.api_用户权限_增加 import 分配或创建角色
from user_auth_zxw.apis.schemas import 返回_login, 请求_获取_登录二维码URL, 返回_获取_登录二维码URL, \
    请求_分配或创建角色
...
router = APIRouter(prefix="/api/account/wechat/qr-login", tags=["微信一键登录"])


@router.post("/get-qrcode", response_model=返回_获取_登录二维码URL)
async def 获取_登录二维码URL(url: 请求_获取_登录二维码URL):
    ...
    try:
        ...
        return 返回_获取_登录二维码URL(qr_code_url=qrcode_url)
    except Exception as e:
        raise HTTPException_AppToolsSZXW(error_code=ErrorCode.生成二维码失败,
...
async def 一键登录(code: str, app_name: str, db: AsyncSession = Depends(get_db)):
    ...
    :return:
    """
    access_key, openid = await get_access_token_and_openid_async(code,
                                                                 WeChatPub.app_id,
    ...
    if openid:
        ...
        if not user:
            ...
            user_info = 请求_分配或创建角色(user_id=user.id,
                                            role_name="l0",
                                            app_name=app_name)
            await 分配或创建角色(user_info, db)
        #
        return await login_user_async(user, db)

    raise HTTPException_AppToolsSZXW(error_code=ErrorCode.微信登录失败,
                                     detail="Login failed",
