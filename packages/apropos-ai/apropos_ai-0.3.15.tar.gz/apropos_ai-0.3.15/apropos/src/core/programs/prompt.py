...
class PromptTemplate(BaseModel):
    ...
    def run(
        ...
        if response_model:
            ...
            response = lm.respond_sync(
                system_message=system, user_message=user, multi_threaded=multi_threaded
            )
        return response
    ...
    async def arun(
        ...
        if response_model:
            response = await lm.respond_async(
                system_message=system, user_message=user, response_model=response_model
            )
        else:
            response = await lm.respond_async(system_message=system, user_message=user)
