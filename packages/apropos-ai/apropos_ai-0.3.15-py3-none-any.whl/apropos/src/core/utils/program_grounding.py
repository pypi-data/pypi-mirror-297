...
async def get_topics_for_prompt(
    prompt: str, other_prompts: List[str], llm: LM
) -> List[TopicInfo]:
    system_message = """
...
Your topics: """
    ...
    topic_response = await llm.respond_async(
...
"""
    assignments = await llm.respond_async(
        system_message=system_message,
        user_message=user_message,
...
"""
    ...
    breakdown = await llm.respond_async(
        system_message=system_message,
        user_message=user_message,
...
async def build_prompt_template(
    ...
    system_message_instances: List[str],
    user_message_instances: List[str],
    response_model: Union[str, BaseModel],
    llm: LM,
):
    # One LM to split the prompt into topics
    system_topics = await get_topics_for_prompt(
        system_message, system_message_instances[0:10], llm
    )
    user_topics = await get_topics_for_prompt(
        user_message, user_message_instances[0:10], llm
    )
    ...
    topic_assignments: List[str] = await assign_topics_to_prompt_section(
        system_message, system_topics, llm
    )
    ...
    system_topic_breakdowns = [
        await break_down_prompt_content_for_topic_into_components(
            system_message, system_message_instances, topic_info, llm
        )
        for topic_info in system_topics
    ]
    user_topics = [
        await break_down_prompt_content_for_topic_into_components(
            user_message, user_message_instances, topic_info, llm
        )
...
async def match_aliases_to_prompt_template_inputs(
    aliases: List[str], prompt_template: PromptTemplate, llm: LM
) -> Dict[str, str]:
    system_message = """
...
async def ground_program_to_single_step_dag(
    ...
    prompt_template = await build_prompt_template(
        ...
        input_aliases,
        output_alias,
        system_message_instances,
        user_message_instances,
        response_model,
        ground_llm,
    )
...
async def messages_to_dag_and_benchmark(
    ...
    template, dag = await ground_program_to_single_step_dag(
        ...
        system_message=messages[0][0],
        user_message=messages[0][1],
        input_aliases=input_keys,
        output_alias="<<<ANSWER>>>",
        system_message_instances=[messages[i][0] for i in range(1, 100)],
        user_message_instances=[messages[i][1] for i in range(1, 100)],
        response_model="str",
