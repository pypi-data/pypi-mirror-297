...
from gravybox.framework import create_app
...
async def test_framework_no_trace_id_on_link_request():
    with TestClient(app) as client:
        ...
        response = client.post("/success", json=test_request)
...
async def test_framework_success():
    with TestClient(app) as client:
        ...
        response = client.post("/success", json=test_request.model_dump())
...
async def test_framework_data_unavailable():
    with TestClient(app) as client:
        ...
        response = client.post("/data_unavailable", json=test_request.model_dump())
        assert response.status_code == 200
        test_response = TestResponse.model_validate(response.json())
        assert test_response.success is False
...
async def test_framework_timeout():
    with TestClient(app) as client:
        ...
        response = client.post("/timeout", json=test_request.model_dump())
...
async def test_framework_failure():
    with TestClient(app) as client:
        ...
        response = client.post("/failure", json=test_request.model_dump())
        assert response.status_code == 500
        test_response = TestResponse.model_validate(response.json())
        assert test_response.success is False
...
async def test_framework_http_error():
    with TestClient(app) as client:
        test_request = TestRequest(trace_id="failure", value_one=3, value_two=False)
        response = client.post("/http_exception", json=test_request.model_dump())
