...
class IngresoRoute(Route):
    ...
    def cliente():
        if request.method == "POST":
            return request.form
        client_view: ViewConfig = CrudEntityStore.getView(ClienteViewCfg.entity_name)
    ...
    def vehiculo():
        if request.method == "POST":
            ...
            return request.form
        CrudEntityStore.getEntity(VehiculoViewCfg.entity_name)
        vehiculoView: ViewConfig = CrudEntityStore.getView(VehiculoViewCfg.entity_name)
        return MenuBase.render(
             "macros/template_factory.html",
...
class EliminacionMantenimientoRoute(Route):
    ...
    def eliminar():
        if request.method == "POST":
            return request.form
        viewDeleteMantenimiento = CrudEntityStore.getView(MantenimientoEliminacionViewCfg.entity_name)
        return MenuBase.renderFromFactory(
            main_title="Eliminación de mantenimientos",
            comps_data=[
                {
                    "type":"form",
                    "form_data": viewDeleteMantenimiento.form.toForm(None) 
                }
            ],
        )
