...
class CasLoginView(LoginView):
    ...
    def successful_login(self, request: HttpRequest, next_page: str) -> HttpResponse:
        ...
        # 在请求中的 session 获取 attributes 信息
        # print(request.session.items())
        attributes = request.session.get('attributes', {})
        # logger.debug(f'CAS successful_login response: user: {user}, attributes: {attributes}')
        avatar = attributes.get('avatar', [])
        if avatar:
            avatar = avatar[0]
        if check_json_format(avatar):
            avatar = json.loads(avatar)
        else:
            avatar = avatar.strip('()').split(',')[1]
        ...
        # 在 session 中添加头像
        # print(f"{avatar}.jpeg")
        request.session['avatar'] = f"{avatar}.jpeg"
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        user = authenticate(ticket=ticket,
                            service=service_url,
                            request=request)
        # print(request.session.items()) 查看单点登录后信息
        # dict_items([('attributes', {'authType': 'ehr', 'avatar': ['(thumb
