...
try:
    from renardo_lib.Patterns import PGroup, Pattern
    from renardo_lib.Root import Note, Root
    from renardo_lib.TimeVar import TimeVar
except ModuleNotFoundError:
    try:
        from FoxDot.lib.Patterns import PGroup, Pattern
...
class Chord(PGroup):
    ...
    TODO
    ----
    - [ ] Allow `Scale` that is not `chromatic` [WIP].
        - [X] chromatic
        - [X] major
        - [ ] majorPentatonic
        - [ ] minor
...
class __chords__:  # noqa: N801
    ...
    def __new(self, chords: Union[str, list], *args: str) -> Pattern | TimeVar:
        if isinstance(chords, str):
            chords = chords.split(',')

        harmony = []
        repets = []
        for chord in map(str.strip, chain(chords, args)):
            repet = 1
