...
from typing import Any, Callable, Dict, Literal, Sequence, Union, get_args, get_origin, get_type_hints
...
def get_tool_defs(tools: list[Callable]) -> tuple[dict[str, Callable], list[ToolTypeDef]]:
    name_to_tool = {}
    tool_defs: list[ToolTypeDef] = []
    for tool in tools:
        name_to_tool[tool.__name__] = tool
        tool_defs.append({"toolSpec": get_tool_spec(tool)})

    return (name_to_tool, tool_defs)


def maybe_execute_tools(message: Message, name_to_tool: dict[str, Callable]) -> Sequence[ToolResultBlockTypeDef]:
    return [execute_tool(block["toolUse"], name_to_tool) for block in message["content"] if block.get("toolUse")]
