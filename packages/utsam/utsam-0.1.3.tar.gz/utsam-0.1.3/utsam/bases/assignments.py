...
from utsam.bases.params import SemesterParams, _LOGS_
...
class AssignmentMarking(AssignmentDir):
    def __init__(
        ...
        self.all_marks_df = all_marks_df
    ...
    def plot_hist_by_criteria_tutor(self, criteria_rank, tutor_rank, marks_type='raw', employee_col_name='employee_first_name', bins=20):
        ...
        col_name = 'criteria_student_score' if marks_type == 'raw' else 'new_criteria_student_score'
    ...
    def plot_hists(self, marks_type='raw', employee_col_name='employee_first_name', bins=20):
        col_name = 'criteria_student_score' if marks_type == 'raw' else 'new_criteria_student_score'
    ...
    def set_score_and_grade(self):
        if len(self.all_new_marks_df) > 0:
            ...
            self.all_new_marks_df['new_weighted_criteria_student_score'] = self.all_new_marks_df['new_criteria_student_score'] * self.all_new_marks_df[PgAssignmentCriteriasTable.criteria_weight]
            new_student_marks = self.all_new_marks_df.groupby(['group_id', PgStudentsTable.student_id]).agg({'new_weighted_criteria_student_score': 'sum'}).reset_index()
            return new_student_marks
