...
def get_server_args(server, content_type):
    return [
            server.host,
            server.db_port,
            server.default_db_name,
...
def backup_server_databases(modeladmin, request, queryset):
    ...
    for server in queryset:
        args = get_server_args(server, content_type)
...
@admin.action(description='Backup all databases - locally')
def backup_server_databases_local(modeladmin, request, queryset):
    content_type = ContentType.objects.get_for_model(Server)
    for server in queryset:
        args = get_server_args(server, content_type)
        args.append(True)
...
def get_db_args(database, content_type):
    return [
            database.db_host,
            database.db_port,
            database.db_name,
...
def backup_databases(modeladmin, request, queryset):
    ...
    for database in queryset:
        args = get_db_args(database, content_type)
        if hasattr(settings, 'CELERY_BROKER_URL'):
            backup_database.delay(*args)
        else:
            backup_database(*args)
    messages.info(request, "Backup for selected databases has started...")

@admin.action(description='Backup database - locally')
def backup_databases_local(modeladmin, request, queryset):
    content_type = ContentType.objects.get_for_model(Database)
    for database in queryset:
        args = get_db_args(database, content_type)
        args.append(True)
