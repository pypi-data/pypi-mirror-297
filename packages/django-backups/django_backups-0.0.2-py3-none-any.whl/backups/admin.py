...
class ServerAdmin(admin.ModelAdmin):
    ...
    actions = [update_connection_status, update_server_databases, backup_server_databases, backup_server_databases_local]

class DatabaseAdmin(admin.ModelAdmin):
    list_display = ['db_name', 'server_id']
    ordering = []
    inlines = [DatabaseInline]
    actions = [update_database_tables, backup_databases, backup_databases_local]

class BackupEntryAdmin(admin.ModelAdmin):
    list_display = ['get_content_type_model', 'get_url']
    def get_url(self, obj):
        return format_html(
                f'''<a href="{obj.file if obj.file[:4] == 'http' else '/' + obj.file}" target="_blank">{obj.file}</a>'''
            )
