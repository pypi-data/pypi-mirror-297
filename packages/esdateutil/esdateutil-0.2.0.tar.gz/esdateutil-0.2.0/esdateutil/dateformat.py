...
# Dateformat String APIs
# TODO What is best api for this? I kinda like how croniter does what it does, mostly.
class DateFormatter:
    def __init__(self, fmt_fns, fmt_names=None):
        if not fmt_fns and type(fmt_fns) is list:
            raise ValueError("DateFormatter must be a list of functions that take a string, tzinfo and pos. Instead received {}".format(fmt_fns))
        self.fmt_fns = fmt_fns
        if fmt_names:
            self.fmt_names = fmt_names
        else:
            self.fmt_names = [fn.__name__ for fn in fmt_fns]

    def __repr__(self):
        return "DateFormatter({})".format('||'.join(self.fmt_names))
...
def dateformat_list(fmts):
    fmt_fns = []
    for fmt in fmts:
        try:
            fmt_fn = DATE_FORMATS[fmt]
        # If the string isn't known, attempt to treat as strptime format
        except KeyError:
            # TODO timezone handling. s can contain TZ information depending on
            # format, check for none and write to datetime
            #fmt_fn = lambda s, tzinfo, pos: datetime.strptime(s, fmt).replace(tzinfo=tzinfo)
            fmt_fn = lambda s, pos: datetime.strptime(s, fmt)
        fmt_fns.append(fmt_fn)
    return DateFormatter(fmt_fns, fmts)

def dateformat_str(fmt_str="", separator="||"):
    if not fmt_str:
        return DateFormatter([strict_date_optional_time, epoch_millis], ["strict_date_optional_time", "epoch_millis"])
    return dateformat_list(fmt_str.split(separator))
dateformat = dateformat_str
