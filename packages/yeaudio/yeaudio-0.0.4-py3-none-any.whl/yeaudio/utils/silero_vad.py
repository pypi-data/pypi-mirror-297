...
def load_silero_vad():
    ...
    model = OnnxWrapper(path, force_onnx_cpu=True)
    return model
...
def get_speech_timestamps(audio,
    ...
    Parameters
    ----------
    audio: numpy.ndarray, One dimensional float

    model: preloaded silero VAD model
    ...
    """
    assert len(audio.shape) == 1, "More than one dimension in audio. Are you trying to process audio with 2 channels?"
...
class VADIterator:
    ...
    def __call__(self, x, return_seconds=False):
        """
        x: audio chunk (see examples in repo)
