...
def convert_to_string_key(user_data: Any, max_length: int) -> StringResult:
    return convert_to_string_truncated(user_data, size=max_length)


def convert_to_string_value(
    user_data: Any, source: Optional[str] = None, show_warning: bool = True
) -> str:
    res = convert_to_string_truncated(user_data, 1000, source=source)
    if res.truncated and show_warning:
        LOGGER.warning(
            CONVERT_UTILS_CONVERT_TO_STRING_TRUNCATED_WARNING, res.full_value
        )
