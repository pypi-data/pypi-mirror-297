...
def _raise_error_configuration():
...
def _get_config_obj(config: Optional[str] = None, default_to_union_semantics: bool = False) -> Config:
    ...
    if _UNION_CONFIG.is_direct_union_cli_call or default_to_union_semantics:
        if config is not None:
            _UNION_CONFIG.config_source = "--config passed in"
        ...
        elif _UNION_CONFIG.config_obj is not None:
            return _UNION_CONFIG.config_obj

        else:
            _raise_error_configuration()
            config = Config.for_endpoint(endpoint=_DEFAULT_GCP_SERVERLESS_ENDPOINT)

        _UNION_CONFIG.config_obj = config
        return config
    else:
        # CLI ran with `pyflyte`, we use the same semantics as `pyflyte`
        cfg_file = get_config_file(config)
        if cfg_file is None:
            # Use the config_obj cache here to return the cached value from --config
        ...
        _UNION_CONFIG.config_obj = Config.auto(cfg_file)
        return _UNION_CONFIG.config_obj
