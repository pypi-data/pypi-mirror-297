...
class ChassisControl:
    ...
    def go_straight(self, speed=0.2):
        ...
        if speed < 0 or speed > 1:
            raise Exception(
                "The movement speed range is 0~1, but the received value is {}".format(speed))
    ...
    def go_back(self, speed=-0.2):
        ...
        if not -1 <= speed <= 0:
            raise Exception(
                "The movement speed range is -1~0, but the received value is {}".format(speed))
        self.Send_Data = [0] * 11
        self.Send_Data[0] = ProtocolCode.header
        self.Send_Data[1] = 0
        self.Send_Data[2] = 0
