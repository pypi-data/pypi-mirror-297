...
class KafkaClient:
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(KafkaClient, "_instance"):
            with KafkaClient._instance_lock:
                if not hasattr(KafkaClient, "_instance"):
                    KafkaClient._instance = object.__new__(cls)
    ...
    def produce_message(self, topic: str, key: str, value: str):
        ...
        try:
            ...
            [producer.poll(0.05) for _ in range(3)]
    ...
    def produce_messages_in_batch(self, topic: str, messages: list, poll_interval: float = 0.1):
        ...
        try:
            for key, value in messages:
                producer.produce(topic, key=key, value=value, callback=delivery_callback)
                producer.poll(0)  # 定期调用以处理异步消息

            while True:
                remaining_messages = producer.flush(0)
    ...
    def consume_messages_with_threads(self, topics: list, handle_func, num_threads=3, timeout=1.0):
        """多线程消费 Kafka 消息"""
