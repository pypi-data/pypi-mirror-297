...
class JerrisClient(BaseModel):
    ...
    api_key: str = Field(
        default=None, description="The API key for authenticating requests"
    )
            ...
            if response is not None:
                if response.status_code == 404:
                    return {
                        ...
                        "status": API_STATUS_ERROR,
                        "error": "ResourceNotFound",
                        "message": "API Resource not found, please check the API base URL.",
                    }
    ...
    def get_report(self, process_id: str) -> AnalyzeResponse:
        return self._make_request(
            path=f"analyse/url/async/report?analyze_photo_result_id={process_id}",
            method="GET",
        )
