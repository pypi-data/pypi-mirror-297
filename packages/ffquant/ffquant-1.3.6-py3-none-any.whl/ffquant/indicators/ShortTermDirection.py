...
class ShortTermDirection(BaseIndicator):
    ...
    def handle_api_resp(self, item):
        ...
        if item['SHORT_TERM_DIR'] == 'BULLISH':
            self.cache[result_time_str] = self.BULLISH
        elif item['SHORT_TERM_DIR'] == 'BEARISH':
            self.cache[result_time_str] = self.BEARISH

        if self.p.debug:
            print(f"{self.__class__.__name__}, result_time_str: {result_time_str}, short_term_dir: {item['SHORT_TERM_DIR']}")
