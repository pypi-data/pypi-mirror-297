...
class TurningPoint(BaseIndicator):
    ...
    def handle_api_resp(self, item):
        ...
        if item['TYPE_TURNING_POINT'] == 'TURNING_UP':
            self.cache[result_time_str] = self.TURNING_UP
        elif item['TYPE_TURNING_POINT'] == 'TURNING_DN':
            self.cache[result_time_str] = self.TURNING_DN

        if self.p.debug:
            print(f"{self.__class__.__name__}, result_time_str: {result_time_str}, type_turning_point: {item['TYPE_TURNING_POINT']}")
