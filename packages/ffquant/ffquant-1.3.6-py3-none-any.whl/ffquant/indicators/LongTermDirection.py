...
class LongTermDirection(BaseIndicator):
    ...
    def handle_api_resp(self, item):
        ...
        if item['LONG_TERM_DIR'] == 'BULLISH':
            self.cache[result_time_str] = self.BULLISH
        elif item['LONG_TERM_DIR'] == 'BEARISH':
            self.cache[result_time_str] = self.BEARISH

        if self.p.debug:
            print(f"{self.__class__.__name__}, result_time_str: {result_time_str}, long_term_dir: {item['LONG_TERM_DIR']}")
