...
class ShortTermDirection(BaseIndicator):
    ...
    def handle_api_resp(self, item):
        ...
        if item['shortTermDir'] == 'BULLISH':
            self.cache[result_time_str] = self.BULLISH
        elif item['shortTermDir'] == 'BEARISH':
            self.cache[result_time_str] = self.BEARISH

        if self.p.debug:
            print(f"{self.__class__.__name__}, result_time_str: {result_time_str}, short_term_dir: {item['shortTermDir']}")
