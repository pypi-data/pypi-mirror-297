...
def add_metrics_plot(strat, views, debug=False):
    ...
    if debug:
        print(f"Total Compound Return: {returns['rtot']:.8%}, Annualized Return: {returns['rnorm']:.8%}")

    sharpe = strat.analyzers.sharpe.get_analysis()
    if debug and sharpe['sharperatio'] is not None:
        print(f"Sharpe Ratio: {sharpe['sharperatio']:.8f}")
    ...
    if debug:
        print(f"Annual Return Volatility: {annual_volatility:.8%}")
    ...
    metrics_data = {
        ...
        "Result": [
            f"{returns['rtot']:.8%}",
            f"{returns['rnorm']:.8%}",
            f"{annual_volatility:.8%}",
            f"{sharpe['sharperatio']:.8f}" if sharpe['sharperatio'] is not None else "NaN",
