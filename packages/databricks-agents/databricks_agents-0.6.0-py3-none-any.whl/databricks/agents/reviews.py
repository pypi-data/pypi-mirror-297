...
def _convert_str_object_output_to_chat_completion_output(output_json):
    ...
    # TODO: rework this code once extensible schemas are supported
    output_json.update(
        {"choices": [{"message": {"content": output_json.pop("content")}}]}
    )
...
def _parse_request_logs(request_logs):
    def transform_request_entry(request_logs_row):
        ...
        request_json[_CONVERSATION_ID] = request_json.get(_DATABRICKS_OPTIONS, {}).get(
            _CONVERSATION_ID, ""
        )
...
def retrieve_payload_logs(spark_df):
    ...
    """
    request_logs_with_trace = request_logs.filter(
        F.expr("response:databricks_output.trace IS NOT NULL")
    )
    if not request_logs_with_trace.isEmpty():
        v2_request_logs = request_logs.filter(
            F.expr(
                f"response:databricks_output.trace['{MLFLOW_TRACE_SCHEMA_VERSION}']==2"
            )
        )
    ...
    return request_logs.withColumn(
    ...
    if review_experiment:
        ...
        can_manage_users = [
            ...
            for permission_tuple in list(
                filter(
                    lambda permission_tuple: permission_tuple[1]
