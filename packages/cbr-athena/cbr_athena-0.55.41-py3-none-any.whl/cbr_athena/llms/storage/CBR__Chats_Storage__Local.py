...
class CBR__Chats_Storage__Local(Type_Safe):
    ...
    def chat_save(self, llm_chat_completion: LLMs__Chat_Completion):
        if cbr_config_athena.aws_enabled():                         # todo: refactor this out from this 'Storage__Local' class
            s3_key = cbr_site_shared_objects.s3_db_chat_threads().save_chat_completion(llm_chat_completion)  # save chat in dedicated S3 bucket
