...
def get_onchain_data(search_term, limit=3, start=0, is_exact_match=False):
    ...
    if len(search_term) >= 40:
        search_condition = f"ats.token_id = '{search_term}' OR ats.pair_id = '{search_term}'"
    elif len(search_term) < 40 and is_exact_match:
        search_condition = f"ats.symbol = '{search_term}' OR ats.name = '{search_term}'"
    else:
        search_condition = f"ats.symbol LIKE '{search_term}%' OR ats.name LIKE '{search_term}%' OR ats.name LIKE ' {search_term}%'"
    ...
    if data := read_db.fetchall(query=query):
        for i in data:
            (symbol, name, is_coin, chain, token_id, pair_id, vol_24_hr, liquidity, marketcap, icon, buy_tax, sell_tax,
             ...
             is_blacklisted, can_burn, is_scam, can_freeze, is_contract_verified, pc_24_hr) = i
