...
class SnowflakeCatalog(Catalog):
    ...
    def config(self):
        return {
            ...
            "RefreshMode": (
                self.refresh_mode.to_proto() if self.refresh_mode else None
            ),
...
class SnowflakeConfig:
    ...
    def __eq__(self, __value: object) -> bool:
        ...
        is_catalog_equal = (self.catalog is None and self.catalog is None) or (
            ...
            and self.catalog.target_lag == __value.catalog.target_lag
            and self.catalog.refresh_mode == __value.catalog.refresh_mode
            and self.catalog.initialize == __value.catalog.initialize
            and self.catalog.table_format == __value.catalog.table_format
        )
        return (
            self.username == __value.username
            and self.password == __value.password
    ...
    def from_proto(source_catalog_table):
        return GlueCatalogTable(
            ...
            table_format=TableFormat.get_format(
                source_catalog_table.table_format
            ),
        )
...
class TrainingSetVariant(ResourceVariant):
    ...
    def get(self, stub):
        return TrainingSetVariant.get_by_name_variant(
            stub, self.name, self.variant
        )
