...
current_ac: str = f'{unknown_ac}_flight_1'
    ...
    def _add_to_dict(pkt: BSTPacket, pkt_data):
        ...
        if pkt.TYPE == PacketTypes.SYSTEM_INITIALIZE:
            ...
            if current_ac.startswith(unknown_ac):
                current_ac = current_ac.replace(unknown_ac, ac_name)
                results = {gcs_name: results[gcs_name], current_ac: results[unknown_ac]}
            elif current_ac.startswith(ac_name) and sys_init_pkt.system_time < ac_sys_time:
                # Same aircraft, increment flight
                current_ac = f'{ac_name}_flight_{int(current_ac.split("_")[-1])+1}'
            else:
                current_ac = f'{ac_name}_flight_1'
