...
class CachedDataGenerator(Sequence):
    ...
    def __init__(self, data, batch_size=32, mode=None, n_jobs=1, dtype=np.float32, cache_folder=None):
        self.data_orig = data
        self.df = None
    ...
    def update_XYw(self, force=False):
        if self.X is None or force:
            self.X, self.Y, self.w = self.calc_XYw(self.df)
    ...
    def calc_XYw(self, df, with_tqdm=None):
        print(f'- INFO: about to calculate XYw (may be slow if not cached; n_jobs={self.n_jobs}) [{self.mode}]')
        ...
        xparallel.x_on_iter(df[:1].itertuples(index=False), self._get_sample_wrapper, with_tqdm=False, n_jobs=1)
        ...
        #
        # gather the samples
        samples = xparallel.x_on_iter(df.itertuples(index=False), self._get_sample_wrapper, with_tqdm=with_tqdm, tqdm_total=len(df), n_jobs=self.n_jobs, tqdm_desc=f'calc XYw - {self.mode}')

        #
        # initialize data format:
        if self._with_weight is None:
            sample0 = samples[0]
            assert isinstance(sample0, DataSample)
        ...
        for idx, shape in enumerate(self._input_shapes):
            arr = np.stack([s.inputs[idx] for s in samples])
        ...
        if self._with_outputs:
            ...
            for idx, shape in enumerate(self._output_shapes):
                arr = np.stack([s.outputs[idx] for s in samples])
                arr = arr.reshape([n]+shape)
                final_outputs.append(arr)

        if self._with_weight:
            final_weights = np.array([s.weight for s in samples])

        return final_inputs, final_outputs, final_weights
