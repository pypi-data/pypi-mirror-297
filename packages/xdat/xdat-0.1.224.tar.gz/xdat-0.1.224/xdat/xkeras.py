...
class CachedDataGenerator(Sequence):
    ...
    def __init__(self, data, batch_size=32, mode=None, n_jobs=1, cache_folder=None):
        self.data_orig = data
        self.df = None
    ...
    def calc_XYw(self, df, with_tqdm=None):
        print(f'- INFO: about to calculate XYw (may be slow if not cached or if n_jobs=1) [{self.mode}]')
        ...
        xparallel.x_on_iter(df[:1].itertuples(index=False), self.get_sample, with_tqdm=False, n_jobs=1)
        ...
        #
        # gather the samples
        samples = xparallel.x_on_iter(df.itertuples(index=False), self.get_sample, with_tqdm=with_tqdm, tqdm_total=len(df), n_jobs=self.n_jobs, tqdm_desc=f'calc XYw - {self.mode}')
