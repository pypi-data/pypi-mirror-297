...
class SoundPlotter(BaseClass):
        ...
        for idx, data in enumerate(self.data):
            ...
            if data["type"] == "waveform":
                self._plot_waveform(
                    data["data"],
                    current_ax,
                    time_offset_sec=data["time_offset_sec"],
                    title=titles[idx],
                )
        ...
        for idx_annot, annot in enumerate(
            ...
            for idx_panel in annot["panel"]:  # for each panel
                ...
                if annot["label"] is not False:
                    ...
                    current_ax.legend(
                        new_handles, unique_labels, loc="upper right"
                    )

                if annot["tag"] is True:
                    bbox_props = dict(
                        boxstyle="square", fc="w", ec="w", alpha=0.8
                    )
    ...
    def _plot_annotations(
        self, annot, ax, label, panel_idx, color, line_width
    ):
        ...
        for index, row in annot.data.iterrows():
            ...
            if panel_type == "spectrogram":
                ...
                if self.unit == "sec":
                    ...
                    time_resolution = self.data[panel_idx][
                        "data"
                    ].time_resolution
    ...
    def _plot_spectrogram(
        self, spectro, current_ax, time_offset_sec=0, title=None
    ):
        ...
        if self.unit == "sec":
            ...
            raise ValueError(
                "Keyword 'unit' must be set to either 'sec' or" " 'samp'."
            )
        current_ax.axis(
            [
                self.time_min,
    ...
    def _plot_waveform(self, sound, current_ax, time_offset_sec=0, title=None):
        ...
        if self.unit == "sec":
            axis_t = np.arange(
                0,
                sound.waveform_duration_sample
                / sound.waveform_sampling_frequency,
                1 / sound.waveform_sampling_frequency,
            )
            ...
            axis_t = axis_t + (
                time_offset_sec * sound.waveform_sampling_frequency
            )
            xlabel = "Time (sample)"
        else:
            raise ValueError(
        ...
        current_ax.plot(
            axis_t[0 : len(sound._waveform)], sound._waveform, color="black"
        )
        current_ax.set_xlabel(xlabel)
