...
from malevich_app.export.abstract.abstract import Keys, KeysValues, Info, Message, OperationWithRunWithKey, OperationWithRun, \
    OSGetBatch, OSGetAll, OSGetKeys, SynchronizeSettings, KeysPresigned, Presigned, KeysWithSynchronize
from malevich_app.export.jls.LocalLogsBuffer import LocalLogsBuffer
from malevich_app.export.jls.df import OBJ
...
from malevich_app.export.secondary.const import STORAGE_PATH, APP_DIR, WORKDIR
...
class Context(Generic[MinimalCfg]):
    class _DagKeyValue:
        ...
        async def __get_bytes(self, key: str) -> bytes:
            ...
            return await send_post_dag(operation.json(), GET_KEYS_VALUES_RAW(self.__dag_host_port), raw_res=True,
                                       headers={'Content-type': 'application/json', 'Accept': 'application/python-pickle'})

        def __get_bytes2(self, key: str) -> bytes:
            operation = OperationWithRunWithKey(operationId=self.__operation_id, runId=self.__run_id, key=key, hostedAppId=C.APP_ID, secret=self.__secret)
            return asyncio.run(send_post_dag(operation.json(), GET_KEYS_VALUES_RAW(self.__dag_host_port), raw_res=True,
                                       headers={'Content-type': 'application/json', 'Accept': 'application/python-pickle'}))
        ...
        def get(self, keys: List[str]) -> Dict[str, Any]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), GET_KEYS_VALUES(self.__dag_host_port)))
        ...
        async def async_get(self, keys: List[str]) -> Dict[str, Any]:
            if self.__is_local:
                return self.__local_storage.kv.get(self.__operation_id, self.__run_id, keys)
            operation = Keys(operationId=self.__operation_id, data=keys, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
            res = await send_post_dag(operation.json(), GET_KEYS_VALUES(self.__dag_host_port))
        ...
        def get_all(self) -> Dict[str, Any]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), GET_KEYS_VALUES_ALL(self.__dag_host_port)))
            return self.__fix_get2(res)
        ...
        async def async_get_all(self) -> Dict[str, Any]:
            if self.__is_local:
                return self.__local_storage.kv.get_all(self.__operation_id, self.__run_id)
            operation = OperationWithRun(operationId=self.__operation_id, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
            res = await send_post_dag(operation.json(), GET_KEYS_VALUES_ALL(self.__dag_host_port))
            return await self.__fix_get(res)
        ...
        async def __update(self, keys_values: Dict[str, Any]):
            ...
            for k, v in keys_values.items():
                if isinstance(v, bytes):
                    ...
                    sends.append(send_post_dag(v, f"{POST_KEYS_VALUES_RAW(self.__dag_host_port)}/{k}/{C.APP_ID}/{self.__operation_id}{run_id_part}",    # TODO secret
                                               headers={'Content-type': 'application/python-pickle', 'Accept': 'application/json'}))
                else:
            ...
            if len(main_keys_values) > 0:
                ...
                sends.append(send_post_dag(operation.json(), POST_KEYS_VALUES(self.__dag_host_port)))
            await gather(*sends)
        ...
        def __update2(self, keys_values: Dict[str, Any]):
            ...
            for k, v in keys_values.items():
                if isinstance(v, bytes):
                    run_id_part = f"/{self.__run_id}" if self.__run_id is not None else ""
                    asyncio.run(send_post_dag(v, f"{POST_KEYS_VALUES_RAW(self.__dag_host_port)}/{k}/{C.APP_ID}/{self.__operation_id}{run_id_part}",    # TODO secret
                                               headers={'Content-type': 'application/python-pickle', 'Accept': 'application/json'}))
                else:
                    main_keys_values[k] = v
            if len(main_keys_values) > 0:
                operation = KeysValues(operationId=self.__operation_id, data=main_keys_values, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
                asyncio.run(send_post_dag(operation.json(), POST_KEYS_VALUES(self.__dag_host_port)))
        ...
        def clear(self) -> None:
            ...
            operation = OperationWithRun(operationId=self.__operation_id, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
            asyncio.run(send_delete_dag(operation.json(), DELETE_KEYS_VALUES_ALL(self.__dag_host_port)))

        async def async_clear(self) -> None:
            if self.__is_local:
                return self.__local_storage.kv.clear(self.__operation_id, self.__run_id)
            operation = OperationWithRun(operationId=self.__operation_id, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
            await send_delete_dag(operation.json(), DELETE_KEYS_VALUES_ALL(self.__dag_host_port))
    ...
    class _ObjectStorage:
        ...
        def get_keys(self, local: bool = False, all_apps: bool = False) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), GET_OBJ_STORAGE_KEYS(self.__dag_host_port)))
            return res["result"]
        ...
        async def async_get_keys(self, local: bool = False, all_apps: bool = False) -> List[str]:
            if self.__is_local:
                raise Exception("object storage not allow in local mode yet")
            operation = OSGetKeys(operationId=self.__operation_id, local=local, hostedAppId=C.APP_ID, secret=self.__secret, allApps=all_apps)
            res = await send_post_dag(operation.json(), GET_OBJ_STORAGE_KEYS(self.__dag_host_port))
            return res["result"]
        ...
        def get(self, keys: List[str], force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), GET_OBJ_STORAGE(self.__dag_host_port)))
            return res["result"]
        ...
        async def async_get(self, keys: List[str], force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            if all_apps and not self.__single_pod and not force:
                ...
                force = True
            operation = OSGetBatch(operationId=self.__operation_id, data=keys, force=force, hostedAppId=C.APP_ID, secret=self.__secret, allApps=all_apps)
            res = await send_post_dag(operation.json(), GET_OBJ_STORAGE(self.__dag_host_port))
            return res["result"]
        ...
        def get_all(self, local: bool = False, force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), GET_OBJ_STORAGE_ALL(self.__dag_host_port)))
            return res["result"]
        ...
        async def async_get_all(self, local: bool = False, force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            if not local and all_apps and not self.__single_pod and not force:
                ...
                all_apps = False
            operation = OSGetAll(operationId=self.__operation_id, local=local, force=force, hostedAppId=C.APP_ID, secret=self.__secret, allApps=all_apps)
            res = await send_post_dag(operation.json(), GET_OBJ_STORAGE_ALL(self.__dag_host_port))
            return res["result"]
        ...
        def update(self, keys: List[str], presigned_expire: Optional[int] = -1) -> Dict[str, str]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), POST_OBJ_STORAGE(self.__dag_host_port)))
            return res["result"]
        ...
        async def async_update(self, keys: List[str], presigned_expire: Optional[int] = -1) -> Dict[str, str]:
            if self.__is_local:
                raise Exception("object storage not allow in local mode yet")
            operation = KeysPresigned(operationId=self.__operation_id, data=keys, presigned=presigned_expire, hostedAppId=C.APP_ID, secret=self.__secret)
            res = await send_post_dag(operation.json(), POST_OBJ_STORAGE(self.__dag_host_port))
            return res["result"]
        ...
        def presigned(self, keys: List[str], expire: Optional[int] = None) -> Dict[str, str]:
            ...
            res = asyncio.run(send_post_dag(operation.json(), POST_PRESIGNED_OBJ_STORAGE(self.__dag_host_port)))
            return res["result"]
        ...
        async def async_presigned(self, keys: List[str], expire: Optional[int] = None) -> Dict[str, str]:
            ...
            assert expire is None or expire >= 0, "expire value should be >= 0 or None"
            operation = Presigned(operationId=self.__operation_id, data=keys, presigned=expire, hostedAppId=C.APP_ID, secret=self.__secret)
            res = await send_post_dag(operation.json(), POST_PRESIGNED_OBJ_STORAGE(self.__dag_host_port))
            return res["result"]
        ...
        def delete(self, keys: List[str]) -> None:
            ...
            asyncio.run(send_delete_dag(operation.json(), DELETE_OBJ_STORAGE(self.__dag_host_port)))

        async def async_delete(self, keys: List[str]) -> None:
            if self.__is_local:
                raise Exception("object storage not allow in local mode yet")
            operation = KeysWithSynchronize(operationId=self.__operation_id, data=keys, hostedAppId=C.APP_ID, secret=self.__secret)
            await send_delete_dag(operation.json(), DELETE_OBJ_STORAGE(self.__dag_host_port))
    ...
    def __share(self, path: str, all_runs: bool, path_prefix: str, force: bool, ignore_not_exists: bool = False) -> bool:
        ...
        if not os.path.exists(realpath):
            ...
            return False
        mnt_path = f"{STORAGE_PATH(self.__operation_id)}{run_id}/{path}"
        ...
        if not self.__single_pod:
            ...
            if C.IS_EXTERNAL:
                ...
                if data is not None and len(data) > 0:
                    asyncio.run(send_post_dag(data, self.__objects_url(all_runs), headers={'Content-type': 'application/octet-stream'}))    # TODO secret
            else:
                ...
                asyncio.run(send_post_dag(settings.json(), SYNCHRONIZE(self.__dag_host_port)))
    ...
    async def async_synchronize(self, paths: Optional[List[str]] = None, all_runs: bool = False) -> None:
        ...
        if not self.__single_pod:
            ...
            if C.IS_EXTERNAL:
                data = zip_raw(paths, self.__operation_id, run_id, self.__japp_logs_buffer)
                if data is not None and len(data) > 0:
                    await send_post_dag(data, self.__objects_url(all_runs), headers={'Content-type': 'application/octet-stream'})   # TODO secret
            else:
                settings = SynchronizeSettings(operationId=self.__operation_id, runId=run_id, paths=paths, hostedAppId=C.APP_ID, secret=self.__secret)
                await send_post_dag(settings.json(), SYNCHRONIZE(self.__dag_host_port))
    ...
    async def __send_msg(self, data, url, headers, wrap, with_result):
        try:
            ...
            if wrap:
                data = Info(data=data, operationId=self.__operation_id, infoType="msg").json()
    ...
    async def async_msg(self, data: Union[str, Dict], url: Optional[str] = None, headers: Optional[Dict[str, str]] = None, wait: bool = False, wrap: bool = True, with_result: bool = False):
        ...
        if wait:
            ...
            send_background_task(self.__send_msg, data, url, headers, wrap, with_result)
    ...
    def email_send(self, message: str, subject: Optional[str] = None, type: str = "gmail") -> None:
        ...
        try:
            asyncio.run(send_post_dag(msg.json(), MESSAGE(self.__dag_host_port), raw_res=True)) # FIXME
        except:
    ...
    def get_scale_part(self, df: pd.DataFrame) -> pd.DataFrame:
        """return scale part of df"""
        div, mod = divmod(df.shape[0], self.__indexes)
        return df.iloc[div * self.__index + min(mod, self.__index):div * (self.__index + 1) + min(mod, self.__index + 1)]
