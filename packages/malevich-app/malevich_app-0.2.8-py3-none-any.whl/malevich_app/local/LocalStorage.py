...
class LocalStorage:
    def __init__(self, local_settings: LocalRunStruct, schemes: Dict[str, Tuple[List[str], Set[str]]]):
    ...
    def save_data(self, operation_id: str, run_id: str, data: Union[pd.DataFrame, List, Dict], scheme: Optional[str], name: str, group_name: str, index: int, is_doc: bool):
        ...
        collection_name = "data" if not is_doc else "json"
        ...
        # extra save
        if self.__results_dir is not None:
            path = os.path.join(self.__results_dir, run_id, name, str(index))
            os.makedirs(os.path.dirname(path), exist_ok=True)
            if is_doc:
    ...
    def __get_doc(self, operation_id: str, id: str):
        id = id.removeprefix(JsonCollection.prefix)
    ...
    def scheme(self, name: str, scheme_json: Optional[str] = None, path: Optional[str] = None):
        assert (scheme_json is not None) + (path is not None) == 1, "scheme create fail: should set scheme_json or path"

        if path is not None:
            with open(path, 'r') as fr:
                scheme_json = fr.read()
        self.__schemes[name] = scheme_json_columns(scheme_json)
