...
def update_dummy_scheme(scheme_name: str):
    if not hasattr(__dummy_schemes_module, scheme_name):
        setattr(__dummy_schemes_module, scheme_name, type(scheme_name, (), {}))
    if not hasattr(__dummy_schemes_module2, scheme_name):   # FIXME
        setattr(__dummy_schemes_module2, scheme_name, type(scheme_name, (), {}))
...
def get_julius_app(app: Optional[App] = None, cfg: Optional[Cfg] = None, mode: str = "run", base_j_app: Optional[Any] = None, app_cfg_extension: Optional[str] = None, *args, **kwargs):
    ...
    with __lock:
        ...
        jls_module = importlib.import_module("jls")
                    ...
                    if name[-3:] == ".py":
                        ...
                        while reimport:
                            try:
                                ...
                                if ("jls" in er.name or "malevich" in er.name or "square" in er.name) and er.msg.startswith("cannot import name '"):
