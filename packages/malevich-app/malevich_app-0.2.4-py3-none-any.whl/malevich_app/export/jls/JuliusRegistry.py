...
from malevich_app.export.abstract.abstract import PipelineApp, Cfg, AppFunctionsInfo, InputFunctionInfo, ProcessorFunctionInfo, \
    OutputFunctionInfo, ConditionFunctionInfo, InitInfo, AnyInit, TempRunScheme
...
from malevich_app.export.secondary.endpoints import RUN_SCHEME
...
class JuliusRegistry(JuliusEmptyRegistry):
    ...
    def __register(self, import_dirs: Optional[List[str]] = None):  # FIXME customize for user
        ...
        lib_modules = [importlib.import_module("jls"), importlib.import_module("malevich"), importlib.import_module("malevich.square")]     # TODO import "defaults"

        for dir in import_dirs:
            assert os.path.isdir(dir), f"{dir} directory should exist"
        imported = set()
        for import_dir in import_dirs:
            for path, _, names in os.walk(import_dir):
                for name in names:
                    if name[-3:] == ".py":
                        reimport = True
                        failed = False
                        ...
                        while reimport:
                            try:
                                ...
                                if er.msg.startswith("cannot import name '") and ("jls" in er.name or "malevich" in er.name or "square" in er.name):
                                    failed = True
    ...
    def info(self) -> AppFunctionsInfo:
        return AppFunctionsInfo(
            inputs={id: InputFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], mode=kwargs["mode"].value) for id, (args, kwargs) in self.__inputs.items()},
            processors={id: ProcessorFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], contextClass=get_context_info(get_fun_info(args[0])[0])) for id, (args, kwargs) in self.__processors.items()},     # TODO contextClass
            outputs={id: OutputFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], collectionOutNames=kwargs["collection_out_names"] if (collection_out_name := kwargs["collection_out_name"]) is None else [collection_out_name]) for id, (args, kwargs) in self.__outputs.items()},
            conditions={id: ConditionFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"]) for id, (args, kwargs) in self.__conditions.items()},
            schemes={id: json_schema(cl) for id, cl in self.__schemes.items()},
            inits={id: InitInfo(id=id, enable=kwargs["enable"], tl=kwargs["tl"], prepare=kwargs["prepare"], argname=get_context_argname(args[0]), doc=args[0].__doc__) for id, (args, kwargs) in self.__inits.items()},
    ...
    async def update_schemes_pipeline(self, operation_id: str):
        ...
        for name, cl in self.__schemes.items():
            ...
            if not self.__is_local:
                tasks.append(post_request_json(RUN_SCHEME, temp_run_scheme.json(), text=True, buffer=self.logs_buffer))
        if len(tasks) > 0:
            await gather(*tasks)
    ...
    def create_app(self, init: AnyInit, bind_id: str, cfg: Optional[Cfg], pipeline_app: Optional[PipelineApp] = None, app_cfg: Any = None, app_cfg_extension: Optional[str] = None, local_dfs: Optional[LocalDfs] = None, *args, **kwargs) -> JuliusApp:
        ...
        if is_init:
            ...
            j_app = JuliusApp(version=2, local_dfs=local_dfs, exist_schemes=self.__exist_schemes, local_storage=self.__local_storage, logger_fun=self.__logger_fun)
