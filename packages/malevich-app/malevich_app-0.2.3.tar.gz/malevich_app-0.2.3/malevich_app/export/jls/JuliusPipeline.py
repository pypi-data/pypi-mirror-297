...
class JuliusPipeline:
    def __init__(self, pipeline: InitPipeline, registry, logs_buffer: io.StringIO, storage: Optional['LocalStorage'] = None):
        ...
        self.__alt_deps: Dict[Tuple[str, int, str, int], Dict[str, int]] = {}                           # (bind_id, bind_id iteration, arg_name, index in alternative arg list) -> proc bind_id -> iteration
    ...
    async def __run(self, run_id: str, iteration: int, items: Dict[str, JuliusPipelineItem], bind_id: str) -> bool:
        ...
        if pipeline_item.japp.get_operation() == EntityType.CONDITION:
            try:
                args = await self.__collections_holder.interpret(pipeline_item.japp, collections, pipeline_item.japp.get_cfg())
            ...
            if not ok:
                ...
                await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, ok, PipelineStructureUpdate(), branch=is_true)
                return ok
                    ...
                    for name, num_conds in name_to_num_conds.items():
                        ...
                        for num, cond in num_conds:
                            ...
                            if (runned and (cond is None or cond == cond_value or (cond_value is None and cond))) or \
                                    (not runned and cond == False):
                                if is_processor:
                                    alt_dep_key = (next_bind_id, next_bind_id_iteration, name, num)
                            ...
                            for name, num_conds in name_to_num_conds.items():
                                ...
                                for num, cond in num_conds:
                                    ...
                                    if (runned and (cond is None or cond == cond_value or (cond_value is None and cond))) or \
                                        ...
                                        if is_processor:
                                            alt_dep_key = (next_bind_id, next_iteration, name, num)
                                            deps = self.__alt_deps.get(alt_dep_key)
                                            if deps is None:
                                                deps = {}
                                                self.__alt_deps[alt_dep_key] = deps
                                ...
                                for name, num_conds in name_to_num_conds.items():
                                    ...
                                    for num, cond in num_conds:
                                        ...
                                        if (runned and (cond is None or cond == cond_value or (cond_value is None and cond))) or \
                                            ...
                                            if first and is_processor:
                                                alt_dep_key = (next_bind_id, cluster_iteration, name, num)
                                                deps = self.__alt_deps.get(alt_dep_key)
                                                if deps is None:
                                                    deps = {}
                                                    self.__alt_deps[alt_dep_key] = deps
