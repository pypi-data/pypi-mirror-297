...
from drt_sdk_cli import dependencies
from drt_sdk_cli.localnet.config_root import ConfigRoot
from drt_sdk_cli.localnet.config_software import (SoftwareComponent,
                                                         SoftwareResolution)
...
def fetch_prerequisites(configfile: Path):
    ...
    if config.software.drt_chain_go.resolution == SoftwareResolution.Remote:
        download_software_component(config.software.drt_chain_go)

    if config.software.drt_chain_proxy_go.resolution == SoftwareResolution.Remote:
        download_software_component(config.software.drt_chain_proxy_go)

    config.software.drt_chain_go.node_config_must_exist()
    config.software.drt_chain_go.seednode_config_must_exist()
    config.software.drt_chain_proxy_go.proxy_config_must_exist()

    is_node_built = config.software.drt_chain_go.is_node_built()
    is_seednode_built = config.software.drt_chain_go.is_seednode_built()
    is_proxy_built = config.software.drt_chain_proxy_go.is_proxy_built()
