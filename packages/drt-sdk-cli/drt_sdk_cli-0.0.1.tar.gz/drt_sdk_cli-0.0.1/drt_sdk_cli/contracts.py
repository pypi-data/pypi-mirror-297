...
from drt_sdk_core import (Token, TokenComputer, TokenTransfer,
                                 Transaction, TransactionPayload)
from drt_sdk_core.address import Address
from drt_sdk_core.transaction_factories import \
    SmartContractTransactionsFactory
from drt_sdk_network_providers.interface import IContractQuery

from drt_sdk_cli import errors
from drt_sdk_cli.accounts import Account
from drt_sdk_cli.constants import DEFAULT_HRP
from drt_sdk_cli.interfaces import IAddress
from drt_sdk_cli.utils import Object
...
def prepare_args_for_factory(arguments: List[str]) -> List[Any]:
    ...
    for arg in arguments:
        if arg.startswith(HEX_PREFIX):
            ...
            raise errors.BadUserInput(f"Unknown argument type for argument: `{arg}`. Use `drtpy contract <sub-command> --help` to check all supported arguments")
