...
from drt_sdk_cli import dependencies, utils, workstation
from drt_sdk_cli.errors import KnownError
from drt_sdk_cli.localnet import libraries
...
def build(configfile: Path, software_components: List[str]):
    ...
    if "node" in software_components:
        ...
        cmd_node = config.software.drt_chain_go.get_cmd_node_folder()
    ...
    if "seednode" in software_components:
        ...
        cmd_seednode = config.software.drt_chain_go.get_cmd_seednode_folder()
    ...
    if "proxy" in software_components:
        ...
        cmd_proxy = config.software.drt_chain_proxy_go.get_cmd_proxy_folder()
...
def _get_chain_vm_go_folder_name(config: ConfigRoot) -> str:
    go_mod = config.software.drt_chain_go.get_path_within_source(Path("go.mod"))
    lines = utils.read_lines(go_mod)
    line = [line for line in lines if "github.com/DharitriOne/drt-chain-vm-go" in line][0]
    parts = line.split()
...
def _set_rpath(cmd_path: Path):
    ...
    For other occurrences of this approach, see:
     - https://github.com/DharitriOne/drt-chain-scenario-cli-go/blob/master/.github/workflows/on_release_attach_artifacts.yml
    """
