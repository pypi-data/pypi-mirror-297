...
from drt_sdk_cli import cli_shared, config, dependencies, errors
from drt_sdk_cli.dependencies.install import get_deps_dict
from drt_sdk_cli.dependencies.modules import DependencyModule

logger = logging.getLogger("cli.deps")


def setup_parser(subparsers: Any) -> Any:
    parser = cli_shared.add_group_subparser(subparsers, "deps", "Manage dependencies or drt-sdk modules")
    subparsers = parser.add_subparsers()

    choices = ['all'] + list(get_deps_dict().keys())

    sub = cli_shared.add_command_subparser(subparsers, "deps", "install", "Install dependencies or drt-sdk modules.")
