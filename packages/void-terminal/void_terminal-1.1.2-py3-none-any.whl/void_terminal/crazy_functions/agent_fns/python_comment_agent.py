...
# TODO: Resolve indentation issues
...
class PythonCodeComment():
    ...
    def find_next_function_begin(self, tagged_code:list, begin_and_end):
        ...
        def extract_number(text):
            ...
            if match:
                # Extract matching numeric parts and convert to integers
    ...
    def _get_next_window(self):
        ...
        # If the remaining lines are very few，Deal with it in one go
    ...
    def begin_comment_source_code(self, chatbot=None, history=None):
        ...
        while True:
            try:
                # yield from update_ui_lastest_msg(f"({self.file_basename}) Reading the next code snippet:\n", chatbot=chatbot, history=history, delay=0)
                next_batch, line_no_start, line_no_end = self.get_next_batch()
                # yield from update_ui_lastest_msg(f"({self.file_basename}) Processing code snippet:\n\n{next_batch}", chatbot=chatbot, history=history, delay=0)
    ...
    def verify_successful(self, original, revised):
        ...
        """
        from void_terminal.crazy_functions.ast_fns.comment_remove import remove_python_comments
