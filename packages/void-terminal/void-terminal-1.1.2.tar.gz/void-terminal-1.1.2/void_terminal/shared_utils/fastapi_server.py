...
def validate_path_safety_replaced(path_or_url, user):
    ...
    from void_terminal.toolbox import FriendlyException
    PATH_PRIVATE_UPLOAD, PATH_LOGGING = get_conf('PATH_PRIVATE_UPLOAD', 'PATH_LOGGING')
    sensitive_path = None
    path_or_url = os.path.relpath(path_or_url)
    if path_or_url.startswith(PATH_LOGGING):    # Log file（Divided by user）
        sensitive_path = PATH_LOGGING
    elif path_or_url.startswith(PATH_PRIVATE_UPLOAD):   # User`s upload directory（Divided by user）
        sensitive_path = PATH_PRIVATE_UPLOAD
    elif path_or_url.startswith('tests') or path_or_url.startswith('build'):   # A commonly used test directory
        return True
    else:
        raise FriendlyException(f"Input file path ({path_or_url}) Exists，But the position is illegal。Please upload the file before executing this task。") # return False
    if sensitive_path:
        ...
        for user_allowed in allowed_users:
            if f"{os.sep}".join(path_or_url.split(os.sep)[:2]) == os.path.join(sensitive_path, user_allowed):
                return True
        raise FriendlyException(f"Input file path ({path_or_url}) Exists，But belongs to another user。Please upload the file before executing this task。") # return False
    return True
