...
def check_date_range_validity(start_date: str, end_date: str):
    """Check if the date range is valid
    Args:
        start_date (str): start date
        end_date (str): end date
    Returns:
        bool: True if the date range is valid
    """
    start_date = date_string_handler(start_date)
    end_date = date_string_handler(end_date)

    if start_date > end_date:
        raise ValueError(
            "An error has occurred: "
            + f"- Start Date {start_date.date()} "
            + f"is not before {end_date.date()}."
        )
    return True
