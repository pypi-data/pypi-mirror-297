...
# Optimized version 5.6.2024
@njit
def total_sample_splits_categorical_eff(no_of_values):
    """
    Compute total number of sample splits that can be generated by categoricals.
    ...
    factorials = np.ones(no_of_values + 1, dtype=np.int64)
    factorials[1:] = np.cumprod(np.arange(1, no_of_values+1, dtype=np.int64))

    no_of_splits = 0
    for i in range(1, no_of_values):
        no_of_splits += (factorials[no_of_values]
                         / (factorials[no_of_values - i] * factorials[i]))
