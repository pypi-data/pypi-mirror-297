...
def test_handle_command_opens_config(mock_open_config_file, mock_input):
    args = argparse.Namespace(config=True, path="", contents=False, structure_only=False, message=None, verbose=False, wait_for_enter=False)
    handle_command(args)
...
def test_handle_command_runs_conversation_without_scan(mock_conversation, mock_input, log_level, caplog, tmp_path):
    ...
    data_dir.mkdir()
    args = argparse.Namespace(config=False, path=str(data_dir), contents=False, structure_only=False, message=None, verbose=False, wait_for_enter=False)
...
def test_handle_command_runs_conversation_with_content_scan(mock_conversation, mock_input, tmp_path):
    data_dir = tmp_path / "data"
    data_dir.mkdir()
    args = argparse.Namespace(config=False, path=str(data_dir), contents=True, structure_only=False, message=None, verbose=False, wait_for_enter=False)
    handle_command(args)
...
def test_handle_command_runs_conversation_with_structure_scan(mock_conversation, mock_input, tmp_path):
    data_dir = tmp_path / "data"
    data_dir.mkdir()
    args = argparse.Namespace(config=False, path=str(data_dir), contents=False, structure_only=True, message=None, verbose=False, wait_for_enter=False)
    handle_command(args)
    mock_conversation.assert_called_once_with(str(data_dir), False, True, None)


@patch("morix.main.parse_args")
@patch("morix.main.handle_command")
def test_main_calls_handle_command(mock_handle_command, mock_parse_args):
    mock_parse_args.return_value = argparse.Namespace(config=True, message=None, path="", contents=False, structure_only=False, verbose=False, wait_for_enter=False)
    main()
    mock_handle_command.assert_called_once()

def test_parse_args_with_wait_for_enter():
    test_args = ["prog", "-w"]
    with patch("sys.argv", test_args):
        args = parse_args()
        assert args.wait_for_enter is True
