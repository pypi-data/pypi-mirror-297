...
response = client.jobs.retrieve_jobs()
print(response.jobs)
```
...
async def main() -> None:
    response = await client.jobs.retrieve_jobs()
    print(response.jobs)
...
client.with_options(max_retries=5).jobs.retrieve_jobs()
```
...
client.with_options(timeout=5.0).jobs.retrieve_jobs()
```
...
job = response.parse()  # get the object that `jobs.retrieve_jobs()` would have returned
print(job.jobs)
```
