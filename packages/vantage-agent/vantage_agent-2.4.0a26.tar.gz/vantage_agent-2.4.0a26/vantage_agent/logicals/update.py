...
async def self_update_agent() -> None:
    ...
    upstream_version_future = asyncio.run_coroutine_threadsafe(
        _fetch_upstream_version_info(), asyncio.get_event_loop()
    )
    upstream_version = upstream_version_future.result()
    logger.debug(
