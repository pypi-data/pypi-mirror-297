...
def cluster_config_task(scheduler: BaseScheduler) -> Job:
    ...
    return scheduler.add_job(
        upload_scontrol_config, "interval", seconds=SETTINGS.TASK_JOBS_INTERVAL_SECONDS, replace_existing=True
    )
...
def cluster_node_task(scheduler: BaseScheduler) -> Job:
    """Schedule a task to report the cluster config."""
    return scheduler.add_job(
        upload_scontrol_node, "interval", seconds=SETTINGS.TASK_JOBS_INTERVAL_SECONDS, replace_existing=True
    )
...
def self_update_task(scheduler: BaseScheduler) -> Job:
    ...
    return scheduler.add_job(
        self_update_agent,
        "interval",
        seconds=SETTINGS.TASK_SELF_UPDATE_INTERVAL_SECONDS,
        replace_existing=True,
    )
...
def status_report_task(scheduler: BaseScheduler) -> Job:
    ...
    return scheduler.add_job(
        report_health_status, "interval", seconds=SETTINGS.TASK_JOBS_INTERVAL_SECONDS, replace_existing=True
    )
