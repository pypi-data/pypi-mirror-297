...
from vantage_agent import lock
...
async def self_update_agent() -> None:
    ...
    with lock:
        ...
        logger.debug(
            "Vantage Agent version info: "
            f"current_version={current_version}, upstream_version={upstream_version}"
        )
        ...
        if _need_update(current_version, upstream_version):
            ...
            logger.debug("Pausing down the scheduler...")
            scheduler.pause()
