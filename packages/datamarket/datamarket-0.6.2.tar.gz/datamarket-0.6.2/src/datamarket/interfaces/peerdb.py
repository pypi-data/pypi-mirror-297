...
class PeerDBInterface:
    ...
    def run_automation(self, schema_name, schema_tables, drop=False, sync=False, resync=False, hard_resync=False):
        ...
        mirror_tables = [table.__tablename__ for table, should_replicate in schema_tables if should_replicate]
        
        self.source.create_user()
        self.source.grant_permissions(schema_name)
        self.source.create_publication(schema_name, mirror_tables)
        self.create_postgres_peer()
        self.create_clickhouse_peer(schema_name)
        self.create_mirror(schema_name, mirror_tables, resync, hard_resync)
        self.destination.create_row_policies(schema_name, mirror_tables)
