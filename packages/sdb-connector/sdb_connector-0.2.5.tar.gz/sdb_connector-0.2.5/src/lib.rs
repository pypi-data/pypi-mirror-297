...
async fn process_additonal_info_data(result: Response, ip_address: &str, name: &str, select_type: u8) -> Result<Vec<(u64, u16, u8, u16, u16, u16, u16, u16, u16, String)>, Box<dyn Error>> {
    let mut data = result;
...
async fn select_additional_info_data_async(
    ...
    let data = process_additonal_info_data(result, &ip,path_name, select_type).await?;
    Ok(data)
}
...
async fn select_measurement_data_async(
    ...
    let data = process_measurement_data(result, &ip, path_name, select_type).await?;
    Ok(data)
}
...
pub async fn process_measurement_data(result: Response, ip_address: &str, name: &str, select_type:u8) -> Result<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<String>)>, Box<dyn Error>> {
    let mut data = result;
    ...
    let number_of_channels = match get_number_of_channels_tag41(&data).await {
        Ok(number_of_channels) => number_of_channels,
        Err(e) => {
            println!("Error getting number of channels: {:?}", e);
            0
...
pub async fn query_general_information(
    ...
    let result_query = format!(
        "SELECT ip_address FROM {} WHERE run_id = {}",
        table_name, run_id
    );
    let result = db.query(&result_query).await?;
    Ok(result)
