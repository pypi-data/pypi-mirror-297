...
class NessusIntegration(ScannerIntegration):
    ...
    def fetch_findings(self, **kwargs) -> Iterator[IntegrationFinding]:  # type: ignore
        """
    ...
    def check_collection(self, file_collection: List[Path], path: str) -> bool:
        """
        Check if any Nessus files were found in the given path.

        :param List[Path] file_collection: List of Path objects for .nessus files
    ...
    def move_files(self, file_collection: List[Path]) -> None:
        """
        ...
        """
        for file in file_collection:
            file_name = (f"{file.stem}_" + f"{get_current_datetime('%Y%m%d-%I%M%S%p')}").replace(" ", "_") + ".nessus"
            new_path = file.parent / "processed" / file_name
            os.makedirs(new_path.parent, exist_ok=True)
            file.rename(new_path)
            logger.info(f"Moved Nessus file {file} to {new_path}")
