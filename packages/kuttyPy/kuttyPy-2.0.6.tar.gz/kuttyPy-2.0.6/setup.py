...
def copy_dir(dir_path):
    base_dir = os.path.join('kuttyPyGui', dir_path)
...
setuptools.setup(
    ...
    url="https://github.com/csparkresearch/kuttypyplus",
    packages=setuptools.find_packages(exclude=['contrib', 'tests']),
    package_data = {
        '': ['*.c','*.qss','*.rules','*.conf']+[f for f in copy_dir('online')]+[f for f in copy_dir('examples')],
    },
    classifiers=[
    ...
    'console_scripts': [
        ...
        'kuttypyide=kuttyPyGui.KuttyPyIDE:run',
