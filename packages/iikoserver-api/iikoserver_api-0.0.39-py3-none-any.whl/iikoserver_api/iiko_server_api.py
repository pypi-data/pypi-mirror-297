...
from iikoserver_api.services.olap_cash_shift_builder import OlapCashShiftBuilder
...
class IikoServerApi:
    ...
    def export_inventory(self, from_date: datetime = datetime.now() - timedelta(days=30), to_date: datetime = datetime.now()):
        ...
        xml_dict_data = xmltodict.parse(data.decode())
        items = xml_dict_data.get('storeReportItemDtoes').get('storeReportItemDto')
...
if __name__ == '__main__':
    with IikoServerApi('https://tutto.iiko.it/resto/api/', 'rsinfo', '5E6MYvS2KO') as api:
        # data = api.export_incoming_invoice(datetime.now() - timedelta(days=30), datetime.now())
        data = api.export_outgoing_service()
        print(data)
