...
Best regards,
The NotificationList Team""")
    
def initial_merging(file_path):
    ...
    df = df.with_columns(pl.col("id").rank(method='dense').alias("Dense Rank"))
    unique_ids = [f"UNQ_{int(rank):05d}" for rank in df['Dense Rank']]
