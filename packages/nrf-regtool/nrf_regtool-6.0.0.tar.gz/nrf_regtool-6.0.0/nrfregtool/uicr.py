...
from .hw_tables import CTRLSEL_DEFAULT, dt_lookup_ctrlsel, dt_lookup_adc_channel_pins
...
def extract_all_configs(
    ...
    adc_pins = extract_adc_pins(uicr.adc_nodes)
...
def _gpio_pins_from_props(
    ...
    for prop in gpio_pin_props:
        ...
        gpio_pin = GpioPin(port=prop_port, pin=prop_pin, secure=secure, ctrlsel=None)
        pins.add(gpio_pin)
...
def _log_trace_configs(trace_log_info):
    ...
    if trace_config.etbsink_sources:
        log_vrb(f"ETB Sources: {_make_enabled_sources_str(trace_config.etbsink_sources)}")
    if trace_config.etrsink_sources:
        log_vrb(f"ETR Sources: {_make_enabled_sources_str(trace_config.etrsink_sources)}")
    if trace_config.tpiusink_sources:
        log_vrb(f"TPIU Sources: {_make_enabled_sources_str(trace_config.tpiusink_sources)}")
