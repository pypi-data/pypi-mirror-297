...
from nrfregtool.common import AddressOffset, NrfPsel, log_dbg
...
_PINCTRL_CTRLSEL_LOOKUP_NRF9280 = {
    ...
    0x5F8D_8000: {
        # P2
        NrfPsel(fun=NRF_FUN_CAN_RX, port=2, pin=10): Ctrlsel.CAN_SERIAL2,
        NrfPsel(fun=NRF_FUN_CAN_TX, port=2, pin=11): Ctrlsel.CAN_SERIAL2,
        # P9
        NrfPsel(fun=NRF_FUN_CAN_RX, port=9, pin=4): Ctrlsel.CAN_SERIAL2,
        NrfPsel(fun=NRF_FUN_CAN_TX, port=9, pin=5): Ctrlsel.CAN_SERIAL2,
    },
    # CAN121
    0x5F8D_B000: {
        # P2
        NrfPsel(fun=NRF_FUN_CAN_RX, port=2, pin=8): Ctrlsel.CAN_SERIAL2,
        NrfPsel(fun=NRF_FUN_CAN_TX, port=2, pin=9): Ctrlsel.CAN_SERIAL2,
        # P9
        NrfPsel(fun=NRF_FUN_CAN_RX, port=9, pin=2): Ctrlsel.CAN_SERIAL2,
        NrfPsel(fun=NRF_FUN_CAN_TX, port=9, pin=3): Ctrlsel.CAN_SERIAL2,
    },
    # I3C121
    0x5F8D_E000: {
...
def dt_lookup_ctrlsel(
    ...
    elif root_compatible.startswith("nordic,nrf9280"):
        lut = _PINCTRL_CTRLSEL_LOOKUP_NRF9280
    else:
        return None
...
_CHANNEL_LOOKUP_NRF9280 = _CHANNEL_LOOKUP_NRF54H20


def dt_lookup_adc_channel_pins(channel: edtlib.Node) -> List[Tuple[int, int]]:
    root_compatible = channel.edt.get_node("/").compats[0]
    if root_compatible.startswith("nordic,nrf54h20"):
        lut = _CHANNEL_LOOKUP_NRF54H20
    elif root_compatible.startswith("nordic,nrf9280"):
        lut = _CHANNEL_LOOKUP_NRF9280
    else:
        return []
