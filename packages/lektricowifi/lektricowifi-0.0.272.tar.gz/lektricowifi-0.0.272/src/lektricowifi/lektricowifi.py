...
class Device:
    ...
    CURRENT_LIMIT_REASON = ["No limit", "Installation current", 
                            ...
                            "EM offline", "EM", "OCPP", "Overtemperature", 
                            "Switching phases", "User limit", 
                            "1P Charging Disabled"]
    ...
    async def device_info(self, type: str) -> Info:
        ...
        if type == self.TYPE_1P7K or type == self.TYPE_3P22K:
            ...
            int_limit_reason = int(data["current_limit_reason"])
            if int_limit_reason >= len(self.CURRENT_LIMIT_REASON):
                data["current_limit_reason"] = "Unknown"
            else:
                data["current_limit_reason"] = self.CURRENT_LIMIT_REASON[int_limit_reason]
