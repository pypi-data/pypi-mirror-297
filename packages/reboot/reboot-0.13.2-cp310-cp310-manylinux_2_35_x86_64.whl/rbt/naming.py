...
# in a unit test or in `rsm dev run`.
LOCAL_USER_ID = 'local'

# A human-readable and recognizable prefix that we use for all Kubernetes
# namespaces that form a Resemble Space. This ensures that all such namespaces
# are grouped together when using e.g. `kubectl get namespaces`.
SPACE_NAMESPACE_PREFIX = "resemble-space-"

# Resemble IDs take the form `[single-letter-prefix][suffix]`. How long is that
...
ApplicationId = rbt.aio.types.ApplicationId
...
ConsensusId = rbt.aio.types.ConsensusId
...
def get_local_application_name_from_serviceables(
    ...
    """
    return str([s.service_name() for s in serviceables])
