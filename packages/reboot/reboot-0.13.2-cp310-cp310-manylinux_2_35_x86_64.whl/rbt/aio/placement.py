...
from rbt.aio.aborted import is_grpc_retryable_exception
...
class PlacementClient(ABC):
    ...
    def shard_for_actor(
        ...
        # TODO: change the following to a single `StateRef` parameter that
        #       encodes a fully-qualified set of service names and actor keys.
        #       See: https://github.com/reboot-dev/respect/issues/2972
    ...
    def address_for_actor(
        ...
        Raises `UnknownApplicationError` if the `application_id` is not known.
        """
        consensus_id = self.consensus_for_actor(
            application_id, service_name, state_ref
        )
...
class StaticPlacementClient(PlacementClient):
    ...
    consensus by making RPCs, like in the (current) Resemble Cloud.
    """
    ...
    async def _handle_plan(
        ...
        for application in plan.applications:
            ...
            self._services_by_application[application.id] = [
                service.name for service in application.services
            ]
