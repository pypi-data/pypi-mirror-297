...
from rbt.settings import DOCS_BASE_URL, MAX_IDEMPOTENCY_KEY_LENGTH
...
class IdempotencyManager:
    ...
    # Map from (service, state_ref, alias) to a generated UUID for an
    # idempotency key.
    _aliases: dict[tuple[ServiceName, StateRef, str], uuid.UUID]
    ...
    # The service, state_ref, and method of the mutation that is uncertain.
        ...
        *,
        ...
        if self._uncertain_mutation:
            ...
            raise RuntimeError(
                ...
                f"{self._mutation_name(service, method)} of state "
                f"'{state_ref.id}' is due to a retry which may cause an undesired "
    ...
    def _idempotency_key_from(
        ...
        *,
        ...
        idempotency_key = self._get_or_create_idempotency_key(
            service, state_ref, method, idempotency
        )
        ...
        if mutation.idempotency_generated:
            raise ValueError(
                f"To call {self._mutation_name(service, method)} of '{state_ref.id}' "
                "more than once using the same context an idempotency "
                "alias or key must be specified"
            )

        if (
            mutation.service != service or mutation.state_ref != state_ref or
    ...
    def _get_or_create_idempotency_key(
        ...
        alias = (
            ...
            f'{self._mutation_name(service, method)}@{state_ref.id}'
        )

        key = (service, state_ref, alias)
    ...
    def generate_idempotent_state_id(
        ...
        if self._seed is None:
            ...
            raise ValueError(
                ...
                f"(of type {state_type}) "
                "without an idempotency seed."
            )
        ...
        service = state_type_to_service(state_type)
        alias = (
            idempotency.alias if idempotency.alias is not None else
            f'{self._mutation_name(service, method)}'
        )
