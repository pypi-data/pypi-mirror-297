...
from rbt.admin.export_import_converters import ExportImportItemConverters
from rbt.admin.export_import_servicer import ExportImportServicer
from rbt.aio.auth.token_verifiers import TokenVerifier
from rbt.aio.contexts import EffectValidation
from rbt.aio.external import ExternalContext
from rbt.aio.interceptors import (
    ResembleContextInterceptor,
    UseApplicationIdInterceptor,
)
from rbt.aio.internals.channel_manager import _ChannelManager
from rbt.aio.internals.contextvars import use_application_id
from rbt.aio.internals.middleware import Middleware
from rbt.aio.internals.tasks_cache import TasksCache
from rbt.aio.internals.tasks_servicer import TasksServicer
from rbt.aio.placement import PlacementClient, StaticPlacementClient
from rbt.aio.react import ReactServicer
from rbt.aio.resolvers import ActorResolver, StaticResolver
from rbt.aio.servicers import (
    ConfigServicer,
    ResembleServiceable,
    Routable,
    Serviceable,
)
from rbt.aio.state_managers import LocalSidecarStateManager, StateManager
...
from rbt.helpers import generate_proto_descriptor_set
from rbt.inspect.servicer import InspectServicer
from rbt.settings import RESEMBLE_STATE_DIRECTORY
...
class Server:
    ...
    def create_on_k8s(
        ...
        *,
        ...
        # `tests/rbt/controller/config_server_test.py` uses this
        ...
        # 'rbt/controller/consensus_managers.py'.
        application_id: str = os.environ[ENVVAR_RESEMBLE_APPLICATION_ID]
        consensus_id: str = os.environ[ENVVAR_RESEMBLE_CONSENSUS_ID]
        ...
        # For now, we can assume that all traffic in a Resemble Cloud deployment
        ...
        #            filter in the mix (e.g. for `rsm serve` outside of the
        #            cloud). In that case, we'll need to implement a
        #            PlacementClient that gets real routing information from a
        #            central authority (e.g. Istio's EDS, a Resemble application
...
class _ServiceServer(Server):
    ...
    def __init__(
        ...
        *,
        ...
        # Construct 'ExportImport' system service for the Resemble servicers.
                    ...
                    await run_application_initializer(
                        ...
                        context=ExternalContext(
                            ...
                            idempotency_seed=uuid.uuid5(
                                uuid.NAMESPACE_DNS,
                                f"{self._application_id}.rsm.cloud",
                            ),
