...
from rbt.aio.headers import AUTHORIZATION_HEADER
from rbt.aio.secrets import SecretNotFoundException, Secrets
from rbt.run_environments import running_rsm_dev
from rbt.settings import ADMIN_SECRET_NAME
...
class AdminAuthMixin:
    ...
    async def ensure_admin_auth_or_fail(
        ...
        """
        if running_rsm_dev():
            # In rsm dev mode, no admin secret needs to be specified - all calls
        ...
        try:
            ...
            await grpc_context.abort(
                code=code_pb2.Code.UNAUTHENTICATED,
        ...
        if bearer_token != admin_secret:
            await grpc_context.abort(
                code=code_pb2.Code.PERMISSION_DENIED,
