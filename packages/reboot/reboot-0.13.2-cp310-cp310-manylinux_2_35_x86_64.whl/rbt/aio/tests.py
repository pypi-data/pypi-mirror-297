...
from rbt.aio.types import ApplicationId, ConsensusId, ServiceName, StateRef
from rbt.controller.application_config import ApplicationConfig
from rbt.controller.application_config_trackers import (
    LocalApplicationConfigTracker,
)
from rbt.controller.config_extractor import LocalConfigExtractor
from rbt.controller.consensus_managers import LocalConsensusManager
...
from rbt.settings import ENVVAR_LOCAL_ENVOY_MODE, ENVVAR_LOCAL_ENVOY_USE_TLS
...
#
# Note that this number does NOT influence the number of partitions in a Resemble
# Cloud deployment, since it gets its `ApplicationConfig` from elsewhere. It may
# still follow `rbt.controller.plan_makers.DEFAULT_NUM_PARTITIONS`.
...
class Resemble:
    ...
    def __init__(
        ...
        """
        base_directory: directory below which applications will store state.
        """
        ...
        self._consensus_manager = LocalConsensusManager(base_directory)
    ...
    async def up(
        ...
        *,
        ...
        if local_envoy:
            # 'Resemble' is part of 'Application', which users will call to run
            # the backend, so we don't want to override the environment variable
            # if it's already set by 'rsm dev run' or 'rsm serve'.
    ...
    async def unique_consensuses(
        ...
        if len(application_configs) != 1:
            # TODO: See https://github.com/reboot-dev/respect/issues/3356.
            raise ValueError(
    ...
    async def consensus_start(self, consensus: Consensus) -> None:
        ...
        # addresses. See https://github.com/reboot-dev/respect/issues/3356
