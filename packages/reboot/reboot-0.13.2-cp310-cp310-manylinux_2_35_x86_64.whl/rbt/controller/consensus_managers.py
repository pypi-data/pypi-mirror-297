...
from rbt.aio.placement import PlanOnlyPlacementClient
from rbt.aio.resolvers import DirectResolver
from rbt.aio.servers import _ServiceServer
from rbt.aio.servicers import Serviceable
from rbt.aio.signals import raised_signal
from rbt.aio.state_managers import LocalSidecarStateManager
...
from rbt.consensus.local_envoy import ConsensusAddress, LocalEnvoy
from rbt.consensus.local_envoy_factory import LocalEnvoyFactory
from rbt.controller.consensuses import Consensus
from rbt.controller.envoy_filter import EnvoyFilter
from rbt.controller.envoy_filter_generator import generate_transcoding_filter
...
from rbt.naming import get_service_account_name_for_application
from rbt.run_environments import on_cloud
    ...
    async def _set_consensus(
        ...
        await self._k8s_client.deployments.create_or_update(
            ...
            env=[
                ...
                # Kubernetes, see 'rbt/aio/servers.py'.
...
async def _run_consensus_process(
    *,
    ...
    try:
        ...
        state_manager = LocalSidecarStateManager(directory, shards)
...
class LocalConsensusManager(ConsensusManager):
    ...
    def __init__(
        ...
        if base_directory is None:
            ...
            self._base_directory = Path(self._tmp_directory.name)
        else:
            if not base_directory.is_dir():
                raise ValueError(
                    f"Base directory at '{base_directory}' "
    ...
    def _application_directory(self, application_id: ApplicationId) -> Path:
        return self._base_directory / application_id
    ...
    async def _validate_stable_consensus_counts(
        ...
        for application_id, consensus_count in consensuses_by_app.items():
            application_directory = self._application_directory(application_id)
            metadata_path = application_directory / "__metadata"
    ...
    async def _start_consensus(
        ...
        consensus_directory = os.path.join(
            self._application_directory(consensus.application_id),
    ...
    async def _launch_subprocess_consensus(
        ...
        if os.getenv(ENVVAR_RSM_NODEJS, "false").lower() == "true":
    ...
    async def _launch_nodejs_subprocess_consensus(
        ...
        pid = await rbt.nodejs.python.launch_subprocess_consensus(
