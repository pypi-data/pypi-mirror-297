...
from rbt.cli.cloud import add_cloud_options
from rbt.cli.directories import (
    add_working_directory_options,
    dot_rsm_dev_directory,
    use_working_directory,
)
from rbt.cli.monkeys import monkeys, no_chaos_monkeys
from rbt.cli.protoc import protoc_direct
# We won't mock the classes in `rc`, so it's safe to import those directly.
from rbt.cli.rc import (
    ArgumentParser,
...
from rbt.cli.watch import watch
from rbt.controller.exceptions import InputError
from rbt.controller.plan_makers import validate_num_consensuses
...
def add_application_options(subcommand: SubcommandParser) -> None:
    """Helper that adds options used to run Resemble applications."""
    ...
    subcommand.add_argument(
        ...
        help=(
            ...
            "In the Reboot Cloud, Secrets are written using `rsm cloud secret write`. "
...
def protoc_parser_if_protoc_watch(
    ...
    specified the necessary arguments in an `.rsmrc`.
    ...
    *,
    ...
    if protoc_parser is not None:
        try:
            ...
            terminal.fail(
                "Failed to run 'rsm protoc' as part of 'rsm dev run' with "
                "'--protoc-watch' flag set.\n"
                "Edit the '.rsmrc' file to set the necessary arguments to run "
                "'rsm protoc', or pass '--no-protoc-watch'."
            )
        ...
        if args.tsc is not None and (
            ...
            terminal.fail(
                "You must pass either '--react' or '--nodejs' to 'rsm protoc' to use '--tsc'."
            )
    ...
    # 'rbt.aio.Application' will be looking for.
    #
    ...
    if args.name is not None:
        ...
        # subdirectory.
        env[ENVVAR_RSM_DIRECTORY] = dot_rsm_dev_directory() + '/' + args.name
    ...
    if args.local_envoy:
        ...
        env[ENVVAR_RESEMBLE_LOCAL_ENVOY] = 'true'
        ...
        env[ENVVAR_RESEMBLE_LOCAL_ENVOY] = 'false'

    env[ENVVAR_RESEMBLE_LOCAL_ENVOY_PORT] = str(
    ...
    env[ENVVAR_RSM_PARTITIONS] = str(args.partitions)

    if args.nodejs:
        env[ENVVAR_RSM_NODEJS] = 'true'
    ...
    env[ENVVAR_RSM_EFFECT_VALIDATION] = args.effect_validation.upper()
    ...
    while True:
        ...
        async with watch(proto_globs) as protos_event_task:
            ...
            async with watch(args.watch or []) as watch_event_task:
                ...
                # '--python' or '--nodejs'.
                if (
                    not args.python and not args.nodejs and
                    ...
                    terminal.fail(
                        ...
                        "or '--nodejs' if you want to launch a Node application."
                    )
...
async def dev_expunge(args: argparse.Namespace) -> None:
    ...
    if args.yes is False:
        terminal.info(
            f"About to expunge '{args.name}' from '{dot_rsm_dev_directory()}'"
        )
        if not ask_for_confirmation("Do you want to continue? [y/n]:"):
            terminal.fail("Expunge cancelled")

    application_directory = (Path(dot_rsm_dev_directory()) / args.name)
    try:
