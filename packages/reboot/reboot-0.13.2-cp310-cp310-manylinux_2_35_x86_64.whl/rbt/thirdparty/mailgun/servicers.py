...
import asyncio
import os
import rbt.aio.auth.token_verifiers
import respect.logging
import uuid
from abc import abstractmethod
from datetime import datetime, timedelta
from rbt.aio.auth import Auth
from rbt.aio.auth.tokens import Token
from rbt.aio.contexts import ReaderContext, WriterContext
from rbt.aio.secrets import (
    EnvironmentSecretSource,
    SecretNotFoundException,
    Secrets,
)
from rbt.aio.tasks import TaskEffect
from rbt.run_environments import on_cloud
from rbt.thirdparty.mailgun.settings import (
...
class _AbstractMessageServicer(Message.Interface):

    class TokenVerifier(rbt.aio.auth.token_verifiers.TokenVerifier):
            ...
            try:
                ...
                if token == api_key.decode():
                    return Auth(
                        ...
                        bearer_token=Token(
                            raw=token,
                            decoded={MAILGUN_API_KEY_VERIFIED: True},
                        ),
                    )
                else:
                    return None
    ...
    class Authorizer(Message.Authorizer):
        ...
        async def authorize(
            ...
            *,
            ...
            assert context.auth.bearer_token is not None
            assert context.auth.bearer_token.decoded is not None
            if MAILGUN_API_KEY_VERIFIED in context.auth.bearer_token.decoded:
                return Ok()
    ...
    def token_verifier(
        self,
    ) -> Optional[rbt.aio.auth.token_verifiers.TokenVerifier]:
