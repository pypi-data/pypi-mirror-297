...
from rbt.options import get_method_options, is_resemble_state
...
def legal_diff_constructor_to_message(
    ...
    """Allows the migration from https://github.com/reboot-dev/respect/pull/2596.
...
def legal_diff_task_to_message(
    ...
    """Allows the migration for https://github.com/reboot-dev/respect/pull/2578.
...
_LEGAL_RESEMBLE_METHOD_OPTION_DIFFS: list[LegalMethodOptionDiffPredicate] = [
...
class ProtoValidationError(Exception):
    ...
    def __init__(
        self,
        reason: str,
        validation_errors: Optional[list['ProtoValidationError']] = None,
    ):
        super().__init__(reason, validation_errors)
        self.reason = reason
...
def validate_descriptor_sets_are_backwards_compatible(
    ...
    errors: list[ProtoValidationError] = []
    original_states_by_name = _get_all_resemble_states_by_name(
        original_file_descriptor_set
    )
    updated_states_by_name = _get_all_resemble_states_by_name(
        ...
        try:
            ...
            errors.append(
                ProtoValidationError(
                    reason=f'State `{state_name}` was deleted'
                )
            )
            continue
...
def _get_all_resemble_states_by_name(
    ...
    for file_descriptor_proto in file_descriptor_set.file:
        ...
        try:
            for message_name, message_descriptor in file_descriptor.message_types_by_name.items(
                ...
                if is_resemble_state(message_descriptor):
...
def _validate_methods_are_backwards_compatible(
    ...
    # `${Message}Interface`: in future we might support defining additional
    # interfaces.
    def get_service_descriptor(state: Descriptor) -> ServiceDescriptor:
        service_name = f'{state.full_name}Interface'

        try:
            return state.file.pool.FindServiceByName(service_name)
        except KeyError:
            raise ValueError(
                f"Service `{service_name}` not found in `{state.file.name}` or imported files",
            )

    original = get_service_descriptor(original_state)
    updated = get_service_descriptor(updated_state)

    exceptions: list[ProtoValidationError] = []
    for method in original.methods_by_name.values():
        ...
        exceptions += _validate_resemble_options(
...
def _validate_resemble_options(
    service_name: str,
    original_method: MethodDescriptor,
    updated_method: MethodDescriptor,
) -> list[ProtoValidationError]:
    """Compare the Resemble options for the original and updated methods to be
    ...
    original_resemble_options = get_method_options(
        original_method.GetOptions()
    )
    updated_resemble_options = get_method_options(updated_method.GetOptions())
    diffs = dict_diff(
        MessageToDict(original_resemble_options),
        MessageToDict(updated_resemble_options)
    )
    # Filter out any legal differences.
    illegal_diffs = [
        d for d in diffs if not any(
            is_legal_diff(
                d, original_resemble_options, updated_resemble_options
            ) for is_legal_diff in _LEGAL_RESEMBLE_METHOD_OPTION_DIFFS
        )
    ]
    if len(illegal_diffs) > 0:
        # TODO: Consider rendering each `illegal_diff`, rather than the entire structure.
        return [
            ProtoValidationError(
                reason=(
                    f'Custom options for method `{service_name}.{original_method.name}` updated '
                    f'from `{original_resemble_options}` '
                    f'to `{updated_resemble_options}`'
                )
            )
        ]
    return []
...
def _compare_reserved_field_numbers(
    ...
    while original_range is not None:
        ...
        if updated_range is None or updated_range.start > original_range.start:
            ...
            exceptions.append(
                ProtoValidationError(
                    reason=f'Field numbers {missing_start} to '
...
def _compare_messages(
    ...
    # removed.
    exceptions: list[ProtoValidationError] = _compare_reserved_field_numbers(
    ...
    for field_number, original_field in original_message.fields_by_number.items(
        ...
        try:
            ...
            if not field_number_reserved:
                exceptions.append(
                    ProtoValidationError(
                        reason=f'Field `{original_field.name}` was deleted from '
                        f'message `{original_message.name}` ({where_message}), '
        ...
        for property_name in ['type', 'label']:
            ...
            exceptions += [ProtoValidationError(reason=error_message)]

        # If this field is a nested proto message, descend into it recursively.
        if original_field.type == FieldDescriptor.TYPE_MESSAGE:
...
def _validate_state_is_backwards_compatible(
    ...
    try:
        ...
        return [ProtoValidationError(reason=str(e))]
