...
if startswith(path, resemble_prefix) then
  ...
  request_handle:headers():replace("x-reboot-service-name", service_name)
  request_handle:headers():replace("x-reboot-state-ref", state_ref)

  request_handle:headers():replace(":path", "/" .. table.concat({ table.unpack(values, 6) }, "/"))
end
