...
from rbt.aio.types import ApplicationId, ConsensusId
from rbt.routing.filters.lua import (
    ADD_HEADER_X_RESEMBLE_APPLICATION_ID_TEMPLATE_FILENAME,
    COMPUTE_HEADER_X_RESEMBLE_CONSENSUS_ID_TEMPLATE_FILENAME,
...
def _http_filter_add_header_x_resemble_application_id(
    ...
    filter_content = render_lua_template(
        ADD_HEADER_X_RESEMBLE_APPLICATION_ID_TEMPLATE_FILENAME, template_input
    )

    return http_connection_manager_pb2.HttpFilter(
        name="rbt.add_header_x_resemble_application_id",
...
def _http_filter_compute_header_x_resemble_consensus_id(
    ...
    return http_connection_manager_pb2.HttpFilter(
        name="rbt.compute_header_x_resemble_consensus_id",
        typed_config=_lua_any(filter_content),
    )
...
def _filter_http_connection_manager(
    ...
    http_connection_manager = http_connection_manager_pb2.HttpConnectionManager(
            ...
            virtual_hosts=[
                route_components_pb2.VirtualHost(
                    ...
                    cors=route_components_pb2.CorsPolicy(
                        ...
                        allow_headers=
                        f"{APPLICATION_ID_HEADER},{SERVICE_NAME_HEADER},{STATE_REF_HEADER},{CONSENSUS_ID_HEADER},{IDEMPOTENCY_KEY_HEADER},{WORKFLOW_ID_HEADER},keep-alive,user-agent,cache-control,content-type,content-transfer-encoding,x-accept-content-transfer-encoding,x-accept-response-streaming,x-user-agent,grpc-timeout,{AUTHORIZATION_HEADER}",
                        max_age="1728000",
        ...
        http_filters=[
            _http_filter_add_header_x_resemble_application_id(application_id),
            ...
            [
                _http_filter_compute_header_x_resemble_consensus_id(
                    consensuses
                ),
