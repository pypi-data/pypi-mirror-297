...
from tensorkube.services.k8s_service import create_aws_secret, create_build_pv_and_pvc, \
    create_secret, list_secrets, delete_secret
...
from tensorkube.services.rich_service import list_tensorkube_deployments, describe_deployment, display_deployment_logs, \
    ssh_into_deployed_service, display_secrets
from tensorkube.services.s3_service import create_s3_bucket, delete_s3_bucket, upload_files_in_parallel, empty_s3_folder
from tensorkube.services.deploy import Config, deploy_knative_service, DEFAULT_GPUS, DEFAULT_CPU, \
    DEFAULT_MEMORY, DEFAULT_MIN_SCALE, DEFAULT_MAX_SCALE, DEFAULT_ENV, DEFAULT_SECRET 
@click.group()
def tensorkube():
    pass
...
def secrets(env):
    ...
    if not secrets:
        if env == DEFAULT_NAMESPACE:
            ...
            click.echo(f"No secrets found in environment " + click.style(env, italic=True, bold=True))
        return
...
@click.option('--config-file', type=str, help='Path to the config.yaml file. you can use this file instead of cli flags. Keys in config.yaml take precedence than cli flags.')
def deploy(gpus, gpu_type, cpu, memory, min_scale, max_scale, env, github_actions, secret, config_file):
    ...
    config.set_if_not_exist('gpus', gpus).set_if_not_exist('gpu_type', gpu_type).set_if_not_exist('cpu', cpu).set_if_not_exist('memory', memory).set_if_not_exist('min_scale', min_scale).set_if_not_exist('max_scale', max_scale).set_if_not_exist('env', env).set_if_not_exist('github_actions', github_actions).set_if_not_exist('secret', secret)
...
def create(secret_name, key_value_pairs, env, force):
    try:
        ...
        click.echo(click.style("Error: Invalid key-value pairs. Please provide key-value pairs in the format KEY=value.", fg=CliColors.ERROR.value))
        return
    
    click.echo("Creating secret " + click.style(secret_name, italic=True, bold=True) 
               + " in environment " + click.style(env, italic=True, bold=True) + " ...")
...
def delete(secret_name, env):
    ...
    if click.confirm(click.style(msg, fg=CliColors.WARNING.value), abort=True):
        click.echo("Deleting secret " + click.style(secret_name, italic=True, bold=True) 
               + " from environment " + click.style(env, italic=True, bold=True) + " ...")
