...
class Progress(object):
    ...
    def display(self, new_file = False, done_message = None):
        ...
        if self.current_position == self.total_size:
            print_size = utils.format_size(self.current_position, True)
            ...
            print_speed = utils.format_size((self.current_position - self.initial_position) / sec_elapsed, True, True)
...
class ProgressCR(Progress):
    ...
    def display(self, new_file = False, done_message = None):
        ...
        timedelta = datetime.datetime.now() - self.time_start
        sec_elapsed = timedelta.days * 86400 + timedelta.seconds + float(timedelta.microseconds)/1000000.0
        if (sec_elapsed > 0):
            print_speed = utils.format_size((self.current_position - self.initial_position) / sec_elapsed, True, True)
        else:
            print_speed = (0, "")
        self.labels["datetime"] = datetime.datetime.now().isoformat()
        self._stdout.write(self.CR_char)
        self._stdout.write(utils.convert(u"[%(datetime)s] %(action)s: '%(source)s' %(extra)s " % self.labels))
        if self.total_size >= 0:
            if self.total_size == 0 or done_message == 'done':
                percent = 100
            else:
                percent = (self.current_position * 100) // self.total_size
            output = u" %(current)s of %(total)s   %(percent)3d%% in %(elapsed)4ds  %(speed)7.2f %(speed_coeff)sB/s" % {
                "current" : str(self.current_position).rjust(len(str(self.total_size))),
                "total" : self.total_size,
                "percent" : percent,
                "elapsed" : sec_elapsed,
            ...
            output = u" %(current)s in %(elapsed)4ds  %(speed)7.2f %(speed_coeff)sB/s" %{
                "current": str(self.current_position).rjust(len(str(self.total_size))),
                "elapsed": sec_elapsed,
                "speed": print_speed[0],
                "speed_coeff": print_speed[1]
...
#             print_speed = (0, "")
#         self._stdout.write(self.CR_char)
#         done = int(50 * self.current_position / float(self.total_size))
#         percent = (self.current_position * 100) // self.total_size
#         sys.stdout.write("Uploading " + self.labels.get('source', 'unknown') + " [" + '#' * done + '.' * (
#                             50 - done) + "] " + '{:.2f}'.format(percent) + "% in %d" + sec_elapsed)
#         self._stdout.write(u"%(action)s: '%(source)s' -> '%(destination)s'  %(extra)s " % self.labels)
#         if self.total_size >= 0:
#             output = " %(current)s of %(total)s   %(percent)3d%% in %(elapsed)4ds  %(speed)7.2f %(speed_coeff)sB/s" % {
#                 "current" : str(self.current_position).rjust(len(str(self.total_size))),
#                 "total" : self.total_size,
#                 "percent" : self.total_size and ((self.current_position * 100) // self.total_size) or 100,
#                 "elapsed" : sec_elapsed,
#                 "speed" : print_speed[0],
#                 "speed_coeff" : print_speed[1]
#             }
#         else:
#             output = "%(current)s in %(elapsed)4ds  %(speed)7.2f %(speed_coeff)sB/s" %{
#                 "current": str(self.current_position).rjust(len(str(self.total_size))),
#                 "elapsed": sec_elapsed,
#                 "speed": print_speed[0],
#                 "speed_coeff": print_speed[1]
