...
class ResponseError(Exception):
    def __init__(self, res, errorType, errorMessage=None):
        if type(res) is int:
            code = res
        else:
            code = res.status_code
            self.res = res
        Exception.__init__(self, errorType + " error: %i, message: %s" % (code, errorMessage))
...
class HTTPResponseError(ResponseError):
    def __init__(self, res):
        ResponseError.__init__(self, res, "HTTP", status_codes._codes.get(res.status_code, (None,))[0])
...
class Client(object):
    ...
    def __init__(self, url, **kwargs):
        ...
        self.put_max_retry = kwargs.get('put_max_retry', 1)
    ...
    def get_file(self, remote_path, local_file=None, **kwargs):
        ...
        if res.status_code == 200:
            ...
            if self.progress:
                ...
                labels = {'source': self._format_oc_path_for_output(remote_path), 'destination': local_file,
                          'action': 'Download',
    ...
    def get_directory_as_zip(self, remote_path, local_file):
        ...
        res = self._session.get(url,
                                stream=True,
                                headers={
                                    'User-Agent': 'gtocclient/{} ({}; {})'.format(pkg_info.version, getpass.getuser(),
                                                                                  socket.gethostname())}
                                )
        if res.status_code == 200:
            if local_file is None:
    ...
    def put_file(self, remote_path, local_source_file, **kwargs):
        ...
        if kwargs.get('chunked', False) or kwargs.get('put_continue', False):
            while True:
                try:
                    result = self._put_file_chunked(
                        remote_path,
                        local_source_file,
                        **kwargs
                    )
                    return result
                except (HTTPResponseError, IOError) as e:
                    retry += 1
                    if retry >= self.put_max_retry:
                        raise e
                    else:
                        print(" Error occurred: %s.\n Trying to upload the file again. Wait for 5 seconds..." % e)
                        kwargs["put_continue"] = True
                        time.sleep(5)

        stat_result = os.stat(local_source_file)
        if stat_result.st_size > 30 * 1024 * 1024 * 1024:
            raise InvalidFileError(
                "Directly uploading files larger than 30G is not allowed. Please do chunked upload on this file instead.")
        if stat_result.st_size > 20 * 1024 * 1024 * 1024:
            print(
                "Warn: Directly uploading files larger than 20G is not guaranteed to be stable. Please do chunked upload on this file instead.")
        headers = {}
        ...
        while True:
            try:
                ...
                if self.progress:
                    ...
                    if os.path.getsize(local_source_file) == 0:
                        res = self._make_dav_request(
                            'PUT',
                            remote_path,
                            data={},
                ...
                retry += 1
                if retry >= self.put_max_retry or (hasattr(e, "status_code") and e.status_code == 507):
                    raise e
                else:
                    print(" Error occurred: %s.\n Trying to upload the file again. Wait for 5 seconds..." % e)
                    time.sleep(5)
    ...
    def _put_file_chunked(self, remote_path, local_source_file, **kwargs):
        ...
        if kwargs.get('put_continue', False):
            if self.exists(remote_path):
                ...
                try:
                    checksums = \
                    prop.attributes['{http://owncloud.org/ns}checksums']['{http://owncloud.org/ns}checksum'][0]
    ...
    def _make_ocs_request(self, method, service, action, **kwargs):
        ...
        res = self._session.request(method, self.url + path, proxies=self.proxies, timeout=self._timeout, **attributes)
        return res
    ...
    def set_password(self, user_id, password,new_password=None):
        self.login(user_id, password)
        base_url = self.url
        sess = requests.Session()
        auth = HTTPBasicAuth(user_id, password)
        sess.auth = auth
        url = "{}/login?user={}".format(base_url,user_id)
        res_text = sess.request('GET', url=url).text
        requesttoken = re.findall('data-requesttoken="(.*)"', res_text)[0]
        payload = 'user={}&password={}&timezone-offset=8&timezone=Asia%2FShanghai&requesttoken={}'.format(user_id,password,requesttoken)
        headers = {
            'Content-Type': 'application/x-www-form-urlencoded',
            'User-Agent': 'gtocclient/{} ({}; {})'.format(pkg_info.version, getpass.getuser(),
                                                                socket.gethostname())
        }
        res_text = sess.request("POST", url, headers=headers, data=payload).text
        requesttoken = re.findall('data-requesttoken="(.*)"', res_text)[0]
        headers["requesttoken"] = requesttoken
        if new_password == None:
            new_password = utils.generate_random_password()
        payload = "oldpassword={}&personal-password={}&personal-password-clone={}".format(password,new_password,new_password)
        url = "{}/settings/personal/changepassword".format(base_url)
        response = sess.request("POST", url, headers=headers, data=payload)
        response_data = response.json()
        if response_data.get("status") == "success":
            return new_password
        elif "未能成功发送重置邮件" in response_data.get("data",{}).get("message",""):
            sys.stdout.write("密码修过成功，但是发送重置邮件失败")
            sys.stdout.flush()
            return new_password
        else:
            raise Exception("err message: {}".format(response.json()))
