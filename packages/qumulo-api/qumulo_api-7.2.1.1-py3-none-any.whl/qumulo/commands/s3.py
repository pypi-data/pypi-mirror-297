...
class ListAccessKeysCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        ...
        if args.json:
            list_access_keys_json(rest_client, args.user)
        else:
            list_access_keys_tab(rest_client, args.user)
...
class ModifyBucketCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        if args.versioning is None and args.enable_locking is None:
            err_msg = (
                'At least one of the following arguments is required: [--enable-versioning,'
                ' --suspend-versioning, --enable-locking]'
            )
            raise ValueError(err_msg)

        patch = s3.BucketPatch()
        if args.enable_locking:
...
class SetPolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            '--file',
            help=(
                'The access policy file to upload. For an access policy template, use the --example'
                ' flag.'
            ),
            required=True,
        )
