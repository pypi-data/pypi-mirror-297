...
class Policy:
    ...
    def to_dict_no_index(self) -> Dict[str, Any]:
        ans: Dict[str, Any] = {}
        ans['Statements'] = [s.to_dict() for s in self.statements]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for i, s in enumerate(ans['Statements']):
            s['Index'] = i + 1
        return ans
    ...
    def from_dict(policy: Dict[str, Any]) -> 'Policy':
        ans = Policy(
            id=None,
            version=None,
            statements=[PolicyStatement.from_dict(s) for s in policy['Statements']],
        )
        if 'Id' in policy:
            ans.id = policy['Id']
