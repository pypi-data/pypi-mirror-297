...
SupportTeam : type[AbstractSupportTeam] = apps.get_model(get_settings().get('SupportTeam','dj_rest_tickets.DefaultSupportTeam'),require_ready=False)
...
SupportTeamMember : type[AbstractSupportTeamMember] = apps.get_model(get_settings().get('SupportTeamMember','dj_rest_tickets.DefaultSupportTeamMember'),require_ready=False)
...
Ticket : type[AbstractTicket] = apps.get_model(get_settings().get('Ticket','dj_rest_tickets.DefaultTicket'),require_ready=False)
...
class AbstractTicketMessage(models.Model):
    ...
    def save(self,*args,**kwargs) -> None:
        self.is_creator = self.user == self.ticket.creator
...
TicketMessage : type[AbstractTicketMessage] = apps.get_model(get_settings().get('TicketMessage','dj_rest_tickets.DefaultTicketMessage'),require_ready=False)
