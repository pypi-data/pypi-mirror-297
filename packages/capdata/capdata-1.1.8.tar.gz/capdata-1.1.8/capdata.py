...
def get_ir_vanilla_instrument_definition(inst_type: [], ccy: [] = None, ir_index: [] = None, inst_codes: [] = None):
    if (inst_type is None or len(inst_type) == 0):
        raise ValueError(f'inst_type 值不能空，可选值为SWAP, CROSS, DEPO')
    return data.refer.get_ir_vanilla_instrument_definition(inst_type, inst_codes=inst_codes, ccy=ccy, ir_index=ir_index)
