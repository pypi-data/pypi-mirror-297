...
def resolve_dns(hostname=None, url=None):
    """Resolves the given domain name to an IP address.

    Args:
        hostname (str, optional): The host name to resolve DNS for. Defaults to None.
        url (str, optional): The URL to resolve DNS for. Defaults to None.
    ...
    if url:
        ...
        try:
            ...
            netloc = parsed_url.netloc.replace(hostname, ip_address)
            new_url = urlunparse(parsed_url._replace(netloc=netloc))
            return new_url

        except socket.gaierror as e:
            raise ValueError(f"Error resolving DNS for {url}: {e}")
        ...
        try:
            ...
            raise ValueError(f"Error resolving DNS for {hostname}: {e}")
    else:
        raise ValueError("No hostname or URL provided.")
