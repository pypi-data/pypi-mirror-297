...
class EnhancedScrollableFrame(EnhancedFrame):
    ...
    def move_children(self, event: Event) -> None:
        for child_widget in self.winfo_children():
            if child_widget in (self._scrollbar, self._canvas):
                continue
            ...
            configure_command(in_=self._frame)
    ...
    def setup_widgets(self) -> None:
        ...
        self._frame: EnhancedFrame = EnhancedFrame(self._canvas)
        self._frame.bind(sequence="<MouseWheel>", func=self.on_mouse_wheel, add=True)
        self._frame.bind(sequence="<Configure>", func=self.on_configure, add=True)
        self._frame_id: int = self._canvas.create_window(0, 0, anchor="nw", window=self._frame)
    ...
    def on_configure(self, event: Event) -> None:
        ...
        self._canvas.itemconfigure(tagOrId=self._frame_id, height=height, width=width)
        self.update_scrollbar()
