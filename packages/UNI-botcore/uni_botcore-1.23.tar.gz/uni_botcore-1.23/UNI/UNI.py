...
from .Uni_cfg import UNI_Handlers as U_Handlers
...
def uni_handler(
	...
	def load_handlers(wrapped_event_handler, type_, simulate_event_handler, func, locals_):

		if type_ == 'events':
			if lock_event_type != []:
				handl.register_handler(handler=wrapped_event_handler, handler_type=type_, handler_simulator=simulate_event_handler, func=func, locals_=locals_)
		else:
			handl.register_handler(handler=wrapped_event_handler, handler_type=type_, handler_simulator=simulate_event_handler, func=func, locals_=locals_)

		print(f'Зарегистрирован хендлер: {type_}')


	def decorator(func):

		# global handler_filters_

		# if handler_filters_d:
		# 	print(f':{handler_filters_d}:')
		# 	handler_filters_ = handler_filters_d

		print(f'получаем данные о хендлере')
		old_func = func
		#old_locals = handler_filters_
		print(f'получаем путь')
		...
		print(f'симуляция и исполнение')
		@wraps(func)
		...
		# else:
		# 	handl.register_handler(handler=wrapped_event_handler, handler_type=type_, handler_simulator=simulate_event_handler)
		print(f'!!!!регаем хендлер!!!!')
		load_handlers(wrapped_event_handler, type_, simulate_event_handler, old_func, handler_filters_)

		return func


	print(f'возвращаем декоратор')
