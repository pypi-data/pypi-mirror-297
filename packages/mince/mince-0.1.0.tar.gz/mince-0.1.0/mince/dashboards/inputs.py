...
import polars as pl
from dash import html, dcc, Dash, Input, Output  # type: ignore
...
def create_inputs(
    ...
    for name, input_spec in ui_spec['inputs'].items():
        if input_spec['type'] == 'button':
            ...
            inputs[name] = create_date_picker(input_spec, dfs)
        else:
            raise Exception('invalid input type')
    return inputs


def create_button(name: str, input_spec: ui_types.InputSpec) -> html.Div:
...
def create_date_picker(
    input_spec: ui_types.InputSpec, dfs: dict[str, pl.DataFrame]
) -> dcc.DatePickerSingle:
    min_date = min(df['timestamp'].min() for df in dfs.values())  # type: ignore
    max_date = max(df['timestamp'].max() for df in dfs.values())  # type: ignore
    return html.Div(
        [
            dcc.DatePickerSingle(
                id='date-picker',
...
def is_visible(state: dict[str, Any], input_spec: ui_types.InputSpec) -> bool:
