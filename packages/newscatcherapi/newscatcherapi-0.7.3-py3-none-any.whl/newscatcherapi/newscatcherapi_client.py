...
class NewsCatcherApiClient(object):
    ...
    def get_latest_headlines(
        ...
        # Send Request
        r = self.request_method.get(self.base_url + const.LATEST_HEADLINES_URL, auth=self.auth, timeout=60, params=payload, proxies=proxies)
    ...
    def get_search(
        ...
        # Send Request
        r = self.request_method.get(self.base_url + const.SEARCH_URL, auth=self.auth, timeout=60, params=payload, proxies=proxies)
    ...
    def get_sources(self,
        ...
        # Send Request
        r = self.request_method.get(self.base_url + const.SOURCES_URL, auth=self.auth, timeout=60, params=payload, proxies=proxies)
    ...
    def get_search_all_articles(
        ...
        while True:
            ...
            print(f'{from_datetime.strftime("%m/%d/%Y %H:%M:%S")} --> {temp_to_.strftime("%m/%d/%Y %H:%M:%S")}')
