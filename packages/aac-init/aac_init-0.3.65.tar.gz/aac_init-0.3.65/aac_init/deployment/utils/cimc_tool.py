...
from aac_init.logging import setup_logger, netmiko_session_logger
...
class CIMC_Tool:
    """CIMC Toolkits, include API and SSH."""
    ...
    def __init__(self, global_policies: dict, apic_connection: dict):
        ...
        for k, v in apic_connection.items():
            setattr(self, k, v)
        self.logger = setup_logger(f"cimctool_{self.cimc_address}.log")
        self.device_ssh = {
            ...
            "session_log": netmiko_session_logger(
                f"ssh_session_{self.cimc_address}.log"
            ),
        }
        self.token = None
        ...
        self.logger.info("CIMC tool initialized successfully.")
    ...
    def _api_login(self):
        ...
        try:
            ...
            if response:
                ...
                if self.token:
                    self.logger.debug(
                        f"Successfully logged in to CIMC. Token: {self.token}"
                    )
                    return self.token
                else:
                    msg = (
                        f"Login failed for CIMC {self.cimc_address}: No token received."
                    )
                    self.logger.error(msg)
            else:
                msg = f"Login request to CIMC {self.cimc_address} failed: No response."
                self.logger.error(msg)
    ...
    def api_validate_apic(self):
        ...
        2. Login CIMC via API.
        3. Perform CIMC health check.
        4. Clear any existing virtual media mappings.
        5. Power down CIMC.
        """

        self.logger.info("Validating APIC Image for APIC installation...")
        if not self._validate_apic_image():
            self.logger.error("Validate APIC Image failed.")
            return False

        self.logger.info("Validating CIMC for APIC installation...")
        ...
        try:
            ...
            self.logger.info("CIMC for APIC installation validated successfully.")
            return True
    ...
    def api_install_apic(self):
        ...
        self.logger.info("Configure APIC installation via CIMC-MAP.")
        ...
        try:
            ...
            self.logger.info("CIMC-MAP successfully configured, start to install APIC.")
            return True
    ...
    def _ssh_connect(self):
        ...
        for device_type in DEVICE_TYPES:
            try:
                ...
                sleep(3)
        self.logger.error(
            f"Failed to connect to {self.cimc_address} with both methods!"
        )
        return False
    ...
    def ssh_install_apic(self):
        ...
        try:
            ...
            interfaces = self.ssh_connection.send_command(
                "ls -l /sys/class/net",
                read_timeout=90,
            )
            ...
            self.logger.info("APIC installation successfully.")
    ...
    # TODO: TBD - handle post-check to for selection2
    def ssh_discover_apic(self):
        """Configure APIC initial setup parameters via CIMC SSH"""

        self.logger.info("Start to discover and initial setup APIC via CIMC SSH.")
    ...
    def gen_install_apic(self):
        ...
        """
        self.logger.info("Start to run APIC installation workflow...")
        if not self.api_install_apic():
            msg = f"APIC {self.hostname} api install phase failed."
            self.logger.error(msg)
            self.errors.append(msg)
            return False
        self.logger.info(f"APIC {self.hostname} api install phase completed successfully.")

        if not self.ssh_install_apic():
            msg = f"APIC {self.hostname} ssh install phase failed."
            self.logger.error(msg)
            self.errors.append(msg)
            return False
        self.logger.info(f"APIC {self.hostname} ssh install phase completed successfully.")

        self.logger.info("APIC installation workflow has been successfully completed.")
        return True

    def gen_discover_apic(self):
        """
        Complete APIC discovery and initial setup workflow:
        1. Validate CIMC for APIC discovery
        2. Configure APIC initial setup parameters
        """

        self.logger.info("Start to run APIC initial setup workflow...")
        self.api_validate_apic()
