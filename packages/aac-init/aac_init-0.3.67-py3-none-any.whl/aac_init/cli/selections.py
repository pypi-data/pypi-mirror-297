...
from aac_init.deployment.utils import Thread_Tool, APIC_CIMC_Tool, ACI_Switch_Tool
    ...
    def fabric_bootstrap(self):
        ...
        if apic_check:
            ...
            for apic_connection in apics:
                apic = APIC_CIMC_Tool(global_policies, apic_connection)
        ...
        if aci_switch_check:
            ...
            for aci_switch_connection in aci_switches:
                ...
                if not aci_switch.validate_aci_switch():
                    ...
                    self.errors.append(msg)
                    return False
                self.logger.info(f"Validate ACI switch '{aci_switch.hostname}' successfully.")
