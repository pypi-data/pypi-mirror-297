...
class APIC_CIMC_Tool:
    ...
    def __init__(self, global_policies: dict, apic_connection: dict):
        ...
        self.device_ssh = {
            ...
            "session_log": netmiko_session_logger(
                f"ssh_session_{self.hostname}.log"
            ),
        }
        self.token = None
        ...
        self.logger.debug("CIMC tool initialized successfully.")
        ...
        try:
            ...
            for i in range(1,4):
                ...
                if response:
                    ...
                    if self.token:
                        self.logger.debug(
                            f"Successfully logged in to CIMC via API. Token: {self.token}"
                        )
                        return self.token
                    else:
                        msg = (
                            f"Login failed for CIMC '{self.cimc_address}': No token received. Retry = {i}"
                        )
                        self.logger.error(msg)
    ...
    def api_install_apic(self):
        ...
        try:
            ...
            self.logger.debug("CIMC-MAP configured successfully, start to install APIC...")
            return True
        ...
        for i in range(1,4):
            for device_type in DEVICE_TYPES:
                try:
                    ...
                    self.logger.error(f"Failed to connect to {self.cimc_address} with {device_type}: {e}")
                    sleep(3)
    ...
    def ssh_discover_apic(self):
        ...
        self.logger.debug("Start to discover and initial setup APIC via CIMC SSH.")
    ...
    def gen_install_apic(self):
        ...
        if not self.api_install_apic():
            ...
            self.errors.append(msg)
            return False
        self.logger.debug(f"APIC '{self.hostname}' API installation phase completed successfully.")
        ...
        if not self.ssh_install_apic():
            ...
            self.errors.append(msg)
            return False
        self.logger.debug(f"APIC '{self.hostname}' SSH installation phase completed successfully.")

        self.logger.debug("APIC installation workflow has been successfully completed.")
        self.logger.info(f"APIC '{self.hostname}' had been successfully installed to '{self.apic_image}'.")
        return True
    ...
    def gen_discover_apic(self):
        ...
        self.logger.debug("Start to run APIC initial setup workflow...")
