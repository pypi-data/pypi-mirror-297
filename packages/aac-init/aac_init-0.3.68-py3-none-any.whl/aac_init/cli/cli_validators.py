...
class CLI_Validator:
    ...
    def _validate_global_policy(self):
        ...
        data = yamale.make_data(self.global_policy_path)
        ...
        for _, _, schema_files in os.walk(schema_folder):
            for schema_file in schema_files:
                self.logger.debug(f"Validating schema: '{schema_file}' for global policy...")
    ...
    def _validate_fabric_mgmt(self):
        ...
        data = yamale.make_data(fabric_mgmt_file)
        schema_folder = os.path.join(settings.SCHEMA_DIR, "01-fabric_mgmt")

        for _, _, schema_files in os.walk(schema_folder):
            for schema_file in schema_files:
                self.logger.debug(f"Validating schema: '{schema_file}' for fabric management...")
                schema_file_path = os.path.join(schema_folder, schema_file)
                try:
                    schema = yamale.make_schema(schema_file_path)
                    yamale.validate(schema, data)
        ...
        fabric_mgmt_policy_apic_node_policies_nodes = fabric_mgmt_policy_apic_node_policies.get("nodes", []) or []
        fabric_mgmt_node_ids = set(i.get("id") for i in fabric_mgmt_policy_apic_node_policies_nodes)

        global_policy_fabric = self.global_policy.get("fabric", {}) or {}
        global_policy_switch_nodes_connection = global_policy_fabric.get("switch_nodes_connection", []) or []
        global_policy_switch_node_ids = set(i.get("id") for i in global_policy_switch_nodes_connection)
