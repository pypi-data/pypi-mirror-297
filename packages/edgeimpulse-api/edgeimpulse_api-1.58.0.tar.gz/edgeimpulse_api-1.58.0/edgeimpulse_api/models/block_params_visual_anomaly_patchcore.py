...
class BlockParamsVisualAnomalyPatchcore(BaseModel):
    ...
    resampling_method: Optional[StrictStr] = Field(None, alias="resamplingMethod", description="The method for resampling the input data")
    ...
    __properties = ["backbone", "numLayers", "poolSize", "resamplingMethod", "samplingRatio", "numNearestNeighbors"]

    @validator('resampling_method')
    def resampling_method_validate_enum(cls, v):
        if v is None:
            return v

        if v not in ('tile', 'interpolate'):
            raise ValueError("must validate the enum values ('tile', 'interpolate')")
        return v
    ...
    def from_dict(cls, obj: dict) -> BlockParamsVisualAnomalyPatchcore:
        ...
        _obj = BlockParamsVisualAnomalyPatchcore.construct(**{
            ...
            "resampling_method": obj.get("resamplingMethod"),
