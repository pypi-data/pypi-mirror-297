...
from zetta.job import get_job, list_jobs, worker_log_job, process_zetta_job_config, init_zetta_job_config, gitea_upload, send_finetune_request
...
async def list(env: str=common_env_option(), json: bool = True):
    try:
        jobs = list_jobs(json_output=json)
        print(jobs)
...
async def get(jobid, env: str=common_env_option(), json: bool = True):
    try:
        job = get_job(jobid, json_output=json)
        if job == 'null':
