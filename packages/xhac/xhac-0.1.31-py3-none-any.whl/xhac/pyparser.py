...
EVETN_VERSION_RE = re.compile("EVENT/(\d+).(\d+)")
...
class HttpParser(object):
    ...
    def _parse_response_line(self, line):
        ...
        # version
        matchv = VERSION_RE.match(bits[0]) or EVETN_VERSION_RE.match(bits[0])
        if matchv is None:
            raise InvalidRequestLine("Invalid HTTP version: %s" % bits[0])
        self._version = (int(matchv.group(1)), int(matchv.group(2)))
        self.protocol = 'HTTP' if VERSION_RE.match(bits[0]) else 'EVENT'
