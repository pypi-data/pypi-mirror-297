...
def get_files(
    ...
    **kwargs,
    ...
    text_pattern += "{extension}" if text_pattern.endswith(".") else ".{extension}"
    ...
    if is_string(extensions):
        ...
        response = []
        [
            response.extend(get_files(path, x, pattern))
            for x in extensions
            if is_string(x, True)
        ]
        return response
...
def create_path(
    *pathes: Path | str,
    ...
    Path(*[x for x in pathes if isinstance(x, (bytes, str, Path))]).mkdir(
...
def set_path(
    *pathes: str | Path,
    create_parent_only: bool = False,
    return_string: bool = False,
):
    path = Path(*[x for x in pathes if isinstance(x, (bytes, str, Path))])
    create_path(path.parent if create_parent_only else path)
