...
class TrendsAnalyzerResults(langchain_core.tools.BaseTool):
  ...
  samples_directory: str = 'path to a folder with json documents to infer trends'
  ...
  def _run(
    ...
    trend_analyzer = analyzer.TrendsAnalyzer(
      llm=llm, vect_store=vectorstore.load_vectorstore(samples_directory=self.samples_directory)
    )
