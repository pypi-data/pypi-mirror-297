...
def show_progress(duration, stop_event, progress_complete):
    ...
    with Progress(
        ...
        task = progress.add_task(f"Generating image... Usually done in {duration} seconds", total=duration)
        for remaining in range(duration, 0, -1):
            if stop_event.is_set():
                break
            progress.update(task, advance=1, description=f"Generating image... Usually done in {remaining} seconds")
            time.sleep(1)
...
def generate_image_with_progress(provider_instance, prompt, duration):
    ...
    if progress_complete.is_set():
        console.print(f"\nImage saved to: {destination_file}", style="green")
    else:
        console.print(f"\nImage generated faster than expected and saved to: {destination_file}", style="green")
