...
@click.pass_context
def upload(ctx: Context, vault_spec: str, directory: str, url: str, token: str | None, tag: str | None) -> None:
    try:
    ...
    try:
        ...
        response_message = client.upload_files_s3(directory=directory, tag=tag_name or tag)
...
@click.argument("vault_spec", required=False)
@click.option(
    "-d",
    "--directory",
    default=DEFAULT_DIRECTORY,
...
def download(
    ...
    if not tag_name_or_file_name:
        message = client.download_latest_version(org_name, vault_name, target_directory=directory)
        click.echo(message)
        return None

    try:
        message = client.download_version_or_tag(
            org_name, vault_name, tag_name_or_file_name, target_directory=directory
        )
        click.echo(message)
        return None
    except ValueError:
...
@cli.command(
    cls=CustomHelpOption,
    name="run",
    help="Run Proofs locally, in a container or remotely on kaas provided compute nodes",
)
@click.option(
    "--mode",
    "-m",
...
@click.pass_context
def run(ctx: Context, mode: str, kontrol_version: KontrolVersion) -> None:
    click.echo(f"Running Kontrol in {mode} mode with version {kontrol_version}")
    run_kontrol = RunKontrol(kontrol_version, mode)
    if run_kontrol.run():
