...
class ApiColegiosTecnicosService(BaseApiGraphiQL):
    ...
    def get(self, query: str) -> List[Usuario]:
        ...
        if content['data'] and content['data']['getDadosAlunosColegiosTecnicos'] and content['data']['getDadosAlunosColegiosTecnicos']['items']:
            resultado = content['data']['getDadosAlunosColegiosTecnicos']['items']
            return UsuarioList.validate_python(resultado)
                # exist = len([u for u in usuarios if u.identificador == item["identificador"]]) > 0
                #
                # if not exist:
                #     username = f'cl{item["identificador"]}'
                #     unidade = item.get('unidade', '')
                #     tipo_membro = TipoMembro.ALUNO_COTIL if str(item['unidade']).lower() == 'cotil' else TipoMembro.ALUNO_COTUCA 
                #
                #     usuario = DataMembro(
                #         nome=str(item['nome']).title(),
                #         identificador=item['identificador'],
                #         email=f'cl{str(item["email"]).lower()}@dac.unicamp.br',
                #         cargo=f'Aluno / {item["cargo"]}',
                #         unidade=unidade,
                #         telefone=None,
                #         username=username,
