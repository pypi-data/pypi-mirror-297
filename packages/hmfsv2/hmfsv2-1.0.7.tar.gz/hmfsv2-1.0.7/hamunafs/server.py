...
@task_deco('fs_put', function_timeout=120, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.NSQ, specify_async_loop=consumer_loop)
...
# @task_deco('fs_put_local', function_timeout=120, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.SQLITE_QUEUE)
...
@task_deco('fs_get_{}'.format(cfg['NODE_ID']), function_timeout=60, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.NSQ, specify_async_loop=consumer_loop)
...
# @task_deco('fs_get_{}_local'.format(cfg['NODE_ID']), function_timeout=60, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.SQLITE_QUEUE)
...
def get_system_status():
    ...
    long_idle_warning = time.time() - last_process_ts > 60 
...
async def extra_tasks():
    global exception_queue, last_exception_sync_time, last_process_ts
    while True:
        try:
            ...
            if time.time() - last_process_ts  > 300:
