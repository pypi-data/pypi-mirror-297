...
def run_query(src: str, month_item: dict, forcast_times: list[str], steps: list[str], vars: list[str],
    ...
    collection = get_collection(src, month_item)
...
def query_mongo_multi_thread(workers, src: str, month_group: dict, steps: list[str], vars: list[str], coords: List[Coordinate]):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_month = {
            executor.submit(run_query, src, month_key, month_group[month_key], steps, vars, coords): month_key
...
def get_collection(src, year_month):
