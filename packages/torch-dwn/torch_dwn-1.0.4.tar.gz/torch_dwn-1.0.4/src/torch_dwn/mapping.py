...
class LearnableMapping(torch.nn.Module):
    def __init__(self, num_inputs, num_output, tau=0.001):
        super().__init__()
        self.weights = torch.nn.Parameter(torch.rand(num_inputs, num_output, dtype=torch.float32), requires_grad=True)
        self.tau = tau
