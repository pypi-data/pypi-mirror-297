...
def remove_empty_directories() -> None:
    for root, folders, files in os.walk(os.getcwd()):
        if f'.DS_Store' in files:
            path = Path(root).joinpath('.DS_Store')
            send2trash(str(path.absolute))
...
def rm(verbose: bool, dry_run: bool) -> None:
    ...
    if dry_run:
        delete_style = click.style('will delete'.ljust(14), fg = 'yellow')
        keep_style = click.style('will keep'.ljust(14), fg = 'green')
    else:
        delete_style = click.style('deleted'.ljust(14), fg = 'yellow')
        keep_style = click.style('kept'.ljust(14), fg = 'green')        
