...
'''
To Do:
1. find more elegant solution for version reporting - only want to have version number in one place
2. Add filtering capability for files (only pictures or files after a certain date)
...
def remove_empty_directories() -> None:
    ...
    for root, folders, files in tqdm(os.walk(os.getcwd()), desc='Removing .DS_Store files from empty directories'):
...
def duple_outputs_exist() -> bool:
    if not Path('duple.delete').exists() or not Path('duple.json').exists():
        click.secho('duple.delete and/or duple.json do no exists - run duple scan to create these files')
        return False
    return True
...
def create_output(all_files: dict, dupes: dict, summary_statistics: dict) -> None:
    ...
    for key, value in summary_statistics.items():
        click.secho(f'{key.ljust(max_len_key + max_len_val - len(str(value)) + 10, ".")}{click.style(str(value), fg = "green")}')
    
    click.secho()
    click.secho('Open the `output summary results` file listed above with a text editor for review', fg = 'yellow')
    ...
    with open('duple.delete', 'w', encoding = 'utf-8') as f:
        f.write(f'Duple Report Generated on {dt.now(timezone.utc).astimezone().isoformat()}, commanded by user: {os.getlogin()}')
...
def hash_stats(path: str) -> None:
    ...
    hash_times = process_map(timed_get_hash, hashes, repeat(path), max_workers = 4, chunksize = 1, desc = 'Hashing Files...')
...
@cli.command()
@click.option('--test_path', '-tp', type = click.Path(), default = os.getcwd(), help = 'path where test directories and files will be created')
@click.option('--numdirs', '-nd', type = click.INT, default = 3, help = 'number of directories to make for the test')
@click.option('--numfiles', '-nf', type = click.INT, default = 3, help = 'number of files to make in each directory, spread through the directories')
...
def make_test_files(test_path: Path, numdirs: int, numfiles: int, max_file_size: int):
    ...
    for i in tqdm(range(numdirs), desc = 'making directories', position = 0):
        leni = len(str(i))
        dir_path = str(Path(os.getcwd()).joinpath(f'folder_{''.ljust(lennumdir - leni, '0')}{i}').absolute())
        ...
        for j in range(numfiles):
            lenj = len(str(j))
            file_name = str(Path(dir_path).joinpath(f'file_{''.ljust(lennumfiles - lenj, '0')}{j}.txt'))
