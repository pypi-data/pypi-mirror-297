...
OUTPUT_FILE = 'wake-detections.html'
...
class WakeExport:
    ...
    def validate(self) -> Self:
        ...
        if self.sources.get('version') != self.WAKE_DEMO_VERSION:
            raise WakeExportError(
        ...
        if not sources or len(sources) == 0:
            raise WakeExportError(
                ...
                f'Use non-empty directory. Check the compilation output for errors'
            )
    ...
    with ui.spinner('Waiting for server confirmation'):
        ...
        while True:
            if time.time() > timeout:
                raise WakeExportError(
                    'Timeout: Server didn\'t confirmed export upload',
                    'Please try again'
                )
            ...
            if state != CheckState.WAITING_FOR_UPLOAD.value:
                break
...
def wait_for_server_execution(project_api: ProjectApi, project_id: str, check_id: str):
    with ui.spinner('Waiting for the remote execution') as spinner:
        ...
        while True:
            ...
            state_logs = project_api.get_vulnerability_check_state_logs(project_id, check_id, last_log_time)

            new_logs = state_logs.get('logs')
            if len(new_logs):
                for log in new_logs:
                    log_time = datetime.datetime.fromisoformat(log.get('createTime'))
        ...
        if check_state == CheckState.ERROR.value:
            ui.error('Remote execution failed', lines=[
                check.get('error')
            ])
            return

        check = project_api.get_vulnerability_check(project_id, check_id)
        result_link = check.get('resultLink')

        spinner.update('Getting the result file')
        if not result_link:
            raise WakeExportError('Check was successful but cannot fetch the result file', 'Please contact ABCH')

        download_result(result_link)
...
def check(ctx):
    try:
        ...
        ui.section_start(f'Compiling the source (Wake {WakeExport.WAKE_DEMO_VERSION})')
        ...
        abs_path = str(Path.cwd() / OUTPUT_FILE)

        ui.section_end()
        ui.success(title='Check is completed', lines=[
            f'Results are available in {ui.highlight(abs_path)}',
        ])
        ...
        ui.error(title=f'Wake Arena API errror [{err.code}]', lines=['Try again or contact ABCH'])
        sys.exit(1)
