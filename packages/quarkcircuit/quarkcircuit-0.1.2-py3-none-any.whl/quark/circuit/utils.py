...
def glob_phase(mat: np.ndarray) -> float:
    """
    Extract the global phase $\alpha$ from a d*d matrix. $U = e^{i\alpha} S$ in which S is in SU(d).
...
def kron_factor_4x4_to_2x2s(mat: np.ndarray) -> Tuple[complex, np.ndarray, np.ndarray]:
    """
    Decompose a 4x4 matrix U into the Kronecker product of two 2x2 unitary matrices $U = kron(A, B)$
    and a global scalar factor.
    ...
    Args:
        mat (np.ndarray): A 4x4 unitary matrix to be factored.

    Returns:
        Tuple[complex, np.ndarray, np.ndarray]: 
            - A complex scalar `g` representing the global factor.
            - `f1` (np.ndarray): A 2x2 matrix, representing part of the Kronecker product.
            - `f2` (np.ndarray): Another 2x2 matrix, representing part of the Kronecker product.

    Raises:
        ValueError: If the input matrix cannot be tensor-factored into two 2x2 matrices.
...
def kak_decompose(mat: np.ndarray) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    ...
    Programmers <https://arxiv.org/abs/quant-ph/0406176>`.

    Args:
        mat (np.ndarray): A 4x4 unitary matrix.

    Returns:
        Tuple[List[np.ndarray], List[np.ndarray]]: 
            - `rots1`: A list of four 2x2 matrices representing the rotation gates acting on the first qubit.
            - `rots2`: A list of four 2x2 matrices representing the rotation gates acting on the second qubit.
...
def zyz_decompose(mat: np.ndarray) -> Tuple[float, float, float, float]:
    ...
    Args:
        mat: A 2x2 unitary matrix to decompose.
...
def u3_decompose(mat: np.ndarray) -> Tuple[float, float, float, float]:
    """
    Decompose a 2x2 unitary matrix into U3 gate and obtain the parameters and global phase.
    ...
    Args:
        mat (np.ndarray): A 2x2 unitary matrix to decompose.

    Returns:
        Tuple[float, float, float, float]: A tuple containing the three parameters $\theta$, $\phi$, $\lambda$ of a standard U3 gate 
        and global phase `p`.
    """
