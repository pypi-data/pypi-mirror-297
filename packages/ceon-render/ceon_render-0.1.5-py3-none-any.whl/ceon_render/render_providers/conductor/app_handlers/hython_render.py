...
def prep_ris(node):
    ...
    for i in range(num_displays):
        print(
            "Set display {} to make intermediate directories if needed".format(
                i
            )
        )
...
def parse_args():
    ...
    parser.add_argument(
        "-r", dest="resolution", nargs=2, type=int, required=False
    )
    parser.add_argument(
        "-o",
        "-out-file",
...
def ensure_posix_paths():
    ...
    for parm, value in refs:
        ...
        print(
            "{} Found a drive letter in path: {}. Stripping".format(
                ident, value
            )
        )
        ...
        try:
            ...
            print(
                "{} Failed to set value for parm {}. Skipping".format(
                    ident, value
                )
            )
            print(ex)
...
def print_available_rops():
    ...
    try:
        ...
        all_rops = hou.nodeType(
            hou.sopNodeTypeCategory(), "ropnet"
        ).instances()
