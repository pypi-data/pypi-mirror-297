...
def put_colour(txt, color='',filename=False):
    if filename:
        return txt
    if color == 'red':
...
def add_context_info(func,level=logging.INFO,filename=False,format='',show=True):
    def wrapper(*args, **kwargs):
        ...
        if func.__name__ == 'debug':
            color = 'gray'
            format_txt = put_colour(format_txt,color=color,filename=filename)
            args = (' '.join([put_colour(str(i),color=color,filename=filename) if not filename else str(i) for i in args ]),)
