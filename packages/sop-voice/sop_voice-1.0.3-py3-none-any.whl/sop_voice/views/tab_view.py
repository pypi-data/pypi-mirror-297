...
class VoiceSiteTabView(View, ActionsMixin, PermissionRequiredMixin, AccessMixin):
    ...
    tab = ViewTab(
        label="Voice",
        badge=lambda obj: count_all_sda_list(SDA_List.objects.filter(delivery__site=obj)).__int__()[0]
    )
    ...
    def get_extra_context(self, request, pk) -> dict:
        ...
        context['sda'] = SDA_List
        context['sda_table'] = self.get_table(SDA_ListTable, SDA_List.objects.filter(delivery__site=site), request)
        ...
        context['rcm'] = get_object_or_create(RingCentralMigration, site)
        context['btip'] = get_object_or_create(BTIPMigration, site)
    ...
    def get(self, request, pk):
        if not request.user.has_perm(get_permission_for_model(SDA_List, 'view')):
