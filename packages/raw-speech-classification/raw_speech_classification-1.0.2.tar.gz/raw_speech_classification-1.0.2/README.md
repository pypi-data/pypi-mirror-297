...
1. Create lists for training, cross-validation and testing.
   ...
   [`datasets/IEMOCAP/F1_lists`](https://github.com/idiap/RawSpeechClassification/tree/v1.0.2/datasets/IEMOCAP/F1_lists).

1. **If you installed from source with Conda:** A `run` script is available that
   concatenates all the steps. Run [`run.sh`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/run.sh). Provide the model architecture as
   an argument. See [`model_architecture.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/model_architecture.py) for valid
...
This is an [example](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/docs/log.txt) of the log printed to the terminal, and you should
obtain the following curve in `results/seg-f1/plot.png`:

![Results](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/docs/plot.png)

## Code components

1. [`wav2feat.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/wav2feat.py) creates directories where the wav files are stored
   as fixed length frames for faster access during training and testing.

1. [`train.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/train.py) is the Keras training script.

1. Model architecture can be configured in
   [`model_architecture.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/model_architecture.py).

1. [`rawdataset.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/rawdataset.py) provides an object that reads the saved
   directories in batches and retrieves mini-batches for training.

1. [`test.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/test.py) performs the testing and generates scores as posterior
...
1. [`plot.py`](https://raw.github.com/idiap/RawSpeechClassification/v1.0.2/rsclf/plot.py) generates and saves the learning curves.
