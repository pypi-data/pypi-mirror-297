...
class Base:
    def __init__(self):
        ...
        self.reset = Style.RESET_ALL
                # Khai báo tên trò chơi
        game_name = "NewBlum"

        # In ra dòng thông báo
        print(f"{self.blue}NhoxNho {self.white}{game_name} Auto Claimer")
    ...
    def create_banner(self, game_name: str):
        ...
        banner = f"""
        {self.blue}NhoxNho {self.white}{game_name} Auto Claimer
    ...
    def parse_proxy_info(self, proxy_info):
        try:
            ...
            return {"user_name": user_name, "pass": password, "ip": ip, "port": po>
        except:
            self.log(
                f"{self.red}Check proxy format: {self.white}http://user:pass@ip:po>
            )
            return None
