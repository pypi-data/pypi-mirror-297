import os,requests,json,time,aiohttp
...
class ResponseManager:
    ...
    async def get_response(self) -> None:
        """
        Processes the response and manages saving via SaveManager.
        """
        try:
            self.api_response = await self.response  # Assuming response is JSON
            self.total_tokens_list.append({
                "time": get_any_value(self.api_response, "created"),
                "tokens": get_any_value(self.api_response, "usage")
            })
        except ValueError:
        ...
        self.save_manager = SaveManager(data=self.query_js, title=json_api_response.get('created', 'response'),
                                        directory=['response_data', 'raw_response'], model=json_api_response.get('model', 'default'))
        self.get_all_instruction(json_api_response)
        self.query_js["response"] = json_api_response
        self.save_manager = SaveManager(data=self.query_js, title=self.generate_title or self.title or 'response',
                                        directory=self.directory, model=self.model)
    ...
    def get_abort(self)->bool:
       if self.abort_js.get("abort") or self.abort_js.get("blank_prompt"):
            return True
    ...
    def check_token_cap(self):
        ...
        tokens_in_one_ls = [token_info for token_info in reversed(self.total_tokens_list) if (token_info.get('time')or 0)>one_minute_ago]
        print(tokens_in_one_ls)
        total_current_minute_tokens= sum([0]+[token_info.get('tokens',{}).get('total_tokens',0) for token_info in tokens_in_one_ls])
        total_plus_next = self.model_mgr.selected_max_tokens + total_current_minute_tokens
        desired_token_position = 80000- self.model_mgr.selected_max_tokens
        if total_plus_next > 80000:
            amount_to_shave = total_plus_next-desired_token_position
            for i,token_info in enumerate(reversed(self.total_tokens_list)):
                amount_to_shave -=token_info.get('tokens')
                if amount_to_shave<0:
                        time_difference = time.time()-token_info.get('time')
                        self.sleep_time = time_difference + time.time()
        self.sleep_time = 0
        ...
        if self.prompt.get('blank_prompt') is None:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url=self.endpoint, json=self.prompt, headers=self.header) as response:
                    self.response = await response.json()  # Assuming the response is JSON
                    await self.get_response()  # Process the response
    ...
    async def initial_query(self) -> list[dict]:
        ...
        for chunk_token_distribution_number, distributions in enumerate(self.chunk_token_distributions):
            for chunk_number, distribution in enumerate(distributions):
                ...
                while response_loop:
                    self.check_token_cap()
                    if self.sleep_time < time.time():
                        await self.send_query()  # Await the asynchronous send_query
