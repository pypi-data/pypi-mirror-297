...
def get_model_mgr(model=None):
    model_mgr = ModelManager(input_model_name = model)
    return model_mgr
def  get_instruction_manager(instructions={},bool_list = {}):
    inst_mgr = InstructionManager(instructions,bool_list)
    return inst_mgr
def get_prompt_manager(prompt=None,
  ...
  instruction_mgr = instruction_mgr or get_instruction_manager(bool_list = instruction_bools,instructions=instruction_data)
...
class nogui_setup_module():
    def __init__(self,api_key=None,
        ...
        self.model_mgr = get_model_mgr(model=self.model)
        self.inst_mgr = InstructionManager(self.additional_instructions,self.instruction_bools)
        self.prompt_mgr = get_prompt_manager(prompt=self.prompt,prompt_data=self.prompt_data,instruction_mgr=self.inst_mgr,model_mgr=self.model_mgr,instruction_data=self.inst_mgr.instructions,completion_percentage = self.completion_percentage,model=self.model,instruction_bools=self.instruction_bools)
        self.resp_mgr = ResponseManager(prompt_mgr=self.prompt_mgr,api_mgr=self.api_mgr)
    def calculate_extra_tokens(self,response=None):
        prompt = prompt or self.prompt_mgr.create_prompt()
        self.extra_tokens = (self.model_mgr.selected_max_tokens*self.completion_percentage) - num_tokens_from_string(self.prompt_mgr.create_prompt(prompt))
    ...
    def make_query(self):
        return asyncio.run(self.make_query())
