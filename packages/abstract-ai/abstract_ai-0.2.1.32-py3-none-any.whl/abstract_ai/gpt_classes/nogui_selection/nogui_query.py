...
def get_db_query(envPath):
    db_mgr = self.dbManager(env_path=envPath)
    file_path = get_last_created_response_file_path()
    return db_mgr.generate_query_from_recent_response(file_path)
...
class nogui_setup_module():
    ...
    def generate_query_from_recent_response(self, file_path):
        ...
        if database_query:
            ...
            self.dbManager.get_query_save_to_excel(database_query, file_path=new_file_path)
            return new_file_path

    def get_db_query(self,file_path=None):
        """Get the latest database query and save the result."""
        file_path = file_path or get_latest_response()
        return self.dbManager.generate_query_from_recent_response(file_path)
