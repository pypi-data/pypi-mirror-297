...
class ResponseManager:
    ...
    def get_response(self):
        ...
        try:
            ...
            self.total_tokens_list.append({"time":get_any_value(self.api_response,"created"),"tokens":get_any_value(self.api_response,"usage")})
        except ValueError:
    ...
    async def send_query(self):
        """
        Handles sending the query asynchronously using aiohttp.
        """
        self.query_js = {}
        ...
        if self.prompt.get('blank_prompt') is None:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.endpoint, json=self.prompt, headers=self.header) as response:
                    self.api_response = await response.json()
                    return self.api_response  # Ensure this is handled prope
   
    async def initial_query(self) -> list[dict]:
        """
        Asynchronous method to manage the initial sequence of sending queries and processing responses.
            ...
            for chunk_number, distribution in enumerate(distributions):
                ...
                while response_loop:
                    ...
                    if self.sleep_time < time.time():
                        response = await self.send_query()  # Send query asynchronously
