import os,threading
from abstract_gui import AbstractBrowser,text_to_key,make_component,ensure_nested_list,expandable,RightClickManager,get_screen_dimensions,AbstractWindowManager,sg
...
class WalletManager:
    ...
    def save_config(self, filename):
        with open(filename, 'w') as f:
            f.write(f"en_keys={self.env_keys}\n")
...
def env_section():
    return make_component("Frame", 'env key', layout=[
    [
...
def get_settings()->list:
    ...
    def get_model_selection_layout():
        ...
        def get_model_selector()->object:
            return make_component("Frame",'Model',[[make_component("Combo",[], default_value=[], key='-MODEL-', enable_events=True)]])
        def get_token_display()->object:
            return make_component("Frame",'tokens',[[make_component("Input",[],key='-MAX_TOKENS-', readonly=True, enable_events=True,disabled=True,size=(5,1))]])
    ...
    return [get_column([[make_component("Frame",'Token Percentage',ensure_nested_list(get_percentage_component()))],
            ...
            [swap_commands()],]),
            get_column([[make_component("Frame",'Title',ensure_nested_list([title_bool,title_input]))],
...
def abstract_browser_layout(section=None,extra_component=None):
    ...
    frames = make_component("Frame",'chunk title',layout=ensure_nested_list(inputs))
    chunk_title = make_component('Column',ensure_nested_list(frames))
    chunk_data_buttons = env_section()
...
def get_instructions()->list:
    ...
    sub_layout = [make_component("Column", ensure_nested_list(sub_layout), **expandable(size=(None,int(window_height))), scroll_vertical=True)]
...
def get_total_layout()->list:
    ...
    prompt_tabs= get_prompt_tabs({"query":get_query_section(),
                                 ...
                                 args={**expandable(size=(None,int(window_height)))})
    return [
        [get_progress_frame()],
        [get_output_options()],
        [get_column([[prompt_tabs]]),get_column(utilities())]
        ]
def while_win(event,values,window):
    
    if event == "Start":
        # Start your monitoring or other functionality
        pass
    elif event == "Stop":
        # Stop the monitoring or other functionality
        pass
    elif event == "-START-":
            get_token_info(values["-ADDRESS_INPUT-"])
    elif event.startswith('-ENV_'):
        if event =="-ENV_DIRECTORY-":
                pass
