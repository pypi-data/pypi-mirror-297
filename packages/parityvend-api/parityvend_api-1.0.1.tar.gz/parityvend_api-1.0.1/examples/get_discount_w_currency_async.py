...
async def main():
    ...
    discount_usd = await parityvend.get_discount_from_ip(ip)
    print("USD discount:", discount_usd)

    # the base currency can be changed
    discount_gbp = await parityvend.get_discount_from_ip(ip, base_currency="GBP")
...
Example "Get discount with custom base currency" (async)
