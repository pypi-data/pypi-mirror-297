...
class Spinner:
    ...
    def __init__(self, message="", finished_message:str|Callable[[], str]="", failed_message=None, delay=None):
    ...
    def get_message(self):
        ...
        full_message = f"{next(self.spinner_generator)} {self.message}"
    ...
    def __exit__(self, exception, value, _):
        ...
        time.sleep(self.delay)
        self.reset_cursor()
        m = self.finished_message
        finished_message = m if isinstance(m, str) else m()
        if finished_message != "":
            final_message = f"▰▰▰▰ {finished_message}"
            self.update(final_message, color="green")
            rich.print()
        elif finished_message == "":
            self.update("")
            self.reset_cursor()
