...
from relationalai.errors import RAITypeError, UnsupportedGraphType
from .. import dsl
...
def between(param:str, value: Numeric, min: Numeric, max: Numeric, type_: NumericType = Number):
...
def positive(param:str, value: Numeric, type_: NumericType = Number):
...
def nonnegative(param:str, value: Numeric, type_: NumericType = Number):
    if (not isinstance(value, type_)) or (value < 0):
...
class Compute:
    ...
    def local_clustering_coefficient(self, node):
        if not self._graph.undirected:
            exception = UnsupportedGraphType("local_clustering_coefficient only supports undirected graphs.")
            raise exception
        return self._lib.local_clustering_coefficient(self._graph, unwrap(node))

    def avg_clustering_coefficient(self):
        if not self._graph.undirected:
            exception = UnsupportedGraphType("avg_clustering_coefficient only supports undirected graphs.")
            raise exception
    ...
    def cosine_similarity(self, node1, node2):
        if not self._graph.undirected:
            exception = UnsupportedGraphType("cosine_similarity only supports undirected graphs.")
            raise exception
    ...
    def eigenvector_centrality(self, node):
        if not self._graph.undirected:
            exception = UnsupportedGraphType("eigenvector_centrality only supports undirected graphs.")
            raise exception
    ...
    def triangle_community(self, node):
        if not self._graph.undirected:
            exception = UnsupportedGraphType("triangle_community only supports undirected graphs.")
            raise exception
