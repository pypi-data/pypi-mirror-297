    ...
    if data:
        ...
        try:
            ...
            with Spinner("Validating imports engine", "Imports engine validated", "Error:"):
                engine_state = provider.get_engine(data["engine"])
            if not engine_state:
                rich.print()
...
def set_imports_engine(name:str|None=None):
    ...
    provider = get_resource_provider()
    if not name:
        selected_name = controls.fuzzy_with_refetch(
            "Select an engine for imports:",
            "engines",
            lambda: [engine["name"] for engine in provider.list_engines()],
            not_found_message="No valid engines found."
        )
        if not selected_name or isinstance(selected_name, Exception):
            if not selected_name:
                ...
                return
        assert isinstance(selected_name, str), "selected_name should not be None"
        name = selected_name
    with Spinner(
        "Setting imports engine",
        "Imports engine set to [cyan]'" + name + "'[/cyan]",
        "Error:",
    ):
