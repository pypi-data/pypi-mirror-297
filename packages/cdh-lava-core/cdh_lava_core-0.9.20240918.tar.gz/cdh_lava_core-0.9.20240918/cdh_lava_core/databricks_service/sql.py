    ...
    def save_workflow_sql(
        ...
        with tracer.start_as_current_span("save_workflow_sql"):
            try:
                ...
                relative_path_sql = os.path.relpath(new_path_sql, current_directory)
                relative_dir_name_sql_local = os.path.relpath(dir_name_sql_local, current_directory)

                # Ensure the paths are prefixed with './' if they are not already
                if not relative_path_sql.startswith('.'):
                    relative_path_sql = './' + relative_path_sql

                if not relative_dir_name_sql_local.startswith('.'):
                    relative_dir_name_sql_local = './' + relative_dir_name_sql_local
