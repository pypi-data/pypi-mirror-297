...
def _make_filtering_bound_logger(min_level: int) -> type:
    ...
    def log(self: Any, level: int, event: str, *args: Any, **kw: Any) -> Any:
        if level < min_level:
            return None
        # log may need to handle logging message from stdlib logging,
        # so use a full list here.
        lvl_name = STD_LEVEL_TO_NAME[level]

        if not args:
            return self._proxy_to_logger(lvl_name, event, **kw)

        return self._proxy_to_logger(lvl_name, event % args, **kw)
