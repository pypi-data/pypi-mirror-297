...
async def run(
    ...
    delayed_transaction_logger = make_logger(
        "delayed_transactions", False, Path(config.log_files["delayed_transactions"])
    )
    stuck_transaction_logger = make_logger(
        "stuck_transactions", False, Path(config.log_files["stuck_transactions"])
    )
    improper_fill_logger = make_logger(
        "improper_fills", False, Path(config.log_files["improper_fills"])
    )
