...
class GraphConvolution(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if node_layers:
            in_dim = node_layers[-1]  # Output from the DeepSet node layers
        else:
            in_dim = input_dim
        for i in range(num_layers):
            ...
            self.gcn_layers.append(GCN(in_dim, out_dim))
            in_dim = out_dim

        # Set layers after GCN
...
def main():
    ...
    # Dummy edge_index for 100 nodes, for simplicity, connecting each node to itself
