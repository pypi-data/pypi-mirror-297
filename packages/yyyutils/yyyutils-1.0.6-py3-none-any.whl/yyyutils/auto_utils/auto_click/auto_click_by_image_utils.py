...
class AutoClickByImageUtils:
    ...
    def _locate_icon(self, img_name: str, x_start_ratio: float = 0, y_start_ratio: float = 0,
                     ...
                     similarity_threshold: float = 0.9, multi_scale: bool = False,
        ...
        for i in range(try_times):
            try:
                ...
                template = self.image_cache.get(img_name) or cv.imread(obj_path)
                if template is None:
    ...
    def check_icon(self, img_name: str, x_start_ratio: float = 0, y_start_ratio: float = 0,
                   ...
                   similarity_threshold: float = 0.9) -> Tuple[bool, int, int]:
        try:
            res = self._locate_icon(img_name, x_start_ratio, y_start_ratio, x_end_ratio, y_end_ratio,
                                    multi_scale=multi_scale, ignore_color=ignore_color, binarize=binarize,
                                    edge_only=edge_only, similarity_threshold=similarity_threshold)
    ...
    def save_click_icon(self, size: Tuple[int, int] = (100, 100), clear_icon_dir: bool = False):
        """
        ...
        with MoniterKeyboardUtils('esc', func) as moniter:
            ...
            while True:
                try:
                    count += 1
                    x, y = MouseUtils.get_mouse_click_position()
                    if finish_flag:
                        break
                    img = ImageGrab.grab(bbox=(x - size[0] // 2, y - size[1] // 2, x + size[0] // 2, y + size[1] // 2))
                    img.save(os.path.join(self.icon_dir, f"icon{count}.png"))
...
class AutoClickGenerator:
    ...
    def add_icon(self, icon_name: str, multi_scale: bool = False, ignore_color: bool = False, binarize: bool = False,
                 edge_only: bool = False, similarity_threshold: float = 0.9):
        try:
    ...
    def click_by_order(self, interval: float = 1, xy_offset: Tuple[int, int] = (0, 0)):
        for icon_name in os.listdir(self.auto_click_utils.icon_dir):
            try:
                res, x, y = self.auto_click_utils.check_icon(icon_name, similarity_threshold=0.9)
                if res:
...
if __name__ == '__main__':
    AutoClickByImageUtils().save_click_icon()
    AutoClickGenerator().add_icons(['icon1.png', 'icon2.png']).click(interval=1)
