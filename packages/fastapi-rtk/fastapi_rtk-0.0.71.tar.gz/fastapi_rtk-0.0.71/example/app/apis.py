...
class AssetApi(ModelRestApi):
    ...
    async def bulk_update_time(
        self, body: dict | List, query: QueryManager
    ) -> Response:
        try:
            stmt = (
                update(Asset).where(Asset.id.in_(body)).values(date_time=datetime.now())
            )
            result = await safe_call(query.session.execute(stmt))
            await safe_call(query.session.commit())
            return GeneralResponse(detail=f"Updated {result.rowcount} records")
        except Exception as e:
            await safe_call(query.session.rollback())
            print(e)
