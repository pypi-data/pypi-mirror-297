...
class SQLAInterface:
    ...
    def generate_schema(
        ...
        self._generate_schema_properties(
            columns_to_generate, optional, schema_dict, self.with_fk
        )
    ...
    def id_schema(self):
        if not self._id_schema.get(self.obj.__name__):
            self._id_schema[self.obj.__name__] = int | str
            self._obj_id[self.obj.__name__] = create_model(
                f"{self.obj.__name__}-ID",
                id=(self._id_schema[self.obj.__name__], Field()),
            )
    ...
    def id_schema(self, value: type):
        ...
        self._obj_id[self.obj.__name__] = create_model(
            f"{self.obj.__name__}-ID", id=(self._id_schema[self.obj.__name__], Field())
        )

    @property
    def obj_id(self):
        return self._obj_id[self.obj.__name__]
    ...
    def schema(self) -> Type[BaseModel]:
        ...
        """
        if not self._schema:
            self._schema = self.generate_schema(self.list_properties.keys())
        return self._schema
