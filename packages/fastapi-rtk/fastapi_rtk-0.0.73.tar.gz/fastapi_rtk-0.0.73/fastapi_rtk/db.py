    ...
    def __init__(
        ...
        if select_cols:
            self._select_cols = [
                ...
                if x in self.datamodel.list_columns
            ]
    ...
    async def scoped_session(self, bind: str | None = None):
        ...
        ***EVEN IF THE SESSION IS SYNC, ASYNC WITH ... AS ... IS STILL NEEDED.***
        ...
        if not scoped_session_maker:
            raise Exception("DatabaseSessionManager is not initialized")
        scoped_session = (
            self._scoped_session_binds.get(bind) if bind else self._scoped_session
        )
        if not scoped_session:
            scoped_session = scoped_session_maker()
            if bind:
                self._scoped_session_binds[bind] = scoped_session
            else:
