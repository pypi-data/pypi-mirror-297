...
impl Dataframe {
    ...
    pub fn filter(&self, cexpr: &CompiledExpr) -> Result<Self> {
        let col = self.eval(cexpr, Type::Bool, "__@@_filter_")?;
        let mut indices = vec![];
    ...
    pub fn assign(&self, cexpr: &CompiledExpr, name: &str, dtype: Type) -> Result<Self> {
        let col = self.eval(cexpr, dtype, name)?;
        Ok(self.append(col)?)
    }
    ...
    pub fn eval(
        ...
        let plexpr: PolarsExpr = cexpr.try_into().map_err(|err| {
            anyhow!(
...
mod tests {
    ...
    fn test_df_eval_basic() {
        ...
        for case in cases {
            ...
            let got = df.eval(&compiled, col.dtype().clone(), "r").unwrap();
