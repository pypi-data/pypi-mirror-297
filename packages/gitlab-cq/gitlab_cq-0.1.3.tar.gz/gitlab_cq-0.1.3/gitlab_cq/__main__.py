...
Options:
  ...
  --echo                Echo linter output
    ...
    if not sys.stdin.isatty():
        ...
        # run linter
        with NamedTemporaryFile(mode="wb") as writer, Path(writer.name).open("rb", 1) as reader:
            proc = subprocess.Popen(  # noqa: S603
                [cmd, *arguments], stdout=subprocess.PIPE, stderr=writer, encoding="utf-8", shell=False
            )
            assert proc.stdout is not None
            while proc.poll() is None:
                linter_output += (c := reader.read().decode("utf-8"))
                if echo:
                    sys.stdout.write(c)
                time.sleep(0.1)
            linter_output += (c := reader.read().decode("utf-8"))
