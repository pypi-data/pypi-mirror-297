...
@auto_schema
def report_aluminum_beam_column(matl: AluMaterial, sect: SteelSection, load: MemberForce, length: SteelLength, eff_len: EffectiveLength, factor: AluMomentModificationFactor):
    dll = load_dll()
    json_data_list = [matl.json(), sect.json(), load.json(), length.json(), eff_len.json(), factor.json()]
    return generate_report_xls(dll, 'Report_Aluminum_BeamColumn', json_data_list)
#     return ResultBytes(type="xlsx", result=read_file_as_binary(file_path))
