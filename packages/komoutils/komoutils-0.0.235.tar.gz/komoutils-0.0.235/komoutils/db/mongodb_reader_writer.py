...
class MongoDBReaderWriter(KomoBase):
    ...
    def read(self, collection: str, filters=None, omit=None, limit: int = 1000000, sort_key: str = '_id', sort_order: int = -1):
        if filters is None:
            filters = {}
        if omit is None:
            omit = {}

        records: list = list(self.db[collection].find(filters, omit).sort(sort_key, sort_order).limit(limit=limit))
        return records
