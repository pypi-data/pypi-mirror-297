                ...
                if os.access(ff, os.X_OK):
                    if f=="sim":
                        ...
                        if not os.path.isdir(pybin):
                            os.mkdir(pybin)
                        shutil.move(ff, os.path.join(pybin, "wrapped_sim"))
                    if f == "play":
                        pybin = os.path.join(extension_dir, "pycytoplay")
                        if not os.path.isdir(pybin):
                            os.mkdir(pybin)
                        shutil.move(ff, os.path.join(pybin, "wrapped_play"))
...
if platform.system() != 'Windows':
	...
	version=version,
        ...
        scripts=["pycytoplay/package/sim","pycytoplay/package/play"],
        ...
        ext_modules=[CMakeExtension('cytoplay', source_dir='pycytoplay')],
        zip_safe=False,
    )
else:
    setup(
