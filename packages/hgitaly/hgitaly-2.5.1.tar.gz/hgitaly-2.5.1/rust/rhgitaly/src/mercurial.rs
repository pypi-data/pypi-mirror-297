...
use hg::repo::Repo;
use hg::revlog::{Graph, GraphError, Node, NodePrefix, Revision, RevlogError, NULL_REVISION};
...
fn git_perms(entry: &ManifestEntry) -> Result<i32, HgError> {
    match entry.flags {
        ...
        Some(b'x') => Ok(git::OBJECT_MODE_EXECUTABLE),
        Some(b'l') => Ok(git::OBJECT_MODE_LINK),
        // 't' is actually only used in treemanifest, but stil possible
        Some(b't') => Ok(git::OBJECT_MODE_TREE),
...
pub fn changelog_entry_manifest(
    ...
    let changelog_entry = changelog.entry_for_rev(rev.into())?;
...
/// An ancestors iterator with offset and limit, that does not emit `NULL_REVISION`
pub struct LimitedAncestorsIterator<G: Graph> {
    inner: AncestorsIterator<G>,
    count: usize,
    max: usize,
    offset: usize,
}

impl<G: Graph> LimitedAncestorsIterator<G> {
    pub fn new(ait: AncestorsIterator<G>, offset: usize, limit: usize) -> Self {
        Self {
            inner: ait,
            offset,
            max: offset + limit,
...
impl<G: Graph> Iterator for LimitedAncestorsIterator<G> {
