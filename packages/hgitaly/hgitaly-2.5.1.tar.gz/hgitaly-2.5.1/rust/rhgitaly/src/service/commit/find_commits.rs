...
use crate::mercurial::LimitedAncestorsIterator;
use crate::message;
...
fn stream_find_commits_ancestors(
    ...
    let iter = LimitedAncestorsIterator::new(
        AncestorsIterator::new(cl, [init_rev], NULL_REVISION, true)
    ...
    stream_chunks(
        ...
        iter.map(|r| {
            r.map_err(|e| RevlogError::Other(revlog_err_from_graph_err(e)))
                .and_then(|r| cl.entry_for_rev(r.into()))
...
pub async fn inner_impl(
    ...
    if request.skip_merges {
        return Err(Status::unimplemented("with skip_merges option"));
    }
        ...
        .map_err(|e| Status::internal(format!("Error resolving revision: {:?}", e)))?
    {
        ...
        Some(node_prefix) => {
            ...
            load_changelog_and_stream(
                ...
                move |_req, _repo, cl, tx| {
                    if let Err(e) =
                        stream_find_commits_ancestors(cl, node_prefix, offset, limit, &tx)
                    {
