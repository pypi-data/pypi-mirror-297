...
from tp_interfaces.helpers.datamodel.fact import mentioned_fact_factory
...
class BratReader(AbstractReader):
    ...
    def read(self) -> Iterator[TalismanDocument]:
        ...
        for path_txt in self._path.glob("*.txt"):
            path_ann = Path(self._path, path_txt.stem + '.ann')
            if path_ann.is_file():
                yield self.read_doc(path_txt, path_ann)
            else:
                logger.warning(f"skip file '{path_txt.stem}'. '{path_ann.stem}' not found.")

    @staticmethod
    def read_doc(path_txt: Path, path_ann: Path) -> TalismanDocument:
