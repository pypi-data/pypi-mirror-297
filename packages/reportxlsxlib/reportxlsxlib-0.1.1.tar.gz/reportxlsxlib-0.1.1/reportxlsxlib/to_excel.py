...
class sqlResult_to_Excel:
    ...
    def caculate_excel(self,path='',dis=''):
        work_bk = workbooks(path)
        work_bk.calculate_formula()
        print('Excel 所有公式计算成功')
        work_bk.save(path)
        print('保存成功')
    ...
    #@validate_parameters(params_to_check=['path'])
    #def caculate_excel_v1(self,path='',dis=''):
...
if __name__ =='__main__':
    ...
    #excl.update_by_template(
        ...
        #sheet_name='促销和可选包'
    #)
    #excl.caculate_excel('template_v2.xlsx','template_v3.xlsx')
