...
def one_step_process(path, company, profile=True, control_group=['lab_blank', 'methanol'], filter_type=1, threshold=10,
                     ...
                     mz_error_alignment=0.015, mz_overlap=1, ms2_analysis=True, frag_rt_error=0.02, split_n=20,rt_overlap=1,
...
def ultimate_peak_picking(ms1, profile=True, split_n=20, threshold=10, i_threshold=500, SN_threshold=3,peak_width=1,area_threshold = 500,
                           height_threshold = 500, noise_threshold = 0, rt_error_alignment=0.05,mz_error_alignment=0.015, mz_overlap=1,
                          sat_intensity=False, long_rt_split_n=1, rt_overlap=1,orbi=False,step_size = 0.02, message = '',isotope_analysis = True):
    ...
    if long_rt_split_n == 1:
        peak_all = split_peak_picking(ms1, profile=profile, split_n=split_n, threshold=threshold, peak_width=peak_width,
                                      i_threshold=i_threshold, SN_threshold=SN_threshold, noise_threshold=noise_threshold,
                                      rt_error_alignment=rt_error_alignment,mz_error_alignment=mz_error_alignment,
        ...
        for n, part in enumerate(parts):
            peak_all = split_peak_picking(part, profile=profile, split_n=split_n, threshold = threshold, i_threshold=i_threshold,
                                          peak_width=peak_width,SN_threshold=SN_threshold,area_threshold = area_threshold,
                                          height_threshold = height_threshold,
                                          rt_error_alignment =rt_error_alignment,mz_error_alignment = mz_error_alignment,
...
def first_process(file, company, profile=True, control_group=['methanol_blank', 'control', 'lab_blank'], threshold=10,
    ...
    mz_round = 4
    ms1, ms2 = sep_scans(file, company,message = message)
    peak_all = ultimate_peak_picking(ms1, profile=profile, split_n=split_n, threshold = threshold, i_threshold=i_threshold,
                                     SN_threshold=SN_threshold,peak_width = peak_width,area_threshold = area_threshold,
    ...
    if len(ms2) == 0:
        ...
        if ms2_analysis is True:
            ...
            if any(item.lower() in basename_file.lower() for item in control_group):
                ...
                peak_all2 = ultimate_peak_picking(ms2, profile=profile, split_n=split_n, threshold = threshold, i_threshold=i_threshold,
                                     SN_threshold=SN_threshold,peak_width = peak_width,area_threshold = area_threshold,
                                     height_threshold = height_threshold, rt_error_alignment = rt_error_alignment,
                                     mz_error_alignment = mz_error_alignment, mz_overlap = mz_overlap,sat_intensity=sat_intensity,
                                     long_rt_split_n=long_rt_split_n,rt_overlap = rt_overlap,step_size = step_size,
                ...
                for i in tqdm(range(len(peak_all)), desc=f'{message}Assign DIA MS2 spectrum',leave = False,colour = 'Green',ncols = 100):
                    ...
                    result = result.sort_values(ascending=False).iloc[:max_frag_num]
...
def multi_process(path, company, profile=True, control_group=['lab_blank', 'methanol'], processors=1,threshold = 10, i_threshold=200,
                  SN_threshold=3, peak_width=1, area_threshold = 500,height_threshold = 500,rt_error_alignment=0.05,mz_error_alignment=0.015,
                  mz_overlap=1, ms2_analysis=True,frag_rt_error=0.02,split_n=20,rt_overlap=1, filter_type=1, sat_intensity=False,
    ...
    for j,file in enumerate(files_mzml):
        ...
        pool.apply_async(first_process,
                         args=(file, company, profile, control_group,threshold, i_threshold, SN_threshold, peak_width,area_threshold,
                             height_threshold,rt_error_alignment,mz_error_alignment, mz_overlap,ms2_analysis, frag_rt_error, split_n,
...
def peak_finding(eic, threshold=10, width=1):
    ...
    if len(peaks) > 5:
        ...
        # 根据z_scores>1找到离群点
        index1 = np.where((z_scores > 1) & (widths > width))[0]
...
def peak_picking(df1, threshold=10, i_threshold=500, SN_threshold=3, peak_width=2, area_threshold=500,
                 height_threshold=500, profile_info=None, isotope_analysis=True, rt_error_alignment=0.05, mz_error_alignment=0.015,
    ...
    if len(data) == 0:
        ...
        if alignment:
            ...
            if len(data) !=0:
                ...
                for idx, scaled_pair in tqdm(enumerate(scaled_data), desc='Aligning all rt_mz pairs', leave=False,
                                             colour='Green'):
                    if idx in visited:
                        continue
                ...
                for i in range(len(df1)):
                    ...
                    for ce in ce_num:
                        ...
                        if len(df_frag) == 0:
                            ...
                            df1.loc[i, f'MS2_spec_DDA_{ce}V'] = str(s_ms2.astype(int))
...
def swath_process(file, precursor_ion_start_mass=99.5, profile=True, peak_width=2,
    ...
    peak_all_ms1 = split_peak_picking(ms1, profile=profile, threshold=threshold, peak_width=peak_width,
                                      i_threshold=i_threshold, SN_threshold=SN_threshold, message=message,orbi = True)
...
def fingerprint_application(data, index_dict, names, target_name, target_place=None, fold_change=5, mz_range=[50, 800],
    ...
    data = pd.read_excel(r'data.xlsx',index_col = 'Unnamed: 0')
    index_dict = pd.read_excel(r'index_dict.xlsx')
