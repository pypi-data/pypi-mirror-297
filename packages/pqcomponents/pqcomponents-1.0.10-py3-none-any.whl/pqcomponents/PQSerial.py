...
class PQSerial(QWidget):
    ...
    def threadEventHandler_ReceiveEvent(self, szChar):
        ...
        if self.RxACKProcess(szChar, nLen):
            if szChar == COM_CR:  # 라인의 끝을 만나면.. 0x0D
                if self.ATMode:
                    nLen = len(self.szRxData)
    ...
    def RxACKProcess(self, szChar, nLen):
        if szChar == COM_ACK and nLen == 1:
            print("ACK Process")
            del self.szRxData[:]
            self.parent.RxACKDataProcess(self.szOldCmd)
            del self.szOldCmd[:]
            return False
        elif szChar == COM_NAK and nLen == 1:
            print("NAK Process")
            del self.szRxData[:]
    ...
    def RxACKNACKCheck(self):
        if self.szRxData[0] == COM_ACK:
            print("ACK Check")
            self.szRxData.pop(0)
            self.RxACKNACKCheck()
        elif self.szRxData[0] == COM_NAK:
            print("NAK Check")
            self.szRxData.pop(0)
            self.RxACKNACKCheck()

    def BoardSerialCheck(self, nLen):
        if nLen < LimitLen: return False
        elif self.szRxData[0] != COM_STX: return False
        elif self.szRxData[nLen - 4] != COM_ETX: return False
        elif self.szRxData[nLen - 1] != COM_CR: return False

        ckSum = []
        self.CheckSum(self.szRxData[1:], nLen - LimitLen, ckSum)
