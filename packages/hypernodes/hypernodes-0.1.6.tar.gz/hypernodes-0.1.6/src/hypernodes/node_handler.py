...
class NodeHandler:
    ...
    def save_dag(self, module: ModuleType, overwrite: bool = True) -> None:
        ...
        file_name = f"{self.node_info.name}_{module_name}.py"
        module_path = Path(self.node_info.folder) / file_name

        if not overwrite and module_path.exists():
            raise ValueError(
                f"DAG '{module_name}' already exists for node '{self.node_info.name}'. "
        ...
        logger.info(f'`{module_name}` is saved as a Hamilton DAG for "{self.node_info.name}"')
    ...
    def _update_hamilton_dags(self, module: ModuleType, module_path: str) -> None:
        ...
        self.registry.update_node(self.node_info.name, self.node_info)
    ...
    def save_hypster_config(
        ...
        """
        config_path = Path(self.node_info.folder) / f"{self.node_info.name}_hypster_config.py"

        if not overwrite and config_path.exists():
            raise ValueError(
                f"Hypster config already exists for node '{self.node_info.name}'. "
                "Use overwrite=True to replace it."
            )
        ...
        logger.info(
            f"`{hypster_config.name}` is saved as a hypster config for {self.node_info.name}"
        )
    ...
    def save(
        ...
        logger.info(
            ...
            f'{" and hypster config" if hypster_config else ""} to node "{self.node_info.name}"'
        )
        self.registry.update_node(self.node_info.name, self.node_info)
