...
DEFAULT_REGISTRY_PATH = os.path.expanduser("conf/node_registry.yaml")
DEFAULT_FOLDER_TEMPLATE = "src/nodes/{node_name}/modules"
logger = logging.getLogger(__name__)

# Schema definition
# TODO: consider finding an elegant solution without "schema"
REGISTRY_SCHEMA = Schema(
    {
        str: {
            "folder": str,
            SchemaOptional("hamilton_dag_paths"): [str],
            SchemaOptional("hypster_config_path"): str,
            SchemaOptional("builder_param_name"): str,
        }
    }
)
...
class StoreHandler:
    """
    Handles loading and saving node data to/from a YAML file.
    """

    def __init__(self, file_path: str):
        """
        Initialize the StoreHandler.

        Args:
            file_path (str): Path to the YAML file for storing node data.
        """
    ...
    def load(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Dictionary of node data, validated against REGISTRY_SCHEMA.
        """
        try:
            with open(self.file_path, "r") as f:
                data = yaml.safe_load(f) or {}
                return REGISTRY_SCHEMA.validate(data)
        except FileNotFoundError:
            return {}

    def save(self, data: Dict[str, Any]) -> None:
        """
        Save node data to the YAML file.

        Args:
            data (Dict[str, Any]): Dictionary of node data to save.
        """
