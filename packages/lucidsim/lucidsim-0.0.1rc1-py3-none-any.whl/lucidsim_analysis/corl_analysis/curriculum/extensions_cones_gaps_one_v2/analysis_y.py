...
with doc:
    ...
    ys = np.linspace(-0.25, 0.25, 20)

    # Placeholder for collecting success rates
    success_rates = []

    prefix = "lucidsim/lucidsim/corl_experiments/corl_analysis/curriculum/extensions_cones_gaps_one_v2/sweep_y"
    loader = ML_Logger(prefix=prefix)
    # Array to hold success rates
    success_rates = np.zeros((len(x), len(ys)))

    # Simulate collecting success rate for each combination of x and s
    ...
    plt.imshow(success_rates, extent=[-0.25, 0.25, 0, 4.5], origin="lower", aspect="auto", cmap="viridis")
