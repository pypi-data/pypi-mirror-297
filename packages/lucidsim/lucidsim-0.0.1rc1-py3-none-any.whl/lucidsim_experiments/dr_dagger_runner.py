...
class DRDaggerRunner(ParamsProto, prefix="dagger"):
    env_name = "Extensions-cones-hurdle_many-heightmap-dr_7-v1"
    dagger_env_name = "Extensions-cones-hurdle_many-dr_7_sampling-v1"

    runner_prefix = "/lucidsim/lucidsim/corl/baseline_datasets/debug/domain_randomization_7_v1/extensions_cones_hurdle_one"

    teacher_queue_name = Proto(env="$ZAKU_USER:lucidsim:dr-teacher-queue-1")
...
def create_initial_sweep(dagger_iter, prev_checkpoint=None):
    ...
    random.seed(DRDaggerRunner.seed)

    if dagger_iter == 0:
        with logger.Prefix(DRDaggerRunner.initial_checkpoints_prefix):
            checkpoint_paths = logger.glob("**/checkpoints/*.pt")

        checkpoint_paths = [f"{DRDaggerRunner.initial_checkpoints_prefix}/{path}" for path in checkpoint_paths] + [
            DRDaggerRunner.teacher_checkpoint
        ] * (len(checkpoint_paths))

        random.shuffle(checkpoint_paths)

        checkpoint_paths = checkpoint_paths[: DRDaggerRunner.traj_count]
    else:
        # from previous iter
        checkpoint_paths = [prev_checkpoint] * DRDaggerRunner.traj_count

    dataset_path = f"{DRDaggerRunner.runner_prefix}/datasets/dagger_{dagger_iter}"
    ...
    with Sweep(Unroll_s) as sweep:
        ...
        Unroll_s.env_name = DRDaggerRunner.env_name if dagger_iter == 0 else DRDaggerRunner.dagger_env_name
        Unroll_s.model_entrypoint = (
            "behavior_cloning.go1_model.transformers.transformer_policy:get_rgb_transformer_policy"
            if dagger_iter > 0
    ...
    return jobs[: 1 if DRDaggerRunner.debug else None], dataset_path
...
def main(_deps=None, **deps):
    ...
    dr_teacher_queue = TaskQ(name=DRDaggerRunner.teacher_queue_name)
    training_queue = TaskQ(name=DRDaggerRunner.training_queue_name)

    DRDaggerRunner._update(_deps, **deps)

    logger.job_started(dagger=vars(DRDaggerRunner))
    ...
    for dagger_iter in range(DRDaggerRunner.num_dagger_iters + 1):
        ...
        print("Adding", starting_len, "jobs to the dr-teacher queue.")

        is_done, tokens = dr_teacher_queue.gather(data_sweep, gather_tokens=None)

        while not is_done(blocking=False):
            dr_teacher_queue.unstale_tasks(10)
            print(f"{len(tokens)} jobs left.")
            time.sleep(10.0)

        print("Flow teacher done, waiting a bit for images to save...")
        time.sleep(20.0)

        # Pull images to local filesystem
        local_destination = Path(DRDaggerRunner.local_data_root) / Path(dataset_path).relative_to("/")
        local_destination.mkdir(parents=True, exist_ok=True)

        dataset_sync_command = [
            "rsync",
            "-avzhr",
            os.path.join(DRDaggerRunner.rsync_source_prefix, str(Path(dataset_path).relative_to("/"))),
        ...
        training_job = {
            ...
            "train.image_type": "rgb",
            "train.stack_size": DRDaggerRunner.stack_size,
            "train.use_batchnorm": False,
            "train.teacher_checkpoint": DRDaggerRunner.teacher_checkpoint,
            "train.student_checkpoint": current_student_checkpoint,
            "train.n_epochs": 70,
            "train.debug": DRDaggerRunner.debug,
...
if __name__ == "__main__":
    q = TaskQ(name="alanyu:lucidsim:dr-teacher-queue-1")
    q.clear_queue()
