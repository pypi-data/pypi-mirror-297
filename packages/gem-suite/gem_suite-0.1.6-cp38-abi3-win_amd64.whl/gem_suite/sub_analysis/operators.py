...
def analyze_individual_bonds(
    ...
    zxz_op_data = (
        data[data.name == "zxz"]
        .groupby(["theta", "component"], as_index=False)
        .value.agg(lambda v: ufloat(np.average(v), np.std(v)))
    )
    ...
    for bond, qubit in zip(bonds, bond_qubits):
        qk_components = [
            Qubit(qubit.index),
            Qubit(qubit.neighbors[0]),
            Qubit(qubit.neighbors[1]),
        ]
...
def analyze_clifford_limit(
    ...
    if len(clif_data) == 0:
        warnings.warn(
            "Experiment data at theta = 0.5 pi of the Clifford "
            "limit is not included in the result.",
            RuntimeWarning,
        )
        return []
