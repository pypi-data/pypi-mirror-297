...
class CxDecoratorDefaultHook(CxDecoratorHook):
  def _check(self, target: Any) -> bool:
    return True
...
def CxDecorator(hook_cls: Optional[Type[CxDecoratorHook]] = None, func_check: Optional[Callable] = None, func_target: Optional[Callable] = None):
  if isinstance(hook_cls, CxDecoratorHook):
    hook_cls = hook_cls.__class__
  elif callable(hook_cls):
    hook_cls = hook_cls

  if not (isinstance(hook_cls, type) and issubclass(hook_cls, CxDecoratorHook)) or hook_cls == CxDecoratorHook:
    hook_cls = CxDecoratorDefaultHook
