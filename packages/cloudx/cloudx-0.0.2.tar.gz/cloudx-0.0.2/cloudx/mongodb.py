...
class CxMongoClient(object):
  def __init__(
    ...
    if isinstance(host, str):
      if '://' in host:
        py_mongo_client = CxMongoClient.create_py_mongo_client_from_url(host)
      else:
        py_mongo_client = CxMongoClient.create_py_mongo_client(host, port, username, password)
    else:
...
class CxMongoModel(object):
  def __init__(self, mongo_client: CxMongoClient, collection_name: str, pk_name: str = '_id', ex_field_names: Optional[Dict] = None, raw_delete: bool = True):
    self._mongo_client = mongo_client
    self._collection_name = collection_name
    self._pk_name = pk_name
    self._ex_field_names = CxMongoModel._process_ex_field_names(ex_field_names)
    self._raw_delete = raw_delete

  def _make_pk_value(self, pk_value: Any) -> Any:
    if self._pk_name == '_id' and CxMongoClient.is_object_id(pk_value):
      pk_value = CxMongoClient.make_object_id(pk_value)
    return pk_value
  ...
  @property
  def mongo_client(self) -> CxMongoClient:
    return self._mongo_client
  ...
  def insert_item(self, item: Dict) -> Dict:
    ...
    if '_id' in item and CxMongoClient.is_object_id(item['_id']):
      item['_id'] = CxMongoClient.make_object_id(item['_id'])
