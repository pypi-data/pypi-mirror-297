...
def get_transmittance_center_of_modes_upper_v2(img_path, seg_path, kernel_size, binary_type = "brightness", model = "tcm", type = "q2"):
    ...
    if model == "tcm":
        ...
        new_shape = tuple(dim // downsizing for dim in array_binary.shape)
        array_transmission = np.zeros(new_shape)
        array_seg_binary = np.zeros(new_shape)
        ...
        half_height = array_transmission.shape[0] // 2  # Calculate half the width of the array

        for ii in range(half_height):
            for jj in range(array_transmission.shape[1]):  # Loop only up to half width
                i = int(downsizing * (ii + 0.5))
                j = int(downsizing * (jj + 0.5))
                if img_arr[i, j] == 1:
                    # Define boundaries for the neighborhood
                    i_min = max(0, i - kernel_size // 2)
                    i_max = min(half_height, i + kernel_size // 2 + 1)
                    ...
                    neighborhood = array_binary[i_min:i_max, j_min:j_max]
                    # Compute the mean of the neighborhood
                    array_transmission[ii, jj] = neighborhood.mean()

                    array_seg_binary[ii, jj] = array_binary[i, j]
                elif img_arr[i, j] == 2:
                    array_transmission[ii, jj] = 1
                    array_seg_binary[ii, jj] = 1
                else:
                    array_transmission[ii, jj] = 0
                    array_seg_binary[ii, jj] = 0
        ...
        new_shape = tuple(dim // downsizing for dim in array_2d.shape)
        array_transmission = np.zeros(new_shape)
        array_seg_binary = np.zeros(new_shape)
