...
# 2024.04.03 - Implement baichuanV2 model by using mindspore
...
"""Baichuan2 models' APIs."""

__all__ = ["BaichuanV2ForCausalLM", "BaichuanV2Model"]
...
from .configuration_baichuan_v2 import BaichuanV2Config
...
try:
    ...
    logger.info(f"{FlashAttention} is available")
except ImportError:
    FLASHATTENTION_VALID = False


class BaichuanV2PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = BaichuanV2Config
    base_model_prefix = "baichuan_v2"


class BaichuanV2Model(BaichuanV2PreTrainedModel):
    r"""
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaDecoderLayer`]
    Args:
        config(BaichuanV2Config): the config of network
    ...
    def __init__(self, config: BaichuanV2Config = None):
        ...
        self.casual_mask = CausalMaskForBaichuanV2(
            seq_length=config.seq_length,
        ...
        self.norm_out = LlamaRMSNorm(
            config.hidden_size, config.rms_norm_eps, compute_type=config.layernorm_compute_type
        )
...
class CausalMaskForBaichuanV2(nn.Cell):
...
class NormHead(nn.Cell):
    ...
    def __init__(self, hidden_size, vocab_size, compute_dtype=mstype.float16, eps=1e-5):
        super().__init__()
        self.weight = Parameter(
            initializer(HeUniform(negative_slope=math.sqrt(5)), [vocab_size, hidden_size], mstype.float16),
            name="weight",
            parallel_optimizer=True,
        )
    ...
    def construct(self, hidden_states):
        ...
        out = self.matmul(hidden_states.astype(self.compute_dtype), norm_weight.astype(self.compute_dtype))
    ...
    def shard(self, parallel_config):
        ...
        self.matmul.shard(((1, 1), (parallel_config.model_parallel * parallel_config.data_parallel, 1)))


class BaichuanV2ForCausalLM(BaichuanV2PreTrainedModel):
    r"""
    Provide baichuan2 training loss or logits through network.
    Args:
        config (BaichuanV2Config): The config of baichuan2 model.
    ...
    @cell_reuse
    def __init__(self, config: BaichuanV2Config = None):
        super(BaichuanV2ForCausalLM, self).__init__(config, auto_prefix=True)
        _check_config(config.parallel_config)
        self.config = config
        ...
        self.model = BaichuanV2Model(config=config)
        self.lm_head = NormHead(
            hidden_size=config.hidden_size, vocab_size=config.vocab_size, compute_dtype=config.compute_dtype
        )
    ...
    def prepare_inputs_for_export(self, full_model=True):
        ...
        if dyn:
            logger.info("Exporting dynamic MindIR...")
        ...
        if full_model:
            logger.info("exporting with batch_size = %s, seq = %s ...", self.config.batch_size, seq_length)
            input_ids = dummy_tensor(shape=[bs, seq_len], dtype=mstype.int32)
        else:
            logger.info("exporting with batch_size = %s, seq = 1 ...", self.config.batch_size)
            input_ids = dummy_tensor(shape=[bs, 1], dtype=mstype.int32)
        return_tuple = collections.namedtuple(
            "return_tuple",
            [
                "input_ids",
                "value1",
    ...
    def construct(
        ...
        """BaichuanV2 ForCausalLM forward."""
