...
class LLamaAttention(nn.Cell):
    ...
    def __init__(
        ...
        if self.n_kv_head % parallel_config.model_parallel != 0:
            error_msg = (
                "For 'MultiHeadAttention', the class variable 'n_kv_head' must be a multiple "
                "of 'parallel_config.model_parallel', but got the n_kv_head is {} and the "
        ...
        if self.use_prompt_flash_attention:
            self.prompt_flash_attention = P.nn_ops.PromptFlashAttention(
        ...
        if self.use_incre_flash_attention and self.use_past:
            self.incre_flash_attention = P.nn_ops.IncreFlashAttention(
                num_heads=self.n_head,
                num_key_value_heads=self.n_kv_head,
                scale_value=1.0 / math.sqrt(self.head_dim),
                input_layout="BNSD",
        ...
        if self.use_past:
            if self.use_paged_attention:
                self.kvcache_mgr = PagedAttentionMgr(
                    self.n_head,
                    self.head_dim,
                    self.hidden_size,
    ...
    def construct(self, x: Tensor, freqs_cis: Tuple[Tensor, Tensor], mask=None, kvcache_inputs=None):
        ...
        if self.is_first_iteration and self.use_prompt_flash_attention:  # PFA
            attention = self.prompt_flash_attention(
                query, key, value, mask, None, None, None, None, None, None, None, None
            )
            ...
            attention = self.incre_flash_attention(
                query, key, value, mask, None, None, None, None, None, None, None, None
            )
            attention = self._merge_heads(attention)
        else:  # SA
...
class LLamaDecodeLayer(nn.Cell):
    ...
    def __init__(
        ...
        self.attention_norm = LlamaRMSNorm(
            self.hidden_size, norm_eps, compute_type=layernorm_compute_dtype, is_dynamic=is_dynamic
        )
        ...
        if self.expert_num == 1:
            ...
            self.feed_forward = MoEV2(
                ffn=ffn, dim=self.hidden_size, moe_config=moe_config, parallel_config=parallel_config
            )
    ...
    def _check_input(self, x, freqs_cis, mask):
        ...
        if mask is not None:
            _check_input_dtype(
                mask.dtype, "input_mask", [mstype.float32, mstype.float16, mstype.bfloat16, mstype.uint8], self.cls_name
            )
        return True
