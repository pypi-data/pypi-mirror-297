...
from mindformers.modules.layers import Linear, _check_input_dtype, RotaryEmbedding
...
class _MicroBatch(nn.Cell):
    ...
    def construct(self, i, *inputs):
        ...
        for each_input in inputs:
            ...
            strided_slice_end = ()
            for j in range(input_shape_len):
            ...
            micro_input = self.strided_slice_list[k](
...
class LLamaAttentionInterleave(nn.Cell):
    ...
    def __init__(
        ...
        if self.hidden_size % self.n_head != 0:
            error_msg = (
                "For 'MultiHeadAttention', the class variable 'hidden_size' must be "
                "a multiple of 'n_head', but got the hidden_size is {} and the n_head is "
                "{}."
        ...
        if self.n_kv_head % parallel_config.model_parallel != 0:
            error_msg = (
                "For 'MultiHeadAttention', the class variable 'n_kv_head' must be a "
                "multiple of 'parallel_config.model_parallel', but got the n_kv_head is {} "
...
class LLamaDecodeLayerInterleave(nn.Cell):
    ...
    def __init__(
        ...
        self.attention = LLamaAttentionInterleave(
        ...
        recompute_condition_1 = isinstance(parallel_config.recompute, bool) and not parallel_config.recompute
        recompute_condition_2 = not parallel_config.recompute.recompute and self.layer_id < (num_layers // 2)
        if parallel_config.recompute.select_recompute or recompute_condition_1 or recompute_condition_2:
        ...
        concat_stra1_tuple = tuple(concat_stra1)
        concat_stra2_tuple = tuple(concat_stra2)
        self.interleaved_concat1.shard(concat_stra1_tuple)
        self.interleaved_concat1.add_prim_attr("skip_redistribution", True)
        self.interleaved_concat_1.shard(concat_stra1_tuple)
        self.interleaved_concat_1.add_prim_attr("skip_redistribution", True)
        self.interleaved_concat2.shard(concat_stra2_tuple)
    ...
    def _check_input(self, x, freqs_cis, mask):
        r"""Check inputs"""
        _check_input_dtype(x.dtype, "x", [mstype.float32, mstype.float16, mstype.bfloat16], self.cls_name)
        freqs_cos, freqs_sin, swap_mask = freqs_cis
        _check_input_dtype(
            freqs_cos.dtype, "freqs_cos", [mstype.float32, mstype.float16, mstype.bfloat16], self.cls_name
        )
        _check_input_dtype(
            freqs_sin.dtype, "freqs_sin", [mstype.float32, mstype.float16, mstype.bfloat16], self.cls_name
        )
        if swap_mask is not None:
            _check_input_dtype(
                swap_mask.dtype, "swap_mask", [mstype.float32, mstype.float16, mstype.bfloat16], self.cls_name
            )
        if mask is not None:
            _check_input_dtype(
                mask.dtype, "input_mask", [mstype.float32, mstype.float16, mstype.uint8, mstype.bfloat16], self.cls_name
            )
        return True
