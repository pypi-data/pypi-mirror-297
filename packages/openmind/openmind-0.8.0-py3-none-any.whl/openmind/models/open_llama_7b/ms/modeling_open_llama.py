...
from .interleave_open_llama import LLamaDecodeLayerInterleave
from .layer_open_llama import LlamaEmbedding, LlamaRMSNorm
from .transformer_open_llama import LLamaDecodeLayer
...
try:
    ...
    logger.info(f"{PromptFlashAttention} is available")
...
try:
    ...
    logger.info(f"{IncreFlashAttention} is available")
...
def layer_compute_dtype(layer, layer_id, offset, parallel_config, n_layers, select_recompute=False):
    ...
    if isinstance(offset, list):
        if len(offset) != parallel_config.pipeline_stage:
            error_msg = (
                f"The length of `offset` {len(offset)} do not match `pipeline stage` "
                f"{parallel_config.pipeline_stage}."
            )
        ...
        error_msg = f"`offset` must be `int` of list of `int`, but got {type(offset)}."
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig = None):
        ...
        for layer_id in range(config.num_layers):
            if config.fine_grain_interleave > 1 and config.parallel_config.model_parallel > 1:
                layer = LLamaDecodeLayerInterleave(
                    config.batch_size,
                    config.seq_length,
                    layer_id,
    ...
    def construct(
        ...
        if not self.use_past:
            ...
            kvcache_inputs = self.kvcache_preprocess(
                bs, batch_valid_length, batch_index, zactivate_len, block_tables, slot_mapping
            )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig = None):
        ...
        if vocab_size % mp != 0:
            logger.warning("The vocab size of Loss is: %s, it is not divide by model_parallel: %s", vocab_size, mp)
    ...
    def prepare_inputs_for_export(self, full_model=True):
        ...
        if use_paged_attention:
            logger.info("Exporting model with paged attention...")
        ...
        if full_model:
            ...
            block_tables = (
                dummy_tensor(shape=[bs, max_num_blocks_pre_batch], dtype=ms.int32) if use_paged_attention else None
            )
