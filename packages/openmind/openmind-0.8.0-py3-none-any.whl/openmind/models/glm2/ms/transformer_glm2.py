...
from .modules_glm2 import ChatGLM2MLP, ChatGLM2RMSNorm
...
class CoreAttention(nn.Cell):
    ...
    def __init__(self, config: ChatGLM2Config, layer_number):
        ...
        if self.multi_query_attention:
            ...
            self.qkv_hidden_size = projection_size + 2 * self.head_dim * config.multi_query_group_num
    ...
    def construct(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        if attention_mask is None and attention_scores.shape[2] == attention_scores.shape[3]:
            attention_mask = ops.ones(
                (attention_scores.shape[0], 1, attention_scores.shape[2], attention_scores.shape[3]), dtype=mstype.bool_
            )
            attention_mask.tril()
...
class ChatGLM2SelfAttention(nn.Cell):
        ...
        if self.use_past:
            ...
            if self.use_flash_attention:
                self.flash_attention = FlashAttention(
                    ...
                    scale_value=1.0 / math.sqrt(self.head_dim),
                    input_layout="BNSD",
                    keep_prob=1.0 - config.attention_dropout,
                    pre_tokens=65536,
                    next_tokens=0,
                )
    ...
    def add_prefix_if_need(self, prefix_key_value, key_layer, value_layer, attention_mask):
        ...
        key_layer, value_layer = Ptuning2Adapter.add_prefix(prefix_key_value, key_layer, value_layer)
    ...
    def construct(
        ...
        if self.use_past:
            context_layer = self.infer_attention(
                query, key, value, batch_valid_length, block_tables, slot_mapping, rotary_pos_emb, attention_mask
            )
        else:
            ...
            key, value, attention_mask = self.add_prefix_if_need(prefix_key_value, key, value, attention_mask)
...
class ChatGLM2Block(nn.Cell):
    ...
    def __init__(self, config: ChatGLM2Config, layer_number: int):
        ...
        self.input_layernorm = layer_norm_func(
            config.hidden_size, eps=config.layernorm_epsilon, param_init_type=self.layernorm_dtype
        )
    ...
    def __init__(self, config: ChatGLM2Config):
        ...
        if self.post_layer_norm:
            ...
            self.final_layernorm = layer_norm_func(
                config.hidden_size, eps=config.layernorm_epsilon, param_init_type=config.layernorm_compute_type
            )
