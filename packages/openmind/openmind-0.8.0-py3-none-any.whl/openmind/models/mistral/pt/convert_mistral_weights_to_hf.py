...
# 2024.06.13 - The hard-coded public addresses are put into the configuration file.
#              Huawei Technologies Co., Ltd.
# 2024.06.17 - Delete irrelevant comments.
#              Huawei Technologies Co., Ltd.
...
try:
    from ...llama.pt import LlamaTokenizerFast
...
from ....utils.constants import DEFAULT_FLAGS, DEFAULT_MODES
...
def write_json(text, path):
    with os.fdopen(open(path, DEFAULT_FLAGS, DEFAULT_MODES), "w") as f:
        json.dump(text, f)
...
def write_model(model_path, input_base_path, model_size, tokenizer_path=None, safe_serialization=True):
    ...
    error_msg = f"Fetching all parameters from the checkpoint at {input_base_path}."
    logger.info(replace_invalid_characters(error_msg))
    # Load weights
    loaded = [
    ...
    logger.info("Loading the checkpoint in a Mistral model.")
...
def write_tokenizer(tokenizer_path, input_tokenizer_path):
    ...
    error_msg = f"Saving a {tokenizer_class.__name__} to {tokenizer_path}."
    logger.info(replace_invalid_characters(error_msg))
...
def main():
    ...
    parser.add_argument(
        ...
        help="'f' models correspond to the finetuned versions, and are specific to the Mistral2 official release. For more details on Mistral2, checkout the original repo: {MISTRAL_REPO_URL}",
    )
    parser.add_argument(
        "--output_dir",
