...
# 2024.04.02 - Change relative path to absolute path and adapt to npu
#              Huawei Technologies Co., Ltd.
# 2024.05.07 - Add kwargs for  LlamaSdpaAttention
#              Huawei Technologies Co., Ltd.
# 2024.05.09 - Rename slice to slice_value
#              Huawei Technologies Co., Ltd.
# 2024.05.27 - The hard-coded public addresses are put into the configuration file
#              Huawei Technologies Co., Ltd.
# 2024.08.05 - Remove example for LlamaForCausalLM
#              Huawei Technologies Co., Ltd.
...
from ....utils.constants import PT_NN_MODULE_URL, BART_PAPER_URL
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            slice_value = self.intermediate_size // self.config.pretraining_tp
            gate_proj_slices = self.gate_proj.weight.split(slice_value, dim=0)
            up_proj_slices = self.up_proj.weight.split(slice_value, dim=0)
            down_proj_slices = self.down_proj.weight.split(slice_value, dim=1)

            gate_proj = torch.cat([F.linear(x, gate_proj_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)
            up_proj = torch.cat([F.linear(x, up_proj_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)

            intermediate_states = (self.act_fn(gate_proj) * up_proj).split(slice_value, dim=2)
            down_proj = [
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if self.layer_idx is None:
                error_msg = (
                    ...
                    f"If you are using {self.__class__.__name__} for auto-regressive decoding with k/v "
                    f"caching, please make sure to initialize the attention class with a layer index."
                )
                raise ValueError(replace_invalid_characters(error_msg))
        ...
        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
                error_msg = (
                    f"Attention mask should be of size {(bsz, 1, q_len, kv_seq_len)}, but "
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                error_msg = f"Unknown RoPE scaling type {scaling_type}"
                raise ValueError(replace_invalid_characters(error_msg))
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # NOTE: Should be removed once Flash Attention for RoCm is bumped to 2.1.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # NOTE: These transpose are quite inefficient but Flash Attention requires the layout
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # NOTE: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            # NOTE: Improve this warning with e.g. `model.config.attn_implementation = "manual"`
...
LLAMA_START_DOCSTRING = rf"""
    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)

    This model is also a PyTorch [torch.nn.Module]({PT_NN_MODULE_URL}) subclass.
...
LLAMA_INPUTS_DOCSTRING = rf"""
    Args:
        ...
        attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            and modify to your needs. See diagram 1 in [the paper]({BART_PAPER_URL}) for more
