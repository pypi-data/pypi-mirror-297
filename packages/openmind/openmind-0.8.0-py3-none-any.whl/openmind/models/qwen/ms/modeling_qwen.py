...
class QwenModel(QwenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # 5. ln_f
        self.ln_f = LlamaRMSNorm(self.embed_dim, eps=config.rms_norm_eps, compute_type=config.layernorm_compute_type)
...
class QwenDecodeLayer(LLamaDecodeLayer):
    ...
    def __init__(self, batch_size, seq_length, layer_id, **kwargs):
...
class QwenFeedForward(nn.Cell):
    ...
    def shard(self, parallel_config):
        ...
        if self.hidden_dim % mp != 0:
            raise ValueError(
                "For 'FeedForward', the class variable 'hidden_dim' must be a multiple of the"
        ...
        if self.dim % mp != 0:
            raise ValueError(
                ...
                "model parallel, but got the dim is {} and the num of model parallel is {}.".format(self.dim, mp)
            )
