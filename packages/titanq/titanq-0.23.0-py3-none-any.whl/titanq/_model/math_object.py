...
class MathObject(ABC):
    ...
    def __lt__(self, other):
        return Equation(self, ConstraintType.LESSER, other)

    def __le__(self, other):
        return Equation(self, ConstraintType.LESSER_EQUAL, other)

    def __gt__(self, other):
        return Equation(self, ConstraintType.GREATER, other)
...
class Term(MathObject):
    ...
    def _populate_weights(self, weights):
        if self.degree() == 2:
            ...
            # make the matrix symmetric if not on the diagonal
            if index1 != index2:
...
class Equation():
    ...
    def generate_constraint(self, variable_list: List[Variable], inequality_tolerance: float) -> Tuple[npt.NDArray, npt.NDArray]:
        """
        ...
        inequality_tolerance
            The epsilon to add to the bound in the case of strictly grater of strictly lower comparison
        """
