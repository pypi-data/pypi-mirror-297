...
def loss_likelihood(x, pi, mu1, alpha, theta1, theta2):
	 ...
	 comp1_t1 = torch.lgamma(x+theta1+eta) - torch.lgamma(x+1+eta) - torch.lgamma(theta1+eta)
	 comp1_t2 = theta1 * torch.log(theta1/(theta1+mu1)+eta) + x * torch.log(mu1/(theta1+mu1)+eta)
	 NB1 = torch.exp(comp1_t1+comp1_t2)
	 
	 comp2_t1 = torch.lgamma(x+theta2+eta) - torch.lgamma(x+1+eta) - torch.lgamma(theta2+eta)
	 comp2_t2 = theta2 * torch.log(theta2/(theta2+mu2)+eta) + x * torch.log(mu2/(theta2+mu2)+eta)
...
def TotalLoss(x, pi, mu1, alpha, theta1, theta2, means, logvars, 
	 kld_weight=0.1, recon_weight=1, penalty_alpha=0.03):
