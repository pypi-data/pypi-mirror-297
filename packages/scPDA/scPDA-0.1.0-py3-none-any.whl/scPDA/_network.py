...
class VAE(nn.Module):
	def __init__(self, input_dim, layer1=100, layer2=50, n_hidden=15,
				 alpha_init=None, theta1_init=None, theta2_init=None):
		super().__init__()
		...
		# Global Parameters
		self.alpha = nn.Parameter(alpha_init.clone().detach() if alpha_init is not None else torch.ones(input_dim)*2)
		self.theta1 = nn.Parameter(theta1_init.clone().detach() if theta1_init is not None else torch.ones(input_dim))
		self.theta2 = nn.Parameter(theta2_init.clone().detach() if theta2_init is not None else torch.ones(input_dim))
	...
	def forward(self, x):
		...
		alpha = F.softplus(self.alpha)+1. # alpha > 1
