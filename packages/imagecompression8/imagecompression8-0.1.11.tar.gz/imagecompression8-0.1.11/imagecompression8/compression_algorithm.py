...
## decompress
def decompress_image_with_svd(compressed_image_path, block_size=64, rank=3):
    """
    使用SVD對壓縮的圖像進行恢復。

    :param compressed_image_path: 壓縮圖像的路徑
    :param block_size: 圖像分塊大小
    :param rank: SVD保留的奇異值數量
    :return: 恢復的圖像 (PIL Image)
    """
    compressed_image = Image.open(compressed_image_path)
    compressed_array = np.array(compressed_image)
    
    h, w = compressed_array.shape[:2]
    recovered_image = np.zeros_like(compressed_array)

    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            block = compressed_array[i:i+block_size, j:j+block_size]
            if block.shape[0] < block_size or block.shape[1] < block_size:
                continue
            U, S, Vt = svd(block, full_matrices=False)
            if rank is not None:
                ...
                S = S[:rank]
                Vt = Vt[:rank, :]
            recovered_block = np.dot(U, np.dot(np.diag(S), Vt))
            recovered_image[i:i+block_size, j:j+block_size] = recovered_block
    
    return Image.fromarray(recovered_image.astype(np.uint8))
