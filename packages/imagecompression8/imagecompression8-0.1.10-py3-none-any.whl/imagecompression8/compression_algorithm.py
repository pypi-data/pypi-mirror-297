...
## decompress
def decompress_image_with_svd(compressed_image, block_size=32, rank=None):
    """
    使用SVD對壓縮圖像進行解壓縮，恢復到接近原始圖像的狀態。

    :param compressed_image: 壓縮的PIL圖像對象
    :param block_size: 圖像分塊的大小
    :param rank: 在壓縮過程中保留的奇異值數量
    :return: 解壓縮後的PIL圖像對象
    """
    compressed_image_array = np.array(compressed_image)
    h, w = compressed_image_array.shape[:2]
    decompressed_image = np.zeros_like(compressed_image_array)
    ...
    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            ...
            decompressed_image[i:i+block_size, j:j+block_size] = decompressed_block

    # 將解壓縮結果轉換回PIL圖像對象
    return Image.fromarray(decompressed_image.astype(np.uint8))