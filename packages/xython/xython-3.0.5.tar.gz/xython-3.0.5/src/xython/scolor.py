...
class scolor:
	...
	def change_excel56_no_to_color_name(self, input_excel56_no):
		result = self.change_excel56_no_to_color_name_kor(input_excel56_no)
		return result

	def change_excel56_no_to_color_name_kor(self, input_excel56_no):
		"""
		엑셀 56색의 번호 -> 한글 색이름
		...
		:return:
		"""
		result = self.vars["excel56_vs_color_kor"][int(input_excel56_no)]
		return result

	def change_hsl_as_bright_level(self, hsl, strong_level=50):
		"""
	...
	def change_hsl_as_dark_level1(self, hsl, strong_level=0.5):
		"""
	...
	def change_hsl_as_gray_level1(self, hsl, strong_level=0.5):
		"""
		입력받은 hsl값을 어두운 쪽으로 이동시키는것
	...
	def change_hsl_as_high_bright(self, hsl):
		"""
		입력된 hsl값 -> 고명도로 바꾸는 것
		"""
		hsl[2] = 80
	...
	def change_hsl_as_high_color(self, hsl):
		"""
		입력된 hsl값 -> 고채도로 바꾸는 것
		"""
		hsl[1] = 80
	...
	def change_hsl_as_pastel_level1(self, hsl, strong_level=0.5):
		"""
	...
	def change_hsl_as_vivid_level1(self, hsl, strong_level=0.5):
		"""
		level1 : 0~1사이의값
		입력받은 hsl값을 어두운 쪽으로 이동시키는것
		bright = [100,100], sharp = [50,100], graish = [100,0], dark = [0,0], black = [50, 0]
	...
	def change_hsl_to_10_rgb_set_step_by_s(self, hsl, step=10):
		"""
	...
	def change_hsl_to_2_near_bo_by_h_level100(self, hsl, h_step=36):
		"""
	...
	def change_hsl_to_2_side_color_by_h_level100(self, hsl, h_step=36):
		"""
		level100 : -100 ~ 100사이의 값
		근접색조합 : 양쪽 근처색
	...
	def change_hsl_to_2_side_color_by_l_level100(self, hsl, l_step=30):
		"""
		level100 : -100 ~ 100사이의 값
		명도차가 큰 2가지 1가지색
	...
	def change_hsl_to_2_side_color_by_s_level100(self, hsl, s_step=30):
		"""
		level100 : -100 ~ 100사이의 값
		채도차가 큰 2가지 1가지색
	...
	def change_hsl_to_2near_bo_hsl(self, hsl, h_step=36):
		"""
		근접보색조합 : 보색의 근처색
		분열보색조합 : Split Complementary
	...
	def change_hsl_to_2near_bo_rgb(self, hsl, h_step=36):
		"""
		근접보색조합 : 보색의 양쪽 근처색
		분열보색조합 : Split Complementary
		근접보색조합 : 보색의 강한 인상이 부담스러울때 보색의 근처에 있는 색을 사용
	...
	def change_hsl_to_2near_rgb(self, hsl, h_step=36):
		"""
	...
	def change_hsl_to_3_hsl_set_step_by_big_l(self, hsl, l_step=30):
		"""
		명도차가 큰 2가지 1가지색
	...
	def change_hsl_to_3_hsl_set_step_by_big_s(self, hsl, s_step=30):
		"""
		채도차가 큰 2가지 1가지색
	...
	def change_hsl_to_3_rgb_set_as_0_120_240(self, hsl):
		result = self.change_hsl_to_3_rgb_set_step_by_h_120_degree(hsl)
		return result

	def change_hsl_to_3_rgb_set_step_by_h_120_degree(self, hsl):
		"""
		등간격 3색조합 : triad
		활동적인 인상과 이미지를 보인다
	...
	def change_hsl_to_4_tetra_style_rgb(self, hsl):
		"""
	...
	def change_hsl_to_bo_rgb(self, hsl):
		"""
	...
	def change_rgb_to_close_excel56_no(self, input_rgb):
		"""
	...
	def change_scolor_to_close_excel56_no(self, input_scolor):
		"""
		...
		result = self.change_rgb_to_close_excel56_no(rgb_value)
		return result
	...
	def change_scolor_to_hsl(self, input_scolor):
		...
		if type(input_scolor) == type([]): #리스트형태 일때는 rgb로 인식
			...
			[value_only, color_name, l_value] = self.check_input_scolor(input_scolor)
	...
	def check_color_name_for_input_color(self, input_color):
		...
		"""
		result = False
		result = self.color.check_color_name(input_color)
		return result
	...
	def control_hsl_by_plusminus100(self, hsl, step_no):
		"""
		hsl값을 명도를 조정하는 방법
		+，-로 조정을 하는것이다
	...
	def control_hsl_for_sl_by_plusminus100(self, hsl, s_step="++", l_step="++"):
		"""
	...
	def control_hsl_to_high_bright(self, hsl):
		"""
		입력된 hsl값 -> 중명도로 바꾸는 것
		"""
		hsl[2] = 90
	...
	def control_hsl_to_high_color_contrast(self, hsl):
		"""
		입력된 hsl값 -> 중채도로 바꾸는 것
		"""
		hsl[0] = 100
	...
	def control_hsl_to_low_bright(self, hsl):
		"""
	...
	def control_hsl_to_low_color_contrast(self, hsl):
		"""
		입력된 hsl값 -> 저채도로 바꾸는 것
		"""
		hsl[0] = 20
	...
	def control_hsl_to_middle_bright(self, hsl):
		"""
		입력된 hsl값 -> 중명도로 바꾸는 것
		"""
		hsl[2] = 50
	...
	def control_hsl_to_middle_color_contrast(self, hsl):
		"""
		입력된 hsl값 -> 중채도로 바꾸는 것
		"""
		hsl[0] = 50
	...
	def get_color_name_for_rgbint(self, rgbint):
		...
		"""
		try:
			rgblist = self.color.change_rgbint_to_rgb(rgbint)
			color_index = self.change_56color_no_to_rgb(rgblist)
			colorname = self.change_color_name_to_color_index(color_index)
		except:
			colorname = None
		return colorname
	...
	def get_nth_hsl_set_for_hsl_control_by_h(self, hsl, input_nth=10):
		result = []
	...
	def get_nth_hsl_set_for_hsl_control_by_l(self, hsl, input_nth=10):
		result = []
	...
	def get_nth_hsl_set_for_hsl_control_by_s(self, hsl, input_nth=10):
		result = []
		for no in range(1, 101, input_nth):
			temp = [hsl[0], no, hsl[2]]
			result.append(temp)
	...
	def get_nth_rgb_set_for_hsl_control_by_h(self, hsl, input_nth=10):
		result = []
		for no in range(1, 361, input_nth):
			temp = self.change_hsl_to_rgb([no, hsl[1], hsl[2]])
			result.append(temp)
		return result

	def get_nth_rgb_set_for_hsl_control_by_l(self, hsl, input_nth=10):
		result = []
		for no in range(1, 101, input_nth):
			temp = self.change_hsl_to_rgb([hsl[0], hsl[1], no])
			result.append(temp)
		return result

	def get_nth_rgb_set_for_hsl_control_by_s(self, hsl, input_nth=10):
		result = []
		for no in range(1, 101, input_nth):
			temp = self.change_hsl_to_rgb([hsl[0], no, hsl[2]])
			result.append(temp)
		return result
	...
	def is_scolor_style(self, input_scolor):
		...
		result1 = self.jf.is_match_all("[한글&영어:2~10][숫자:0~7]", str(input_scolor))
		result2 = self.jf.is_match_all("[한글&영어:2~10][+-:0~7]", str(input_scolor))
