...
class han:
	def ___terms___(self):
		...
		parameter_id : 각 action에 연결된 parameter의 번호, parameter id와 같은 개념
	...
	def __init__(self, file_name=""):
		...
		if file_name == "":
			if self.get_doc_obj_for_active_doc():
				pass
			else:
	...
	def count_char_in_doc(self):
		...
		result = parameter_set_obj.Item("DetailCharCount")
		return result

	def count_char_in_selection(self):
		"""
		선택영역 전체의 글자갯수 (공백도 1개임)
		"""
	...
	def count_page_in_doc(self):
		...
		result = parameter_set_obj.Item("DetailPageCount")
		return result
	...
	def get_all_parameter_data_for_action_name(self, input_action_name):
		...
		result["parameter_id"] = self.get_parameter_id_for_action_name(input_action_name)
		result["parameter_items"] = None
		result["parameter_option"] = {}
		if result["parameter_id"]:
			result["parameter_items"] = self.get_all_parameter_item_by_parameter_id(result["parameter_id"])
	...
	def get_all_parameter_item_by_parameter_id(self, input_parameter_set_id):
		"""
		parameter_id : 각 action에 연결된 parameter의 번호, parameter id와 같은 개념
		parameter_item_set : all parameter element, parameter id의 모든 파라미타 들
		parameter_item_option_set : 한개의 parameter_item 의 option 사항들
	...
	def get_doc_obj_for_active_doc(self):
		"""
	...
	def get_parameter_id_for_action_name(self, input_action):
		"""
	...
	def get_test(self):
		lpp = self.han_program.GetPosBySet()
		print(lpp["List"], lpp["Para"], lpp["Pos"])
	...
	def search_action(self, input_action):
		# all_data = basic_data_for_han.basic_data().vars
		temp = self.check_action_name(input_action)
		for index, a_action in enumerate(temp):
			parameter_set_id = self.get_parameter_id_by_action_name(a_action)
			if parameter_set_id == "없음":
				print("                  Parameter들은 ==> ", "없음")
			else:
				parameters = self.get_parameter_set_by_parameter_name(parameter_set_id)
				print("                  Parameter들은 ==> ", parameters)
				for a_parameter in parameters:
					print("                                 Parameter는 ==> ", a_parameter)
					option = self.get_parameter_option_for_paramater(a_parameter)
