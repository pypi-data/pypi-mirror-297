...
import pynal, jfinder  # xython 모듈
...
class mailmail:
	...
	def filter_mail_set_as_body_by_jf_sql(self, input_mail_set, jf_sql):
		...
		for index, one_mail in enumerate(mail_l1d):
			if self.jf.search_all_with_jf_sql(jf_sql, one_mail.Body):
	...
	def get_mail_object_between_two_date_in_input_folder(self, input_folder, start_date, end_date,  sort_by="") :
		dt_obj_from =self.nal.change_any_time_to_dt_obj(start_date)
		dt_obj_to =self.nal.change_any_time_to_dt_obj(end_date)
	...
	def get_mail_set_in_folder_between_date(self, input_folder, dt_obj_from, dt_obj_to):
		...
		:return:
		"""
		dt_obj_from = self.nal.change_any_time_to_dt_obj(dt_obj_from)
		dt_obj_to = self.nal.change_any_time_to_dt_obj(dt_obj_to)
	...
	def get_mail_set_in_folder_from_today_to_day_no(self, input_folder, input_day_no):
		...
		result = []
		dt_obj_to = self.nal.get_dt_obj_for_today()
		ymd_list_today = self.nal.get_ymd_list_for_dt_obj(dt_obj_to)
		mails = input_folder.Items
		mails.Sort("ReceivedTime", True)
		for one_mail in mails:
			dt_obj_rt = self.nal.change_any_text_time_to_dt_obj(one_mail.ReceivedTime)
			ymd_list_rt = self.nal.get_ymd_list_for_dt_obj(dt_obj_rt)
	...
	def get_mail_set_in_folder_on_yyyymmdd(self, input_folder, input_yyyy_mm_dd):
		...
		folder_object = self.check_any_folder(input_folder)
		dt_obj_to = self.nal.change_any_time_to_dt_obj(input_yyyy_mm_dd)
	...
	def print_mail_set_for_basic_datas_with_easy_format(self, input_mail_set):
		...
		try:
			for one_mail in input_mail_set:
				...
				temp.append(self.nal.change_dt_obj_to_format_time_as_input_format(one_mail.ReceivedTime))
	...
	def zzz_sample_001(self):
		"""
		받은편지함의 오늘날짜의 메일들에서,
		첨부자료가 있는것들을 특정 폴더안에 앞부분에 이름을 덧붙여서 저장하는 것이다
		"""
		yyyy_mm_dd = self.nal.get_today_as_yyyy_mm_dd_style()
	...
	def zzz_test_check_outlook_email_01(self):
		"""
		테스트를 위한것
