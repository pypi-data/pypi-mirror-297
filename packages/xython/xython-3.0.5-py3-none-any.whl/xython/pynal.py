# -*- coding: utf-8 -*-
import re, time, calendar, arrow  # 내장모듈
import datetime
...
class pynal():
	...
	def change_any_text_time_to_dt_obj(self, input_text_time):
		...
		resql_result = self.jf.search_all_by_jf_sql("[+-:1~1][숫자:2~2]:[숫자:2~2]", dt_string)
		if resql_result:
		...
		resql_result = self.jf.search_all_by_jf_sql(resql, dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.jf.search_all_by_jf_sql("[숫자:2~2]:[숫자:2~2]:[숫자:2~2]", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		if self.jf.search_all_by_jf_sql("[숫자:4~4]-[숫자:1~2]-[숫자:1~2]", dt_string):
			resql_result = self.jf.search_all_by_jf_sql("[숫자:4~4]-[숫자:1~2]-[숫자:1~2]", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.jf.search_all_by_jf_sql("[숫자:1~2]-[숫자:1~2]-[숫자:1~4]", dt_string)
		if resql_result:
			temp = resql_result[0][0].split("-")
			result["yea"] = temp[2]
			result["mon"] = temp[1]
		...
		if old_dt_string != "":
			...
			dt_string = dt_string.strip()
			old_dt_string = dt_string
			resql_result = self.jf.search_all_by_jf_sql("(20|19)[숫자:6~6]", dt_string)
		if resql_result:
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.jf.search_all_by_jf_sql("[숫자:6~6][공백:1~1][숫자:6~6]", dt_string)
		if resql_result:
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.jf.search_all_by_jf_sql("([영어:3~10])[공백&,.:0~3]([숫자:1~2])[공백&,.:1~3]([숫자:1~4])", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.jf.search_all_by_jf_sql("[숫자:1~2][영어:0~3][공백&,:1~3][영어:3~9][공백&,:0~3][숫자:4~4]", dt_string)
		if resql_result:
			found_text = resql_result[0][0]

			bbb = self.jf.search_all_by_jf_sql("[영어:3~9]", found_text)
			for num in self.vars["dic_no_mon"].keys():
				if bbb[0][0] in self.vars["dic_no_mon"][num]:
					result["mon"] = num
			found_text = found_text.replace(bbb[0][0], "")

			ccc = self.jf.search_all_by_jf_sql("[숫자:4~4]", found_text)
			result["yea"] = ccc[0][0]
			found_text = found_text.replace(ccc[0][0], "")

			ddd = self.jf.search_all_by_jf_sql("[숫자:1~2]", found_text)
			result["day"] = ddd[0][0]

			dt_string = dt_string.replace(resql_result[0][0], "")
			dt_string = dt_string.strip()

		resql_result = self.jf.search_all_by_jf_sql("\.[숫자:6~6]", dt_string)
		if resql_result:
			dt_string = dt_string.replace(resql_result[0][0], "")
			# .586525
			# 초단위 이하의 자료
		...
		resql_result = self.jf.search_all_by_jf_sql("[숫자:4~4]", dt_string)
		if resql_result:
			# print(resql_result)
		...
		resql_result = self.jf.search_all_by_jf_sql("[숫자:2~2]", dt_string)
		if resql_result:
			result["day"] = resql_result[0][0]
			dt_string = dt_string.replace(resql_result[0][0], "")
			dt_string = dt_string.strip()

		resql_result = self.jf.search_all_by_jf_sql("pm[또는]am", dt_string)
		if resql_result:
			# print(resql_result)
	...
	def change_any_time_to_formatted_text_time(self, input_text_time, input_time_format='%Y-%m-%d'):
		"""
		입력시간을 입력된 형태로 바꾸는 것
	...
	def change_any_time_to_ymd_list(self, any_time_style):
		"""
		...
		:return:
		"""
		dt_obj = self.check_input_time(any_time_style)
		result = self.get_ymd_list_for_dt_obj(dt_obj)
		return result

	def change_anytime_to_utc_timestamp(self, input_time=""):
		"""
		...
		if input_time == "":  # 만약 아무것도 넣지 않으면 localtime을 돌려준다
			...
			result = self.change_ymd_list_to_utc_timestamp(input_time)
			...
			result = self.change_ymd_list_to_utc_timestamp(ymd_list)
		else:
	...
	def change_dic_time_to_dt_obj(self, input_dic):
		...
		text_time = str(temp["yea"]) + "-" + str(temp["mon"]) + "-" + str(temp["day"]) + "-" + str(temp["hou"]) + "-" + str(temp["min"]) + "-" + str(temp["sec"])
	...
	def change_dt_obj_to_dic_time(self, input_dt_obj):
		...
		result = self.get_all_data_for_dt_obj(input_dt_obj)
		return result

	def change_dt_obj_to_format_time_as_input_format(self, input_dt_obj, input_format="%Y-%m-%d %H:%M:%S"):
		"""
	...
	def change_dt_obj_to_sec(self, input_dt_obj):
		...
		"""
		result = self.get_all_data_for_dt_obj(input_dt_obj)["sec"]
		return result
	...
	def change_dt_obj_to_total_day(self, input_dt_obj):
		...
		시간 객체를 날수로 계산
		"""
		result = self.get_all_data_for_dt_obj(input_dt_obj)["day"]
		return result
	...
	def change_dt_obj_to_utc_timestamp(self, input_dt_obj):
		"""
		dt_obj를 utc로 변경후에 timestamp로 만드는 것
		datetime는 utc local
	...
	def change_dt_obj_to_utc_timestamp_1(self, input_dt_obj):
		"""
		timestamp : utc기준으로 1970년 1월1일부터 1초마다 숫자를 더해서 사용
		"""
		result = self.get_all_data_for_dt_obj(input_dt_obj)["utc_timestamp"]
		return result
	...
	def change_dt_obj_to_ymd_list(self, input_dt_obj):
		...
		datetime객체 => [년, 월, 일]
		"""
		result = self.get_all_data_for_dt_obj(input_dt_obj)["ymd_list"]
		return result


	def change_dt_obj_to_yyyy_mm_dd(self, input_dt_obj):
		temp = self.get_all_data_for_dt_obj(input_dt_obj)["ymd_list"]
	...
	def change_dt_obj_to_ymdhms_list(self, input_dt_obj):
		...
		:return:
		"""
		result = self.get_all_data_for_dt_obj(input_dt_obj)["ymdhms_list"]
		return result

	def change_format_text_time_to_dt_obj(self, input_text_time, input_format):
		"""
		datetime는 utc local
	...
	def change_format_text_to_hms_list(self, hms_list, end_text=["시", "분", "초"]):
		"""
	...
	def change_input_local_to_utc_timestamp(self, local_time_zone=9):
		"""
	...
	def change_input_time_to_another_formatted_text_time(self, input_text_time, input_time_format):
		"""
		입력시간을 다른 형식으로 바꾸는 것
	...
	def change_input_time_to_dt_obj(self, input_text_time):
		result = self.check_input_time(input_text_time)
		return result

	def change_input_time_to_utc_timestamp(self, input_text_time):
		"""
	...
	def change_input_time_to_ymd_list(self, input_text_time):
		"""
		문자형시간 => [년, 월, 일]
	...
	def change_month_to_eng(self, month):
	...
	def change_time_to_utc_timestamp(self, input_data=""):
		"""
		현재의 시간을 돌려준다
		입력값이 ""이면 지금의 시간을 뜻한다
		시간형태 : 1648037614.4801838 (의미 : 2022-03-23T21:13:34.480183+09:00)
		"""
		localtime = self.change_anytime_to_utc_timestamp(input_data)
	...
	def change_utc_timestamp_to_end_of_day_for_month(self, input_data=""):
		"""
		입력한 날짜나 시간의 마지막날을 게산하는것
		입력값 : 시간 또는 날짜
		출력값 : 그달의 마지막날
		"""
		lt = self.change_anytime_to_utc_timestamp(input_data)
	...
	def change_utc_timestamp_to_ymd_dash(self, input_data=""):
		lt = self.change_anytime_to_utc_timestamp(input_data)
		atime = arrow.get(lt)
	...
	def change_utc_to_dt_obj(self, input_utc):
		...
		dt_obj = self.change_any_time_to_dt_obj(dt_timestamp)
		return dt_obj
	...
	def change_ymd_list_to_utc_timestamp(self, input_list):
		"""
		입력 : [년, 월, 일]
		출력 : utc값
		"""
	...
	def check_input_time_1(self, input_data=""):
		...
		if input_data == "":
			...
			result = self.change_ymd_list_to_utc_timestamp(input_data)
		else:
	...
	def check_text_time_with_jf_sql(self, input_text_time, jf_sql):
		...
		:return:
		"""
		result = self.jf.search_all_by_jf_sql(input_text_time, jf_sql)
		return result
		...
		for start_ymd_list, end_ymd_list in period_list_moon:
			...
			for one_holiday in holiday_list2d:
				...
				if one_holiday[1][2] == "음":
					...
					if base_start_no <= holiday_no and base_end_no >= holiday_no and one_holiday[0][0] <= holiday_no and one_holiday[0][1] >= holiday_no:
	...
	def delta_two_date_1(self, date_1, date_2):
		result = self.delta_two_date(date_1, date_2)
		return result
	...
	def get_all_information_for_now_as_dic_style(self, time_char=""):
		"""
		"""
		if time_char == "":
			lt = time.localtime()
	...
	def get_day_for_now(self, time_char=time.localtime(time.time())):
		# 일 -----> '05'
		return time.strftime('%d', time_char)

	def get_day_list_for_utc(self, input_text_time=""):
		"""
	...
	def get_day_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		일 -----> ['05']
		닞은숫자 -> 많은글자 순으로 정리
		"""
		utf_local_time = self.change_anytime_to_utc_timestamp(input_data)
	...
	def get_dt_obj_by_date_obj_and_time_obj(self, input_date_obj, input_time_obj):
		dt_obj = self.combine_date_obj_and_time_obj(input_date_obj, input_time_obj)
		return dt_obj

	def get_dt_obj_for_last_day_of_any_time(self, input_text_time):
		"""
	...
	def get_dt_obj_for_now_as_yyyy_mm_dd_style(self):
		"""
	...
	def get_dt_obj_for_now_as_yyyy_mm_dd_style_1(self):
		"""
		날짜와 시간(datetime) -> 문자열로 : strftime
		날짜와 시간 형식의 문자열을 -> datetime으로 : strptime
	...
	def get_dt_timestamp_from_dt_obj(self, input_dt_obj):
		result = input_dt_obj.timestamp()
		return result
	...
	def get_end_year_of_datetime(self):
		result = datetime.MAXYEAR
		return result

	def get_hms_for_now_by_local(self):
		"""
		utc시간이 아니고 현재 지역적인 시간으로 변경하는 것
		3.6버전이후에 나타나는 astimezone를 사용하면 알아서 적용되는 것이다
		"""
	...
	def get_holiday_list_between_day1_and_day2(self, input_any_time1, input_any_time2):
		"""
		날짜사이의 휴일의 리스트 얻기
		...
		dic_time_1 = self.get_all_data_for_dt_obj(input_any_time1)
		dic_time_2 = self.get_all_data_for_dt_obj(input_any_time2)
		...
		for list1d in self.vars["holiday_list"]:
			if list1d[0][0] <= int(start_day) and list1d[0][1] >= int(end_day):
				if list1d[1][0] <= int(dic_time_1["mon"]) and list1d[1][0] <= int(dic_time_2["mon"]) and list1d[1][1] <= int(dic_time_1["day"]) and list1d[1][1] <= int(dic_time_2["day"]):
	...
	def get_hour_for_now(self, time_char=time.localtime(time.time())):
		# 시 -----> '10'
		return time.strftime('%I', time_char)

	def get_hour_set_for_now(self, time_char=time.localtime(time.time())):
		# 시 -----> ['10', '22', 'PM']
	...
	def get_hour_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		시 -----> ['10', '22']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_min_set_for_utc(self, input_utc):
		"""
		입력값 : utc시간숫자, 1640995200.0 또는 ""
		분 -----> ['07']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_min_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		분 -----> ['07']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_month_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		월 -----> ['04', Apr, April]
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_sec_set_for_dt_obj(self, input_dt_obj):
		"""
	...
	def get_sec_set_for_now(self, time_char=time.localtime(time.time())):
		# 초 -----> ['48']
		return [time.strftime('%S', time_char)]

	def get_sec_set_for_utc(self, input_utc):
		"""
		utc : utc의 timestamp, utc시간숫자, 1640995200.0 또는 ""
	...
	def get_sec_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		초 -----> ['48']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_week_list_by_nth_week_later(self, n_week_later, base_date=""):
		...
		dic_time = self.get_dic_time_for_dt_obj(dt_obt)
		print(dic_time['yea'], int(dic_time['yearweekno'])+n_week_later)
		week_list = self.get_7_day_list_for_yearweekno(dic_time['yea'], int(dic_time['yearweekno'])+n_week_later)
		return week_list

	def get_week_list_for_weekno_start_by_monday(self, year, week_no):
		"""
		입력값 : 년도, 위크번호
		한 주의 시작은 '월'요일 부터이다
		"""
	...
	def get_week_set_for_utc_timestamp(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		주 -----> ['5', '13', 'Fri', 'Friday']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def get_weekno_for_utc_timestamp(self, input_data=""):
		"""
		시간이 들어온면
		입력값 : 년도, 위크번호
		한 주의 시작은 '월'요일 부터이다
	...
	def get_weekno_for_ymd_list(self, input_date=""):
		...
		if input_date == "":
			...
			dt_obj = self.change_any_time_to_dt_obj(input_date)
	...
	def get_year_set_for_now(self, time_char=time.localtime(time.time())):
		# 년 -----> ['02', '2002']
		return [time.strftime('%y', time_char), time.strftime('%Y', time_char)]

	def get_year_set_for_utc(self, input_utc):
		"""
		utc : utc의 timestamp, utc시간숫자, 1640995200.0 또는 ""
	...
	def get_year_set_for_utc_timestamp(self, input_data=""):
		"""
		년 -----> ['22', '2022']
		닞은숫자 -> 많은글자 순으로 정리
		"""
	...
	def get_ymd_for_now_by_local(self):
		"""
		utc시간이 아니고 현재 지역적인 시간으로 변경하는 것
		3.6버전이후에 나타나는 astimezone를 사용하면 알아서 적용되는 것이다
		"""
	...
	def get_ymd_list_for_input_time(self, input_any_time):
		"""
		datetime는 utc local
		datetime객체 => [년, 월, 일]
		...
		:return:
		"""
		dt_obj = self.change_any_time_to_dt_obj(input_any_time)
	...
	def get_ymdhms_for_now_by_local(self):
		dt_obj = datetime.datetime.now()
		changed_dt_obj = dt_obj.astimezone()
	...
	def minus_two_date_1(self, date_1, date_2):
		time_big = self.check_input_datetime(date_1)
		time_small = self.check_input_datetime(date_2)
		if time_big.lt_utc > time_small.lt_utc:
			pass
		else:
			time_big, time_small = time_small, time_big
		time_big.last_day = self.getMonthRange(time_big.year, time_big.month)[3]
		time_small.last_day = self.getMonthRange(time_small.year, time_small.month)[3]
	...
	def plus_dt_obj_and_hms_list(self, dt_obj, hms_list_2):
		...
		:return:
		"""
		sec_1 = self.get_dt_timestamp_for_dt_obj(dt_obj)
		sec_2 = self.change_hms_list_to_sec(hms_list_2)
		temp = sec_1 + sec_2
	...
	def shift_week_for_dt_obj(self, dt_obj, input_no):
		"""
		날짜를 이동
		...
		:return:
		"""
		new_dt_obj = dt_obj + datetime.timedelta(days=input_no*7)
		return new_dt_obj
	...
	def shift_hour_for_dt_obj(self, dt_obj, input_no):
		"""
		시간을 이동
	...
	def shift_min_for_dt_obj(self, dt_obj, input_no):
		"""
		분을 이동
	...
	def shift_month_for_dt_obj(self, dt_obj, input_month_no):
		"""
		월을 이동
	...
	def shift_month_for_input_time(self, input_text_time, input_month_no):
		"""
		기준날짜에서 월을 이동시키는것
		...
		"""
		dt_obj = self.check_input_time(input_text_time)
		changed_dt_obj = self.shift_month_for_dt_obj(dt_obj, input_month_no)
		result = self.get_ymd_list_for_dt_obj(changed_dt_obj)
		return result
	...
	def shift_sec_for_dt_obj(self, dt_obj, input_sec):
		"""
		날짜객체를 초단위로 이동시키는 것
	...
	def shift_year_for_dt_obj(self, dt_obj, input_year_no):
		"""
		년을 이동
	...
	def minus_date0_date1(self, input_date1, input_date2):
		utc1 = self.change_ymdlist_to_utc(input_date1)
		utc2 = self.change_ymdlist_to_utc(input_date2)
		result = abs((float(utc1) - float(utc2)) / (60 * 60 * 24))
		return result
