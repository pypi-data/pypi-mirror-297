# -*- coding: utf-8 -*-
import re, math, string, random, os, itertools, copy, time, sys
...
class pcell:
	...
	2024-08-10 : 비슷한것들을 제거하고 전체적으로 재확인함
	"""
	...
	def __start_pcell(self, filename="", cache_on_off=""):
		...
		if filename in [None, "", "activeworkbook", "active_workbook", "active"]:
			...
			for index in range(self.xlapp.Workbooks.Count):
				one_file = self.util.check_filepath(self.xlapp.Workbooks[index].FullName)
	...
	def arange_all_sheet_by_name(self):
		"""
		이름순으로 시트를 정렬하는것
		"""
	...
	def change_excel56_to_color_name(self, input_excel56):
		...
		"""
		result = self.color.change_excel56_no_to_color_name(input_excel56)
		return result
	...
	def change_value_in_range_by_jfsql(self, sheet_name, xyxy, iy, jfsql, new_value):
		...
		for index, list_1d in enumerate(list_2d):
			try:
				aa = self.jf.replace_with_jf_sql(jfsql, new_value, list_1d[iy])
				if aa == list_1d[index]:
					pass
				else:
	...
	def check_excel_filename(self, input_file):
		...
		input_file = self.util.check_filepath(input_file)
	...
	def delete_all_empty_sheet_in_workbook(self):
		"""
		...
		for one_sheet_name in all_sheet_name:
			...
			if check_sheet:
				self.delete_sheet(one_sheet_name)
	...
	def delete_file(self, old_path):
		...
		:return:
		"""
		old_path = self.util.check_filepath(old_path)
		os.remove(old_path)
	...
	def delete_jf_sql_from_found_to_end_for_selection(self, jf_sql):
		...
		for x in range(xyxy[0], xyxy[2] + 1):
			for y in range(xyxy[1], xyxy[3] + 1):
				value = self.read_value_in_cell("", [x, y])
				aaa = self.jf.search_all_with_jf_sql(jf_sql, value)
				if aaa:
	...
	def draw_detail_line_in_range(self, **input):
		...
		:param input:
		"""
		enum_line = self.vars["line"]["end_style_vs_enum"]
	...
	def draw_line(self, sheet_name, xyxy, *input_list):
		...
		for one in input_list[0]:
			if type(one) == type([]) and (type(input_list[0]) == type([]) or type(input_list[0]) == type(())):
				temp_position = one
			elif one in self.vars["check_line_position"].keys():
				...
				temp_color = one
		print("temp_color, temp_position, temp_thickness, temp_line ==> ", temp_color, temp_position, temp_thickness,
			  temp_line)
	...
	def draw_line_basic(self, sheet_name, xyxy, *para_list):
		...
		:return:
		 """
		print("*para_list[0] ==> ", para_list)
		self.draw_line(sheet_name, xyxy, para_list)

	def draw_line_for_easy(self, sheet_name, xyxy, *input_list):
		"""
		몇가지 형태중 기본적인 형태로 입력을 하면 알아서 parameter로 적용하는 형태입니다
		예제1 : excel.draw_line_for_easy("", [3, 3, 10, 10], [7, 8, 9], rgbint, "t0", "all)
		예제2 : excel.draw_line_for_easy("", [3, 3, 10, 10], [7, 8, 9], rgbint, "all")
		예제3 : excel.draw_line_for_easy("", [3, 3, 10, 10], [7, 8, 9], "all", "t0", rgbint)
		...
		for one in input_list[0]:
			if type(one) == type([]) and (type(input_list[0]) == type([]) or type(input_list[0]) == type(())):
				temp_position = one
			elif one in self.vars["check_ line_position"].keys():
				temp_position = self.vars["check_line_position"][one]
			elif one in self.vars["check_ line_style"].keys():
				temp_line = self.vars["check_line_style"][one]
			elif one in self.vars["check_ line_thickness"].keys():
				temp_thickness = self.vars["check_line_thickness"][one]
			else:
				temp_color = one
			print("temp_color, temp_position, temp_thickness, temp_line ==> ", temp_color, temp_position,
				  temp_thickness, temp_line)
			for abc in temp_position:
				self.range_object.Borders(abc).Color = self.color.change_scolor_to_rgbint(temp_color)
				self.range_object.Borders(abc).Weight = temp_thickness
				self.range_object.Borders(abc).LineStyle = temp_line

	def draw_line_for_selection_as_well_used_1(self):
		"""
		...
		self.draw_line("", "", [[7, 8, 9, 10, 11], "bla", "실선", "t0"])
		self.draw_line("", "", [[12], "bla", "실선", "t-1"])
	...
	def draw_line_in_range(self, sheet_name, xyxy, *input_list):
		...
		보관용
		"""
		self.draw_line(sheet_name, xyxy, input_list)

	def draw_line_in_range_as_basic(self, sheet_name="", xyxy=""):
		"""
	...
	def draw_line_with_for_speed(self, sheet_name="", xyxy="", position="", rgbint="", line_style="", thickness=""):
		"""
		영역안의 라인을 그리는것
		예제 : excel.draw_line_with_speed("", [3, 3, 10, 10], [7, 8, 9], rgbint, 1, 1)
	...
	def get_address_for_merged_range(self, sheet_name="", xyxy=""):
		"""
		영역안의 병합된 자료를 알려주는것
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				range_object = sheet_object.Cells(x, y)
				if range_object.MergeCells:
					range_object.Select()
	...
	def insert_all_picture_of_folder_in_sheet(self, sheet_name, folder_name, ext_list, xywh, link=0J, image_in_file=1):
		...
		aaa = self.util.get_all_file_name_in_folder_by_extension_name(folder_name, ext_list)
		sheet_object = self.check_sheet_name(sheet_name)
	...
	def insert_image_at_range_name(self, sheet_name, folder_path, ext_list=["jgp", "png"]):
		...
		all_files = self.util.get_all_file_name_in_folder_by_extension_name(folder_path, ext_list)  # 1
		all_files.sort()  # 2
	...
	def is_cell_in_merge(self, sheet_name, xy=""):
		"""
		결과를 True / Flase로 나타내는 것
	...
	def make_dict_by_first_value_in_range(self, sheet_name="", xyxy=""):
		...
		l2d_changed = self.util.delete_empty_line_for_list_2d(l2d)
	...
	def make_title_by_range(self, sheet_name="", xyxy=""):
		...
		for y in range(xyxy[1], xyxy[3] + 1):
			...
			for x in range(xyxy[0], xyxy[2] + 1):
				...
				aa = self.check_merge_status(sheet_name, [x, y])
				if aa:
	...
	def merge_for_xyxy(self, sheet_name="", xyxy=""):
		...
		if x1 == x2:
			...
			for a in range(x2 - x1 + 1):
				sheet_object.Range(sheet_object.Cells(x1 + a, y1), sheet_object.Cells(x1 + a, y2)).Merge(0)
	...
	def paint_same_value_in_range_by_scolor(self, sheet_name, xyxy, input_scolor="gray"):
		...
		:return:
		"""
		self.paint_samevalue_in_range_by_scolor(sheet_name, xyxy, input_scolor)
	...
	def print_label_style(self, sheet_name, input_list_2d, line_list, start_xy, size_xy, y_line, position):
		...
		changed_list_2d = self.util.pick_ylines_at_list_2d(input_list_2d, line_list)
		for index, list_1d in enumerate(changed_list_2d):
	...
	def replace_many_times_with_jf_sql(self, jf_sql_list, replace_word_list, input_text):
		...
		for one in jf_sql_list:
			re_sql_list.append(self.jfinder(one))
	...
	def replace_with_jf_sql_as_selection_directly(self, jf_sql, replace_text):
		...
		for x in range(xyxy[0], xyxy[2] + 1):
			for y in range(xyxy[1], xyxy[3] + 1):
				value = self.read_value_in_cell("", [x, y])
				aaa = self.jf.replace_with_jf_sql(jf_sql, replace_text, value)
	...
	def search_jf_sql_for_selection_with_new_sheet(self, jf_sql):
		...
		result_list_2d = self.jf.search_result_only_with_jf_sql_for_list_2d(jf_sql, list_2d)
	...
	def search_value_by_jf_sql_in_range_with_paint(self, sheet_name, xyxy, jf_sql, input_scolor="yel"):
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				one_value = sheet_object.Cells(x, y).Value2
				found_or_not = self.jf.search_all_with_jf_sql(jf_sql, str(one_value))
				if found_or_not:
	...
	def write_list_in_range(self, sheet_name, xyxy, input_list):
		...
		list_2d = self.util.change_any_data_to_list_2d(input_list)
	...
	def write_unique_value_in_range(self, xyxy):
		"""
		선택한 자료중에서 고유한 자료만을 골라내는 것이다
		1. 관련 자료를 읽어온다
		2. 자료중에서 고유한것을 찾아낸다
	...
	def write_value_in_range(self, sheet_name, xyxy, input_any_type):
		...
		checked_list_2d = self.util.change_any_data_to_list_2d(input_any_type)  # 1
	...
	def write_value_in_range_by_range_priority(self, sheet_name, xyxy, input_value):
		...
		sheet_object = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		l2d = self.util.change_any_data_to_list_2d(input_value)
	...
	def write_value_in_range_by_value_priority(self, sheet_name, xyxy, input_any_type):
		...
		sheet_object = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		l2d = self.util.change_any_data_to_list_2d(input_any_type)
	...
	def write_value_in_range_except_none(self, sheet_name, xyxy, input_any_type):
		...
		list_2d = self.util.change_any_data_to_list_2d(input_any_type)
