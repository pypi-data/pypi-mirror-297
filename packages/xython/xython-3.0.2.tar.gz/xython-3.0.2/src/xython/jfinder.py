...
class jfinder:
    ...
    def delete_from_begin_to_searched_value(self, jf_sql, input_text, include_serarched_value=True):
        """
        찾는자료 앞의 모든 자료를 삭제하기
        ...
        :return:
        """
        found_data = self.search_all_with_jf_sql(jf_sql, input_text)
        start_x= 0
        if found_data:
    ...
    def delete_from_searched_value_to_end(self, jf_sql, input_text, include_serarched_value=True):
        """
        찾는 자료뒤의 모든 자료를 삭제하기
        옵션으로 찾은 자료를 포함할지 아닐지를 선택한다
    ...
    def delete_no_meaning_words(self, input_list, change_word_dic):
        ...
        for one in input_list:
            one = str(one).strip()
            if self.fullmatch_with_jf_sql(sql_3, one):
                if one in list(change_word_dic.keys()):
                    #print("발견 ==> 바꿀문자", one)
                    one = change_word_dic[one]

            if self.fullmatch_with_jf_sql(sql_4, one):
                #print("발견 ==> (문자)   :  ", one)
            ...
            if len(one) <= 1:
                one = ""
            elif self.fullmatch_with_jf_sql(sql_1, one):
                #print("발견 ==> 숫자만", one)
                one = ""
            elif self.fullmatch_with_jf_sql(sql_2, one):
                #print("발견 ==> 숫자+1글자", one)
                one = ""
    ...
    def delete_with_jf_sql (self, jf_sql, input_text):
        ...
        result = self.search_all_with_jf_sql(re_sql, input_text)
        return result

    def is_fullmatch(self,jf_sql,input_text):
        result = self.fullmatch_with_jf_sql(jf_sql,input_text)
        return result

    def is_fullmatch_with_jf_sql(self,jf_sql,input_text):
        """
    ...
    def is_match(self, jf_sql, input_text):
        result = self.search_all_with_jf_sql(jf_sql, input_text)
        return result

    def is_match_all(self,jf_sql,input_text):
        result = self.is_fullmatch_with_jf_sql(jf_sql,input_text)
        return result

    def is_match_with_jf_sql (self,jf_sql, input_text):
        """
    ...
    def run_with_jf_sql(self, jf_sql, input_text):
        ...
        re_sql = self.jfinder(jf_sql)
        result = self.search_all_with_re_sql(re_sql, input_text)
        return result
    ...
    def search_all_eng(self, input_text):
        """
    ...
    def search_all_with_jf_sql(self, jf_sql, input_text):
        """
        결과값을 얻는것이 여러조건들이 있어서 이것을 하나로 만듦
        [[결과값, 시작순서, 끝순서, [그룹1, 그룹2...], match결과].....]
    ...
    def search_all_with_re_sql (self, re_sql, input_text):
        """
    ...
    def search_between_brackets_with_jf_sql(self, input_text):
        """
        괄호안의 문자 갖고오기
        괄호 내부 내용만 추출 : '\(([^)]+)'
        앞 뒤 괄호까지 포함 : '\([^)]+\)'
    ...
    def search_between_word1_word2_with_jf_sql(self, input_text, word_a, word_b):
        """
        두 단어사이의 글자를 갖고오는 것
    ...
    def search_in_result(self, result_list_2d, jf_sql):
        """
        ...
        for list_1d in result_list_2d:
            start_no = list_1d[1]
            temp_2d = self.search_all_with_jf_sql(jf_sql, list_1d[0])
            if temp_2d:
    ...
    def search_number_between_len1_len2(self, len1, len2, input_text):
        """
        m,n개사이인 것만 추출
    ...
    def search_result_only_with_jf_sql_for_list_2d(self, jf_sql,input_l2d):
        re_sql = self.jfinder(jf_sql)
        result = []
        for ix, l1d in enumerate(input_l2d):
            temp_l1d = []
            for iy, value in enumerate(l1d):
                one_result = self.search_all_with_re_sql(re_sql, value)
    ...
    def setup_for_begin_end(self, begin=False, end=False):
        """
