...
class xy_list(object):
	...
	def delete_by_value(self, input_data):
		"""
	...
	def read_value_by_ixy(self, ixy):
		"""
		i : index의 값 (index : 0부터 시작하는 값)
		ixy : 무조건 숫자여야 한다
		"""
	...
	def read_value_by_ixyxy(self, xyxy):
		"""
		i : index의 값 (index : 0부터 시작하는 값)
		ixyxy : 무조건 숫자여야 한다
		"""
	...
	def read_value_by_xyxy(self, xyxy):
		"""
		xyxy : 무조건 숫자여야 한다
		"""
		x1, y1, x2, y2 = [min(xyxy[0], xyxy[2]), min(xyxy[1], xyxy[3]), max(xyxy[0], xyxy[2]), max(xyxy[1], xyxy[3])]
	...
	def read_value_by_yy(self, y1, y2):
		"""
	...
	def read_value_from_no1_to_no2(self, start, end):
		"""
	...
	def write(self, input_no, value):
		dic_data = self.check_input_no(input_no)
		no = dic_data["start"]
		self.main_list[no-1] = value

	def write_value_by_no(self, input_no, input_data):
		"""
		n번째에 값을 넣는 것이다
		"""
		input_data = self.to_general_list(input_data)
	...
	def write_value_by_xx(self, x1, x2):
		"""
	...
	def write_value_by_xy(self, xy, input_data=None):
		"""
		2차원 리스트 자료의 값을 바꾸는것
		"""
		input_data = self.to_general_list(input_data)
	...
	def xyinsert(self, no, value):
		self.check_no(no)
		return self.main_list.insert(no-1, value)
