...
def check_optional():
    ...
    for argv in sys.argv:
        ...
        if argv == "--help" or argv == "-help" or argv == "-h":
            ...
            print("   --radius=[Integer]:             set the radius of the circular graph")
            print("   --pop-up=true/false:            set whether to pop up the generated image")
            print("   --edges=undirected/directed:    set the shape of the graph")
            exit()
        if argv.startswith('--'):
            if argv.startswith('--save-temp-files='):
                ...
                if argv == '--pop-up=false':
                    pop_up = False
                argv_remove_list.append(argv)
            elif argv.startswith('--edges='):
                if argv == '--edges=directed':
                    directed = True
                argv_remove_list.append(argv)
            else:
    ...
    return save_temp_files, temp_path, output_format, shape, radius, pop_up, directed
...
def generate_code(EdgeSet, shape="circle", radius=3, directed=False):
    ...
    if shape == "tree":
        if not isTree(EdgeSet):
            ...
            exit()
        root = EdgeSet[0][0]
        completeLaTeXCode = LaTeXCode(EdgeSet, isTree=True, root=root, directed=directed)
    else:
        if shape == "line":
            completeLaTeXCode = LaTeXCode(EdgeSet, isLine=True, directed=directed)
        else:
            completeLaTeXCode = LaTeXCode(EdgeSet, radius, directed=directed)
...
def main(save_temp_files=False, temp_path='temp', output_format='png', shape="circle", radius=3, pop_up=True, input_file='graph.txt', output_file='graph', directed=False):

    if save_temp_files == False and temp_path == 'temp' and output_format == 'png' and shape == "circle" and radius == 3 and pop_up and directed == False:
        save_temp_files, temp_path, output_format, shape, radius, pop_up, directed = check_optional()
    
    EdgeSet = read_graph(input_file)

    completeLaTeXCode = generate_code(EdgeSet, shape, radius, directed)
