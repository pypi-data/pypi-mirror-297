...
def line(VertexSet, EdgeSet, directed):
    ...
    for (u, v, t) in EdgeSet:
        if last_dict[u] == v or last_dict[v] == u:
            if directed:
                GraphMeta += "  \draw[->] (" + str(u) + ") to node[above] {" + str(t) + "} (" + str(v) + ");\n"
            else:
...
def tree(VertexSet, EdgeSet, root, directed):
    ...
    while Queue:
        ...
        for (v, t) in Graph.edge[u]:
            if count % 2 == 0:
                ...
                right_last = v
            if directed:
                GraphMeta += "  \draw[->] (" + str(u) + ") to node[left] {" + str(t) + "} (" + str(v) + ");\n"
            else:
...
def circle(VertexSet, EdgeSet, radius, directed):
...
def LaTeXCode(EdgeSet, radius=3, isTree=False, isLine=False, root=None, directed=False):
    ...
    if isLine:
        GraphMeta = line(VertexSet, EdgeSet, directed)

    elif isTree:
        GraphMeta = tree(VertexSet, EdgeSet, root, directed)

    else:
        GraphMeta = circle(VertexSet, EdgeSet, radius, directed)
