...
class Occurrence(QGraphicsRectItem):
    ...
    def contextMenuEvent(self, event):
        if self.time_pane.occurrence_under_creation is None:
            ...
            for occurrence in self.timeline.occurrences():
                if (
                    occurrence.end_position == self.begin_position
            ...
            for occurrence in self.timeline.occurrences():
                if (
                    self.end_position == occurrence.begin_position
                    and self.event == occurrence.event
                ):
                    can_merge_next = True
                    break
    ...
    def merge_previous(self):
        for occurrence in self.timeline.occurrences():
            if (
                self.begin_position == occurrence.end_position
                and self.event == occurrence.event
            ):
                break
    ...
    def draw_name(self, painter):
        ...
        if self.event:
            ...
            painter.drawText(
                self.rect(), Qt.AlignmentFlag.AlignCenter, self.event.name
            )
