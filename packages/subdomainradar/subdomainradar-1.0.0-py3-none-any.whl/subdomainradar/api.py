...
if __name__ == "__main__":
    ...
    try:
        # List enumerators
        print('Enumerators:')
        enumerators = api.list_enumerators()
        print(enumerators)

        # List enumerator groups
        print('Enumerator Groups:')
        groups = api.list_enumerator_groups()
        print(groups)
        ...
        # Enumerate domains
        """ print('Enumerate Domains:')
        enumerate_task_id_response = api.enumerate_domains(domains=domains, group="Fast")
        print(enumerate_task_id_response) """

        # Enumerate domains and wait for results
        print('Enumerate Domains with Results:')
        enumerate_results = api.enumerate_domains_with_results(domains=domains, group="Fast")
        print(enumerate_results)

        # Reverse search with additional parameters
        print('Reverse Search:')
        reverse_search = api.reverse_search(
            subdomain_part="admin",
            domain_part="car",
            tld_part="com",
            exclude_generic_hosting_domains=True,
            exclude_gov_ed_domains=True
        )
        print(reverse_search)

        # Get excludes
        print('Excludes:')
        excludes = api.get_excludes()
        print(excludes)
        ...
        # Get task status using a task from the list of tasks
        if tasks:
            task_id = tasks[0]['task_id']
            print('Task Status:')
            print(api.get_task(task_id))
