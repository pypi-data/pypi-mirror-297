...
from mipi_datamanager.core.types import JoinLiterals, Mask
from mipi_datamanager.core import meta
from mipi_datamanager.core.database_tools import query, odbc, generate_inserts
from mipi_datamanager.core.error_handler import ConfigError
from mipi_datamanager.core.database_tools.jinja import JinjaWorkSpace, JungleWorkSpace
from mipi_datamanager.core.jungle import Jungle
...
def _get_df_and_sql_from_jungle(jungle_source, inner_path, connection, jinja_parameters_dict):
    jenv = JungleWorkSpace(jungle_source.root_dir)
...
def _get_config_from_master(inner_path, jungle_source):
    _inner_path = Path(inner_path)
    with open(os.path.join(jungle_source.root_dir, "master_config.json"), 'r') as f:
...
class DataManager:
    ...
    def __init__(self, frame: meta.Frame,
        ...
        self.jungle_source = jungle_source
    ...
    @classmethod
    def from_jungle(cls, inner_path,
                    jungle_source: Jungle,
        ...
        config = _get_config_from_master(inner_path, jungle_source)

        if not config["meta"]["population"]:
            raise ConfigError(f'expected population status is True script got: {config["meta"]['population']}')

        con = jungle_source.conn_list[config['meta']['connection']]
        df, sql = _get_df_and_sql_from_jungle(jungle_source, inner_path, con, jinja_parameters_dict)
        frame = meta.Frame(config["meta"]["name"], "Jungle", df, sql=sql)

        return cls(frame,
                   jungle_source=jungle_source,
                   store_deep_frames=store_deep_frames,
                   store_deep_base=store_deep_base,
                   default_format_func_dict=default_format_func_dict,
                   dialect=dialect)

    def set_jungle_source(self, jungle_source):
        if self.jungle_source:
            raise AttributeError(
                f"Mipi object {repr(self)} has already been set. Create a new mipi object, or clone this one to set a different jungle source")
        else:
            self.jungle_source = jungle_source
    ...
    def from_dataframe(cls, df: pd.DataFrame, frame_name=None,
        ...
        return cls(frame, jungle_source=jungle_source,
    ...
    def from_sql(cls, script_path: str, connection: odbc.Odbc, format_parameters_list: list = None, frame_name=None,
        ...
        return cls(frame, jungle_source=jungle_source, store_deep_frames=store_deep_frames,
                   store_deep_base=store_deep_base,
    ...
    def join_from_jungle(self, inner_path, how: JoinLiterals = "left", jinja_parameters_dict: dict = None,
        ...
        config = _get_config_from_master(inner_path, self.jungle_source) #TODO replace with jungle.pull
        ...
        con = self.jungle_source.conn_list[config['meta']['connection']]

        sql = self.resolve_join_jungle_template(inner_path, right_on,jinja_parameters_dict,rename_columns=rename_dict)
    ...
    def clone(self, base_name=None,
        ...
        _jungle_source = change_jungle_source or self.jungle_source
        ...
        mipi2 = cls(frame, jungle_source=_jungle_source,
                    store_deep_frames=store_deep_frames, store_deep_base=store_deep_base,
                    default_format_func_dict=new_format_dict)

        return mipi2

    def get_insert_statements(self, key: str, frame=None, rename_columns: dict | None = None,
                              make_temptable=True):  # TODO test from specific frame
        ...
        if self.dialect == 'sql_server':
            if isinstance(key, str):
                return generate_inserts.generate_tsql_inserts(df[[key]], make_temptable=make_temptable)
    ...
    def resolve_join_format_sql_file(self, file_path: str, key: str, format_parameters_list=None, frame=None,
        ...
        return self.resolve_join_format_sql(sql, key, format_parameters_list, frame = frame,rename_columns = rename_columns, make_temptable = make_temptable)
    ...
    def _maybe_get_jinja_insert_dict(self,key,jinja_parameters_dict = None, frame = None,rename_columns= None,make_temptable = None):
        if jinja_parameters_dict:
            jinja_parameters_dict["MiPiTempTable"] = self.get_insert_statements(key,frame = frame,rename_columns = rename_columns, make_temptable=make_temptable)
        else:
            jinja_parameters_dict = {"MiPiTempTable": self.get_insert_statements(key,frame = frame,rename_columns = rename_columns, make_temptable=make_temptable)}
        return jinja_parameters_dict
    def resolve_join_jungle_template(self, inner_path, key, jinja_parameters_dict, frame=None,
                                     rename_columns: dict | None = None, make_temptable=True):
        jenv = JungleWorkSpace(self.jungle_source.root_dir)
        jinja_parameters_dict = self._maybe_get_jinja_insert_dict(key, jinja_parameters_dict,frame,rename_columns, make_temptable)
