...
class PlayerDataSystem:
    ...
    def load_player_data(self, username: str, player_instance: Any) -> None:
        try:
            ...
            if online_data and self.is_online():
    ...
    def get_file_timestamp(self, path: str) -> float:
        try:
            response = requests.head(self.db._get_file_url(path), headers=self.db.headers)
            if response.status_code == 200:
                return float(response.headers['Last-Modified'])
        except Exception as e:
            print(colored_text("RED", f"Error retrieving file timestamp: {e}"))
        return 0
...
class DataSystem:
    ...
    def save_offline_data(self, key: str, value: Any) -> None:
        ...
        encrypted_data: bytes = self.encrypt_data(json.dumps(value))
        path: str = f"gitbase/data/{key}.gitbase"
    ...
    def load_data(self, key: str) -> Optional[Any]:
        try:
            ...
            offline_path: str = f"gitbase/data/{key}.gitbase"
            if os.path.exists(offline_path):
                print(colored_text("GREEN", f"Loading offline backup for {key}."))
    ...
    def load_offline_data(self, key: str) -> Optional[Any]:
        ...
        if os.path.exists(path):
            ...
            print(colored_text("RED", f"No offline data found for {key}"))
            return None
        return None
    ...
    def delete_data(self, key: str) -> int:
        try:
            ...
            print(colored_text("RED", f"Error: {e}"))
            print(colored_text("GREEN", "Attempting to delete offline backup version."))
            self.delete_offline_data(key)

    def delete_offline_data(self, key: str) -> None:
        path: str = f"gitbase/data/{key}.gitbase"
