...
def _prepare_patch_schema(input_schema: Optional[Schema]) -> Optional[Schema]:
    ...
    if input_schema:
        ...
        OriginalSchemaClass = input_schema.__class__
        PatchSchemaClass = type(
            f"Patch{OriginalSchemaClass.__name__}",
            (OriginalSchemaClass,),
            {}
        )

        # Create an instance of the new schema class
        put_input_schema = PatchSchemaClass()

        # Deepcopy the declared fields to avoid modifying the original class's fields
        put_input_schema._declared_fields = deepcopy(put_input_schema._declared_fields)
        for field_dict in [put_input_schema._declared_fields, put_input_schema.load_fields, put_input_schema.fields]:
