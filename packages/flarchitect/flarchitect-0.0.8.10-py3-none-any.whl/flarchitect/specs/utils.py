...
def _prepare_patch_schema(input_schema: Optional[Schema]) -> Optional[Schema]:
    """Helper function to prepare a patch schema with all fields optional.

    Args:
        input_schema (Optional[Schema]): The input schema to prepare the patch schema for.

    Returns:
        Optional[Schema]: The prepared patch schema.
    """
    if input_schema:

        put_input_schema = copy.deepcopy(input_schema)

        if callable(put_input_schema):
            put_input_schema = put_input_schema()

        model = put_input_schema.get_model() if hasattr(put_input_schema, "get_model") else None
        if hasattr(put_input_schema, "__name__"):
            put_input_schema.__name__ = f"patch_{convert_case(put_input_schema.__name__, get_config_or_model_meta('API_SCHEMA_CASE', model=model, default='camel'))}".replace(
                "Schema", ""
            )
        else:
            put_input_schema.__class__.__name__ = f"patch_{convert_case(put_input_schema.__class__.__name__, get_config_or_model_meta('API_SCHEMA_CASE', model=model, default='camel'))}".replace(
                "Schema", ""
            )

        fields = put_input_schema._declared_fields.values() or put_input_schema.load_fields.values()
        for field in fields:
            if field.required:
