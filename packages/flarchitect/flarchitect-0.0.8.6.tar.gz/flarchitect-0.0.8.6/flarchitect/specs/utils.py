...
def _prepare_patch_schema(input_schema: Optional[Schema]) -> Optional[Schema]:
    ...
    if input_schema:
        ...
        if hasattr(put_input_schema, "__name__"):
            put_input_schema.__name__ = f"patch_{convert_case(input_schema.__name__, get_config_or_model_meta('API_SCHEMA_CASE', model=model, default='camel'))}".replace(
                "Schema", ""
            )
        else:
            put_input_schema.__class__.__name__ = f"patch_{convert_case(input_schema.__class__.__name__, get_config_or_model_meta('API_SCHEMA_CASE', model=model, default='camel'))}".replace(
                "Schema", ""
            )
