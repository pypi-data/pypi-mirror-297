...
from flarchitect.schemas.auth import TokenSchema
    ...
    def route_function_factory(
        ...
        def route_function(id: Optional[int] = None, **hook_kwargs) -> Any:
            hook_kwargs = pre_process(
                pre_hook, id=id, field=get_field, join_model=join_model, **hook_kwargs
            )
...
class RouteCreator(AttributeInitializerMixin):
    ...
    def _validate_authentication_setup(self):
        ...
        if not current_app.config.get("SECRET_KEY") and auth:
            raise ValueError(
        ...
        if auth and not user and callable(auth):
            raise ValueError(
        ...
        if auth and not auth_method:
            raise ValueError(
                "If API_AUTHENTICATE is set to True, API_AUTHENTICATE_METHOD must be set to either 'basic', 'jwt', or 'api_key'."
            )

        if (
            auth
            and callable(auth)
            and not (hasattr(user, "email") and hasattr(user, "password"))
        ):
            raise ValueError(
                "The user model must have an email and password or api_key field if an authentication function is set."
            )
    ...
    def _create_jwt_login_route(self, user: Callable):
        ...
        def login(data):
            ...
            if usr and getattr(usr, check_method)(getattr(usr, hash_field), password):
                login_user(usr)
                access_token = create_access_token(
                    identity={"id": usr.id, "username": usr.username},
    ...
    def _create_jwt_refresh_route(self, user: Callable):
