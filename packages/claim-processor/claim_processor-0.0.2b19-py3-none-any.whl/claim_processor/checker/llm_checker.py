...
class LLMChecker(CheckerBase):
    ...
    def _check(
        ...
        """
        self.logger.info(f"Starting claim checking process. Is joint: {is_joint}")
        ...
        if is_joint:
            ...
            claim_nums = []
            p_id = 0
            for claims_per_batch, references_per_batch, question_per_batch in zip(
                claims, references, questions
            ):
                ...
                for ref in references_per_batch:
                    ...
                    for _ci, c in enumerate(claims_per_batch):
                        ...
                        _claim_cnt += 1
                        if (
                            _claim_cnt >= joint_check_num
                            or _ci == len(claims_per_batch) - 1
                        ):
                            prompt = prompt_template.replace(
                                "[QUESTION]", question_per_batch
                            )
            ...
            self.logger.info(
                f"Joint checking completed. Total prompts: {len(prompt_list)}"
            )
            ...
            for _i, claim_n in enumerate(claim_nums):
                if len(labels_list[_i]) < claim_n:
                    labels_list[_i] = labels_list[_i] + ["Neutral"] * (
                        claim_n - len(labels_list[_i])
                    )
            ...
            for _i, claim_num in enumerate(batch_claim_nums):
                if claim_num > 0:
                    one_batch_labels = merged_label_list[
                        _index : _index + batch_ref_nums[_i]
                    ]  # [ref_num, claim_num]
                ...
                if isinstance(claim, list) and len(claim) == 3:
                    if question is None:
                        prompt = self.prompt_temp.format(
                            reference=reference, claim=claim_text
                        )
                    else:
                        prompt = self.prompt_temp_wq.format(
                            question=question, reference=reference, claim=claim_text
                        )
            ...
            self.logger.info(f"Non-joint checking. Total prompts: {len(prompt_list)}")
            ...
            self.logger.info(
                f"Claim checking process completed. Total labels: {len(ret_labels)}"
            )
            return ret_labels
