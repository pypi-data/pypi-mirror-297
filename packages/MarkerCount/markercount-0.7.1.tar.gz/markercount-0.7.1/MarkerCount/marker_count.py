...
def get_marker_profile( df_ofreq, of_th = 0.9, N_mkr = 0, stype = 'max', pwr_nt = 2):
    ...
    for c in list(ct):
        ...
        if stype == 'max':
            ...
            # nt2 = df_ofreq.loc[~b,:].idxmax(axis=0)
        else:
        ...
        if N_mkr == 0:
            ...
            N_sel = int(min(N_mkr, np.sum(score > 0)))
            odr = (-s).argsort()
            b[odr[:N_sel]] = True
            bs1 = b | bf1
...
def get_score( X_in, df_mkr ):
    ...
    for ct in cell_types:
        ...
        gsel = list(set(gsel).intersection(genes))
...
def gen_marker_profile(X, cell_type, N_mkrs, cells_to_excl = [], \
    ...
    cells_to_test = list(set(cells_to_test).intersection(cells_cur))
...
def apply_knn(X_pca, cell_type, p_cells_dict, p_min, Nsel):
    ...
    for k in range( dist_mat.shape[0] ):
        ...
        odr = dx.argsort()[:int(min(Nsel, len(dx)))]
        ...
        if (len(n_cells) <= 1): # only one cell type
            ...
            if (np.sum(b&bua) > np.sum(b)*(1-p_maj)):
                ...
                if len(cell_name_lst_sel) == 1:
                    ...
                    by = ct_sel == cell_name_lst_sel[0]
                    centroid = X_pca_ua.loc[by,:].mean(axis = 0)
                    dist_vec = distance_matrix( x = X_pca_ua,  y = centroid[None, :] )
                    max_dist_intra = dist_vec[by,0].max()
                    bz = dist_vec[:,0] <= (max_dist_intra)*0.85 #(1-p_maj) # 6.21. 21:53

                    cell_type_new = copy.deepcopy(ct_sel)
                    cell_type_new[bz] = cell_name_lst_sel[0]
                    cell_type_pred_ary[b] = cell_type_new                    
                    pass
                    #'''
                    ...
                    for m, ct in enumerate(cell_name_lst_sel):
                        bc = ct_sel == ct
                        cent = X_pca_ua.loc[bc,:].mean(axis = 0)
                        dist_vec = distance_matrix( x = X_pca_ua,  y = cent[None, :] )
                        by = ct_sel == ct

                        md_intra = dist_vec[by,0].max()*0.85 #(1-p_maj) 
                        if ct in list(p_cells_dict.keys()):
                            md_intra = md_intra #/(p_cells_dict[ct]+0.1)
                            dist_vec = dist_vec #/(p_cells_dict[ct]+0.1)
                            
                        max_dists.append(md_intra)
                        if m == 0:
                            df_dist_vec = pd.DataFrame({ct: list(dist_vec[:,0])}, index = X_pca_ua.index.values)
                        else:
                            df_dist_vec[ct] = list(dist_vec[:,0])

                    cell_type_new = copy.deepcopy(ct_sel)
                    for m in range(df_dist_vec.shape[0]):
                        # if cell_type_new[m] == STR_UNASSIGNED:
                        ds = np.array(df_dist_vec.iloc[m])
                        md = ds.min()
                        idx = np.argmin(ds)
                        # if md <= max_dists[idx]: 
                        cell_type_new[m] = cell_name_lst_sel[idx]
...
def MkrCnt_Ref( X_ref, cell_type_ref, X_test = None, df_mkr_mat = None, \
    ...
    if ((X_ref is None) | (cell_type_ref is None)):
        ...
        if X_test is not None:
            ...
            if verbose:
                print('Ref. has %i genes,' % (len(genes_ref)))        
...
def MarkerCount_Ref( X_ref, cell_type_ref, X_test = None, df_mkr_mat = None, \
            N_mkrs = 18, of_th = 0.9, cell_types_to_excl = [], log_transformed = False, \
            N_clusters = None, cluster_label = None, X_pca = None, N_pca = 15, \
            target_FPR = 0.05, stype = 'mean', file_to_save_marker = None, verbose = True ):
    ...
    df_res, df_mkr, df_param, df_score = \
        MkrCnt_Ref( X_ref = X_ref, cell_type_ref = cell_type_ref, X_test = X_test, \
            ...
            target_FPR = target_FPR, stype = stype,
            file_to_save_marker = file_to_save_marker, verbose = verbose )
