...
class BizServiceAgent:
    ...
    def _ExecuteCore(self, request, operation):
        ...
        # biz_request_json = json.dumps(request.to_dict(), default=self.convert_default)
        biz_request_json = json.dumps(request.to_dict())

        print(biz_request_json)

        resp = httpx.post(bizService_endpoint + "/" + operation, headers=headers, data=biz_request_json)
