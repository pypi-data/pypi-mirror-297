...
class RsetGridColRenderer(tableview.RsetTableColRenderer):
    """Same as cubicweb_web.views.tableview.RsetTableColRenderer
    but render data that Slick.Grid accepts as header definitions.
    """

    def render_header(self):
        stream = UStringIO()
        super().render_header(stream.write)
        return stream.getvalue()

    def render_cell(self, rownum):
        stream = UStringIO()
        super().render_cell(stream.write, rownum)
        return stream.getvalue()


class GridLayout(tableview.TableLayout):
    """Same as cubicweb_web.views.tableview.TableLayout but
    ...
    def render_header(self, colrenderer):
        ...
        header = {"id": colid, "field": colid, "name": colrenderer.render_header()}
    ...
    def render_table_body(self, colrenderers):
        return [
            self.render_row(rownum, colrenderers)
            for rownum in range(self.cw_rset.rowcount)
        ]

    def render_row(self, rownum, renderers):
        row = {"id": rownum}
        row.update((str(cr.colid), self.render_cell(rownum, cr)) for cr in renderers)
        return row
    ...
    def render_assets(self):
        cnx = self._cw
        cnx.add_css(("SlickGrid/slick.grid.css", "SlickGrid/slick-default-theme.css"))
        cnx.add_js(
            (
...
class RsetGridView(tableview.RsetTableView):
    ...
    :class:`cubicweb_web.view.tableview.RsetTableView` plus the following:
