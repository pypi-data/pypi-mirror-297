...
class ZombieClusterOperations(AWSPolicyOperations):
    ...
    def __init__(self):
        super().__init__()
        self._cluster_prefix = self._environment_variables_dict.get('CLUSTER_PREFIX')
    ...
    def process_and_delete_resources(self, zombie_cluster_resources: dict, resource_id_key: str,
                                     resource_type: str, tags_name: str, create_date: str) -> dict:
        """
        ...
        for zombie_cluster_tag, resources in zombie_cluster_resources.items():
            ...
            response = self.zombie_cluster_verify_and_delete_resources(resource_ids, tags=tags)
            zombie_cluster_resources_response[zombie_cluster_tag] = {
                'ResourceIds': resource_ids,
                'DaysCount': self.get_clean_up_days_count(tags),
                'ClenUpResult': response,
                'User': self.get_tag_name_from_tags(tags=tags, tag_name='User'),
                'ZombieClusterTag': zombie_cluster_tag,
                'CreateDate': str(found_create_date),
            }
            if not response:
                self.update_resource_tags(tags=tags, resource_ids=resource_ids, resource_type=resource_type,
                                          cleanup_days=self.get_clean_up_days_count(tags))
        return zombie_cluster_resources_response

    def update_resource_tags(self, resource_ids: list, cleanup_days: int, tags: list, resource_type: str):
        """
