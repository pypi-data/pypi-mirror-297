...
class Connection:
    ...
    def __init__(self, username='', password='', match_id='', verbose=False):
        ...
        if self.get_record()['status'] == 0:
            ...
            if self.get_playload()['status'] == 0:
                ...
                self.element_module_dict = self.get_module_elements(verbose)
            else:
    ...
    def get_module_elements(self, verbose=False):
        """
        获取所有的模块和组合id
        """
        ret = {}
    ...
    def setbasic(self, verbose=False):
        ...
        for index, row in self.resources.iterrows():
            ...
            if resource_name == 'master' or resource_name == 'slave1' or resource_name == 'slave2':
                ...
                out, err = self.ssh_exec(resource_name=resource_name, command="source /etc/profile")
                out, err = self.ssh_exec(resource_name=resource_name, command="setvar")
                # 设置ip
    ...
    def get_check(self, resource_name, verbose=True):
        """
        获取远程的日志文件内容
        """
        hostname_df = self.resources[self.resources['resource_name']==resource_name]
            ...
            if verbose:
                ...
                for ret_data in ret_list:
                    ...
                    if "cat" in command and "grep" in command:
                        ...
                        ret_data['success'] = "mkdir -p {} && echo {},{} >> {}".format('/'.join(path_name.split('/')[:-1]), keyword, value, path_name)
                    if 'file' in ret_data:
                        if 'strs' in ret_data:
                            ret_data['success'] = "echo {} >> {}".format(ret_data['strs'], ret_data['file'])
                        else:
                            ret_data['success'] = "touch {}".format(ret_data['file'])
    ...
    def add_hostname(self, resource_name, hostname):
        hostname_df = self.resources[self.resources['resource_name']==resource_name]
    ...
    def getsql(self, resource_name, sql_name):
        ...
        out, err = self.ssh_exec(resource_name=resource_name, command=download_sql)
