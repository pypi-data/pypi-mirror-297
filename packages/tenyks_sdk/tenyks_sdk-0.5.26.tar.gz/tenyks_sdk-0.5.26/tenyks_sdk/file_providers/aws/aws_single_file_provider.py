...
class AWSSingleFileProvider(SingleFileProvider):
    def __init__(
        ...
        self.aws_location: AWSLocation = AWSLocation.from_dict(
            aws_location
        )
    ...
    def save_file(self, file_storage: FileStorage) -> Dict[str, object]:
        ...
        file_location: AWSLocation = AWSLocation.from_dict(
            self.aws_location.to_dict()
        )

        file_location.s3_uri = os.path.join(
            file_location.s3_uri, file_storage.filename
        )
