...
class NEUSetter:
    ...
    def setFieldByName(self,fname):
        ...
        if isinstance(fname,str):
            ...
            try:
                ...
                for num,f in enumerate(fields):
                    ...
                    print(msg)
                fnames=[]
                which=Prompt.__init2__(None,func=FormBuilderMkText,ptext="which index?: ",helpText="which index(es), use comma to separate multiple fields",data="list")
                if which in [None,]:
                    return
                elif which in ['d',]:
                    which=[0,]
        ...
        while True:
            try:
                barcode=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"[{fnames}] Barcode|Code|Name:",helpText="what are you looking for?",data="string")
                ...
                with Session(ENGINE) as session:
                    ...
                    for num,entry in enumerate(results):
                        ...
                        print(msg)
                    which=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"[{fnames}]which index?: ",helpText="which index",data="integer")
                    if which in [None,]:
                        continue
                    elif which in ['d',]:
                        which=0
