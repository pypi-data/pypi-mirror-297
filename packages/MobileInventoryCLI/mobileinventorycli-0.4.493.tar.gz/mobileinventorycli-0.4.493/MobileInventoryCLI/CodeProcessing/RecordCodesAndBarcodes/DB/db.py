...
def saveHistory(cmd,ageLimit,executed,data):
    with Session(ENGINE) as session:
        ph=PH(cmd=cmd,ageLimit=ageLimit,executed=str(executed),data=str(data),dtoe=datetime.now())
        session.add(ph)
        session.commit()
        session.refresh(ph)
...
class HistoryUi:
    def clean_old(self):
        with Session(ENGINE) as session:
            ALL=session.query(PH).all()
            total=0
            for num,i in enumerate(ALL):
                if datetime.now() >= (i.dtoe+timedelta(seconds=i.ageLimit)):
                    session.delete(i)
                    total+=1
                if num % 1000:
                    session.commit()
    ...
    {Fore.light_steel_blue}cmds are order ascending and grouped by cmd to reduce output to screen with too many results{Style.reset}
        ...
        {Fore.dark_goldenrod}show,sa,show all{Fore.green_yellow} - show all cmds{Style.reset}
