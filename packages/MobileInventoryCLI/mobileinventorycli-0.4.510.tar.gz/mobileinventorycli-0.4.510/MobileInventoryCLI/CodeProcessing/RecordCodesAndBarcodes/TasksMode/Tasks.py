...
{Fore.magenta}set field cmds{Fore.light_sea_green}
    'set field','sf','set'  #scan barcode,select item,display fields with index,select index,get data,commit
                                ...
                                if text not in ['',]:
                                    ...
                                    if text.lower() in ['ilcv','invert load count value','invert_load_count_value']:
                                        if mkTextStore:
                                            ...
                                            return value,text,''
                                    if text.lower() in ['iccv','invert case count value','invert_case_count_value']:
                                        if mkTextStore:
                                    ...
                                    if text.lower() in ['ilcvc','invert load count value cases','invert_load_count_value_cases']:
                                        if mkTextStore:
                                            ...
                                            print(f"Inverted Load Count is {Fore.light_steel_blue}{value}{Style.reset}")
                                            return value,text,''
                                    if text.lower() in ['iccvc','invert case count value cases','invert_case_count_value_cases']:
                                        if mkTextStore:
                                            if mkTextStore.CaseCount in [None,] or mkTextStore.CaseCount < 1:
                                                print(f"{Fore.orange_red_1}There is an issue this Entry's {Fore.cyan}CaseCount{Fore.orange_red_1} -> {mkTextStore}/n{Fore.green_yellow}Setting it to 1!{Style.reset}")
                                                mkTextStore.CaseCount=1
...
{Style.bold}{Fore.orange_red_1}Code={Fore.spring_green_3a}{CD}{Style.reset}'''
                        ...
                        ptext=f'''{hafnhaf}
{Fore.light_red}Enter {Style.bold}{Style.underline}{Fore.orange_red_1}Quantity/Formula{Style.reset} amount|+amount|-amount|a,+a,-a(advanced)|r,+r,-r(ReParseFormula) (Enter==1)|{Fore.light_green}ipcv={Fore.dark_goldenrod}PalletCount-value|{Fore.light_green}iscv={Fore.dark_goldenrod}ShelfCount-value|{Fore.light_green}ilcv={Fore.dark_goldenrod}LoadCount-value|{Fore.light_green}iccv={Fore.dark_goldenrod}CaseCount-value|{Fore.light_green}ipcvc{Fore.dark_goldenrod}=(PalletCount-value)/CaseCount|{Fore.light_green}iscvc{Fore.dark_goldenrod}=(ShelfCount-value)/CaseCount|{Fore.light_green}ilcvc{Fore.dark_goldenrod}=(LoadCount-value)/CaseCount|{Fore.light_green}iccvc{Fore.dark_goldenrod}=(CaseCount-value)/CaseCount{Style.reset}'''
