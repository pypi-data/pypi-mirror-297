...
from pyLaaang import Laang, CMD_HELP
from pyLaaang.pyrogram import eor

from . import yins

@Laang(["q", "quotly"], langs=True)
...
@Laang(["calo", "tikel"], langs=True)
async def calo(client: Client, message: Message, _):
    user = client.me
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            await Laang.edit("<i>File Tidak Didukung</i>")
            return
        
        media_ = await client.download_media(replied, file_name="assets/")
    else:
        await Laang.edit(_['reply_media'])
        return
    ...
    if media_:
        ...
        while True:
            ...
            if exist.set.count >= limit:
                pack += 1
                packname = f"a{user.id}_by_laang_{pack}"
                packnick = f"{custom_packnick} Vol.{pack}"
                if is_anim:
                ...
                await Laang.edit(_['sticker_2'].format(pack))
                continue
            break
        ...
        if exist is not False:
            try:
                ...
                return await Laang.edit(_['err'].format(e))
            ...
            while limit in await get_response(message, client):
                ...
                if is_video:
                    packname += "_video"
                    packnick += " (Video)"
                await Laang.edit(_['sticker_2'].format(str(pack)))
                await client.send_message("stickers", packname)
                await asyncio.sleep(2)
                if await get_response(message, client) == "Invalid pack selected.":
                    ...
                    await client.send_message("Stickers", packname)
                    await asyncio.sleep(2)
                    await Laang.edit(_['sticker_3'].format(packname))
                    return
...
async def get_response(message: Message, client: Client):
    return [x async for x in client.get_chat_history("Stickers", limit=1)][0].text

@Laang(["tiny"], langs=True)
async def tinying(client: Client, message: Message, _):
    ...
    YB = await eor(message, _['p'])
    ik = await client.download_media(reply)
    im1 = Image.open("assets/blank.png")
    
    if ik.endswith(".tgs"):
        await client.download_media(reply, "laang.tgs")
        await yins.bash("lottie_convert.py laang.tgs json.json")
        ...
        await yins.bash("lottie_convert.py json.json laang.tgs")
        file = "laang.tgs"
        os.remove("json.json")
...
async def memify(client: Client, message: Message, _):
    ...
    if len(text) < 1:
        return await YB.edit(_['sticker_6'])
...
@Laang(["getsticker", "mllg"], langs=True)
...
CMD_HELP.update(
    {"stickers": (
        "stickers",
        {
            ...
            "calo": "Balas Ke Sticker Atau Gambar Untuk Menambahkan Ke Sticker Pack.",
            "mlg": "Menambahkan text di sticker.",
            "mllg": "Mengubah Sticker Menjadi Gambar.",
