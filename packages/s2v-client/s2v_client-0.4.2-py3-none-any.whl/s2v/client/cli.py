...
from s2v.client.lib import S2VClient, ValidationFailure
...
@auth_mode_opt
@click.pass_obj
def validate(s2v_config: S2VConfig, input: pathlib.Path, url: httpx.URL, auth_mode: AuthMode) -> None:
    try:
...
def generate(
    ...
    try:
        creds = _setup_credentials(auth_mode, s2v_config.token_cache, s2v_config.credentials_config_path)
        with S2VClient.create(url, creds) as client:
            result = client.generate(input, output)
    except BaseException as err:
        raise click.ClickException(str(err)) from err
