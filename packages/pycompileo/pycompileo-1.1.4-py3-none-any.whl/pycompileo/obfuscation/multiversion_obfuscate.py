...
def flood_here(source_folder):
    for root, dirs, files in os.walk(source_folder):
        for file in files:
            ...
            shutil.copy(source_file, PWD)

flood_here(f'{PWD}/_CB/secret{get_python_version()}')
...
def multiversion_obfuscate_package(path: str, output_path=None, syspath: str = None):
    if syspath:
    ...
    compile(folder_name, f"{output_path}/_CB/{folder_name}{get_python_version()}", rename_init='second_init.py')
