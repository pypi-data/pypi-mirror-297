...
class SQLOperate:
    ...
    def custom_jsonable_encoder(self, obj, depth: int, current_depth: int = 0, exclude=None):
        if exclude is None:
            exclude = set()
        if isinstance(obj, dict):
            if current_depth >= depth:
                return {}
            return {key: self.custom_jsonable_encoder(value, depth, current_depth + 1, exclude)
                    for key, value in obj.items() if key not in exclude}
        if isinstance(obj, list):
            if current_depth >= depth:
                return []
            return [self.custom_jsonable_encoder(item, depth, current_depth + 1, exclude)
                    for item in obj]
        ...
        if isinstance(obj, Base):
            ...
            return self.custom_jsonable_encoder(data, depth, current_depth, exclude)
