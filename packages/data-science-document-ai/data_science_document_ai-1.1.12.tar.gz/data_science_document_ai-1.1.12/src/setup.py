...
from src.io import logger, download_dir_from_bucket, get_storage_client
...
def setup_params(args=None):
    ...
    downloaded_meta = download_dir_from_bucket(
        bucket,
        params["g_model_data_folder"],
        Path(params["local_model_data_folder"])
    )
    if not downloaded_meta:
        logger.info(f"Could not load models metadata from cloud.")

    params["models"] = load_models_data(Path(params["local_model_data_folder"]) / params["g_model_data_folder"])
...
def load_models_data(directory: Path):
    """
    Loads models meta data, such as confidence thresholds.
    ...
    if directory.exists():
        for model in directory.iterdir():
            ...
            if path_to_confidence.is_file():
                with open(path_to_confidence, 'r') as f:
                    result[model.name] = {
