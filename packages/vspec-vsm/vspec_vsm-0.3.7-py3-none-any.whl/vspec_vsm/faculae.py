...
class Facula:
    ...
    stellar surface with a hot wall and cool floor :cite:p:`1976SoPh...50..269S`. Because if this,
    faculae appear bright when they are near the limb (hot wall is visible) and dark when near
    ...
    def age(self, time: Quantity):
        ...
        """
        if self.is_growing:
            time_from_max = -1*np.log(self.radius/self.r_max)*self.lifetime*0.5
            if time_from_max <= time:
                self.is_growing = False
                time = time - time_from_max
...
class FaculaCollection:
    ...
    *faculae : tuple
    ...
    def map_pixels(self, pixmap, star_rad, star_teff):
        ...
        """
        warnings.warn(
            'Use `Star.add_faculae_to_map` method instead.', DeprecationWarning)
