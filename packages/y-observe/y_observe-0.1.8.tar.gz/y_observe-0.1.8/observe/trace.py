...
def instrument(
    ...
    *,
    ...
    :param allow_private: By default private functions are ignored, set this to True to include them:bool
    ...
    def decorate_class(cls):
        for name, method in inspect.getmembers(cls, inspect.isfunction):
            ...
            if not name.startswith("_") or allow_private:
...
def add_dict_to_span_attributes(span: Span, name: str, item: Dict[str, Any]):
    def get_name(k):
        return f"{name}.{k}"
    ...
    for key in item.keys():
        try:
            ...
            if isinstance(item[key], list) or isinstance(item[key], dict):
                span.set_attribute(get_name(key), json.dumps(item[key]))
            # handle set
            elif isinstance(item[key], set):
                span.set_attribute(get_name(key), json.dumps(list(item[key])))
            # handle None
            elif item[key] is None:
                pass
            # handle everything else
            else:
                span.set_attribute(get_name(key), item[key])
        except Exception as e:
            logger.error(
                "Error adding value to span attributes",
                key=key,
                value=item[key],
                error=str(e),
            )
