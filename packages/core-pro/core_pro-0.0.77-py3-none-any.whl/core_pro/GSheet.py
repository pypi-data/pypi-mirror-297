...
class Sheet(GoogleAuthentication):
    ...
    def create_new_sheet(self, sheet_title):
        ...
        logger.info(f"{self.status} Create New Sheet: {sheet_title}")
        return new_sheet_id
...
class SheetFormat(GoogleAuthentication):
    ...
    def title(self, ws_id: str, position: str):
        ...
        self.service.spreadsheets().batchUpdate(spreadsheetId=self.spreadsheet_id, body=body).execute()
        logger.info(f'{self.status} Highlight title at {position}:{get_column_letter(self.my_range['endColumnIndex'])}')
    ...
    def header(self, ws_id: str, position: str, num_col: int):
        ...
        body = {"requests": request}
        self.service.spreadsheets().batchUpdate(spreadsheetId=self.spreadsheet_id, body=body).execute()
        logger.info(f'{self.status} Highlight header at {position}')
    ...
    def frozen_view(self, ws_id: str, frozen_rows: int = 2):
        ...
        body = {"requests": request}
        self.service.spreadsheets().batchUpdate(spreadsheetId=self.spreadsheet_id, body=body).execute()
        logger.info(f'{self.status} Frozen views up to {frozen_rows} rows')
