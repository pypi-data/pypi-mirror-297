            ...
            async for chunk in response.aiter_lines():
                if (chunk := chunk.strip("data:")) and chunk.startswith("{"):
                    ...
                    try:
                        ...
                        if data.get("lawQaRelatedLaws") and data.get("lawQaRelatedCases"):  # todo: 表格 思维导图
                        ...
                        if data.get("viewpointQaRelatedArticles"):
                            df = pd.DataFrame(data['viewpointQaRelatedArticles'])
                            from meutils.pandas_utils.pd_utils import to_html_plus
                            html_content = to_html_plus(df, title="Viewpoint Qa Related Articles", subtitle="")
                            with tempfile.NamedTemporaryFile(mode='w+', suffix='.html', delete=False) as file:
                                logger.debug(file.name)
                                file.write(html_content)
                                file.seek(0)

                            yield f"\n\n[Viewpoint Qa Related Articles](https://api.chatfire.cn/render/{file.name})"
