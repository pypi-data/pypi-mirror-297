...
class ProdDataManager:
    ...
    def putAndRegister(self, lfn, localfile, filemetadata, DataType):
        ...
        if res["OK"]:
            ...
            if not res["OK"]:
                DIRAC.gLogger.warn(f"Failed to set file meta data to {lfn}")
    ...
    def setTransformationFileStatus(self, lfn, status):
        if status == "PROCESSED":
            ...
            DIRAC.gLogger.notice("self.TransformationID: ", self.TransformationID)
            DIRAC.gLogger.notice("res set Processed: ", res)
        elif status == "PROBLEMATIC":
            res = self.transClient.setFileStatusForTransformation(
                self.TransformationID, "Problematic", lfn, force=True
            )
            DIRAC.gLogger.notice("self.TransformationID: ", self.TransformationID)
            DIRAC.gLogger.notice("res set Problematic: ", res)
        elif status == "UNUSED":
            res = self.transClient.setFileStatusForTransformation(
