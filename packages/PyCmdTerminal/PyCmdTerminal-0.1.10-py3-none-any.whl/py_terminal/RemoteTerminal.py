...
from typing import Optional, Union, List, Callable, Mapping
import threading
import getpass
import logging
...
class RemoteTerminal(SSHClient, _Terminal):
    ...
    def async_execute_command(self, command: str) -> Optional[int]:
        ...
        if self.command_exists('bash'):
            ...
            sh = "sh -c "
        out_tmp = f"/tmp/SSHCLIECT_out_{time.time_ns()}_XXXXXX"
        err_tmp = f"/tmp/SSHCLIECT_err_{time.time_ns()}_XXXXXX"
        full_command = f"nohup {stdbuf}{sh}'{command}' 1> {out_tmp} 2> {err_tmp} & echo $!"
        try:
            _, stdout, stderr = self._exec_command(full_command)
            pid = stdout.strip()
            ...
            if pid.isdigit():
                ...
                self._async_log[int(pid)] = {"out": out_tmp, "err": err_tmp}
                return int(pid)
            else:
    ...
    async def _sync_process_std(self, pid: int, _callable: Callable[[bytes, bytes], None] = print, *, event: asyncio.Event = None):
        ...
        try:
            out_log = self._async_log[pid]["out"]
            err_log = self._async_log[pid]["err"]
            out_last_position = 0
            err_last_position = 0
