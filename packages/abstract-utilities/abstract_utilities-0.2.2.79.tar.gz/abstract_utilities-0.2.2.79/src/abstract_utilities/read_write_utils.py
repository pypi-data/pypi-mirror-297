...
def check_read_write_params(*args, **kwargs):
    file_path_key = string_in_keys(['file','location','dir','directory','filepath','file_path', 'path'], kwargs)
    file_path = kwargs.get(file_path_key) or get_path(list(args))
    contents_key = string_in_keys(['contents', 'data','json_data','jsondata','content', 'text', 'content', 'string'], kwargs)
    contents = kwargs.get(contents_key) if contents_key else next((arg for arg in args if arg != file_path), None)
    if None in [contents,file_path]:
        final = {"file_path" if obj and isinstance(obj,str) and get_path([obj]) else "data": obj for obj in list(args)+list(kwargs.values())}
