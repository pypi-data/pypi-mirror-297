...
from mtmai.crud.curd_chat import user_threads
...
class SQLAlchemyDataLayer(BaseDataLayer):
    ...
    async def update_thread(
        ...
        if context.session.user.identifier is None:
            raise ValueError("User identifier not found in session context")
        async with get_async_session() as session:
            await ChatThread.upsert(
                ChatThread(
                    id=thread_id,
                    ...
                    tags=tags,
                    user_id=user_id,
                    name=name or metadata.get("name") or "no title",
                ),
                session,
            )
            await session.commit()
