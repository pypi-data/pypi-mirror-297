...
def running_compare_1(logger: logging.Logger, crd: CompareRunningData):
    ...
    with connect(**crd.src_dsn) as source_con, connect(**crd.dst_dsn) as target_con:
        while True:
            query_statement_src, query_params_src = get_query_full_table_statement_params(crd.src_db, crd.src_tab, crd.table_keys, ["*"], crd.batch_size, _keyval)
            query_statement_dst, query_params_dst = get_query_full_table_statement_params(crd.dst_db, crd.dst_tab, crd.table_keys, ["*"], crd.batch_size, _keyval)

            logger.info(f"batch_id[{batch_id}] source query statment: '{query_statement_src}', params '{query_params_src}'")
            logger.info(f"batch_id[{batch_id}] target query statment: '{query_statement_dst}', params '{query_params_dst}'")
            ...
            logger.info(f"batch_id[{batch_id}] source rows: {len(source_rows)}")
            logger.info(f"batch_id[{batch_id}] target rows: {len(target_rows)}")

            if len(source_rows) == 0:
                break

            crd.table_rows_total += len(source_rows)

            _temp_diff_rows = find_missing_in_b(source_rows, target_rows)

            logger.info(f"batch_id[{batch_id}] find diff rows: {len(_temp_diff_rows)}")

            crd.temp_diff_rows += _temp_diff_rows

            _keyval = extract_keyvals(source_rows[-1], crd.table_keys)

            processed_progress = round(crd.table_rows_total / crd.table_rows_total * 100, 2)
            logger.info(f"batch_id[{batch_id}] source rows: {len(source_rows)}, target rows: {len(target_rows)}.")
            logger.info(
                f"batch_id[{batch_id}] processed rows number: {crd.table_rows_total}/{crd.table_rows_total}, progress: {processed_progress}%, elapsed time: {get_elapsed_time(_start1, 2)}s."
            )
            batch_id += 1
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=cargs.parallel) as executor:
        for src_db, src_tab, dst_db, dst_tab in compare_tables:
            compare_name = f"{cargs.work_dir}/{src_db}.{src_tab}__{dst_db}.{dst_tab}.forward"
        ...
        for future in concurrent.futures.as_completed(future_to_task):
            ...
            success_count += 1
            print(f"{compare_name} -- progress {success_count}/{len(compare_tables)}")
