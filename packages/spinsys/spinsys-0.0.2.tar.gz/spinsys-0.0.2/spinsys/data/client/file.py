...
class File(spec.AbstractBufferedFile):
    ...
    def _upload_chunk(self, final=False):
        ...
        for i, data_chunk in enumerate(data_chunks):
            ops.append(
                data.BitOp(type=data.BitOpType.PutBitOp, bytes=data_chunk),
            )
        ...
        if resp.error != "":
            raise OSError(f"bit ops upload: {resp.error}")

        if len(resp.outcomes) != len(ops):
            raise OSError(f"expected {len(ops)} outcomes, got {len(resp.outcomes)}")

        for o in resp.outcomes:
            self.dir_entry.blocks.append(
                data.EntryBlock(
                    address=o.store_ref.address,
                    reference=o.store_ref.reference,
    ...
    def _fetch_range(self, start, end):
        ...
        for b, (s, e) in self._blocks_in_range(start, end):
            data, err = self.fs._get(b.address, b.reference)
            if err:
