...
class ProteinClusters:
    ...
    def replace_with_cluster(
        ...
        if not isinstance(self.clustered_proteins, pd.DataFrame):
            self.clustered_proteins.to_frame()
        else:
            # Added following issue #5
            self.clustered_proteins = self.clustered_proteins[["Representative"]]
