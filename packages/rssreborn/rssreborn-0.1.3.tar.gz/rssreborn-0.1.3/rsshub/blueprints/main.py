...
from rsshub.rssfeeder.rss import save_rss_source
from rsshub.rssfeeder.store.dbs import RSSFeed, RSSSource, session
...
def add_rss():
    ...
    creator = data.get("user_nick_name")
    creator_url = data.get("user_avatar_url")

    if not rss_url:
        return jsonify({"error": "RSS URL is required"}), 400

    res = save_rss_source(rss_url=rss_url, creator=creator, creator_url=creator_url)
    if res:
        return jsonify({"status": "ok", "message": "rss添加成功"}), 201
    else:
        return (
            jsonify({"status": "error", "message": "rss解析失败，请确保rss源是否有效"}),
            400,
        )
...
def get_paginated_news():
    ...
    result = {
        "message": "you rss news",
        "status": "ok",
        "num": len(paginated_feeds),
        "data": [r.to_json() for r in paginated_feeds],
    }
    return jsonify(result)


@bp.route("/rss_sources", methods=["GET"])
def get_rss_sources():
    page = request.args.get("page", default=1, type=int)
    per_page = request.args.get("per_page", default=30, type=int)
    offset = (page - 1) * per_page
    paginated_feeds = (
        ...
        .order_by(RSSSource.created.desc())
        .offset(offset)
        .limit(per_page)
        .all()
    )
...
def filter_content(ctx):
    ...
    if include_title:
        include_keywords = (
            include_title.split("|") if "|" in include_title else [include_title]
        )
        items = [
            item
            for item in items
            if any(keyword in item["title"] for keyword in include_keywords)
        ]
    ...
    if include_description:
        ...
        items = [
            item
            for item in items
            if any(keyword in item["description"] for keyword in include_keywords)
        ]

    if exclude_title:
        exclude_keywords = (
            exclude_title.split("|") if "|" in exclude_title else [exclude_title]
        )
        items = [
            item
            for item in items
            if all(keyword not in item["title"] for keyword in exclude_keywords)
        ]
    ...
    if exclude_description:
        ...
        items = [
            item
            for item in items
            if all(keyword not in item["description"] for keyword in exclude_keywords)
        ]
...
@bp.route("/gscore/rss")
def gscore_rss():
    from rsshub.spiders.gscore.rss import ctx

    return render_template("main/atom.xml", **filter_content(ctx()))


@bp.route("/ruanyifeng/rss")
def ruanyifeng_rss():
    from rsshub.spiders.ruanyifeng.rss import ctx
...
@bp.route("/36kr/rss")
def kr36_rss():
    from rsshub.spiders.kr36.rss import ctx

    return render_template("main/atom.xml", **filter_content(ctx()))


@bp.route("/taimeiti/rss")
def taimeiti_rss():
    from rsshub.spiders.taimeiti.rss import ctx
...
@bp.route("/huxiu/rss")
def huxiu_rss():
    from rsshub.spiders.huxiu.rss import ctx

    return render_template("main/atom.xml", **filter_content(ctx()))


@bp.route("/geekpark/rss")
def geekpark_rss():
    from rsshub.spiders.geekpark.rss import ctx
...
@bp.route("/zhihu/daily")
def zhihu_daily():
    from rsshub.spiders.zhihu.daily import ctx
