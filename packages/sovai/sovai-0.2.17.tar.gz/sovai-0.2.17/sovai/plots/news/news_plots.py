...
def plot_news_daily(ticker='NVDA'):
    ...
    # Run the app
    app_name = f"news_daily_sentiment"
    app.run_server(debug=False, port=get_unique_port(app_name), jupyter_mode="inline", jupyter_height=1100)
...
def run_dash_news_ts():
    ...
    df_sentiment = data("news/sentiment_score")
    df_polarity = data("news/polarity_score")
    df_topic = data("news/topic_probability")

    app = dash_news_ts_analysis(df_sentiment, df_polarity, df_topic)
    app_name = "news_topic_valuest"
    app.run_server(debug=False, port=get_unique_port(app_name), jupyter_mode="inline", jupyter_height=1300)
