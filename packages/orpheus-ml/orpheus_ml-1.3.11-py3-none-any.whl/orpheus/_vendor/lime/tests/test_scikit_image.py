import unittest
from orpheus._vendor.lime.wrappers.scikit_image import BaseWrapper
from orpheus._vendor.lime.wrappers.scikit_image import SegmentationAlgorithm
...
class TestBaseWrapper(unittest.TestCase):
    ...
    def test_filter_params(self):
        ...
        self.assertEqual(
            obj_.filter_params(baz_fn, override={"c": False}),
            {"a": 10, "b": 100, "c": False},
        )
...
class TestSegmentationAlgorithm(unittest.TestCase):
    def test_instanciate_segmentation_algorithm(self):
        ...
        fn = SegmentationAlgorithm("quickshift", kernel_size=3, max_dist=6, ratio=0.5, random_seed=133)
        fn_result = fn(img)
        original_result = quickshift(img, kernel_size=3, max_dist=6, ratio=0.5, random_seed=133)
