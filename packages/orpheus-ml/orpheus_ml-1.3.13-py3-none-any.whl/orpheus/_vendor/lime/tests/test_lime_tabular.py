...
from orpheus._vendor.lime.discretize import (
    QuartileDiscretizer,
    DecileDiscretizer,
    EntropyDiscretizer,
)
...
from orpheus._vendor.lime.lime_tabular import LimeTabularExplainer
...
class TestLimeTabular(unittest.TestCase):
    ...
    def test_lime_explainer_good_regressor_synthetic_data(self):
        ...
        explainer = LimeTabularExplainer(X, feature_names=feature_names, discretize_continuous=True)
    ...
    def test_lime_explainer_no_regressor(self):
        ...
        exp = explainer.explain_instance(self.test[i], rf.predict_proba, num_features=2)
        self.assertIsNotNone(exp)
        keys = [x[0] for x in exp.as_list()]
        self.assertEqual(
            1,
    ...
    def test_lime_tabular_explainer_equal_random_state(self):
        ...
        explainer_1 = LimeTabularExplainer(
            X,
    ...
    def testFeatureValues(self):
        ...
        explainer = LimeTabularExplainer(training_data=training_data, categorical_features=[0, 1, 2])
        ...
        assert_array_equal(explainer.feature_frequencies[0], np.array([0.5, 0.5]))
        assert_array_equal(explainer.feature_frequencies[1], np.array([0.25, 0.25, 0.25, 0.25]))
        assert_array_equal(explainer.feature_frequencies[2], np.array([0.5, 0.5]))
    ...
    def test_lime_explainer_with_data_stats(self):
        ...
        descritizer = QuartileDiscretizer(self.train, [], self.feature_names, self.target_names, random_state=20)
