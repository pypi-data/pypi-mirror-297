...
from orpheus._vendor.lime.discretize import QuartileDiscretizer
from orpheus._vendor.lime.discretize import DecileDiscretizer
from orpheus._vendor.lime.discretize import EntropyDiscretizer
from orpheus._vendor.lime.discretize import BaseDiscretizer
from orpheus._vendor.lime.discretize import StatsDiscretizer
...
class LimeTabularExplainer(object):
    ...
    def explain_instance(
        ...
        if not isinstance(inverse, pd.DataFrame):
            inverse = pd.DataFrame(inverse, columns=data_row.index.to_list())
        yss = predict_fn(inverse)
        if yss.ndim == 3 or self.mode == "regression":
            yss = np.nanmean(yss, axis=0)
