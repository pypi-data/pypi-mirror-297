...
import warnings
warnings.filterwarnings("ignore", category=numba.errors.NumbaTypeSafetyWarning)

@njit((float32[:,:], ), cache=True, parallel=True)
def calculate_core_dist_32p(data):
    n, d = data.shape
...
def _prim_mst_32p(observations, core_dist):
    ...
    heap = [(np.float64(0), start_vertex)] 
...
def dens_sep_32p(data_i, data_j, inter_core_dists_i, inter_core_dists_j):
...
@njit((float32[:, :], int32[:], int32[:], int32[:]), cache=True, parallel=True)
def _dbcv_32p(X, labels, uniqal, counts_for_uniq):

    dsbcs = np.zeros(uniqal.size, dtype=np.float64)

    internal_obj = List([Dict.empty(key_type=int32, value_type=int64[:]) for i in prange(uniqal.size)])
    internal_core_dist = List([Dict.empty(key_type=int32, value_type=float64[:]) for i in prange(uniqal.size)])


    min_dspcs = np.full((uniqal.size - 1, uniqal.size), np.inf, dtype=np.float64)

    for i in prange(uniqal.size):
        index = np.flatnonzero(labels == uniqal[i]).astype(np.int64)
        
        core_dists = calculate_core_dist_32p(X[index])
        row, col = _prim_mst_32p(X[index], core_dists)
        ...
        dsbcs[i] = np.max(val)
        internal_obj[i][i] = index[columns]
        internal_core_dist[i][i] = core_dists[columns]

    for i in prange(uniqal.size):
        for j in range(i + 1, uniqal.size):
            index_i = internal_obj[i][i]
            index_j = internal_obj[j][j]
            core_dists_i = internal_core_dist[i][i]
            core_dists_j = internal_core_dist[j][j]

            dspc_ij = dens_sep_32p(X[index_i], X[index_j], core_dists_i, core_dists_j)
