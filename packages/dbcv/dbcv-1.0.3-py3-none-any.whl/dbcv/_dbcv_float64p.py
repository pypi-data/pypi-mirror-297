...
@njit((float64[:,:], ), cache=True, parallel=True)
def calculate_core_dist_64p(data):
    n, d = data.shape
...
def _prim_mst_64p(observations, core_dist):
...
def dens_sep_64p(data_i, data_j, inter_core_dists_i, inter_core_dists_j):
...
@njit((float64[:, :], int32[:], int32[:], int32[:]), cache=True, parallel=True)
def _dbcv_64p(X, labels, uniqal, counts_for_uniq):
    ...
    for i in prange(uniqal.size):
        ...
        core_dists = calculate_core_dist_64p(X[index])
        row, col = _prim_mst_64p(X[index], core_dists)
    ...
    for i in prange(uniqal.size):
        for j in range(i + 1, uniqal.size):
            ...
            dspc_ij = dens_sep_64p(X[index_i], X[index_j], core_dists_i, core_dists_j)
