...
def calculate_core_dist_64np(data):
    n, d = data.shape
...
def _prim_mst_64np(observations, core_dist):
...
def dens_sep_64np(data_i, data_j, inter_core_dists_i, inter_core_dists_j):
...
def _dbcv_64np(X, labels, uniqal, counts_for_uniq):
    ...
    for i in range(uniqal.size):
        ...
        core_dists = calculate_core_dist_64np(X[index])
        row, col = _prim_mst_64np(X[index], core_dists)
    ...
    for i in range(uniqal.size):
        for j in range(i + 1, uniqal.size):
            ...
            dspc_ij = dens_sep_64np(X[index_i], X[index_j], core_dists_i, core_dists_j)
