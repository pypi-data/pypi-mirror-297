...
def _dbcv_64p(X, labels, uniqal, counts_for_uniq):
    ...
    internal_obj = List([np.array([-1], dtype=np.int64) for i in range(uniqal.size)])
    internal_core_dist = List([np.array([-1.0], dtype=np.float64) for i in range(uniqal.size)])
