...
class Request:
            ...
            if "boundary" in self.ctype.lower():
                ...
                try:
                    ...
                    if type(jsonload)==list:
                        ...
                        if "query" in jsonload and "variables" in jsonload and len(jsonload["variables"])>0:
    ...
    def reBuildAll(self,payloadpos="append",fuzzheaderslist=[]):
        variations = []
        body = ""
        headers = []
        body = self.body
        ...
        if self.baseParams["headers"]!={}:
            for mainheader in self.baseParams["headers"]:
                ...
                variations.append((url,headers,body))
        if fuzzheaderslist != []:
            for mainheader in fuzzheaderslist:
