...
from .output import dump_file, dump_string
from .pdf import parse_file


class Parser:
    def __init__(
        self, infile=None, stream=None, config=Config, outfile=None, file_format=None
    ):
        self.config = Config
        if infile:
        ...
        self.raw_scouts = []
        self.scouts = parse_file(infile=infile, stream=stream)
