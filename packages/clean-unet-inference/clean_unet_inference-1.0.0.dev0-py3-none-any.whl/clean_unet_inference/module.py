...
def padding(x, D, K, S):
    ...
    L = torch.tensor(x.size()[-1])
    for _ in range(D):
        L = 1 + torch.max(torch.Tensor([0]).to(torch.int64), torch.ceil((L - K) / S))

    for _ in range(D):
        L = (L - 1) * S + K

    x = F.pad(x, (0, (L - torch.tensor(x.size(-1))).to(torch.int64)))
    return x
...
class CleanUNet(nn.Module):
    ...
    def forward(self, noisy_audio):
        ...
        len_s = x.size(-1)  # length at bottleneck
