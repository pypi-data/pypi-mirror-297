...
class Command(BaseCommand):
    ...
    def handle(self, options: argparse.Namespace) -> None:
        ...
        # Definir el path de la carpeta "res" en relación al nuevo directorio
        new_res_path = os.path.join(parent_dir, 'mi_template', '{{cookiecutter.out_dir}}', 'android', 'app', 'src', 'main', 'res')

        # Definir el path de la carpeta "res" en flutter_dir
        flutter_res_path = os.path.join(self.flutter_dir, 'android', 'app', 'src', 'main', 'res')

        # Verificar si existe la carpeta "res" en flutter_dir, y si existe, eliminarla
        if os.path.exists(flutter_res_path):
            shutil.rmtree(flutter_res_path)
            print(f"Carpeta 'res' existente eliminada en {flutter_res_path}")

        # Copiar la nueva carpeta "res" a flutter_dir
        shutil.copytree(new_res_path, flutter_res_path)
        print(f"Carpeta 'res' copiada desde {new_res_path} a {flutter_res_path}")


        #{{cookiecutter.out_dir}}/android/app/src/main/res
