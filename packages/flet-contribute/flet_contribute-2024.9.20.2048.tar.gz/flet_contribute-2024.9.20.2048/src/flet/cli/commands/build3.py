...
class Command(BaseCommand):
    ...
    def handle(self, options: argparse.Namespace) -> None:
        ...
        # Definir el path de la carpeta "android" en relación al nuevo directorio
        new_android_path = os.path.join(parent_dir, 'mi_template', '{{cookiecutter.out_dir}}', 'android')

        # Definir el path de la carpeta "android" en flutter_dir
        flutter_android_path = os.path.join(self.flutter_dir, 'android')

        # Verificar si existe la carpeta "android" en flutter_dir, y si existe, eliminarla
        if os.path.exists(flutter_android_path):
            shutil.rmtree(flutter_android_path)
            print(f"Carpeta 'android' existente eliminada en {flutter_android_path}")

        # Copiar la nueva carpeta "android" desde cookiecutter a flutter_dir
        if os.path.exists(new_android_path):
            shutil.copytree(new_android_path, flutter_android_path)
            print(f"Carpeta 'android' copiada desde {new_android_path} a {flutter_android_path}")
        else:
            print(f"No se encontró la carpeta 'android' en {new_android_path}")
