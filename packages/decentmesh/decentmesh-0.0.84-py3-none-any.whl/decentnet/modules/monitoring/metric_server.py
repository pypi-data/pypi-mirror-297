...
def metric_server_start():
    ...
    asyncio.run(server.start_server(port=8008))


async def ping(timeout=0.1):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"http://{METRICS_SERVER_HOST}:{METRICS_SERVER_PORT}", timeout=timeout):
...
async def send_metric(name, value, metric_type="observe", timeout=1):
...
if __name__ == '__main__':
    ...
    asyncio.run(_server.start_server(port=8008))
