...
from decentnet.consensus.block_sizing import MAXIMUM_BLOCK_SIZE, BLOCK_PREFIX_LENGTH_BYTES
...
from decentnet.consensus.dev_constants import RUN_IN_DEBUG, BLOCK_SHARE_LOG_LEVEL
from decentnet.consensus.share_server_config import INCOMING_CHANNEL_BYTE_LEN, UNSUB_BYTE_LEN, \
    UNSUB_BYTE_STRING, SUBSCRIBE_ACK
...
class BlockShareServer:
    def __init__(self, host='localhost', port1=8009, port2=8010):
        ...
        self.server_socket_1.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, MAXIMUM_BLOCK_SIZE)
        self.server_socket_1.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, MAXIMUM_BLOCK_SIZE)
        self.server_socket_1.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,
        ...
        self.server_socket_2.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, MAXIMUM_BLOCK_SIZE)
        self.server_socket_2.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, MAXIMUM_BLOCK_SIZE)
        self.server_socket_2.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,
                                        1)  # Disable Nagle's Algorithm
