...
class BasicInterface:
    ...
    def _init_out_encryption(self, beam: Beam, difficulty: Difficulty = BlockchainParams.low_diff_sha256):
        """
    ...
    def send(self, data: bytes, difficulty: Difficulty = BlockchainParams.low_diff_sha256,
    ...
    def fetch(self, return_latency_ms: bool = False) -> dict | None | Tuple[dict, float]:
        ...
        block = self._beam_receive.fetch_message()[0]
        data = self._beam_receive.encryptor_beacon.decrypt(memoryview(block.data))
        if data[-4:] == b'ACK!':
            self._beam_receive.send_communication_data(HANDSHAKE_MSG)
        try:
            if return_latency_ms:
                return cbor2.loads(data), (time.time() - block.timestamp) * 1000
            else:
