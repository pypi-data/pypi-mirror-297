...
class Block:
    ...
    async def to_bytes(self) -> bytes:
        ...
        if self.diff.compression_level > 0:
            data, data_size = CompressionWrapper.compress_lz4(self.data, self.diff.compression_level)
        else:
            data = self.data
        ...
        packed_block_len = INDEX_SIZE + MERGED_DIFFICULTY_BYTE_LEN + HASH_LEN_BLOCK + NONCE_SIZE + TIMESTAMP_SIZE + data_size
