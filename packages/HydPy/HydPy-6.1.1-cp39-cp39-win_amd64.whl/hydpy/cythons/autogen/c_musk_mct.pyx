...
cdef class Model:
    ...
    def get_wqmodel(self) -> Optional[masterinterface.MasterInterface]:
        return self.wqmodel
    def set_wqmodel(self, wqmodel: Optional[masterinterface.MasterInterface]) -> None:
    ...
    cpdef inline void calc_referencewaterdepth_v1(self) noexcept nogil:
        ...
        if isnan(wl) or isinf(wl):
            ...
            mn, mx = 0.9 * wl, 1.1 * wl
        self.sequences.factors.referencewaterdepth[i] = self.pegasusreferencewaterdepth.find_x(            mn, mx, 0.0, 1000.0, self.parameters.solver.tolerancewaterdepth, self.parameters.solver.tolerancedischarge, 100        )
    ...
    cpdef inline void calc_reynoldsnumber_v1(self) noexcept nogil:
        ...
        cdef numpy.int64_t i
        i = self.idx_segment
        denom = (            self.sequences.factors.correctingfactor[i]            * self.sequences.factors.surfacewidth[i]            * self.parameters.control.bottomslope            * self.sequences.factors.celerity[i]            * (1000.0 * self.parameters.derived.segmentlength)        )
        if denom == 0.0:
            self.sequences.states.reynoldsnumber[i] = 0.0
        else:
            self.sequences.states.reynoldsnumber[i] = self.sequences.fluxes.referencedischarge[i] / denom
