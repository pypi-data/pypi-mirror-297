...
class Bot(discord.Bot):
    def __init__(self, token=None, error_handler_webhook=None, intents=None,lang=None, *args, **kwargs):
        if token is None:
         return
    ...
    async def on_error(self, event, *args, **kwargs):
        ...
        # Check if the error is from a command
        if event == "on_command_error":
            command_name = args[0].name if args else None
