...
class ModelAdminBase(ModelAdminAbstract):
    ...
    @classmethod
    def format_p(cls, data, fold=1, text='·····', style=None, obj=None):
        if len(data) <= fold:
            ...
            data = OrderedDict((html.escape(k), v) for k, v in data.items())
            return mark_safe(f"""<a{stl} href="javascript:void(0)" data-dhp='{json.dumps(data)}'>{text}</a>""")
