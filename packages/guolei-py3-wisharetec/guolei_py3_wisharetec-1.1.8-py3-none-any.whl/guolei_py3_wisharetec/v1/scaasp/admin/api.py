    ...
    def download_export_file(
        ...
        def _retry_func(
            ...
            if Draft202012Validator({"type": "boolean", "const": True}).is_valid(
                ...
                login_with_cache_func_kwargs = {}
            login_with_cache_func_kwargs = Dict(login_with_cache_func_kwargs)
            query_export_with_paginator = self.login_with_cache(**login_with_cache_func_kwargs.to_dict()).query_export_with_paginator(
                **query_export_with_paginator_func_kwargs.to_dict())
            if Draft202012Validator({
                "type": "object",
                "properties": {"resultList": {"type": "array", "minItems": 1}},
                "required": ["resultList"]
            }).is_valid(query_export_with_paginator):
                for i in query_export_with_paginator.resultList:
                    if Draft202012Validator({
                        "type": "object",
                        "properties": {
